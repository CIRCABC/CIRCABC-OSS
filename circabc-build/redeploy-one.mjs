...
console.log(chalk.cyan('Redeploying CIRCABC application without building...'));
console.log(chalk.cyan(`Environment: ${environment}`));
console.log(chalk.cyan(`Target: ${wlTargets[0]} (first node only)`));

let user = process.env.USER_NAME;
let password = process.env.PASSWORD;
let ldapUser = process.env.LDAP_USER;

// Deploy to only the first target (one node)
const wlTarget = wlTargets[0];
...
try {
  console.log(chalk.yellow('Initiating redeploy request...'));
  ...
  if (resp.ok) {
    ...
    console.log(chalk.green(`SMT Ticket: ${response.SMT.ticket.number}`));
    ...
    while (true) {
      ...
      if (deployStatusResp.ok) {
        ...
        if (status === 'IN PROGRESS') {
          console.log(chalk.yellow('Redeploy in progress...'));
          await sleep(60000);
        } else if (status === 'SUCCESS') {
          console.log(chalk.green('Redeploy succeeded!'));
          break;
        } else if (status === 'ERROR') {
          console.log(chalk.red('Redeploy failed!'));
          break;
        }
      } else {
        console.log(chalk.red('Failed to check deploy status'));
        break;
      }
    }
  } else {
  ...
  console.error(chalk.red('Error during redeploy:'), error);
  process.exit(5);
}

console.log(chalk.green('Redeploy operation completed.'));

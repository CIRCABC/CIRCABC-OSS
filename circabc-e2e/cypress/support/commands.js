...
Cypress.Commands.add('setPassword', (username, password) => {
  cy.request({
    method: 'POST',
    url:
      `http://${Cypress.env('host')}:8080${Cypress.env('path')}/service/api/person/changepassword/${username}`,
    body: {
      newpw: password,
    },
...
Cypress.Commands.add('login', (username, password) => {
  ...
  cy.request({
    method: 'POST',
    url:    `http://${Cypress.env('host')}:8080${Cypress.env('path')}/service/api/login?guest=true`,
    body: {
      username: username,
      password: password,
    },
    ...
    cy.request({
      method: 'GET',
      url:
        `http://${Cypress.env('host')}:8080${Cypress.env('path')}/service/circabc/users/${username}`,

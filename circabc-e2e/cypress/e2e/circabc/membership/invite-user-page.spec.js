describe("The Invite user Page", function () {
  ...
  it("successfully invite user and sending notification", function () {
    ...
    cy.get('[data-cy="clean-button"]').should("be.visible").click();
    cy.get('[data-cy="search-user"]').type(Cypress.env("reviewer.username"));
    cy.get('[data-cy="search-button"]').should("be.visible").click();
    cy.get("[data-cy=users]").select(
      Cypress.env("reviewer.first.name") +
        " " +
        Cypress.env("reviewer.last.name") +
        " (" +
        Cypress.env("reviewer.email") +
        ")",
    );
    cy.get('#profileName').select(1);
    cy.get("[data-cy=select]").should("be.visible").click();
    cy.wait(500);

describe('Create users', function () {
  ...
  const shouldCreate = Cypress.env('circabc_admin_create') !== false;

  if (shouldCreate) {
    it('successfully created circabc admin', function () {
      cy.visit('/me/roles', {
        failOnStatusCode: false,
      });
      cy.get('[data-cy=create-user]').click();
      cy.get('[data-cy=username]').type(Cypress.env('circabc.admin.username'));
      cy.get('[data-cy=firstname]').type('Circabc');
      cy.get('[data-cy=lastname]').type('Admin');
      cy.get('[data-cy=email]').type('admin@circabc.eu');
      cy.get('[data-cy=phone]').type('27021971');
      cy.get('[data-cy=postalAddress]').type('Uknown address');
      cy.get('[data-cy=password]').type(Cypress.env('circabc.admin.password'));

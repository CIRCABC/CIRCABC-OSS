# Docker OpenLDAP
This docker container is based on Bitnami OpenLDAP image : [https://github.com/bitnami/containers/tree/main/bitnami/openldap](https://github.com/bitnami/containers/tree/main/bitnami/openldap)
...
To use OpenLDAP with WebLogic and Oracle in the same Docker network, please have a look to 
["../docker-compose-weblogic.yml"](./docker-compose-weblogic.yml)

Please refer to ["bitnami README.md"](https://github.com/bitnami/bitnami-docker-openldap.git) for other LDAP configuration options.  

### 2.Launching a docker instance.

Execute the command `docker-compose up` to launch the docker container.

You can also use  *openldap.sh* script which cleans volumes and starts the container. 
...
## Configurating Circabc
Basic LDAP configuration that targets CEDProd LDAP is defined in `alfresco-global.properties` file of general config1 buildconfig in `circabc-backend/buildconfig/config1/WEB-INF/classes/alfresco/alfresco-global.properties`

In order to use OpenLDAP instead, you should overhide this configuration in `alfresco-global.properties` file of your proper buildconfig directory (see `circabc-backend/buildconfig/weblogic-docker/WEB-INF/classes/alfresco-global.properties`).

To use the OpenLDAP container, please add the following lines:
```
#LDAP Configuration

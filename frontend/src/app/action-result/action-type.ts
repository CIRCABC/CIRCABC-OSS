/**
 * Enumeration-like constant map of all action identifiers used across the
 * application to describe operations that can produce an action result.
 *
 * Each key is a stable, human-readable action name (for example
 * `CREATE_SPACE` or `DELETE_CONTENT`), and each value is the corresponding
 * dot-separated string identifier used by the backend and by the i18n /
 * result-handling layers to reference that specific action.
 *
 * The object is declared `as const` so that both the keys and the exact
 * string literal values are preserved in the type system. This provides a
 * single source of truth for action identifiers and enables the companion
 * {@link ActionType} type alias to represent the union of all valid values.
 */
...
/**
 * Union type of all valid action identifier string values contained in the
 * {@link ActionType} constant map.
 *
 * Use this type wherever a variable, parameter or property must hold one of
 * the recognised action identifiers, ensuring only the string literals
 * declared in {@link ActionType} are accepted. Note that this type alias
 * intentionally shares its name with the constant, following the common
 * TypeScript pattern of pairing a `const` object with a same-named type.
 */

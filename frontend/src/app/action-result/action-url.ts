...
/**
 * Describes a single mapping rule used to associate an outgoing backend
 * request (identified by its URL pattern and HTTP verb) with a semantic
 * {@link ActionType}.
 *
 * These rules are evaluated by {@link getActionType} to translate raw
 * REST calls into user-facing action results.
 */
interface ActionUrlRegexMethod {
  /** Regular expression matched against the request URL to identify the action. */
  urlRegex: RegExp;
  /** HTTP method that must match the request for this rule to apply. */
  method: 'GET' | 'PUT' | 'POST' | 'DELETE';
  /** The semantic action type this URL/method combination represents. */
  actionType: ActionType;
  /**
   * Whether the resulting action should be surfaced to the user. When
   * `false`, {@link getActionType} treats the action as internal and does
   * not report it.
   */
  show: boolean;
}
...
/**
 * Ordered lookup table mapping backend request URL patterns and HTTP
 * methods to their corresponding {@link ActionType}.
 *
 * The list is scanned top to bottom by {@link getActionType}, so more
 * specific patterns are placed before broader ones to ensure the correct
 * rule is matched first. Rules flagged with `show: false` represent
 * actions that are not reported to the user.
 */
const availableActions: ActionUrlRegexMethod[] = [
  {
    urlRegex: /\/autoupload/,
    method: 'POST',
  ...
  },
  {
    ...
    method: 'POST',
    actionType: ActionType.ADD_HELP_SECTION,
    show: true,
  },
  {
    urlRegex: /\/help\/links/,
  ...
  {
    urlRegex: /\/categories\/[a-f0-9-]+\/request\/delete\/group\/approval/,
    method: 'POST',
    actionType: ActionType.REQUEST_DELETE_GROUP,
    show: true,
  },
  {
    urlRegex: /\/categories\/[a-f0-9-]+\/groups\/deletion\/request/,
    method: 'POST',
    actionType: ActionType.REQUEST_DELETE_GROUP,
    show: true,
  },
  ...
  },
  {
    urlRegex: /\/help\/categories/,
    method: 'DELETE',
    actionType: ActionType.DELETE_HELP_SECTION,
    show: true,
  },
  {
    urlRegex: /\/help\/links/,
  ...
  {
    urlRegex: /\/keywords\/bulk/,
    method: 'POST',
    actionType: ActionType.IMPORT_KEYWORD,
    show: true,
  },
  {
  ...
  },
  {
    urlRegex: /\/help\/categories/,
    method: 'PUT',
    actionType: ActionType.UPDATE_HELP_SECTION,
    show: true,
  },
  {
  ...
  },
  {
    urlRegex: /\/admins/,
    method: 'POST',
    actionType: ActionType.INVITE_CIRCABC_ADMIN,
    show: true,
  },
];

/**
 * Resolves the semantic {@link ActionType} associated with a given backend
 * request.
 *
 * The function looks up the first rule in {@link availableActions} whose
 * HTTP method matches `method` and whose `urlRegex` matches `url`. The
 * action type is only returned when the matched rule is flagged as
 * user-visible (`show: true`); otherwise the request is considered internal.
 *
 * @param url - The request URL to classify.
 * @param method - The HTTP method used for the request.
 * @returns The matching {@link ActionType} when a visible rule matches,
 * or `undefined` when no visible rule applies.
 */
export function getActionType(
  url: string,
  method: 'GET' | 'PUT' | 'POST' | 'DELETE'
) {
  const result = availableActions.find(

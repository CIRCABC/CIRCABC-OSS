...
var mooJSON = new Hash({
	...
	$replaceChars: function(chr){
		return mooJSON.$specialChars[chr] || '\\u00' + Math.floor(chr.charCodeAt() / 16).toString(16) + (chr.charCodeAt() % 16).toString(16);
	},

	encode: function(obj){
		switch ($type(obj)){
			case 'string':
				return '"' + obj.replace(/[\x00-\x1f\\"]/g, mooJSON.$replaceChars) + '"';
			case 'array':
				return '[' + String(obj.map(mooJSON.encode).filter($defined)) + ']';
			case 'object': case 'hash':
				var string = [];
				Hash.each(obj, function(value, key){
					var json = mooJSON.encode(value);
					if (json) string.push(mooJSON.encode(key) + ':' + json);
				});
				return '{' + string + '}';

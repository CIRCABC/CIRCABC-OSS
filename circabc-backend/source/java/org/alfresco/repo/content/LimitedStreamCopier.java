...
/**
 * This class is a simple utility to copy bytes from an {@link InputStream} to an {@link OutputStream}. The copy can be performed with an optional byte limit and as soon as this limit is reached, the copy will be stopped immediately and a {@link ContentLimitViolationException} will be thrown.
 * 
 * @since Thor
 */
public final class LimitedStreamCopier
{
    ...
    private static final int BYTE_BUFFER_SIZE = 16384; // 16K buffer size
     ...
     * 
     * @param in
     *            the stream from which to read content.
     * @param out
     *            the stream to which to write content.
     * @param sizeLimit
     *            the maximum number of bytes that will be copied between the streams before a {@link ContentLimitViolationException} will be thrown. A negative number or zero will be deemed to mean 'no limit'.
     */

...
public class CircabcBrowseBean extends BrowseBean {
  ...
  public static final String PREFIXED_WAI_CLOSE_NAVIGATION_OUTCOME =
    CircabcNavigationHandler.WAI_PREFIX + CLOSE_NAVIGATION_OUTCOME;
  ...
  public static final String MSG_NODE_UNAVAILABLE =
    "browse_generic_error_node_unavailable";
  ...
  public static final String ALTERNATIVE_DOWNLOAD_URL =
  ...
  private static final String MSG_REJECT_DETAILS =
    "post_moderation_reject_detail";
   ...
   * (&gt; 100)
   */
  public NodePropertyResolver resolverTrunckedOneHundredName =
   ...
   */
  public NodePropertyResolver resolverAttachmentDownload =
    new NodePropertyResolver() {
   ...
   * (&gt; 400)
   */
  public NodePropertyResolver resolverTrunckedFourHundredName =
    new NodePropertyResolver() {
      private static final long serialVersionUID = 5928155255659015928L;

      public Object get(final Node node) {
        final String title = (String) node
          .getProperties()
   ...
   */
  public NodePropertyResolver resolverSimpleDisplayPath =
    new NodePropertyResolver() {
   ...
   * Resolver to get a more beautiful display path
   */
  public NodePropertyResolver resolverSimpleDisplayPathLight =
    new NodePropertyResolver() {
      private static final long serialVersionUID = 7987625426368819373L;

      public Object get(final Node node) {
        final Object displayPath = node
          .getProperties()
          .get(ResolverHelper.DISPLAY_PATH);
  ...
  public NodePropertyResolver resolverReplies = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      ...
      final ServiceRegistry serviceRegistry =
        Services.getAlfrescoServiceRegistry(context);

      // TODO : Alfesco code -> optimize avec lucene ?
      // query for the number of posts within the given node
      final String repliesXPath =
        SUBTYPE_OF + ForumModel.TYPE_POST + END_BRACKETS;

      final List<NodeRef> replies = serviceRegistry
        .getSearchService()
        .selectNodes(
          node.getNodeRef(),
          repliesXPath,
   ...
   */
  public NodePropertyResolver resolverContentFourHundred =
    new NodePropertyResolver() {
      ...
      public Object get(final Node node) {
        ...
        // amounts of content)
        final ContentReader reader = serviceRegistry
          .getContentService()
  ...
  public NodePropertyResolver resolverFromTopic = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      final NodeService nodeService = getNodeService();
      final NodeRef parent = nodeService
        .getPrimaryParent(node.getNodeRef())
        .getParentRef();

      final Node topic = new Node(parent);
      final String name = topic.getName();
      final String title = (String) topic
        .getProperties()
        .get(ContentModel.PROP_TITLE.toString());

      final String copedName = UtilsCircabc.cropEncodeLight(
        (title == null || title.length() < 0) ? name : title,
        50
      );
      ...
      String buf =
        ...
        MessageFormat.format(
          Application.getMessage(fc, MSG_FROM_TOPIC),
          copedName
        ) +
        OPEN_BLOCKQUOTE +
        resolverContentFourHundred.get(node) +
        CLOSE_BLOCKQUOTE;
   ...
   * (&gt; 100)
   */
  public NodePropertyResolver resolverBrowseWaiUrl =
    new NodePropertyResolver() {
   ...
   */
  public NodePropertyResolver resolverCreatorUserName =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532907187778250014L;
    ...
    public Object get(final Node node) {
      ...
      if (lastPost != null) {
        ...
        return (
          formatedDate +
          " (" +
          lastPost.getProperties().get(ContentModel.PROP_CREATOR) +
          ")"
        );
      } else {
        return "";
  ...
  public NodePropertyResolver resolverAlternativeDownload =
    new NodePropertyResolver() {
      private static final long serialVersionUID = 4540744401261261034L;

      public Object get(Node node) {
        return SimpleDownloadServlet.generateDownloadURL(
          node.getNodeRef(),
          node.getName()
        );
      }
    };
  public NodePropertyResolver resolverAlternativeBrowse =
    new NodePropertyResolver() {
      /**
       *
       */
      ...
      public Object get(Node node) {
        return SimpleDownloadServlet.generateBrowseURL(
          node.getNodeRef(),
          node.getName()
        );
      }
   ...
   */
  public NodePropertyResolver resolverCreatorProfile =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532907187778250014L;
        ...
        if (creator != null && personService.personExists(creator)) {
          if (authorityService.isAdminAuthority(creator)) {
            ...
            final CircabcServiceRegistry circabcServiceRegistry =
              Services.getCircabcServiceRegistry(context);
            final ManagementService managementService =
              circabcServiceRegistry.getManagementService();

            final NodeRef nodeRef = node.getNodeRef();
            final NodeRef igRoot = managementService.getCurrentInterestGroup(
              nodeRef
            );

            // try to get IGRoot profile
            String profileName = getProfileName(
              circabcServiceRegistry.getIGRootProfileManagerService(),
              igRoot,
              creator
            );
            ...
            if (profileName == null) {
              profileName = getProfileName(
                circabcServiceRegistry.getCategoryProfileManagerService(),
                managementService.getCurrentCategory(igRoot),
                creator
              );
            }
            ...
            if (profileName == null) {
              profileName = getProfileName(
                ...
                managementService.getCircabcNodeRef(),
                creator
              );
            }

            return profileName == null
              ? Application.getMessage(context, REGISTRED)
              : profileName;
          }
        } else {
          return "";
      ...
      private String getProfileName(
        final ProfileManagerService profileService,
        final NodeRef nodeRef,
        final String creator
      ) {
        final String profileStr = profileService.getPersonProfile(
          nodeRef,
          creator
        );
        if (profileStr != null) {
   ...
   */
  public NodePropertyResolver resolverDocumentNumber =
    new NodePropertyResolver() {
   ...
   */
  public NodePropertyResolver resolverWaitingForApproval =
    new NodePropertyResolver() {
   ...
   */
  public NodePropertyResolver resolverIsAbuseSignaled =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -5329099997788790014L;
   ...
   * Resolver to get if a node is moderated
   */
  public NodePropertyResolver resolverNumberWaitingPost =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532907187778259999L;
      ...
      public Object get(final Node node) {
        ...
        if (getModerationService().isContainerModerated(node.getNodeRef())) {
          for (final FileInfo fileInfo : getPosts(node)) {
            if (
              getModerationService().isWaitingForApproval(fileInfo.getNodeRef())
            ) {
              count++;
            }
          }
   ...
   * Resolver to get if a node is moderated
   */
  public NodePropertyResolver resolverNumberRejectedPost =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532907187778259999L;
   ...
   * Resolver to get if a node is moderated
   */
  public NodePropertyResolver resolverNumberApprovedPost =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532907187778259999L;
   ...
   */
  public NodePropertyResolver resolverAbuseMessages =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532909999778111999L;

      public Object get(final Node node) {
        final List<AbuseReport> reports = getModerationService()
          .getAbuses(node.getNodeRef());
        if (reports.size() > 0) {
          ...
          for (final AbuseReport report : reports) {
            ...
            final String safeMessage = (message == null ||
                message.trim().length() < 1)
              ? NO_MESSAGE
              : message.replace("\n", " ");
            final String i18nMessage = MessageFormat.format(
              Application.getMessage(fc, MSG_ABUSE_REPORT),
              reporter,
              date,
              safeMessage
            );
   ...
   */
  public NodePropertyResolver resolverRejectMessages =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2532909999778111999L;
      ...
      public Object get(final Node node) {
        if (getModerationService().isRejected(node.getNodeRef())) {
          final FacesContext fc = FacesContext.getCurrentInstance();
          final String moderator = (String) node
            .getProperties()
            .get(ModerationModel.PROP_REJECT_BY.toString());
          final Date date = (Date) node
            .getProperties()
            .get(ModerationModel.PROP_REJECT_ON.toString());
          final String message = (String) node
            .getProperties()
          ...
          final String safeMessage = (message == null ||
            ...
            : message.replace("\n", " ");
          final String i18nMessage = MessageFormat.format(
            Application.getMessage(fc, MSG_REJECT_DETAILS),
            moderator,
            date,
            safeMessage
          );
   ...
   * Resolver to the I18N message for Reject a moderated post
   */
  public NodePropertyResolver resolverHasAttachement =
    new NodePropertyResolver() {
   ...
   * Resolver to get the node creator profile
   */
  public NodePropertyResolver resolverCreatorAvatar =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -1132907187778250014L;

      public Object get(final Node node) {
        final PersonService personService = getServiceRegistry()
        ...
        if (creator != null && personService.personExists(creator)) {
          try {
            ...
            if (logger.isErrorEnabled()) {
              logger.error(
                "Impossible to retreive avatar of " + creator,
                ignore
              );
            }
          }
   ...
   * Resolver to get the node creator profile
   */
  public NodePropertyResolver resolverCreatorSignature =
    new NodePropertyResolver() {
  ...
  public void setupContentAction(String id, boolean invalidate) {
    ...
    if (node != null) {
      node.addPropertyResolver(
        ...
        this.resolverAlternativeDownload
      );
      node.addPropertyResolver(
        ALTERNATIVE_BROWSE_URL,
        this.resolverAlternativeBrowse
      );
    }
  }
  ...
  public void clickCircabcHome() {
    ...
    if (igHomeNode != null) {
      ...
      WaiApplication.getNavigationManager()
      ...
      new RetryingTransactionCallback<Object>() {
        public Object execute() throws Exception {
          ...
          if (component instanceof UINavigationList) {
            final UINavigationList navigationList =
              (UINavigationList) component;
  ...
  public void clickWai(final String id) {
    if (id != null && id.length() != 0) {
      try {
        ...
        if (
          ApplicationModel.TYPE_FOLDERLINK.equals(getNodeService().getType(ref))
        ) {
          ref = (NodeRef) getNodeService()
            .getProperty(ref, ContentModel.PROP_LINK_DESTINATION);

          fc
            .getExternalContext()
            .getSessionMap()
            .put(
              LibraryBean.FROM_LINK_TO_SHARED,
              new NodeRef(Repository.getStoreRef(), id)
            );
          fc
            .getExternalContext()
            .getSessionMap()
            .put(LibraryBean.TARGET_GROUP_LINK_TO_SHARED, ref);
        }
  ...
  public void clickWai(final NodeRef ref) {
    ...
    boolean canRead =
      getPermissionService().hasPermission(ref, PermissionService.READ) ==
      AccessStatus.ALLOWED;
    boolean isLibNoAccess =
      getPermissionService()
        .hasPermission(ref, LibraryPermissions.LIBNOACCESS.toString()) ==
      AccessStatus.ALLOWED;
  ...
  private void clickWaiSecure(final NodeRef ref) {
    ...
    if (!refToSend.equals(ref)) {
      ...
      final Serializable nodeName = getNodeService()
        .getProperty(refToSend, ContentModel.PROP_NAME);
      Utils.addErrorMessage(
        MessageFormat.format(
          Application.getMessage(fc, MSG_REDIRECTED_TO),
          nodeName
        )
      );
    }
    ...
    if (typeDef != null) {
      ...
      if (
        (ContentModel.TYPE_FOLDER.equals(modelType) ||
          getDictionaryService()
            .isSubClass(modelType, ContentModel.TYPE_FOLDER)) &&
        (ApplicationModel.TYPE_FOLDERLINK.equals(modelType) ||
          getDictionaryService()
            .isSubClass(modelType, ApplicationModel.TYPE_FOLDERLINK))
      ) {
        setActionSpace(node);
  ...
  public void clickWaiIgService(final ActionEvent event) {
    ...
    applyWaiBrowsing(
      getCircabcNavigator().getCurrentIGService().getId(),
      false
    );
  }
  ...
  public void clickNodeDiscussion(final ActionEvent event) {
    ...
    if (id != null && id.length() != 0) {
      try {
        ...
        if (
          getNodeService().hasAspect(ref, ForumModel.ASPECT_DISCUSSABLE) ==
          false
        ) {
          if (logger.isErrorEnabled()) {
            logger.error(
              "discuss called for an object that does not have a discussion! " +
              id
            );
          }
  ...
  public void clickNativeSpace(final ActionEvent event) {
    ...
    if (
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "node is a multilingual node, get the pivot translation:" + ref
        );
      }
    }
  ...
  public void searchWai(final ActionEvent event) {
    ...
    final UIViewRoot viewRoot = viewHandler.createView(
      fc,
    ...
    if (nodeId != null) {
      ...
      getCircabcNavigator()
        .setupDispatchContext(
          new Node(new NodeRef(Repository.getStoreRef(), nodeId))
        );

      if (informListeners) {
        // inform any listeners that the current space has changed
        UIContextService.getInstance(
          FacesContext.getCurrentInstance()
        ).spaceChanged();
      }
  ...
  public int getListElementNumber() {
    if (cachedListElementNumber == null) {
      final String numberStr = CircabcConfiguration.getProperty(
  ...
  private List<FileInfo> listAllFiles(final NodeRef nodeRef) {
    ...
    for (final FileInfo fileInfo : getFileFolderService()
      .listFolders(nodeRef)) {
  ...
  private NodeRef getSecureNodeRef(final NodeRef fromNodeRef) {
    if (
      getNodeService().exists(fromNodeRef) &&
      getPermissionService()
        .hasPermission(fromNodeRef, PermissionService.READ) ==
      AccessStatus.ALLOWED
    ) {
      return fromNodeRef;
    } else {
      if (logger.isWarnEnabled()) {
        if (!getNodeService().exists(fromNodeRef)) {
          logger.warn(
            "Node reference does not exists : " + fromNodeRef.toString()
          );
        } else if (
          getPermissionService()
            .hasPermission(fromNodeRef, PermissionService.READ) !=
          ...
          logger.warn(
            ...
            " does not have read permission on node  : " +
      ...
      if (isNodeSecure(getDocument())) {
        ...
        isNodeSecure(getCircabcNavigator().getCurrentCategoryHeader())
      ) {
  ...
  private boolean isNodeSecure(final NodeRef nodeRef) {
    if (nodeRef == null) {
      ...
      if (
        getPermissionService().hasPermission(nodeRef, PermissionService.READ) ==
        AccessStatus.ALLOWED
      ) {
  ...
  protected final CircabcServiceRegistry getServiceRegistry() {
    if (serviceRegistry == null) {
      serviceRegistry = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
      );
    }
  ...
  public String getBannerRenderPropertyName() {
    ...
    if (
       ...
       */
      renderPropertyName = categoryNode
        .getProperties()
        .get(CircabcModel.PROP_NAVIGATION_LIST_RENDER_TYPE.toString())
        .toString();
    } else {

...
public class CircabcCCCheckoutFileDialog extends BaseWaiDialog {
  ...
  public void checkoutFileDirect(ActionEvent event) {
    ...
    NodeRef igRef = getManagementService()
      .getCurrentInterestGroup(getActionNode().getNodeRef());
    logRecord.setIgID(
      (Long) getNodeService().getProperty(igRef, ContentModel.PROP_NODE_DBID)
    );
    logRecord.setIgName(
      getNodeService().getProperty(igRef, ContentModel.PROP_NAME).toString()
    );
    ...
    try {
      ...
      Utils.addErrorMessage(
        translate(MSG_FAILURE, getActionNode().getName(), t.getMessage())
      );
      getLogService().log(logRecord);
    }
  }

...
public abstract class IEBaseDownloadContentServlet extends BaseServlet {
  ...
  private static final String POWER_POINT_DOCUMENT_MIMETYPE =
    "application/vnd.powerpoint";
  private static final String POWER_POINT_2007_DOCUMENT_MIMETYPE =
  ...
  protected transient ThreadLocal<Boolean> permissionDenied =
    new ThreadLocalFalse();
   ...
   */
  protected static String generateUrl(
    String pattern,
    NodeRef ref,
    String name
  ) {
  ...
  protected void processDownloadRequest(
    ...
    boolean attachment =
      URL_ATTACH.equals(attachToken) || URL_ATTACH_LONG.equals(attachToken);
    ...
    try {
      ...
      if (
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "User does not have permissions to read content for NodeRef: " +
            nodeRef.toString()
          );
        }
      ...
      if (tokenCount > 6) {
        ...
        try {
          NodeRef parentRef = serviceRegistry
            .getNodeService()
      ...
      if (modified != null) {
        ...
        setHeader(
          res,
          HEADER_ETAG,
          "\"" + Long.toString(modified.getTime()) + "\"",
          req
        );
      }
      ...
      String headerContentDispo =
        CommonUtils.generateFilenameContentDispositionHeader(
          attachment,
          filename,
          req
        );
      ...
      if (
        !attachment &&
        (mimetype.equals(POWER_POINT_2007_DOCUMENT_MIMETYPE) ||
      ...
      throw new AlfrescoRuntimeException(
        "Error during download content servlet processing: " + err.getMessage(),
        err
      );
    }
  ...
  private boolean processMultiRange(
    ...
    // get the content reader
    ContentService contentService = getServiceRegistry(
      getServletContext()
    ).getContentService();
  ...
  private void streamRangeBytes(
    ...
    long bytesLeft = span;
    int read = 0;
    byte[] buf = new byte[((int) bytesLeft) < CHUNKSIZE
      ? (int) bytesLeft
      : CHUNKSIZE];
    while ((read = is.read(buf)) > 0 && bytesLeft != 0L) {
      ...
      if (bytesLeft != 0L) {
        int resize = ((int) bytesLeft) < CHUNKSIZE
          ? (int) bytesLeft
          : CHUNKSIZE;

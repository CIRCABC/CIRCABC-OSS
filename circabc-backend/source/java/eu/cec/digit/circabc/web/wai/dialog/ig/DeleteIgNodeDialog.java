...
public class DeleteIgNodeDialog extends DeleteNodeDialog {
  ...
  private static final String MSG_IG_CONTAINS_LOCKED_DOCUMENTS =
    "ig_contains_locked_documents";
   ...
   */
  private transient NodeArchiveService nodeArchiveService;
  ...
  public void init(final Map<String, String> parameters) {
    try {
      ...
      final List<Profile> profiles = profileManagerService.getProfiles(
        igNodeRef
      );
      ...
      for (final Profile profile : profiles) {
        if (profile.isExported()) {
          igs = new ArrayList<>(
            profileManagerService.getRefInExportedProfile(
              igNodeRef,
      ...
      sharedSpaces.clear();
      final List<ShareSpaceItem> spItems = getLinksBusinessSrv()
        .findSharedSpaces(igNodeRef);
      if (spItems != null) {
      ...
      logRecord.setIgID(
        (Long) getNodeService()
          .getProperty(
            getManagementService().getCurrentCategory(igNodeRef),
      ...
      logRecord.setInfo(
        "Ig Title: " +
        (getNodeService().getProperty(igNodeRef, ContentModel.PROP_TITLE) !=
            null
            ? getNodeService()
      ...
      if (archivedNode != null) {
        if (this.getNodeService().exists(archivedNode)) {
          ...
          if (logger.isWarnEnabled()) {
            ...
            logger.warn(
              "Archive node " + archivedNode.toString() + " does not exists"
            );
          }
        }
      }
  ...
  public List<ProfilesListWrapper> getProfiles() {
    ...
    try {
      ...
      for (final Entry<
        Profile,
        List<NodeRef>
      > entry : profilesExportedUsedByIG.entrySet()) {
    ...
    try {
      ...
      if (getActionNode() != null && actionNodeType != null) {
        ...
        if (logger.isDebugEnabled()) {
          ...
          logger.debug(
            "Trying to delete Interest Group: " +
            name +
            " with ID:" +
            getActionNode().getId()
          );
        ...
        try {
          ...
          if (purgeData) {
            this.getNodeService()
              .addAspect(currentNode, ContentModel.ASPECT_TEMPORARY, null);
          }
          this.getNodeService().deleteNode(currentNode);
          Utils.addStatusMessage(
            FacesMessage.SEVERITY_INFO,
            translate(MSG_SUCCESS, currentName)
          );
        } catch (final NodeLockedException nle) {
          interestGroupID = -1;
          Utils.addErrorMessage(
            Application.getMessage(context, MSG_IG_CONTAINS_LOCKED_DOCUMENTS)
          );
          return null;
        } finally {
        ...
        archivedNode = this.getNodeArchiveService()
          .getArchivedNode(currentNode);
      } else {
        if (logger.isWarnEnabled()) {
  ...
  public NodeArchiveService getNodeArchiveService() {
    ...
    if (nodeArchiveService == null) {
      nodeArchiveService = (NodeArchiveService) FacesHelper.getManagedBean(
        FacesContext.getCurrentInstance(),
        "nodeArchiveService"
      );
    }

...
public class UISortLink extends UICommand {
  ...
  public void encodeBegin(FacesContext context) throws IOException {
    ...
    HtmlFormRendererBase.renderScrollHiddenInputIfNecessary(
      RendererUtils.findNestingForm(this, context).getForm(),
      context,
  ...
  public void broadcast(FacesEvent event) throws AbortProcessingException {
    if (event instanceof SortEvent == false) {
      ...
      if (s_logger.isDebugEnabled()) {
        s_logger.debug(
          "Handling sort event for column: " + ((SortEvent) event).Column
        );
      }

      if (
        getColumn().equals(getDataContainer().getCurrentSortColumn()) == true
      ) {

...
public class EventBean extends InterestGroupBean {
  ...
  private static final String EVENT_VIEW_EVENT_MEETING =
    "event_view_event_meeting";
  private static final String EVENT_VIEW_EVENT_MEETING_TOOLTIP =
    "event_view_event_meeting_tooltip";
  private static final String WAI_DIALOG_VIEW_EVENTS_MEETINGS_WAI =
    "wai:dialog:viewEventsMeetingWai";

  private static final String EVENT_CREATE_EVENT = "event_create_event";
  private static final String EVENT_CREATE_EVENT_TOOLTIP =
    "event_create_event_tooltip";
  private static final String WAI_WIZARD_CREATE_EVENT_WAI =
    "wai:wizard:createEventWai";

  private static final String EVENT_CREATE_MEETING = "event_create_meeting";
  private static final String EVENT_CREATE_MEETING_TOOLTIP =
    "event_create_meeting_tooltip";

  private static final String WAI_WIZARD_CREATE_MEETING_WAI =
    "wai:wizard:createMeetingWai";
  private static final String DIALOG_MANAGER_SETUP_PARAMETERS =
    "DialogManager.setupParameters";
  private static final String WIZARD_MANAGER_SETUP_PARAMETERS =
  ...
  public void init(Map<String, String> parameters) {
    ...
    weekStart = (String) getCurrentNode()
      .getProperties()
  ...
  public Map<DateValue, List<AppointmentUnit>> getAppointmentUnitByDay(
    final Calendar from,
    final Calendar to
  ) {
    // Check why this is necessary for SOLR
    if (
      "solr".equals(CircabcConfiguration.getProperty("index.subsystem.name")) &&
      getViewMode().equals(UICalendar.VIEW_MODE_DAY)
    ) {
      to.add(Calendar.DAY_OF_MONTH, 1);
    }

    final DateValue fromDateValue = AppointmentUtils.convertDateToDateValue(
      from.getTime()
    );
    final DateValue toDateValue = AppointmentUtils.convertDateToDateValue(
      to.getTime()
    );
    ...
    final Map<DateValue, List<AppointmentUnit>> appointmentMap = new HashMap<>(
      dateDiff + 1
    );
    ...
    for (final EventItem item : appointements) {
      ...
      unit.setType(
        item.getEventType().equals(AppointmentType.Event) ? "E" : "M"
      );
    ...
    for (Map.Entry<
      DateValue,
      List<AppointmentUnit>
    > entry : appointmentMap.entrySet()) {
  ...
  public List<ActionWrapper> getActions() {
    ...
    if (
      type.equals(NavigableNodeType.EVENT) ||
      type.equals(NavigableNodeType.EVENT_CHILD)
    ) {
      NavigableNode enventRoot = (NavigableNode) getNavigator()
        .getCurrentIGRoot()

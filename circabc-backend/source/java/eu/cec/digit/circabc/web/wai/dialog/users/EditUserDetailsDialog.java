...
public class EditUserDetailsDialog extends BaseUserDetailsBean {
  ...
  private static final String MSG_ERR_UNEXPECTED_ERROR =
    "edit_user_details_erreor_unexpected";
  private static final String MSG_AVATAR_DELETED =
    "edit_user_details_avatar_del_success";
  private static final String MSG_DATA_LOADED =
    "edit_user_details_reload_success";
  private static final String MSG_SUCCESS = "edit_user_details_success";
  private static final Log logger = LogFactory.getLog(
    EditUserDetailsDialog.class
  );
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    userDetails.setSignature(
      getSecurityService().getCleanHTML(userDetails.getSignature(), true)
    );
    ...
    try {
      ...
      final Map<QName, Serializable> updatedPreferences =
        userDetails.getUpdatedPreferences();
      if (updatedPreferences.containsKey(UserService.PREF_INTERFACE_LANGUAGE)) {
        final UserPreferencesBean userPreferencesBean =
          getUserPreferencesBean();
        final String language = (String) updatedPreferences.get(
      ...
      if (
        updatedPreferences.containsKey(UserService.PREF_CONTENT_FILTER_LANGUAGE)
      ) {
        final UserPreferencesBean userPreferencesBean =
          getUserPreferencesBean();
        final Serializable contentLang = updatedPreferences.get(
        ...
        if (contentLang instanceof String) {
          ...
          userPreferencesBean.setContentFilterLanguage(
            ((Locale) contentLang).getLanguage()
          );
          ...
          if (logger.isErrorEnabled()) {
            logger.error(
              "PREF_CONTENT_FILTER_LANGUAGE has invalid type not String neither Local"
            );
          }
        }
      }
  ...
  public void getFromLdap(ActionEvent event) {
    try {
      getBusinessRegistry()
        .getRemoteUserBusinessSrv()
        .reloadDetails(getUserDetails());

      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,
        translate(MSG_DATA_LOADED)
      );
    } catch (final BusinessStackError validationErrors) {
      for (final String msg : validationErrors.getI18NMessages()) {
        Utils.addErrorMessage(msg);
  ...
  public void removeAvatar(ActionEvent event) {
    try {
      ...
      getUserDetails()
        .setAvatar(
          userDetailsBusinessSrv.getAvatar(getUserDetails().getNodeRef())
        );

      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,
        translate(MSG_AVATAR_DELETED)
      );
    } catch (final BusinessStackError validationErrors) {
      for (final String msg : validationErrors.getI18NMessages()) {
        Utils.addErrorMessage(msg);
  ...
  public void launchUpdateAvatarDialog(ActionEvent event) {
    final UpdateAvatarDialog dialog = (UpdateAvatarDialog) Beans.getBean(
      UpdateAvatarDialog.BEAN_NAME
    );
    dialog.start(event);
  }
  ...
  public boolean isReloadbuttonAvailable() {
    return getBusinessRegistry()
      .getRemoteUserBusinessSrv()
      .isRemoteManagementAvailable();
  }
  ...
  public final String getContentFilterLanguage() {
    final Locale contentFilterLanguage = getUserDetails()
  ...
  public final void setContentFilterLanguage(String contentFilterLanguage) {
    if (
      contentFilterLanguage == null ||
      contentFilterLanguage.equalsIgnoreCase(
        UserPreferencesBean.MSG_CONTENTALLLANGUAGES
      )
    ) {
      ...
      getUserDetails()
        .setContentFilterLanguage(I18NUtil.parseLocale(contentFilterLanguage));
    }
  }

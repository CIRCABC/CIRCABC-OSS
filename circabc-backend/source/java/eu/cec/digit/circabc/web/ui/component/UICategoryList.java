...
public class UICategoryList extends SelfRenderingComponent {
  ...
  public void encodeBegin(final FacesContext context) throws IOException {
    ...
    final List<CategoryHeader> categoryHeaders = (List<
        CategoryHeader
      >) vbValue.getValue(getFacesContext());

    final ValueBinding vbChooseHeader = getValueBinding("chooseHeader");

    final String chooseHeader =
      (vbChooseHeader != null
          ? (String) vbChooseHeader.getValue(getFacesContext())
          : "");

    final Map<?, ?> attrs = this.getAttributes();
    final Boolean displayCategories =
      ((Boolean) (attrs.get("displayCategories") != null)
          ? (Boolean) attrs.get("displayCategories")
          : Boolean.FALSE);

    final ResponseWriter writer = context.getResponseWriter();

    writer.append(
      "<div id=\"panelCatListHeaderGlobal\" class=\"panelCatListGlobal\">"
    );
    writer.append(
      "<div id=\"panelCatListHeader\" class=\"panelCatListLabel\" >"
    );
    ...
    if (displayCategories) {
      ...
      for (final CategoryHeader tmp : categoryHeaders) {
        ...
        final String categoryHeaderTitle = (String) navigableNode.getTitle();
        final String categoryHeaderId = (String) navigableNode.getId();

        writer
          .append("<div id=\"panel")
          .append(categoryHeaderId)
  ...
  private void generateGoToTop(final ResponseWriter writer, final int index)
    ...
    writer
      .append(" id=\"topOfPageAnchorCatListEndImg")
  ...
  private void generateTopAnchorImg(final ResponseWriter writer)
    ...
    writer
      ...
      .append("/images/extension/top_ns.gif\"");
    writer.append(" alt=\"Back to the top of the page\"");
    writer.append(" title=\"Back to the top of the page\" />");
  }

  private void generateCategoryHeadersHasList(
    final ResponseWriter writer,
    final List<CategoryHeader> categoryHeaders
  ) throws IOException {
    ...
    if (categoryHeaderCount <= 10) {
      ...
      int currentRow = 1;
      for (final CategoryHeader tmp : categoryHeaders) {
        final String liClass = (currentRow % 2 == 0)
          ? "recordSetRow"
          : "recordSetRowAlt";
        writer.append("<li class=\"").append(liClass).append("\">");
        writer
          .append("<a href=\"#n")
          .append(tmp.getCategoryHeaderItem().getId())
          .append("\"");
        writer
          .append(" >")
          .append(tmp.getCategoryHeaderItem().getTitle())
          .append("</a>");
        writer.append("</li>");
        currentRow++;
      }
      ...
      final int maxCategoryHeaderByList =
        (categoryHeaders.size() / desiredColumnCount) + 1;

      final Iterator<CategoryHeader> categoryHeaderIterator =
        categoryHeaders.iterator();

      while (categoryHeaderIterator.hasNext()) {
        int currentCategoryHeaderCount = 0;
        final List<CategoryHeader> subCategoryHeaderList = new ArrayList<>(10);
        while (
          categoryHeaderIterator.hasNext() &&
        ...
        writer.append(
          "<ul class=\"categoryListItem\" style=\"width:" +
  ...
  private void generateCategoriesHasList(
    final ResponseWriter writer,
    final CategoryHeader tmp
  ) throws IOException {
    // merge the items
  ...
  private void writeColumn(
    ...
    writer
      .append("<ul class=\"categoryListItem\" style=\"width:")
  ...
  private void writeDummyLink(final ResponseWriter writer, String liClass)
    throws IOException {
    writer.append("<li class=\"").append(liClass).append("\">");
    writer.append("&nbsp;");
    writer.append("</li>");
  ...
  private void writeCategoryLink(
    final ResponseWriter writer,
    ...
    String url =
      path +
      WebClientHelper.getGeneratedWaiUrl(
        category,
  ...
  private void writeExternalLink(
    final ResponseWriter writer,
    CategoryItem externalLink,
    String liClass
  ) throws IOException {
    String path = FacesContext.getCurrentInstance()
      .getExternalContext()
   ...
   */
  protected String getNonJavaScriptAnchor(
    final FacesContext facesContext,
    final String clientId
  ) throws IOException {
    if (logger.isInfoEnabled()) {
      logger.info(
        "UICategoryList : encodeBegin - renderNonJavaScriptAnchorStart - Start"
      );
    }
    final ViewHandler viewHandler = facesContext
      .getApplication()
      .getViewHandler();
    ...
    final FormInfo forInfo = RendererUtils.findNestingForm(this, facesContext);

    final String hiddenFieldName =
      HtmlRendererUtils.getHiddenCommandLinkFieldName(forInfo);
    if (logger.isInfoEnabled()) {
      logger.info(
        "NavigationListRenderer : hiddenFieldName |" + hiddenFieldName + "|"
      );
    }
    ...
    hrefBuf.append("&amp;");
    hrefBuf.append(
      RendererUtils.findNestingForm(this, facesContext).getFormName()
    );
    hrefBuf.append("_SUBMIT");
    hrefBuf.append('=');
    hrefBuf.append(1);

    final StateManager stateManager = facesContext
      .getApplication()
      .getStateManager();
    ...
    if (stateManager.isSavingStateInClient(facesContext)) {
      hrefBuf.append("&amp;");
      logger.info(
        "NavigationListRenderer : isSavingStateInClient |" +
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "UICategoryList : encodeBegin - renderNonJavaScriptAnchorStart - Stop"
      );
    }
    return href;
  }
  ...
  public void restoreState(final FacesContext context, final Object state) {
    ...
    this.value = (List<CategoryHeader>) restoreAttachedState(
      context,
      values[1]
    );
  }

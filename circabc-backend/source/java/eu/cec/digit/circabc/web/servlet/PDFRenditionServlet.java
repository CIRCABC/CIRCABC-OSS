...
public class PDFRenditionServlet extends BaseServlet {
  ...
  public void init() throws ServletException {
    ...
    circabcRenditionHelper =
      (CircabcRenditionHelper) Services.getCircabcServiceRegistry(
        getServletContext()
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (
      ...
      logger.error(
        ...
        "' has no access to the content of document: " +
        documentId
      );
      resp.getWriter().write("");
      return;
    ...
    try {
      ...
      if ("content".equals(response)) {
        ...
        resp.getWriter().write(req.getContextPath() + "/" + tempFile.getName());
      }
   ...
   */
  private File getDocumentTempFile(
    ...
    if (path != null && path.endsWith(WEB_INF_CLASSES_SUFFIX)) {
      path = path.substring(0, path.length() - WEB_INF_CLASSES_SUFFIX.length());
    } else if (path == null) {
      return null;
    }
    ...
    if (reader == null) {
      logger.error(
        "Reader is null when accessing " + "the content node. Unexpected error."
      );
      return null;
    }
    ...
    // URL or image
    if (
      reader.getMimetype().startsWith("image") ||
    ...
    if (success) {
      ...
      if (!deleted) {
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Could not delete temp file name: " +
            tempFile.getName() +
            " path: " +
            tempFile.getPath()
          );
   ...
   */
  protected boolean fillTempFile(
    final File tempFile,
    final NodeRef currentNodeRef,
          ...
          RunAsWork<Boolean> work = new RunAsWork<Boolean>() {
            ...
            public Boolean doWork() throws Exception {
              ...
              if (!convertible) {
                ...
                // Rendition pdf
                renditionNodeRef = circabcRenditionHelper.getRenditionNodeRef(
                  currentNodeRef
                );
              }

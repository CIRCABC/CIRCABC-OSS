...
public class TranslatePropertyDialog extends BaseWaiDialog {
  ...
  public static final String MSG_ERROR_LOCALE_REQUIRED =
    "translate_property_dialog_error_locale_required";
  public static final String MSG_ERROR_VALUE_REQUIRED =
    "translate_property_dialog_error_value_required";
  public static final String MSG_ERROR_LOCALE_DUPLICATED =
    "translate_property_dialog_error_locale_duplicated";
  ...
  public void init(final Map<String, String> parameters) {
    ...
    if (parameters != null) {
      ...
      propertyId = parameters.get(
        BaseDialogLauncherGenerator.PARAM_PROPERTY_KEY
      );
    ...
    if (idx < 0) {
      ...
      final String uri = getNamespaceService()
        .getNamespaceURI(propertyId.substring(0, idx));
    ...
    final MLText values = getMLPropertyValue(
    ...
    if (values != null) {
      ...
      for (Map.Entry<Locale, String> translation : values.entrySet()) {
        propertyTranslation.add(
          new MLPropertyWrapper(
            getSecurityService().getCleanHTML(translation.getValue(), false),
            translation.getKey()
          )
        );
      }
    ...
    if (
      !"".equals(
          CircabcConfiguration.getProperty(CircabcConfiguration.MT_ENABLED)
        )
    ) {
      machineTranslationEnabled = Boolean.parseBoolean(
        CircabcConfiguration.getProperty(CircabcConfiguration.MT_ENABLED)
      );
    }
  }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Trying to add translation to the property " +
        propertyId +
        " of node " +
    ...
    try {
      ...
      getNodeService()
        .setProperty(getActionNode().getNodeRef(), this.propertyQname, mlText);
    } finally {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Property " +
        propertyId +
        "  successfully updated for the node " +
        getActionNode().getName() +
        "\n	node: " +
        getActionNode().getNodeRef() +
        "\n	prop: " +
    ...
    if (getCircabcService().syncEnabled()) {
      if (
        (propertyId.equals("title") ||
          propertyId.equals(CI_LIGHT_DESCRIPTION)) &&
        igNodeRef != null
      ) {
  ...
  public void addSelection(ActionEvent event) {
    final UIInput inputText = (UIInput) event
      .getComponent()
      .findComponent("translate-property-value-input");
    final UIInput inputArea = (UIInput) event
      .getComponent()
    ...
    final String selValue = inputText != null
      ? ((String) inputText.getValue()).trim()
  ...
  private void addSelectionImpl(final String selLang, final String selValue) {
    ...
    if (!error) {
      propertyTranslation.add(
        new MLPropertyWrapper(finalValue, new Locale(selLang))
      );
      this.value = null;
    }
  }
  ...
  public String getDialogCloseAndLaunchAction() {
    ...
    final UIInput inputText = (UIInput) currentInstance
      .getViewRoot()
      .findComponent("FormPrincipal:translate-property-value-input");
    final UIInput inputArea = (UIInput) currentInstance
      .getViewRoot()
      .findComponent("FormPrincipal:translate-property-value-area");
    final UISelectOne select = (UISelectOne) currentInstance
      .getViewRoot()
  ...
  public void removeSelection(ActionEvent event) {
    MLPropertyWrapper wrapper =
      (MLPropertyWrapper) this.translationDataModel.getRowData();
    if (wrapper != null) {
  ...
  public void saveSelection(ActionEvent event) {
    ...
    if (!error) {
      propertyTranslation.set(
        index,
        new MLPropertyWrapper(finalValue, new Locale(selectedLanguage))
      );
  ...
  public String getContainerDescription() {
    return translate(
      "translate_property_dialog_description",
      i18nPropety,
      getActionNode().getName()
    );
  }
  ...
  private boolean isTextArea(final Map<String, String> parameters) {
    if (parameters == null) {
      ...
      final String area = parameters.get(
        TranslateLongTextPropertyGenerator.PARAM_PROPERTY_TEXT_AREA
      );
  ...
  private boolean isRichTextArea(Map<String, String> parameters) {
    if (parameters == null) {
      ...
      final String area = parameters.get(
        TranslateLongTextPropertyGeneratorNoRichText.PARAM_PROPERTY_USE_RICH_TEXT
      );
  ...
  public String getSelectedLanguageName() {
    String labelByCode = getContentFilterLanguagesService()
      .getLabelByCode(selectedLanguage);
    if (labelByCode == null) {
      return selectedLanguage;
    } else {
      return labelByCode;

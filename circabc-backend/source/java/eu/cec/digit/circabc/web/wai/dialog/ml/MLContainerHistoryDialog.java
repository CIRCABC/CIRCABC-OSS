...
public class MLContainerHistoryDialog extends BaseWaiDialog {
  ...
  public void init(Map<String, String> parameters) {
    ...
    final DocumentDetailsDialog docDetails =
    ...
    this.name = (String) getNodeService()
      .getProperty(mlRef, ContentModel.PROP_NAME);

    this.currentEdition = getEditionService()
      .getEditions(mlRef)
      .getVersion(label);

    isLastVersion = getVersionService()
      .getCurrentVersion(mlRef)
      .getVersionLabel()
      .equals(label);
  ...
  private void initTranslations() {
    ...
    if (isLastVersion) {
      for (final NodeRef tr : getMultilingualContentService()
        .getTranslations(this.currentEdition.getVersionedNodeRef())
        .values()) {
        // get the properties of the current translations
        addClientNodeProperties(
          translations,
          tr,
          getNodeService().getProperties(tr)
        );
      }
    } else {
      for (final VersionHistory versionHistory : getEditionService()
        ...
        List<Version> orderedVersions = new ArrayList<>(
        ...
        addClientNodeProperties(
          translations,
          lastVersion.getFrozenStateNodeRef(),
  ...
  private void addClientNodeProperties(
    List<MapNode> clientNodes,
    NodeRef nodeRef,
    ...
    clientLastVersion.put(
      ...
      getContentFilterLanguagesService()
        .convertToNewISOCode(language.getLanguage())
        .toUpperCase()
    );

    if (
      getNodeService()
        .hasAspect(nodeRef, ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION)
    ) {
  ...
  protected final EditionService getEditionService() {
    if (editionService == null) {
      editionService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getEditionService();
    }
  ...
  protected final VersionService getVersionService() {
    if (versionService == null) {
      versionService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getVersionService();
    }

...
public class UICircabcAssociationEditor extends UIAssociationEditor {
  ...
  private static final Log logger = LogFactory.getLog(
    UICircabcAssociationEditor.class
  );
  private static final String FIELD_AVAILABLE = "_available";
  private static final String MSG_WARN_CANNOT_VIEW =
    "warn_cannot_view_target_details";
  ...
  private void customGetAvailableOptions(
    ...
    if (assocDef != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Found " + this.availableOptions.size() + " available options"
        );
      }
    }
  }

  private void getAvailableUserOptions(
    FacesContext context,
    String contains,
    ...
    for (Map.Entry<NodeRef, String> entry : this.getAllUsers(
        context
      ).entrySet()) {
  ...
  private void getAvailableProfileOptions(
    FacesContext context,
    String contains
  ) {
    String safeContains = null;
    ...
    for (Map.Entry<NodeRef, String> entry : this.getAllProfiles(
        context
      ).entrySet()) {
      if (safeContains != null) {
        if (entry.getValue().toLowerCase().contains(safeContains)) {
  ...
  protected void renderAvailableOptions(
    FacesContext context,
    ResponseWriter out,
  ...
  private void customRenderAvailableOptions(
    FacesContext context,
    ResponseWriter out,
    NodeService nodeService,
    String targetType,
    ...
    boolean itemsPresent =
      (this.availableOptions != null && this.availableOptions.size() > 0);
      ...
      for (NodeRef item : this.availableOptions) {
        ...
        if (!item.toString().equals(currentNode.getNodeRef().toString())) {
          if (ContentModel.TYPE_PERSON.equals(nodeService.getType(item))) {
            String fullNameAndUserId = getAllUsers(context).get(item);
            ...
            String profileDisplayName = getAllProfiles(context).get(item);
            ...
            out.write("'>");
            out.write(
              Utils.encode(Repository.getDisplayPath(nodeService.getPath(item)))
            );
            out.write("/");
            out.write(
              Utils.encode(Repository.getNameForNode(nodeService, item))
            );
            out.write("</option>");
          }
        }
  ...
  @Override
  protected void renderExistingAssociation(
    FacesContext context,
    ResponseWriter out,
    NodeService nodeService,
    NodeRef targetRef,
    ...
    if (ContentModel.TYPE_PERSON.equals(nodeService.getType(targetRef))) {
      if (getIgNodeRef() != null) {
        ...
        out.write(
          Utils.encode(User.getFullNameAndUserId(nodeService, targetRef))
        );
      }
    } else if (
      ContentModel.TYPE_AUTHORITY_CONTAINER.equals(
        nodeService.getType(targetRef)
      )
    ) {
      if (getIgNodeRef() != null) {
        ...
        if (groupDisplayName == null || groupDisplayName.length() == 0) {
          String group = (String) nodeService.getProperty(
            targetRef,
            ContentModel.PROP_AUTHORITY_NAME
          );
          groupDisplayName = group.substring(
            PermissionService.GROUP_PREFIX.length()
          );
        }
      ...
      PermissionService permissionService = Repository.getServiceRegistry(
        context
      ).getPermissionService();
      if (
        permissionService.hasPermission(targetRef, PermissionService.READ) ==
        AccessStatus.ALLOWED
      ) {
        out.write(
          Utils.encode(
            Repository.getDisplayPath(nodeService.getPath(targetRef))
          )
        );
        out.write("/");
        out.write(
          Utils.encode(Repository.getNameForNode(nodeService, targetRef))
        );
      } else {
        accessDenied = true;
    ...
    if (accessDenied) {
      ...
      out.write(
        generateFormSubmit(
          context,
          ACTION_REMOVE + ACTION_SEPARATOR + targetRef.toString()
        )
      );
      out.write("\"><img src='");
      out.write(context.getExternalContext().getRequestContextPath());
      out.write(
        "/images/icons/delete.gif' border='0' width='13' height='16'/></a>"
      );
      ...
      if (!allowMany) {
        ...
        out.write(
          generateFormSubmit(
            context,
            ACTION_CHANGE + ACTION_SEPARATOR + targetRef.toString()
          )
        );
        out.write("\"><img src='");
        out.write(context.getExternalContext().getRequestContextPath());
        out.write(
          "/images/icons/edit_icon.gif' border='0' width='12' height='16'/></a>"
        );
      }
    }
    out.write("</td></tr>");
  ...
  private Map<NodeRef, String> getAllProfiles(FacesContext context) {
    if (allProfiles == null) {
      ...
      try {
        ...
        List<Profile> profilesList = getProfileService()
          .getProfiles(getIgNodeRef());
        this.allProfiles = new HashMap<>(profilesList.size());

        // get the NodeRef for each matching group
        AuthorityDAO authorityDAO =
          (AuthorityDAO) FacesContextUtils.getRequiredWebApplicationContext(
            context
          ).getBean("authorityDAO");
        if (authorityDAO != null) {
          for (Profile profile : profilesList) {
            if (profile == null) {
              continue;
            }
            NodeRef groupRef = authorityDAO.getAuthorityNodeRefOrNull(
            ...
            if (
              ...
              !profileDisplayName.equalsIgnoreCase(
                AuthenticationUtil.getGuestUserName()
              )
            ) {
              allProfiles.put(groupRef, profileDisplayName);
            }
          }
        }
  ...
  private Map<NodeRef, String> getAllUsers(FacesContext context) {
    if (allUsers == null) {
      ...
      try {
        ...
        Set<String> invitedUsers = getProfileService()
          .getInvitedUsersProfiles(igNodeRef)
          .keySet();
        PersonService personService =
          (PersonService) FacesContextUtils.getRequiredWebApplicationContext(
            context
          ).getBean("PersonService");

        NodeService nodeService =
          (NodeService) FacesContextUtils.getRequiredWebApplicationContext(
            context
          ).getBean("NodeService");
  ...
  @Override
  protected void renderReadOnlyAssociations(
    FacesContext context,
    ResponseWriter out,
    NodeService nodeService
  ) throws IOException {
    if (this.originalAssocs.size() > 0) {
      ...
      for (Object o : this.originalAssocs.values()) {
        ...
        if (nodeService.exists(targetNode)) {
          if (
            ContentModel.TYPE_PERSON.equals(nodeService.getType(targetNode))
          ) {
            if (getIgNodeRef() != null) {
              ...
              out.write(
                Utils.encode(User.getFullNameAndUserId(nodeService, targetNode))
              );
            }
          } else if (
            ContentModel.TYPE_AUTHORITY_CONTAINER.equals(
              nodeService.getType(targetNode)
            )
          ) {
            if (getIgNodeRef() != null) {
            ...
            try {
              displayString =
                Repository.getDisplayPath(nodeService.getPath(targetNode)) +
                "/" +
                Repository.getNameForNode(nodeService, targetNode);
            } catch (AccessDeniedException ade) {
              displayString = Application.getMessage(
                context,

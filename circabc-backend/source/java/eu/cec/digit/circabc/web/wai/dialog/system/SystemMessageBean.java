...
public class SystemMessageBean {
  ...
  public void updateProperties(boolean showMessage, String message) {
    ...
    this.getNodeService()
      .setProperties(this.getSystemMessageNode(), properties);
  }
  ...
  private NodeRef getSystemMessageNode() {
    ...
    List<ChildAssociationRef> children = nodeService.getChildAssocs(
      rootRef,
      Collections.singleton(SystemMessageModel.TYPE_SYSTEMMESSAGE)
    );
    if (children.size() == 0) {
      try {
        ...
        logger.error(
          "Error when create system message,  user " +
  ...
  private NodeRef getSystemMessageNodeLucene() {
    if (messageNodeRef == null) {
      ...
      NodeRef rootRef = Beans.getWaiNavigator()
        .getCircabcHomeNode()
        .getNodeRef();
        ...
        if (results.length() > 0) {
          ...
          try {
            ...
            logger.warn(
              "User " +
              AuthenticationUtil.getFullyAuthenticatedUser() +
              " has no right to access the System Message Node",
              ade
            );
          }
        }

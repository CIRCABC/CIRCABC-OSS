...
public class TrashcanDialog extends BaseWaiDialog implements IContextListener {
  ...
  private static final String MSG_RECOVERED_ITEM_INTEGRITY_S =
    "recovered_item_integrity_short";
  private static final String MSG_RECOVERED_ITEM_PERMISSION_S =
    "recovered_item_permission_short";
  private static final String MSG_RECOVERED_ITEM_PARENT_S =
    "recovered_item_parent_short";
  private static final String MSG_RECOVERED_ITEM_FAILURE_S =
    "recovered_item_failure_short";
  ...
  private static final String NAME_ATTR = Repository.escapeQName(
    ContentModel.PROP_NAME
  );
  private static final String DATE_ATTR = Repository.escapeQName(
  ...
  private static final String SEARCH_TEXT =
    "PARENT:\"%s\" AND ASPECT:\"%s\" AND TEXT:%s" +
    "  AND @" +
    PROP_IG_ROOT_NODE_ID_ARCHIVED_ATTR +
    ":\"%s\"";
  private static final String SEARCH_NAME_QUOTED =
    "PARENT:\"%s\" AND ASPECT:\"%s\" AND @" +
    NAME_ATTR +
    ":\"%s\"" +
    "  AND @" +
    PROP_IG_ROOT_NODE_ID_ARCHIVED_ATTR +
    ":\"%s\"";
  private static final String SEARCH_TEXT_QUOTED =
    "PARENT:\"%s\" AND ASPECT:\"%s\" AND TEXT:\"%s\"" +
    "  AND @" +
    PROP_IG_ROOT_NODE_ID_ARCHIVED_ATTR +
    ":\"%s\"";
  ...
  private transient SearchService internalSearchService;
  private NodePropertyResolver resolverLocationPath =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -2501720368642759082L;

      public Object get(Node node) {
        final ChildAssociationRef childRef = (ChildAssociationRef) node
          .getProperties()
  ...
  private NodePropertyResolver resolverDisplayPath =
    new NodePropertyResolver() {
      ...
      public Object get(Node node) {
        ...
        if (getInternalNodeService().exists(childRef.getParentRef())) {
          return Repository.getDisplayPath(
            getInternalNodeService().getPath(childRef.getParentRef()),
            true
          );
        } else {
          return "";
  ...
  private NodePropertyResolver resolverFileType32 =
    new TrashcanDialog.ResolverFileType32();
  private NodePropertyResolver resolverLargeIcon =
    new TrashcanDialog.ResolverLargeIcon();
  private NodePropertyResolver resolverMimetype =
    new TrashcanDialog.ResolverMimetype();
  private NodePropertyResolver resolverSize = new TrashcanDialog.ResolverSize();
  private NodePropertyResolver resolverEncoding =
    new TrashcanDialog.ResolverEncoding();
  private NodePropertyResolver resolverDeletedDate =
    new TrashcanDialog.ResolverDeletedDate();
  private NodePropertyResolver resolverDeletedBy =
    new TrashcanDialog.ResolverDeletedBy();
  private NodePropertyResolver resolverIsFolder = new NodePropertyResolver() {
    ...
    public Object get(Node node) {
      return getDictionaryService()
        .isSubClass(node.getType(), ContentModel.TYPE_FOLDER);
    }
  };
  // Added methods
    ...
    if (isAdminUser() == false) {
      msg =
        ...
        MessageFormat.format(
          Application.getMessage(fc, MSG_DELETED_ITEMS_FOR),
          Application.getCurrentUser(fc).getUserName()
        );
    }
    return msg;
  }
  ...
  public String getListedItemsTable() {
    return buildItemsTable(
      property.getListedItems(),
      "recoveredItemsList",
      false,
      true
    );
  ...
  public List<Node> getItems() {
    ...
    try {
      ...
      tx = Repository.getUserTransaction(
        FacesContext.getCurrentInstance(),
        true
      );
      tx.begin();
    ...
    property.setListedItems(
      (itemNodes != null ? itemNodes : Collections.<Node>emptyList())
    );
  ...
  private String buildSearchQuery() {
    String query;
    final NodeRef igRootNode =
      this.getNavigator().getCurrentIGRoot().getNodeRef();
    final String id = igRootNode.getId();
    if (
      property.getSearchText() == null || property.getSearchText().length() == 0
    ) {
      query = String.format(
        SEARCH_ALL,
      ...
      final String safeText = QueryParser.escape(property.getSearchText());
    ...
    if (FILTER_DATE_ALL.equals(property.getDateFilter()) == false) {
      ...
      if (fromDate != null) {
        ...
        final String strFromDate = QueryParser.escape(df.format(fromDate));
  ...
  private String buildItemsTable(
    ...
    try {
      ...
      // outer table
      buf.append(
        "<table width=100% cellspacing=1 cellpadding=1 border=0 class='"
      );
      buf.append(cssClass);
      buf.append("'>");
      // title row
      ...
      for (Node node : items) {
        ...
        buf
          .append("<img width=16 height=16 alt='' src='")
          .append(contextPath)
          .append(img)
          .append("'>");
        buf.append("</td><td>");
        ...
        if (report) {
          ...
          String msg;
          switch (
            (RestoreStatus) node.getProperties().get(PROP_RECOVERSTATUS)
          ) {
          ...
          if (archivedPath) {
            ...
            if (getNodeService().exists(childRef.getParentRef())) {
              buf.append(
                getDisplayPath(
                  getNodeService().getPath(childRef.getParentRef()),
                  true,
                  5
                )
              );
            ...
            buf.append(
              Repository.getNamePath(
                getNodeService(),
                getNodeService().getPath(node.getNodeRef()),
                null,
                "/",
                null
              )
    ...
    if (parameters != null) {
      ...
      if (nodeRef != null) {
        ...
        FacesContext.getCurrentInstance()
          .getExternalContext()
          .getSessionMap()
          .put("currentSpaceNode", currentSpaceNode);
        FacesContext.getCurrentInstance()
          .getExternalContext()
          .getSessionMap()
          .put("currentSpacePath", currentSpacePath);
      }
    }
    clearSearch(null);
  }
  ...
  private NodeRef getRelevantNode() {
    ...
    if (
      actionNode.hasAspect(CircabcModel.ASPECT_LIBRARY) ||
      actionNode.hasAspect(CircabcModel.ASPECT_LIBRARY_ROOT)
    ) {
      nodeRef = this.getManagementService()
        .getCurrentLibrary(actionNode.getNodeRef());
    }
    if (
      actionNode.hasAspect(CircabcModel.ASPECT_NEWSGROUP) ||
      actionNode.hasAspect(CircabcModel.ASPECT_NEWSGROUP_ROOT)
    ) {
      nodeRef = this.getManagementService()
        .getCurrentNewsGroup(actionNode.getNodeRef());
    }
    return nodeRef;
  }
  ...
  public SearchService getInternalSearchService() {
    if (internalSearchService == null) {
      internalSearchService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  private static class ResolverLargeIcon implements NodePropertyResolver {
    ...
    public Object get(Node node) {
      ...
      return (
        "/images/icons/" +
        (icon != null ? icon : CreateSpaceWizard.DEFAULT_SPACE_ICON_NAME) +
        ".gif"
      );
    }
  }
  ...
  private static class ResolverMimetype implements NodePropertyResolver {
    ...
    public Object get(Node node) {
      ContentData content = (ContentData) node
        .getProperties()
  ...
  private static class ResolverSize implements NodePropertyResolver {
    ...
    public Object get(Node node) {
      ContentData content = (ContentData) node
        ...
        .get(ContentModel.PROP_CONTENT.toString());
      return (
        content != null ? Long.valueOf(content.getSize()) : Long.valueOf(0L)
      );
    }
  }
  ...
  private static class ResolverDeletedDate implements NodePropertyResolver {
    ...
    public Object get(Node node) {
      return node
        .getProperties()
        .get(ContentModel.PROP_ARCHIVED_DATE.toString());
    }
  }

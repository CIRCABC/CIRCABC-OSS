...
public class UIClipboardShelfItem
  extends org.alfresco.web.ui.repo.component.shelf.UIClipboardShelfItem {
   ...
   * I18N messages
   */
  private static final String MSG_CLIPBOARD_COPY_TOOLTIP =
    "clipboard_copy_tooltip";
  private static final String MSG_CLIPBOARD_CUT_TOOLTIP =
    "clipboard_cut_tooltip";
  private static final String MSG_CLIPBOARD_ITEM_ICON_FILE_TOOLTIP =
    "clipboard_item_icon_file_tooltip";
  private static final String MSG_CLIPBOARD_ITEM_ICON_FOLDER_TOOLTIP =
    "clipboard_item_icon_folder_tooltip";
  private static final String MSG_CLIPBOARD_PASTE_ALL = "clipboard_paste_all";
  private static final String MSG_CLIPBOARD_PASTE_ALL_TOOLTIP =
    "clipboard_paste_all_tooltip";

  // ------------------------------------------------------------------------------
  // Strongly typed component property accessors
  private static final String MSG_CLIPBOARD_PASTE_ITEM_TOOLTIP =
    "clipboard_paste_item";
  private static final String MSG_CLIPBOARD_PASTE_LINK_TOOLTIP =
    "clipboard_paste_link";
  private static final String MSG_CLIPBOARD_REMOVE_ALL = "clipboard_remove_all";

  // ------------------------------------------------------------------------------
  // Private helpers
  private static final String MSG_CLIPBOARD_REMOVE_ALL_TOOLTIP =
    "clipboard_remove_all_tooltip";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL =
    "clipboard_download_all";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL_TOOLTIP =
    "clipboard_download_all_tooltip";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL_DISABLED =
    "clipboard_download_all_disabled";
  private static final String MSG_CLIPBOARD_REMOVE_ITEM_TOOLTIP =
    "clipboard_remove_item";
  private static final String MSG_CLIPBOARD_TITLE = "clipboard_title";

  // ------------------------------------------------------------------------------
  // Private data
  private static final String MSG_CLIPBOARD_TITLE_COLLAPSED =
    "clipboard_title_collapsed";
  private static final String MSG_CLIPBOARD_TITLE_EXPANDED =
    "clipboard_title_expanded";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL_LIMIT =
    "clipboard_download_all_limit";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL_CUMULATED =
    "clipboard_download_all_cumulated";
  private static final String MSG_CLIPBOARD_DOWNLOAD_ALL_REMAINING =
    "clipboard_download_all_remaining";
  /**
   * Download All action
   */
  ...
  public void restoreState(final FacesContext context, final Object state) {
    ...
    this.downloadAllActionListener = (MethodBinding) restoreAttachedState(
      context,
      values[3]
    );
  }
  ...
  public void decode(final FacesContext context) {
    ...
    if (reqValue != null && reqValue.equals(clientId)) {
      ...
      if (paramValueCollapse != null) {
        ...
        final String paramValueActionClip = (String) requestMap.get(
          "actionClip"
        );
        // decode the values - we are expecting an action identifier and an index
        final int sepIndex = paramValueActionClip.indexOf(
          NamingContainer.SEPARATOR_CHAR
        );
        final int action = Integer.parseInt(
          paramValueActionClip.substring(0, sepIndex)
        );
        final int index = Integer.parseInt(
          paramValueActionClip.substring(sepIndex + 1)
        );
  ...
  public void encodeBegin(final FacesContext context) throws IOException {
    ...
    if (this.collapsed) {
      ...
      outputAttribute(
        out,
        UtilsCircabc.generateHrefPart(context, this, getId(), attributes),
        HREF
      );
      outputAttribute(
        out,
        bundle.getString(MSG_CLIPBOARD_TITLE_COLLAPSED),
        TITLE
      );
      outputAttribute(
        out,
      ...
      out.write(">");
      out.write(
        UtilsCircabc.buildImageTag(
          context,
          WebResourcesCircabc.IMAGE_CLIPBOARD_COLLAPSED,
          bundle.getString(MSG_CLIPBOARD_TITLE_COLLAPSED),
          tempClass
        )
      );
      ...
      out.write(
        UtilsCircabc.buildImageTag(
          context,
          WebResourcesCircabc.IMAGE_CLIPBOARD_EXPANDED,
          bundle.getString(MSG_CLIPBOARD_TITLE_EXPANDED),
          bundle.getString(MSG_CLIPBOARD_TITLE_EXPANDED),
          tempClass
        )
      ...
      if (items.size() != 0) {
        final DictionaryService dd = Repository.getServiceRegistry(
          context
        ...
        for (int i = 0; i < items.size(); i++) {
          ...
          if (
            ...
            totalDownloadSize += computeContentSizeOfFolder(
              item.getNodeRef(),
              context
            );
          }
          ...
          // then the icon item
          type = item.getType();
          isFolder = (ContentModel.TYPE_FOLDER.equals(type) ||
            dd.isSubClass(type, ContentModel.TYPE_FOLDER));
          if (isFolder) {
          ...
          if (isFolder) {
            ...
            out.write(
              DownloadContentServlet.generateBrowserURL(
                item.getNodeRef(),
                item.getName()
              )
            );
          ...
          // output actions
          buildActionLinkImage(
            out,
            ACTION_REMOVE_ITEM,
            i,
            bundle.getString(MSG_CLIPBOARD_REMOVE_ITEM_TOOLTIP),
            WebResources.IMAGE_REMOVE
          );
          out.write(NBSP);
          buildActionLinkImage(
            out,
            ACTION_PASTE_ITEM,
            i,
            bundle.getString(MSG_CLIPBOARD_PASTE_ITEM_TOOLTIP),
            WebResources.IMAGE_PASTE
          );
          if (
            item.supportsLink() &&
            item.getMode() == ClipboardStatus.COPY &&
            (ContentModel.TYPE_LINK.equals(item.getType()) ||
              dd.isSubClass(item.getType(), ContentModel.TYPE_LINK)) ==
            false
          ) {
            out.write(NBSP);
            buildActionLinkImage(
              ...
              ACTION_PASTE_LINK,
              i,
              bundle.getString(MSG_CLIPBOARD_PASTE_LINK_TOOLTIP),
              WebResources.IMAGE_PASTE_LINK
            );
          }
        ...
        if (items.size() > 0) {
          ...
          buildActionLink(
            ...
            ACTION_PASTE_ALL,
            -1,
            bundle.getString(MSG_CLIPBOARD_PASTE_ALL),
          ...
          buildActionLink(
            ...
            ACTION_REMOVE_ALL,
            -1,
            bundle.getString(MSG_CLIPBOARD_REMOVE_ALL),
          ...
          if (totalDownloadSize < downloadLimitMB * MB) {
            ...
            out.write(
              ...
              MessageFormat.format(
                bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL_DISABLED),
                downloadLimitMB
              ) +
              "</i>"
            );
          ...
          out.write("<br/><br/>");
          double remaining =
            (double) downloadLimitMB - (double) totalDownloadSize / MB;
          out.write(
            bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL_LIMIT) +
            ": " +
            downloadLimitMB +
            " MB"
          );
          out.write("<br/>");
          out.write(
            bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL_CUMULATED) +
            ": " +
            String.format("%.2f", (double) totalDownloadSize / MB) +
            " MB"
          );
          out.write("<br/>");
          out.write(
            "<font color=\"red\">" +
            bundle.getString(MSG_CLIPBOARD_DOWNLOAD_ALL_REMAINING) +
            ": " +
            (remaining < 0 ? 0 : String.format("%.2f", remaining)) +
            " MB</font>"
          );
          out.write(END_DIV);
        }
  ...
  private long computeContentSizeOfFolder(
    NodeRef nodeRef,
    FacesContext context
  ) {
    final SearchService searchService = Repository.getServiceRegistry(
      context
    ).getSearchService();
    final NodeService nodeService = Repository.getServiceRegistry(
      context
   ...
   */
  private void buildActionLinkImage(
    final ResponseWriter out,
    final int action,
    final int index,
    final String text,
    final String image

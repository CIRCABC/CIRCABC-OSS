...
public class RefuseApplicantDialog extends BaseWaiDialog {
  ...
  private static final String MESSAGE_ID_DESCRIPTION =
    "refuse_applicant_page_description";
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (getActionNode() == null) {
      throw new IllegalArgumentException(
        "The node ID is a manadtory parameter"
      );
    }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    final ProfileManagerService profManagerService =
      getProfileManagerServiceFactory().getProfileManagerService(currentSpace);
    ...
    try {
      ...
      for (WebApplicant webApplicant : applicants) {
        // remove the currenty
        profManagerService.removeApplicantPerson(
          currentSpace,
          webApplicant.getApplicant().getUserName()
        );
      ...
      // message
      Map<Object, Object> session = FacesContext.getCurrentInstance()
        .getExternalContext()
        .getSessionMap();
      session.put(
        ManageApplicantDialog.MANAGE_APPLICANT_DIALOG_INFORMATION_MESSAGE,
        translate(ManageApplicantDialog.MESSAGE_REFUSE_APPLICATION_SUCCESS)
      );
    } catch (Exception e) {
      logger.error(
        "Impossible to refuse this/these appliation(s): " +
        applicants +
      ...
      Utils.addErrorMessage(
        translate(ManageApplicantDialog.MESSAGE_REFUSE_APPLICATION_FAILED),
        e
      );
    }
  ...
  public void refuseApplicantAction(ActionEvent event) {
    ...
    final Map<String, Applicant> allApplicants =
      profManagerService.getApplicantUsers(getActionNode().getNodeRef());
  ...
  private void sendMail() {
    ...
    for (final WebApplicant applicant : applicants) {
      final NodeRef personNodeRef = getPersonService()
        .getPerson(applicant.getUserName());

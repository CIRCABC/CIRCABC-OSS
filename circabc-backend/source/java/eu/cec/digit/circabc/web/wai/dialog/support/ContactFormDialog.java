...
public class ContactFormDialog extends BaseWaiDialog {
  ...
  private static final String helpdesk_selection =
    "contact_dialog_request_helpdesk_selection";
  private static final String helpdesk_selection_guessed =
    "contact_dialog_request_helpdesk_selection_guessed";
  private static final String organisation_inside =
    "contact_dialog_request_helpdesk_selection_inside";
  private static final String organisation_outside =
    "contact_dialog_request_helpdesk_selection_outside";
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (username != null) {
      ...
      if (
        org.equalsIgnoreCase(INTERNAL_COM) || org.equalsIgnoreCase(INTERNAL_EXT)
      ) {
        ...
        if (!CircabcConfig.ECHA) {
          ...
          contactAsItems.add(
            new SelectItem(selectedContact, getHelpdeskContactAsString())
          );
        }
      } else {
        ...
        contactAsItems.add(
          new SelectItem(selectedContact, getCentralContactAsString())
        );
      }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (validateFields()) {
      ...
      if (getCurrentUserName() == null) {
        result = this.supportService.sendSupportRequestAsGuest(
            subject,
            description,
            SupportTypes.getById(selectedType),
            mail,
            supportService.getContactById(selectedContact),
            actionNode
          );
      } else {
        result = this.supportService.sendSupportRequest(
            subject,
            description,
            SupportTypes.getById(selectedType),
            getCurrentUserName(),
      ...
      if (result) {
        Utils.addStatusMessage(
          ...
          supportService.getContactById(selectedContact).getEmail()
        );
      } else {
        Utils.addStatusMessage(
          FacesMessage.SEVERITY_ERROR,
          "Mail unccessfully sent to :" +
          supportService.getContactById(selectedContact).getEmail()
        );
      }
      reinit();
  ...
  public String computeOrganisationHelpdesk() {
    return translate(
      helpdesk_selection,
      (insideStaff == true
          ? translate(organisation_inside)
          : translate(organisation_outside))
    );
  }

...
public abstract class AbstractMailToUsersBean extends BaseWaiWizard {
  ...
  protected abstract String finishImpl(
    ...
    if (to != null && to.length() != 0) {
      ...
      if (this.usingTemplate != null) {
        ...
        if (extraModelParams.containsKey(ATTACHED_FILES)) {
          ...
          if (listW.size() > 0) {
            bodyToSend +=
              "<h3>" + translate("attachment_links_label") + "</h3>";
            bodyToSend += "<ul>";

            for (AttachementWrapper aw : listW) {
              bodyToSend +=
                "<li>" + buildLinkFromAttachmentWrapper(aw) + "</li>";
            }
            bodyToSend += "</ul>";
          }
        }
      ...
      try {
        ...
        getMailService()
          .send(
            ...
            to,
            from,
            applyParams(getSubject(), extraBodyParams),
            bodyToSend,
            isMailHtml(),
            false
          );
  ...
  public void insertTemplate(final ActionEvent event) {
    if (
      this.template != null &&
      !this.template.equals(TemplateSupportBean.NO_SELECTION)
    ) {
  ...
  public List<SelectItem> getEmailTemplates() {
    ...
    for (final MailWrapper wrap : templates) {
      if (!wrap.isOriginalTemplate()) {
        items.add(
          new SelectItem(wrap.getTemplateNodeRef().getId(), wrap.getName())
        );
      }
    }
    return items;
  ...
  protected final TemplateService getTemplateService() {
    if (templateService == null) {
      templateService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getTemplateService();
    }
  ...
  protected NodeRef getTemplatePerson() {
    final String templateUser = getMailPreferencesService()
  ...
  public void setNavigationBusinessSrv(
    final NavigationBusinessSrv navigationBusinessSrv
  ) {

...
public class UIRichList extends UIComponentBase implements IDataContainer {
  ...
  public UIRichList(final String rederer) {
    ...
    views.add(
      "eu.cec.digit.circabc.web.ui.common.renderer.data.RichListRenderer$CircaViewRenderer"
    );
    ...
    for (String view : views) {
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Added view '" + renderer.getViewModeID() + "' to UIRichList"
          );
        }
      } catch (Exception e) {
  ...
  private void setPageInSession(int page) {
    ...
    if (session.getAttribute(CIRCABC_PAGER_MAP) != null) {
      pagerMap = (HashMap<String, Integer>) session.getAttribute(
        CIRCABC_PAGER_MAP
      );
    } else {
  ...
  private Integer getPageFromSession() {
    ...
    if (session != null) {
      ...
      if (objPagerMap != null) {
        HashMap<String, Integer> pagerMap = (HashMap<
            String,
            Integer
          >) objPagerMap;
  ...
  private HttpSession getSession() {
    return (HttpSession) this.getFacesContext()
      .getExternalContext()
      .getSession(false);
  }

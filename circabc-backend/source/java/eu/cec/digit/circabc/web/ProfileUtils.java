...
public class ProfileUtils extends AbstractSearchUtils {
   ...
   */
  public static List<SortableSelectItem> buildAssignableProfileItems(
    final Node node,
    final Log logger
  ) {
    return generateProfilesSelectItems(
      node,
      false,
      false,
      false,
   ...
   */
  public static List<SortableSelectItem> buildAllProfileItems(
    final Node node,
    final Log logger
  ) {
   ...
   * profiles <b>YES</b>.
   */
  public static List<SortableSelectItem> buildMailableProfileItems(
    final Node node,
    final Log logger
  ) {
  ...
  public static String getProfilename(
    ...
    IGRootProfileManagerService profileManager,
    ManagementService managementService
  ) {
    if (profileManager == null) {
      profileManager = getProfileManagerServiceFactory()
    ...
    final Profile profile = profileManager.getProfileFromGroup(
      igRoot,
      authority
    );
  ...
  private static List<SortableSelectItem> generateProfilesSelectItems(
    final Node node,
    final boolean addAllProfileChoice,
    final boolean addGuest,
    final boolean addRegistred,
    final boolean addImported,
    final String match,
    final List<String> alreadyInvitedAuthority,
    final Log logger
  ) {
    ...
    final List<SortableSelectItem> selectItems = new ArrayList<>(
      profiles.size() + 2
    );
    ...
    if (addAllProfileChoice) {
      selectItems.add(
        new SortableSelectItem(
          ALL_PROFILE,
          Application.getMessage(fc, ALL_PROFILE),
          ALL_PROFILE
        )
      );
    ...
    for (final Profile profile : profiles) {
      if (filteredAuthority.contains(profile.getProfileName())) {
        ...
        needNameFilter == true && profileMatch(profile, match, logger) == false
      ) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "  --  Profile " +
  ...
  private static boolean profileMatch(
    ...
    if (profile.getProfileName().contains(match)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "  --  Profile " +
          profile.getProfileName() +
          " matches with its current title."
        );
      }
      return true;
    } else {
  ...
  private static List<Profile> getProfiles(
    ...
    if (profiles.size() > 0 && logger.isDebugEnabled()) {
      logger.debug(
        "  --  " +
        profiles.size() +
        " profiles successfully found for Node " +
        rootNode
      );
    }
  ...
  private static SortableSelectItem buildProfileItem(
    final Profile profile,
    final Log logger
  ) {

...
public class AddContentBean extends BaseWaiDialog {
  ...
  protected static final String FILENAME_REGEX =
    "(.*[\\\"\\*\\\\\\>\\<\\?\\/\\:\\|]+.*)|(.*[\\.]?.*[\\.]+$)|(.*[ ]+$)";
  static final Log logger = LogFactory.getLog(AddContentBean.class);
  private static final String FILE_LIMIT_REACHED_TITLE =
    "file_limit_reached_title";
  ...
  private static final String MSG_ERR_UNEXPECTED_ERROR =
    "add_content_unexpected_error";
  private static final String MSG_ERR_ONE_ERROR = "add_content_error_one";
  private static final String MSG_UPLOAD_SUCCESS =
    "library_add_upload_successfull";
   ...
   */
  private String locationBeforeUpload =
  ...
  public void init(final Map<String, String> parameters) {
    ...
    possibleStatus.add(
      0,
      new SelectItem(
        DocumentModel.STATUS_VALUE_DRAFT,
    ...
    possibleStatus.add(
      1,
      new SelectItem(
        DocumentModel.STATUS_VALUE_FINAL,
    ...
    possibleSecurityRanking.add(
      1,
      new SelectItem(
        DocumentModel.SECURITY_RANKINGS_SENSITIVE,
    ...
    possibleSecurityRanking.add(
      2,
      new SelectItem(
        DocumentModel.SECURITY_RANKINGS_SPECIAL_HANDLING,
        translate(
          DocumentModel.SECURITY_RANKINGS_SPECIAL_HANDLING.toLowerCase()
        )
      )
    );
    possibleSecurityRanking.add(
      3,
      new SelectItem(
        DocumentModel.SECURITY_RANKINGS_PUBLIC,
    ...
    possibleSecurityRanking.add(
      4,
      new SelectItem(
        DocumentModel.SECURITY_RANKINGS_INTERNAL,
    ...
    possibleSecurityRanking.add(
      5,
      new SelectItem(
        DocumentModel.SECURITY_RANKINGS_LIMITED,
    ...
    List<Keyword> lKeyword = keywordsService.getKeywords(
      getManagementService()
        .getCurrentInterestGroup(this.getActionNode().getNodeRef())
    );
  ...
  public /*synchronized*/String finish() {
    ...
    if (!this.isFinished) {
      ...
      try {
        // Case to fix the DuplicateChildException
        Lock lock = null;

        boolean uploadSyncFixEnabled = Boolean.parseBoolean(
          CircabcConfiguration.getProperty("upload.sync.fix.enabled")
        );

        if (
          uploadSyncFixEnabled &&
          hazelcastInstanceFactory != null &&
          hazelcastInstanceFactory.isClusteringEnabled()
        ) {
          // Lock execution until other members in the cluster are
          // finished
          HazelcastInstance hazelcastInstance =
            hazelcastInstanceFactory.getInstance();
          lock = hazelcastInstance.getLock(VALID_NAME_LOCK);
          logger.debug("About to acquire the valid name execution lock.");
          lock.lock();
          logger.debug("Valid name execution lock acquired.");
        }
        ...
        try {
          ...
          if (
            ...
            lock.unlock();
            logger.debug("Valid name execution lock released.");
          }
        }
        ...
        context
          .getExternalContext()
          .getSessionMap()
          .remove(AlfrescoNavigationHandler.EXTERNAL_CONTAINER_SESSION);
      } catch (Throwable e) {
      ...
      Utils.addErrorMessage(
        Application.getMessage(context, "error_wizard_completed_already")
      );
    }
      ...
      if (translationsAreValid()) {
        try {
          ...
          for (SelectItem item : pivotDocuments) {
            CircabcUploadedFile bean = getBeanFromUploadedFiles(
          ...
          Utils.addErrorMessage(
            translate(MSG_ERR_UNEXPECTED_ERROR, t.getMessage())
          );
          this.isFinished = false;
  ...
  private boolean translationsAreValid() {
    ...
    for (CircabcUploadedFile file : uploadedFiles) {
      if (file.getIsTranslation()) {
        CircabcUploadedFile pivot = getBeanFromUploadedFiles(
          file.getIsTranslationOfDocument()
        );

        if (file.getLanguageName().equals(pivot.getLanguageName())) {
          Utils.addErrorMessage(
            translate(
              "add_content_translation_pivot_same_language",
              file.getFileName(),
              file.getLanguageTitle(),
              pivot.getFileName()
            )
          );
          result = false;
        ...
        for (String key : file.getSubmitedProperties().keySet()) {
          if (key.startsWith(":")) {
            if (usedLanguages.containsKey(file.getSubmitedProperty(key))) {
              Utils.addErrorMessage(
                translate(
                  "add_content_translations_same_language",
                  key.substring(1),
  ...
  public void uploadFile(ActionEvent event) {
    if (submittedFile != null) {
      ...
      try {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "Problem during upload content, file does not exists",
            e
          );
        }
        failed = true;
        ...
        if (fs != null) {
          try {
            ...
            if (logger.isErrorEnabled()) {
              logger.error(
                "Problem when closing temporary file output stream",
                e
              );
            }
          }
      ...
      if (!failed) {
        ...
        try {
          ...
          String mimetype = metadataManager.guessMimetype(
            submittedFile.getName()
          );
          String encoding = metadataManager.guessEncoding(
            new BufferedInputStream(new FileInputStream(newFile.getFile())),
            mimetype
          );
          ...
          if (logger.isErrorEnabled()) {
            logger.error(
              "Error during extracting metadata of one file in the Add COntent Page",
              e
            );
          }
        }
        ...
        if (props != null) {
          if (props.containsKey(ContentModel.PROP_AUTHOR)) {
            newFile.addSubmitedProperty(
              CircabcUploadedFile.AUTHOR,
          ...
          if (props.containsKey(ContentModel.PROP_TITLE)) {
            newFile.addSubmitedProperty(
              CircabcUploadedFile.TITLE,
              cleanTranslatationMetadata(
                props.get(ContentModel.PROP_TITLE).toString()
              )
            );
          } else {
          ...
          if (props.containsKey(ContentModel.PROP_DESCRIPTION)) {
            newFile.addSubmitedProperty(
              CircabcUploadedFile.DESCRIPTION,
              cleanTranslatationMetadata(
                props.get(ContentModel.PROP_DESCRIPTION).toString()
              )
            );
          }
        }
        ...
        if (isMultiLingualDocument) {
          ...
          newFile.addSubmitedProperty(
            CircabcUploadedFile.IS_TRANSLATION,
            "false"
          );
          newFile.addSubmitedProperty(
            CircabcUploadedFile.MAIN_LANGUAGE,
            submittedLanguageDocument
          );
          newFile.addSubmitedProperty(
            CircabcUploadedFile.MAIN_LANGUAGE_TITLE,
            getFilterLanguage(submittedLanguageDocument)
          );

          pivotDocuments.add(
            new SelectItem(newFile.getFileName(), newFile.getFileName())
          );
        }
  ...
  public void removeMultilinguism(ActionEvent event) {
    CircabcUploadedFile fileBean =
      (CircabcUploadedFile) this.uploadedFilesDataModel.getRowData();
    ...
    for (CircabcUploadedFile file : uploadedFiles) {
      if (
        file
          .getSubmitedProperties()
          .containsKey(CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE)
      ) {
        if (
          file
            .getSubmitedProperty(
              CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE
            )
  ...
  private void deleteMultilinguismProperties(CircabcUploadedFile fileBean) {
    ...
    fileBean
      .getSubmitedProperties()
      .remove(CircabcUploadedFile.MAIN_LANGUAGE_TITLE);
  ...
  public void valiteFileSize(
    ...
    if (
      ...
      message.setDetail(
        translate(
          FILE_LIMIT_REACHED,
          this.getFileUploadLimit().getMaxSizeInMegaBytes()
        )
      );
  ...
  protected NodeRef createContent(
    ...
    if (!createdNodeRefs.containsValue(bean.getFileName())) {
      ...
      if (bean.getSubmitedProperties().containsKey(CircabcUploadedFile.NAME)) {
        if (
          bean.getSubmitedProperties().get(CircabcUploadedFile.NAME).length() >
          0
        ) {
          createdRef = getContentBusinessSrv()
            .addContent(
              parent,
              bean.getSubmitedProperty(CircabcUploadedFile.NAME),
              bean.getFile(),
      ...
      super.getNodeService()
        .setProperty(createdRef, ContentModel.PROP_AUTO_VERSION_PROPS, false);
      ...
      if (bean.getSubmitedProperties().containsKey(CircabcUploadedFile.TITLE)) {
        if (
          bean.getSubmitedProperties().get(CircabcUploadedFile.TITLE).length() >
          0
        ) {
          String tmpTitle = getSecurityService()
            .getCleanHTML(
              bean.getSubmitedProperties().get(CircabcUploadedFile.TITLE),
              false
            );

          super.getNodeService()
            .setProperty(createdRef, ContentModel.PROP_TITLE, tmpTitle);
        }
      }
      if (
        bean
          .getSubmitedProperties()
          .containsKey(CircabcUploadedFile.DESCRIPTION)
      ) {
        if (
          bean
            .getSubmitedProperties()
            .get(CircabcUploadedFile.DESCRIPTION)
            .length() >
          0
        ) {
          String tmpDescription = getSecurityService()
            .getCleanHTML(
              bean.getSubmitedProperties().get(CircabcUploadedFile.DESCRIPTION),
              true
            );
      ...
      if (
        bean.getSubmitedProperties().containsKey(CircabcUploadedFile.STATUS)
      ) {
        if (
          ((String) bean
              .getSubmitedProperties()
              .get(CircabcUploadedFile.STATUS)).length() >
          0
        ) {
          super.getNodeService()
            .setProperty(
      ...
      if (
        bean.getSubmitedProperties().containsKey(CircabcUploadedFile.AUTHOR)
      ) {
        if (
          bean
            .getSubmitedProperties()
            .get(CircabcUploadedFile.AUTHOR)
            .length() >
          0
        ) {
          String tmpAuthor = getSecurityService()
            .getCleanHTML(
              bean.getSubmitedProperties().get(CircabcUploadedFile.AUTHOR),
              false
            );

          super.getNodeService()
            .setProperty(createdRef, ContentModel.PROP_AUTHOR, tmpAuthor);
        }
      }
      if (
        bean.getSubmitedProperties().containsKey(CircabcUploadedFile.REFERENCE)
      ) {
        if (
          bean
            .getSubmitedProperties()
            .get(CircabcUploadedFile.REFERENCE)
            .length() >
          0
        ) {
          super.getNodeService()
      ...
      if (
        bean
          .getSubmitedProperties()
          .containsKey(CircabcUploadedFile.SECURITY_RANKING)
      ) {
        if (
          bean
            .getSubmitedProperties()
            .get(CircabcUploadedFile.SECURITY_RANKING)
            .length() >
          0
        ) {
          super.getNodeService()
      ...
      if (bean.getExpirationDate() != null) {
        super.getNodeService()
          .setProperty(
            ...
            bean.getExpirationDate()
          );
      }
      if (bean.getKeywords() != null) {
        for (String k : bean.getKeywords()) {
          keywordsService.addKeywordToNode(
            createdRef,
            keywordsService.buildKeywordWithId(k)
          );
        }
      }
      // Add encrypted aspect to the content
      Object encrypted = bean
        .getSubmitedProperties()
        .get(CircabcUploadedFile.ENCRYPTED);
      if (encrypted != null && (Boolean) encrypted) {
        ...
        getNodeService()
          .addAspect(
            createdRef,
            DocumentModel.ASPECT_ENCRYPTABLE,
            allProperties
          );
      }
      ...
      if (bean.getIsPivotMultilingual()) {
        ...
        final Locale locale = I18NUtil.parseLocale(
          bean.getSubmitedProperty(CircabcUploadedFile.MAIN_LANGUAGE)
        );
        ...
        if (nodeAuthor == null || nodeAuthor.length() < 1) {
          getNodeService()
            .setProperty(createdRef, ContentModel.PROP_AUTHOR, "");
        }
        ...
        getMultilingualContentService()
          .addTranslation(
            createdRef,
            getPivotRef(
              bean.getSubmitedProperty(
                CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE
              )
            ),
            I18NUtil.parseLocale(
              bean.getSubmitedProperty(CircabcUploadedFile.MAIN_LANGUAGE)
            )
          );
      }
  ...
  public DataModel getUploadedFilesDataModel() {
    if (this.uploadedFilesDataModel == null) {
      ...
      if (uploadedFiles == null) {
        this.uploadedFilesDataModel.setWrappedData(
            Collections.<CircabcUploadedFile>emptySet()
          );
      } else {
  ...
  public void removeSelection(final ActionEvent event) {
    final CircabcUploadedFile fileBean =
      (CircabcUploadedFile) this.uploadedFilesDataModel.getRowData();
  ...
  private void deleteTranslationProperties(final CircabcUploadedFile fileBean) {
    ...
    fileBean
      .getSubmitedProperties()
      .remove(CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE);
    fileBean.addSubmitedProperty(
      CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE,
      ""
    );
  ...
  protected void clearUpload() {
    ...
    final FacesContext fc = FacesContext.getCurrentInstance();
    final HttpSession session = (HttpSession) fc
      .getExternalContext()
      .getSession(false);
  ...
  protected String doPostCommitProcessing(
    ...
    if (toEditPropsNode != null) {
      ...
      parameters.put(
        BaseWaiDialog.ACTIVITY_PARAMETER,
        "EditDocumentProperties"
      );
      parameters.put(
        EditNodePropertiesDialog.NOTIFY_AFTER_SETTING_PROPERTIES,
      ...
      FacesContext.getCurrentInstance()
        .getViewRoot()
        .setViewId(this.locationBeforeUpload);
      newOutcome =
  ...
  protected void setLogRecord(Node node) {
    logRecord.setDocumentID(
      (Long) getNodeService()
        .getProperty(node.getNodeRef(), ContentModel.PROP_NODE_DBID)
    );
    final NodeRef igNodeRef = getManagementService()
      .getCurrentInterestGroup(node.getNodeRef());
    logRecord.setIgID(
      (Long) getNodeService()
   ...
   */
  public void setPossibleSecurityRanking(
    List<SelectItem> possibleSecurityRanking
  ) {
  ...
  public boolean isHasPermission() {
    final String currentAlfrescoUserName = getNavigator()
      .getCurrentAlfrescoUserName();
    ...
    if (hasPermission == AccessStatus.DENIED) {
      if (logger.isErrorEnabled()) {
        logger.error(
          "User " +
          currentAlfrescoUserName +
          " does not have requred permision " +
          permission +
          " on node " +
   ...
   */
  public void setHazelcastInstanceFactory(

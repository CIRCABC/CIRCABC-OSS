...
public class MachineTranslationPropertyDialog extends BaseWaiDialog {
  ...
  private static final String MSG_ERROR_TEXT_TO_TRANSLATE_SHOULD_NOT_BE_EMPTY =
    "machine_translate_text_empty";
    ...
    if (values != null) {
      ...
      for (Map.Entry<Locale, String> translation : values.entrySet()) {
        ...
        existingTranslations.add(
          new SelectItem(lang, labelByCode, translation.getValue())
        );
        existingTranslationsSet.add(lang);
      }

      if (existingTranslations.size() > 0) {
        sourceLanguage = (String) existingTranslations.get(0).getValue();
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (FacesContext.getCurrentInstance().getMessages().hasNext()) {
      ...
      translationService.translateProperty(
        getActionNode().getNodeRef(),
        this.propertyQname,
  ...
  private void validate() {
    if (text == null || text.trim().equals("")) {
      Utils.addErrorMessage(
        translate(MSG_ERROR_TEXT_TO_TRANSLATE_SHOULD_NOT_BE_EMPTY)
      );
    }

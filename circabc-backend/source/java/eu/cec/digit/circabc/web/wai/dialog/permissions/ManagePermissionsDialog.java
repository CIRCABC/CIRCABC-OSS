...
public class ManagePermissionsDialog extends BaseWaiDialog {
  ...
  private static final Log logger = LogFactory.getLog(
    ManagePermissionsDialog.class
  );
  private static final String MSG_SUCCESS_INHERIT_NOT =
    "success_not_inherit_permissions";
  private static final String MSG_SUCCESS_INHERIT =
    "success_inherit_permissions";
  ...
  public List<Map> getUsers() {
    personNodes = null;
    personNodes = PermissionUtils.getInterestGroupAuthorities(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        personNodes.size() +
        " users or access/profile are re-defined in space " +
  ...
  public void inheritPermissionsValueDeepTree(ActionEvent event) {
    ...
    final Boolean inheritParentPermissions =
      this.getPermissionService().getInheritParentPermissions(currentNode);

    this.getPermissionService()
      .setInheritParentPermissions(currentNode, !inheritParentPermissions);

    String query =
      "(PATH:\"" + getLucenePathFromSpaceRef(currentNode) + "/*\")";
  ...
  public void inheritPermissionsValueChanged(ActionEvent event) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "The inherit permission property is succesffully setted as " +
        inheritPermissions +
        " for the space " +
        getActionNode().getName()
      );
  ...
  public String getContainerTitle() {
    return translate(
      "manage_space_permission_page_tilte",
      getActionNode().getName()
    );
  }

...
public class UserNameValidator {
   ...
   */
  public static void evaluate(String userEnteredUsernameString)
    throws Exception {
    ...
    for (int i = 0; i < userEnteredUsernameString.length(); i++) {
      if (
        Character.isLetterOrDigit(userEnteredUsernameString.charAt(i)) == false
      ) {
   ...
   * @throws Exception Launch an exception with the corresponding message
   */
  public static void evaluateUnicity(
  ...
  public static void evaluateUserExists(
    PersonService personService,
    String oldUser
  ) throws Exception {
    // Test if user exists

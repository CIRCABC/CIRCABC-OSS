...
public class ManageLogoPreference extends BaseWaiDialog {
  ...
  private static final String INFO_CLICKOK =
    "manage_iglogo_dialog_info_clickok";

  private static final Log logger = LogFactory.getLog(
    ManageLogoPreference.class
  );

  private static final String MSG_ERROR_GET_LOGO =
    "manage_iglogo_dialog_error_get_definition";
  private static final String MSG_ERROR_SET_LOGO =
    "manage_iglogo_dialog_error_set_definition";
  private static final String MSG_ERROR_BAD_WIDTH =
    "manage_iglogo_dialog_error_width";
  private static final String MSG_ERROR_BAD_HEIGHT =
    "manage_iglogo_dialog_error_height";

  private static final String MSG_ERROR_UPLOAD =
    "manage_iglogo_dialog_define_upload_logo_error";
  private static final String MSG_ERROR_UPLOAD_IO =
    "manage_iglogo_dialog_define_upload_logo_error_io";
  private static final String MSG_ERROR_UPLOAD_NAME =
    "manage_iglogo_dialog_define_upload_logo_error_name";
  private static final String MSG_ERROR_UPLOAD_NULL =
    "manage_iglogo_dialog_define_upload_logo_error_upload";
  private static final String MSG_ERROR_NOT_CONTENT =
    "manage_iglogo_dialog_define_upload_logo_error_not_content";

  private static final String MSG_UPLOAD_SUCC_LINK =
    "manage_iglogo_dialog_define_upload_logo_success_link";
  private static final String MSG_UPLOAD_SUCC_FILE =
    "manage_iglogo_dialog_define_upload_logo_success_file";
  ...
  private transient DataModel logoDataModel;
  private transient DataModel servicesDataModel;
  ...
  private void reset() throws IllegalStateException {
    ...
    try {
      final DefaultLogoConfiguration defaultConfiguration =
        getLogoPreferencesService().getDefault(ref);
      resetLogos(ref);

      this.logoDisplayedOnMainPage =
        defaultConfiguration.isLogoDisplayedOnMainPage();
      this.logoDisplayedOnAllPages =
        defaultConfiguration.isLogoDisplayedOnAllPages();
      ...
      this.mainPageLogoWidth = (widthIgHome > 0)
        ? String.valueOf(widthIgHome)
        : "";
      this.mainPageLogoHeight = (heightIgHome > 0)
        ? String.valueOf(heightIgHome)
        : "";
      this.otherPagesLogoWidth = (widthOthers > 0)
        ? String.valueOf(widthOthers)
        : "";
      this.otherPagesLogoHeight = (heightOthers > 0)
        ? String.valueOf(heightOthers)
        : "";

      if (
        defaultConfiguration.getLogo() != null &&
      ...
      if (isServiceConfigAllowed()) {
        final InterestGroupNode interestGroup =
          (InterestGroupNode) getNavigator().getCurrentIGRoot();
        this.serviceConfigs = new ArrayList<>();
        for (final NavigableNode service : interestGroup.getNavigableChilds()) {
          this.serviceConfigs.add(
              new ServiceConfigWrapper(
      ...
      throw new IllegalStateException(
        translate(MSG_ERROR_GET_LOGO, e.getMessage()),
        e
      );
    }
  }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    try {
      ...
      if (width == null || height == null) {
        if (width == null) {
          Utils.addErrorMessage(
            translate(MSG_ERROR_BAD_WIDTH, this.mainPageLogoWidth)
          );
        }
        if (height == null) {
          Utils.addErrorMessage(
            translate(MSG_ERROR_BAD_HEIGHT, this.mainPageLogoHeight)
          );
        }
        ...
        getLogoPreferencesService()
          .setDefault(currentLocationRef, defaultLogoRef);

        getLogoPreferencesService()
          .setMainPageLogoConfig(
            currentLocationRef,
            this.logoDisplayedOnMainPage,
            height,
            width,
        ...
        if (serviceConfigs != null && isServiceConfigAllowed()) {
          for (final ServiceConfigWrapper wrapper : serviceConfigs) {
            if (
              wrapper.needToBeDeleted(defaultLogoRef, this.otherPagesSizeForced)
            ) {
              getLogoPreferencesService()
                .removeConfiguration(wrapper.getService().getNodeRef());
            } else {
              if (wrapper.needToChangeLogo()) {
                getLogoPreferencesService()
                  .setDefault(
                    wrapper.getService().getNodeRef(),
                    wrapper.getSelectedLogo()
                  );
              }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Unexpected error during settings of logo definition on " +
          getActionNode().getNodePath(),
          t
        );
      }
  ...
  public String getLibraryId() {
    final NavigableNode library = (NavigableNode) getNavigator()
      .getCurrentIGRoot()
  ...
  public List<LogoWrapper> getLogoDefinitions() {
    ...
    for (final LogoDefinition def : logos) {
      wrappers.add(
        new LogoWrapper(def, currentNode, nodeService, selectedLogid)
      );
    }
  ...
  public boolean isHomeConfigAllowed() {
    ...
    return (
      NavigableNodeType.IG_ROOT.equals(type) ||
      NavigableNodeType.CATEGORY.equals(type)
    );
  }
   ...
   */
  public final void setLogoDisplayedOnAllPages(
    Boolean logoDisplayedOnAllPages
  ) {
   ...
   */
  public final void setLogoDisplayedOnMainPage(
    Boolean logoDisplayedOnMainPage
  ) {
  ...
  protected final LogoPreferencesService getLogoPreferencesService() {
    if (this.logoPreferencesService == null) {
      this.logoPreferencesService = Services.getCircabcServiceRegistry(
   ...
   */
  public final void setLogoPreferencesService(
  ...
  protected final InterestGroupLogoBean getInterestGroupLogoBean() {
    if (interestGroupLogoBean == null) {
      interestGroupLogoBean = (InterestGroupLogoBean) Beans.getBean(
        InterestGroupLogoBean.BEAN_NAME
      );
    }
   ...
   */
  public final void setInterestGroupLogoBean(
    InterestGroupLogoBean interestGroupLogoBean
  ) {
  ...
  public final void setIconLink(NodeRef iconLink) {
    ...
    if (iconLink != null) {
      final String name = (String) getNodeService()
        .getProperty(iconLink, ContentModel.PROP_NAME);
      ...
      if (ContentModel.TYPE_CONTENT.equals(type) == false) {
        ...
        try {
          inputStream = getContentService()
            .getReader(iconLink, ContentModel.PROP_CONTENT)
            .getContentInputStream();
          getLogoPreferencesService().addLogo(currentRef, name, inputStream);

          // Return at the previous selection
          this.iconLink = getNodeService()
            .getPrimaryParent(iconLink)
            .getParentRef();

          Utils.addStatusMessage(
            FacesMessage.SEVERITY_INFO,
            translate(MSG_UPLOAD_SUCC_LINK, name)
          );

          resetLogos(currentRef);
        } catch (CustomizationException e) {
          Utils.addErrorMessage(
            translate(MSG_ERROR_UPLOAD, name, e.getMessage())
          );
        } catch (IOException e) {
          Utils.addErrorMessage(
            translate(MSG_ERROR_UPLOAD_IO, name, e.getMessage())
          );
        } finally {
          try {
            if (inputStream != null) {
   ...
   *
   */
  private Boolean isDuplicateLogoName(
    ...
    for (final LogoDefinition def : logos) {
      if (
        def.getName().equalsIgnoreCase(name) &&
        def.getDefinedOn().equals(currentRef)
      ) {
  ...
  public final void setIconFile(String name) {
    ...
    final FileUploadBean fileBean = (FileUploadBean) ctx
      .getExternalContext()
      .getSessionMap()
    ...
    if (fileBean != null) {
      ...
      if (isDuplicateLogoName(currentRef, name) == false) {
        try {
          ...
          Utils.addStatusMessage(
            FacesMessage.SEVERITY_INFO,
            translate(MSG_UPLOAD_SUCC_FILE, name)
          );
  ...
  protected final ContentService getContentService() {
    if (contentService == null) {
      contentService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getContentService();
    }

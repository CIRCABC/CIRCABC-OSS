...
public abstract class CreateContentBaseDialog extends BaseWaiDialog {
  ...
  private static final String MSG_ERROR_UPLOAD =
    "create_content_dialog_attachement_error_upload";
  private static final String MSG_LINK_EXISTS =
    "create_content_dialog_attachement_error_link_exist";
  ...
  public String getDefaultMode() {
    try {
      Object mode = FacesContext.getCurrentInstance()
        .getExternalContext()
        .getSessionMap()
        .get("EDIT_MODE");
      ...
      for (final AttachementWrapper wrapper : attachementWrappers) {
        if (wrapper.isCreated() == false) {
          getAttachementBusinessSrv()
            .addAttachement(getEditablePost(), wrapper.getAttachRef());
  ...
  protected String computeContent(final String content) {
    ...
    if (
      content != null &&
      content.length() > 0 &&
      content.equals(HTML_EMPTY_CONTENT) == false
    ) {
      return content;
    } else {
      return null;
  ...
  public boolean isAttachementFound() {
    ...
    return (
      this.attachementWrappers != null && this.attachementWrappers.size() > 0
    );
  }
  ...
  public String getRootId() {
    final InterestGroupNode currentIGRoot = (InterestGroupNode) getNavigator()
      .getCurrentIGRoot();
    ...
    if (this.attachmentDataModel == null) {
      ...
      if (isAttachementAllowed()) {
        if (getEditablePost() != null) {
          ...
          final List<Attachement> attachments = serv.getAttachements(
            getEditablePost()
          );
  ...
  public void removeAttachement(final ActionEvent event) {
    try {
      final AttachementWrapper wrapper =
        (AttachementWrapper) this.attachmentDataModel.getRowData();

      if (wrapper.isCreated()) {
        getAttachementBusinessSrv()
          .removeAttachement(
            getEditablePost(),
            wrapper.getAttachement().getNodeRef()
          );

        Utils.addStatusMessage(
          FacesMessage.SEVERITY_WARN,
          translate(MSG_INFO_REMOVE)
        );
      } else {
  ...
  public void setAttachFile(String name) {
    ...
    if (fileBean != null && fileBean.getFile() != null) {
      ...
      this.attachementWrappers.add(
          new AttachementWrapper(tempAttach, name, false)
        );
    } else {
  ...
  public final void setAttachLink(NodeRef refLink) {
    ...
    if (refLink != null) {
      final String name = (String) getNodeService()
        .getProperty(refLink, ContentModel.PROP_NAME);
      ...
      if (exists) {
        ...
        this.attachementWrappers.add(
            new AttachementWrapper(refLink, name, true)
          );
      }
    }
    attachLink = null;
  ...
  protected final AttachementBusinessSrv getAttachementBusinessSrv() {
    if (attachementBusinessSrv == null) {
      attachementBusinessSrv = getBusinessRegistry()
        .getAttachementBusinessSrv();
    }
  ...
  public static class AttachementWrapper implements Serializable {
    ...
    public int hashCode() {
      ...
      int result = 1;
      result =
        PRIME * result + ((attachRef == null) ? 0 : attachRef.hashCode());
      result =

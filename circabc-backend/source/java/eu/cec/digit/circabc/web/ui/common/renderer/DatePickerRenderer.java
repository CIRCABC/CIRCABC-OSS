...
public class DatePickerRenderer extends BaseRenderer {
  ...
  public static final String RENDERER_TYPE =
    "eu.cec.digit.circabc.faces.DatePickerRenderer";
  ...
  private int roundMinute(int minute, int step) {
    if ((60 % step != 0) || (60 <= step) || (step < 1)) {
      throw new IllegalArgumentException(
        "step should be in {1,2,3,4,5,6,10,12,15,20,30} "
      );
    }
    if ((minute < 0) || (minute > 59)) {
      throw new IllegalArgumentException(
        "minute should greater or equal 0 and less then 60"
      );
    }

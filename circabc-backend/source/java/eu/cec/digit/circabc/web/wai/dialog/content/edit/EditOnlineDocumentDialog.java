...
public class EditOnlineDocumentDialog extends CreateContentBaseDialog {
  ...
  private static final Log logger = LogFactory.getLog(
    EditOnlineDocumentDialog.class
  );
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (
      getNodeService()
        .hasAspect(documentNodeRef, ContentModel.ASPECT_WORKING_COPY)
    ) {
    ...
    if (workingCopyNodeRef != null && selectedContent != null) {
      try {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Trying to update content node Id: " + workingCopyNodeRef.getId()
          );
        }
        ...
        cociContentBusinessSrv.checkIn(
          workingCopyNodeRef,
          true,
          translate("edit_inline_action_title"),
          false
        );
      } catch (Throwable err) {

...
public class ManageKeywordsDialog extends BaseWaiDialog {
  ...
  public SelectItem[] getLanguages() {
    if (languages == null) {
      ...
      String allLanguagesStr = translate(
  ...
  public void updateList(ValueChangeEvent event) {
    ...
    if (
      event.getComponent().getId().contains("language") &&
    ...
    // change in the number selector
      ...
      event.getComponent().getId().contains("number") &&
      event.getNewValue() != null
    ) {
  ...
  protected void initKeywords() {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Trying to retreive the keyword list for the interest group : " +
        getInterestGroup().getName() +
        "(" +
    ...
    languages = null;
    keywords = getKeywordsService()
      .getKeywords(getInterestGroup().getNodeRef());

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Keyword successfully retreived for the Interest Group : " +
        getInterestGroup().getName() +
        "(" +
        getInterestGroup().getNodeRef() +
        ")" +
  ...
  public String export(ActionEvent event) {
    ...
    try {
      ...
      response.setHeader(
        "Content-Disposition",
        "attachment;filename=KeywordExport.xls"
      );
      ...
      logger.error(
        "Error during export of keywords in:" +
        this.getActionNode().getNodeRefAsString(),
        ex
      );
    } finally {
      if (outStream != null) {
  ...
  private void writeXLS(
    Map<Integer, List<KeywordEntry>> data,
  ...
  private Map<Integer, List<KeywordEntry>> generateExportTable() {
    ...
    for (Keyword keyword : keywords) {
      if (keyword.getMLValues().size() > 0) {
        ...
        for (Entry<Locale, String> translation : keyword
          .getMLValues()
          .entrySet()) {
          kList.add(
            new KeywordEntry(
              translation.getKey().getLanguage(),
  ...
  public String importFile(ActionEvent event) {
    if (importedFile != null) {
      if (
        importedFile.getName().matches(".*xls") ||
        importedFile.getName().contains(".*xlsx")
      ) {
        try {
          ByteArrayInputStream is = new ByteArrayInputStream(
            importedFile.getBytes()
          );
  ...
  private Map<String, Integer> importKeywords(
    Map<Integer, List<KeywordEntry>> impData
  ) {
      ...
      if (keywordToUpdate != null) {
        for (KeywordEntry kEntry : lTmp) {
          if (
            !keywordToUpdate
              .getMLValues()
              .containsKey(new Locale(kEntry.getLanguage()))
          ) {
            keywordToUpdate.addTranlatation(
              new Locale(kEntry.getLanguage()),
              kEntry.getValue()
            );
  ...
  private Map<Integer, List<KeywordEntry>> prepareImportedData(
    HSSFWorkbook wb
  ) {
  ...
  public void listUnusedKeywords() {
    ...
    for (Keyword kTmp : keywords) {
      ...
      if (
        keywordsService
          .getNodesForKeywords(this.getActionNode().getNodeRef(), lTmp)
          .size() ==
        0
      ) {
  ...
  public void downloadTemplate() {
    ...
    response.setHeader(
      "Content-Disposition",
      "attachment;filename=template-keywords.xls"
    );

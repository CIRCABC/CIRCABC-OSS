...
public class ServiceConfigWrapper implements Serializable {
  ...
  /*package*/ServiceConfigWrapper(
    DefaultLogoConfiguration configuration,
    final IGServicesNode service,
    final List<LogoDefinition> availableImages
  ) {
    super();
    ...
    this.overrided = configuration
      .getConfiguredOn()
      .equals(service.getNodeRef());
  ...
  public final List<SelectItem> getAvailableImages() {
    ...
    items.add(
      new SelectItem(VALUE_NO_LOGO, Application.getMessage(fc, MSG_NO_LOGO))
    );
  ...
  /*package*/boolean needToBeDeleted(
    final NodeRef igLogo,
    final Boolean igDisplay
  ) {
  ...
  /*package*/boolean needToChangeLogo() {
    return (
      selectedImageRef.equals(getDefinitionLogoRef(this.configuration)) == false
    );
  }
  ...
  /*package*/NodeRef getSelectedLogo() {
    return this.selectedImageRef.equals(VALUE_NO_LOGO)
      ? null
      : new NodeRef(this.selectedImageRef);
  }

...
public class SharingSpaceWizard extends AbstractMailToUsersBean {
  ...
  private void fillItems() {
    ...
    for (final InterestGroupItem item : interestGroups) {
      availableInterestGroups.add(
        new SelectItem(item.getNodeRef().toString(), item.getTitle())
      );
    }

    Collections.sort(availableInterestGroups, new SelectItemLabelComparator());
    ...
    for (final String perm : perms) {
      ...
      this.orderedLibraryPermissions.add(
          new SelectItem(perm, PermissionUtils.getPermissionLabel(perm))
        );
    }
  ...
  public boolean getNextButtonDisabled() {
    if (
      availableInterestGroups == null || availableInterestGroups.size() == 0
    ) {
      return true;
    } else {
  ...
  private void reapplyRules(
    ...
    Action action =
      this.getActionService().createAction(ExecuteAllRulesActionExecuter.NAME);
      ...
      for (ChildAssociationRef assoc : assocs) {
        ...
        if (
          getDictionaryService()
            .isSubClass(className, ContentModel.TYPE_FOLDER) ==
          true
        ) {
  ...
  protected String finishImpl(FacesContext context, String outcome) {
    ...
    inheritParentPermissions = getPermissionService()
      .getInheritParentPermissions(shareSpace);
    if (!inheritParentPermissions) {
    ...
    try {
      final LibraryPermissions libPerm =
        LibraryPermissions.withPermissionString(libraryPermission);
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        getNodeService().getProperty(targetIgRef, ContentModel.PROP_NAME) +
        " successfully invited to the space " +
    ...
    if (notifyLeaders) {
      AuthenticationUtil.runAs(
        new RunAsWork<Object>() {
          public Object doWork() throws Exception {
            final NodeRef currentCategory = getManagementService()
              .getCurrentCategory(targetIgRef);
            final ProfileManagerService profileManager =
              getProfileManagerServiceFactory()
                .getProfileManagerService(currentCategory);
            Set<String> categoryAdmins = profileManager.getInvitedUsers(
              currentCategory
            );

            final IGServicesNode library = new InterestGroupNode(
              targetIgRef
            ).getLibrary();
            ...
            for (final String libAdmin : libAdmins) {
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                  "Invitation successufully send to " +
                  getNodeService()
      ...
      AuthenticationUtil.runAs(
        new AuthenticationUtil.RunAsWork<Object>() {
          public Object doWork() {
            ...
            final String info =
              "Invited interest group " + igName + " to space " + spaceName;
   ...
   */
  public void setOrderedLibraryPermissions(
    List<SelectItem> orderedLibraryPermissions
  ) {
  ...
  public final RuleService getRuleService() {
    if (ruleService == null) {
      ruleService = Services.getAlfrescoServiceRegistry(
  ...
  protected final ActionService getActionService() {
    if (actionService == null) {
      actionService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getActionService();
    }

...
public abstract class PermissionUtils extends AbstractSearchUtils {
  ...
  public static final String CIRCABC_CACHE_NAME =
    "eu.cec.digit.circabc.service.dynamic.authority.circabcDynamicAuthority";
  ...
  private static final List<String> PROFILES_LIST_TO_FILTER = new ArrayList<>(
    3
  );
  ...
  public static List<SelectItem> getPermissions(
    final Node actionNode,
    final Log logger
  ) {
    if (actionNode.hasAspect(CircabcModel.ASPECT_LIBRARY)) {
      final List<String> libPermissions =
        LibraryPermissions.getOrderedLibraryPermissions();
      final List<SelectItem> itemsAsList = new ArrayList<>(
        libPermissions.size()
      );
      ...
      for (final String libPerm : libPermissions) {
        ...
        if (!libPerm.equals(LibraryPermissions.LIBNOACCESS.toString())) {
          itemsAsList.add(new SelectItem(libPerm, getPermissionLabel(libPerm)));
        }
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          itemsAsList.size() +
          " Permission(s) found for the node " +
          actionNode.getName() +
          " having aspect " +
      ...
      final HashSet<NewsGroupPermissions> permissions =
        NewsGroupPermissions.getPermissions();
      ...
      final HashSet<SurveyPermissions> permissions =
        SurveyPermissions.getPermissions();
      final List<SelectItem> itemsAsList = new ArrayList<>(permissions.size());
      ...
      final HashSet<InformationPermissions> permissions =
        InformationPermissions.getPermissions();
      final List<SelectItem> itemsAsList = new ArrayList<>(permissions.size());
      ...
      final HashSet<EventPermissions> permissions =
      ...
      logger.error(
        ...
        actionNode +
        ". The node aspects are not taken in account yet."
      );
  ...
  public static List<Map> getInterestGroupAuthorities(
    final NodeRef anyNode,
    final List<String> withPermission
  ) {
    final FacesContext context = FacesContext.getCurrentInstance();

    final NodeRef igRoot = getManagementService()
      .getCurrentInterestGroup(anyNode);
    final ProfileManagerService igProfileManagerService =
      getProfileManagerServiceFactory().getIGRootProfileManagerService();
    final ProfileManagerService catProfileManagerService =
      getProfileManagerServiceFactory().getCategoryProfileManagerService();
    final ProfileManagerService rootProfileManagerService =
    ...
    final RetryingTransactionCallback<List<Map>> callback =
      new RetryingTransactionCallback<List<Map>>() {
        public List<Map> execute() throws Throwable {
          ...
          try {
            ...
            final Map<String, List<String>> permissionMap = new HashMap<>(
              8,
              1.0f
            );
            ...
            for (final AccessPermission permission : permissions) {
              ...
              if (
                (withPermission == null ||
                  withPermission.contains(permission.getPermission())) &&
            ...
            for (final Map.Entry<
              String,
              List<String>
            > entry : permissionMap.entrySet()) {
              if (entry.getKey().trim().length() < 1) {}
              // check if we are dealing with a person (User Authority)
              else if (
                AuthorityType.getAuthorityType(entry.getKey()) ==
                  AuthorityType.GUEST ||
                getPersonService().personExists(entry.getKey())
              ) {
                final NodeRef nodeRef = getPersonService()
                  .getPerson(entry.getKey());
                if (nodeRef != null) {
                ...
                if (
                  entry.getKey().startsWith(PermissionService.GROUP_PREFIX) ==
                  true
                ) {
                  Profile profileFromGroup =
                    igProfileManagerService.getProfileFromGroup(
                      igRoot,
                      entry.getKey()
                    );
                  ...
                  if (profileFromGroup == null) {
                    ...
                    profileFromGroup =
                      catProfileManagerService.getProfileFromGroup(
                        getManagementService().getCurrentCategory(igRoot),
                        entry.getKey()
                      );
                  }
                  ...
                  if (profileFromGroup == null) {
                    ...
                    profileFromGroup =
                      rootProfileManagerService.getProfileFromGroup(
            ...
            Utils.addErrorMessage(
              MessageFormat.format(
                Application.getMessage(context, Repository.ERROR_NODEREF),
                refErr.getNodeRef()
              )
            );
  ...
  public static SelectItem[] getDomainFilters(
    final boolean withFilter,
    final boolean accessProfile,
    final boolean applicant
  ) {
    final List<SelectItem> filters = new ArrayList<>(11);

    final String allUsersMessage = Application.getMessage(
    ...
    if (!withFilter) {
      ...
      for (final Map.Entry<String, String> item : getUserService()
        .getEcasUserDomains()
        .entrySet()) {
      ...
      filters.add(
        new SelectItem("" + ++index, allUsersMessage, PermissionUtils.ALLUSERS)
      );
      for (final Map.Entry<String, String> item : getUserService()
        .getEcasUserDomains()
        .entrySet()) {
        filters.add(
          new SelectItem("" + ++index, item.getValue(), item.getKey())
        );
      }

      if (accessProfile) {
        filters.add(
          new SelectItem("" + ++index, "Access Profile", FILTER_VALUE_PROFILES)
        );
      }
      if (applicant) {
        filters.add(
          new SelectItem("" + ++index, "Applicants", FILTER_VALUE_APPLICANT)
        );
      }
    }
   ...
   */
  public static boolean isUserCanJoinNode(
    ...
    final CircabcServiceRegistry services = Services.getCircabcServiceRegistry(
      fc
    );
    final ProfileManagerService profileManagerService = services
      .getProfileManagerServiceFactory()
    ...
    boolean canJoin = !profileManagerService
      .getInvitedUsers(nodeRef)
    ...
    if (canJoin && nodeService.hasAspect(nodeRef, CircabcModel.ASPECT_IGROOT)) {
      final AccessStatus accesStatus = permissionService.hasPermission(
        nodeRef,
        JoinPermissions.JOIN.toString()
      );
      ...
      if (canJoin) {
        ...
        final ProfileManagerServiceFactory profileManagerServiceFactory =
          services.getProfileManagerServiceFactory();
        final ProfileManagerService categoryProfileManagerService =
          profileManagerServiceFactory.getProfileManagerService(
            categoryNodeRef
          );
        canJoin = !categoryProfileManagerService
          .getInvitedUsers(categoryNodeRef)
          .contains(currentUser);
      }
    }
    return canJoin;
   ...
   */
  public static List<SortableSelectItem> buildInvitableUserItems(
    final String domain,
    final String match,
    final Log logger
  ) {
  ...
  public static List<SortableSelectItem> buildInvitableUserItems(
    final String domain,
    final String match,
    final List<String> alreadyInvitedAuthority,
    final Log logger
  ) {
    return buildInvitedUserItems(
      getAllAvailableUsers(domain, match, logger),
      MAX_ELEMENTS_IN_LIST,
      null,
      alreadyInvitedAuthority,
      logger
    );
  }
   ...
   */
  public static List<SortableSelectItem> buildInvitedUserItems(
    final Node node,
    final Log logger,
   ...
   * @param alreadyInvitedAuthority filter of some profile authority to remove
   */
  public static List<SortableSelectItem> buildCircabcUserItems(
    final Node node,
    final String match,
    final boolean onlineUsers,
    final List<String> alreadyInvitedAuthority,
    ...
    return buildInvitedUserItems(
      getCircabcUsers(node, logger, onlineUsers, true),
      MAX_ELEMENTS_IN_LIST,
      match,
      alreadyInvitedAuthority,
      logger
   ...
   * @param alreadyInvitedAuthority filter of some profile authority to remove
   */
  public static List<SortableSelectItem> buildCategoryUserItems(
    final Node node,
    final String match,
    final boolean onlineUsers,
    final List<String> alreadyInvitedAuthority,
    ...
    return buildInvitedUserItems(
      getCategoryUsers(node, logger, onlineUsers, true),
      MAX_ELEMENTS_IN_LIST,
      match,
      alreadyInvitedAuthority,
      logger
  ...
  public static List<SortableSelectItem> buildInvitedUserItems(
    ...
    return buildInvitedUserItems(
      getInvitedUsers(node, logger, onlineUsers, false),
      MAX_ELEMENTS_IN_LIST,
      match,
      alreadyInvitedAuthority,
      logger
   ...
   * @param alreadyInvitedAuthority filter of some profile authority to remove
   */
  public static List<SortableSelectItem> buildInvitedUserItemsRecursivly(
    final Node node,
    final String match,
    final boolean onlineUsers,
    final List<String> alreadyInvitedAuthority,
    ...
    return buildInvitedUserItems(
      getInvitedUsers(node, logger, onlineUsers, true),
      MAX_ELEMENTS_IN_LIST,
      match,
      alreadyInvitedAuthority,
      logger
  ...
  private static List<SortableSelectItem> buildInvitedUserItems(
    final Collection<SearchResultRecord> records,
    final int maxUserToReturn,
    final String match,
    final List<String> alreadyInvitedAuthority,
    final Log logger
  ) {
    ...
    for (final SearchResultRecord record : records) {
      ...
      if (
        needNameFilter &&
        !userText.toLowerCase().contains(match.toLowerCase()) &&
        !userId.equals(match)
      ) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "  --  User " + userId + " already found in another location."
          );
        }
      } else {
        boolean disableItem = false;
        if (
          alreadyInvitedAuthority != null &&
          alreadyInvitedAuthority.contains(userId)
        ) {
        ...
        SortableSelectItem newItem = new SortableSelectItem(
          record.getId(),
          userText,
          userText
        );
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "  --  User " + userId + " successfully added to the list."
          );
        }
      }
    }
  ...
  private static Collection<SearchResultRecord> getAllAvailableUsers(
    final String domain,
    ...
    if (!criteria.contains(BLANK_DELIM)) {
      if (criteria.indexOf('@') == -1) {
        users = getUserService()
          .getUsersByDomainFirstNameLastNameEmail(domain, criteria, true);
      } else {
  ...
  private static Collection<SearchResultRecord> getCircabcUsers(
    final Node where,
    final Log logger,
    final boolean onlineUsers,
    ...
    if (logger.isDebugEnabled()) {
      startTime = System.currentTimeMillis();
      logger.debug(
        "Start to search all invited users users in " + where + " ..."
      );
    }
    ...
    final List<SearchResultRecord> userRecords = buildUsersList(
      invitedUsers,
      onlineUsers
    );
  ...
  private static Collection<SearchResultRecord> getCategoryUsers(
    final Node where,
    final Log logger,
    final boolean onlineUsers,
    final boolean recursive
  ...
  private static Collection<SearchResultRecord> getInvitedUsers(
    final Node where,
    final Log logger,
    final boolean onlineUsers,
    final boolean recursive
    ...
    final ProfileManagerService profileManager =
      getProfileManagerServiceFactory().getProfileManagerService(rootNode);

    if (rootNode == null) {
      throw new IllegalStateException(
        "The node received has not Interest Group"
      );
    }

    if (
      logger.isDebugEnabled() && rootNode.equals(where.getNodeRef()) == false
    ) {
      logger.debug(
    ...
    if (recursive) {
      if (NavigableNodeType.CIRCABC_ROOT.equals(type)) {
        final CircabcRootProfileManagerService circabcProfileManager =
          (CircabcRootProfileManagerService) profileManager;
        circabcProfileManager.getMasterUsers(rootNode);
        for (final NodeRef ref : managementService.getCategories()) {
          userRecords.addAll(
            getInvitedUsers(new Node(ref), logger, onlineUsers, recursive)
          );
        }
        ...
        final Set<String> persons = profileManagerService.getPersonInProfile(
          circabcNodeRef,
        ...
        for (final NodeRef ref : managementService.getInterestGroups(
          rootNode
        )) {
          userRecords.addAll(
            getInvitedUsers(new Node(ref), logger, onlineUsers, recursive)
        ...
        final NodeRef categoryNodeRef = managementService.getCurrentCategory(
          where.getNodeRef()
        );
        final ProfileManagerService profileManagerService =
          getProfileManagerServiceFactory()
  ...
  private static List<SearchResultRecord> buildUsersList(
  ...
  public static String computeUserLogin(
    final String username,
    final PersonService personService,
    final NodeService nodeService
  ) {
    return computeUserLogin(personService.getPerson(username), nodeService);
  }

  public static String computeUserLogin(
    final NodeRef userRef,
    final NodeService nodeService
  ) {

...
public class NewsGroupBean extends InterestGroupBean {
  ...
  public static final String MSG_PAGE_DESCRIPTION =
    "newsgroups_forums_title_desc";
  public static final String MSG_PAGE_ICON_ALT =
    "newsgroups_forums_icon_tooltip";
  private static final String PARAM_HIDE = "hide";
  private static final String IMAGES_ICONS_FORUMS_GIF =
    "/images/icons/forums.gif";
  private static final String CREATE_FORUM = "CreateForum";
  private static final String NEWSGROUPS_FORUM_CREATE_FORUM =
    "newsgroups_forum_create_forum";
  private static final String WAI_DIALOG_CREATE_FORUM_WAI =
    "wai:dialog:createForumWai";
  private static final String NEWSGROUPS_FORUM_CREATE_FORUM_TOOLTIP =
    "newsgroups_forum_create_forum_tooltip";
  private static final String NEWSGROUPS_FORUM_CREATE_TOPIC =
    "newsgroups_forum_create_topic";
  private static final String WAI_DIALOG_CREATE_TOPIC_WAI =
    "wai:dialog:createTopicWai";
  private static final String NEWSGROUPS_FORUM_CREATE_TOPIC_TOOLTIP =
    "newsgroups_forum_create_topic_tooltip";
  private static final String ID = "id";
  private static final String NEWSGROUPS_TOPIC_CREATE_POST_TOOLTIP =
    "newsgroups_topic_create_post_tooltip";
  private static final String WAI_DIALOG_CREATE_POST_WAI =
    "wai:dialog:createPostWai";
  private static final String NEWSGROUPS_TOPIC_CREATE_POST =
    "newsgroups_topic_create_post";
  private static final String GET_TOPIC_NODES_TIME_TO_QUERY_AND_BUILD_FORUM_NODES =
    "getTopicNodes | Time to query and build forum nodes: ";
  private static final String GET_FORUM_NODES_TIME_TO_QUERY_AND_BUILD_FORUMS_NODES =
    "GetForumNodes | Time to query and build forums nodes: ";
  private static final String MS = "ms";
  private static final String GET_POST_NODES_TIME_TO_QUERY_AND_BUILD_POST_NODES =
    "getPostNodes | Time to query and build post nodes: ";
  private static final String TYPE = ", type = ";
  private static final String FOUND_INVALID_OBJECT_IN_DATABASE_ID =
    "Found invalid object in database: id = ";
  private static final String PROP_DATE =
  ...
  public List<ActionWrapper> getActions() {
    ...
    if (
      getNavigator().getCurrentNodeType().equals(NavigableNodeType.NEWSGROUP)
    ) {
      actions.add(
        new ActionWrapper(
          CREATE_FORUM,
      ...
      getNavigator()
        .getCurrentNodeType()
        .equals(NavigableNodeType.NEWSGROUP_FORUM)
    ) {
      actions.add(
        new ActionWrapper(
          CREATE_FORUM,
      ...
      getNavigator()
        .getCurrentNodeType()
        .equals(NavigableNodeType.NEWSGROUP_TOPIC)
    ) {
  ...
  public NavigationPreference getForumNavigationPreference() {
    return getNavigationPreferencesService()
  ...
  public List<NavigableNode> getForums() {
    ...
    final RetryingTransactionCallback<List<NavigableNode>> callback =
      new RetryingTransactionCallback<List<NavigableNode>>() {
        public List<NavigableNode> execute() throws Throwable {
          ...
          try {
            final IGServicesNode serviceNode = (IGServicesNode) getNavigator()
              .getCurrentIGService();

            final List<FileInfo> files = getFileFolderService()
              .listFolders(serviceNode.getNodeRef());
            ...
            for (final FileInfo fileInfo : files) {
              ...
              if (typeDef != null) {
                // extract forums
                if (
                  ContentModel.TYPE_SYSTEM_FOLDER.equals(type) ||
                  ...
                  if (
                    ...
                    node = ResolverHelper.createForumRepresentation(
                      fileInfo,
                      getNodeService(),
                      getBrowseBean()
                    );
                ...
                if (logger.isWarnEnabled()) {
                  logger.warn(
                    FOUND_INVALID_OBJECT_IN_DATABASE_ID + nodeRef + TYPE + type
                  );
                }
              }
            }
    ...
    final List<NavigableNode> forums = txnHelper.doInTransaction(
      callback,
      true
    );
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        GET_FORUM_NODES_TIME_TO_QUERY_AND_BUILD_FORUMS_NODES +
        (endTime - startTime) +
        MS
      );
    }
  ...
  public List<NavigableNode> getTopics() {
              ...
              if (typeDef != null) {
                ...
                if (
                  ...
                  if (
                    ...
                    // create our Node representation
                    node = ResolverHelper.createTopicRepresentation(
                      fileInfo,
                      getNodeService(),
                      getBrowseBean()
                    );
    ...
    final List<NavigableNode> topics = txnHelper.doInTransaction(
      callback,
      true
    );
    ...
    if (logger.isDebugEnabled()) {
      long endTime = System.currentTimeMillis();
      logger.debug(
        GET_TOPIC_NODES_TIME_TO_QUERY_AND_BUILD_FORUM_NODES +
        (endTime - startTime) +
        MS
      );
    }
  ...
  private List<NavigableNode> getPosts(final boolean onlyPending) {
    ...
    final NavigationPreference topicNavigationPreference =
      getTopicNavigationPreference();
      ...
      new RetryingTransactionCallback<List<NavigableNode>>() {
        public List<NavigableNode> execute() throws Throwable {
          ...
          try {
            ...
            final List<FileInfo> files = getFileFolderService()
              .listFiles(serviceNode.getNodeRef());
            ...
            for (final FileInfo fileInfo : files) {
              ...
              if (
                onlyPending == false ||
                getModerationService().isWaitingForApproval(nodeRef)
              ) {
                // find it's type so we can see if it's a node we are interested in
                type = getNodeService().getType(nodeRef);
          ...
          final int desc =
            topicNavigationPreference.isInitialSortPostsDescending() ? -11 : 1;

          // Sort posts descending/ascending
          Collections.sort(
            posts,
            new Comparator<NavigableNode>() {
              @Override
    ...
    if (logger.isDebugEnabled()) {
      long endTime = System.currentTimeMillis();
      logger.debug(
        GET_POST_NODES_TIME_TO_QUERY_AND_BUILD_POST_NODES +
        (endTime - startTime) +
        MS
      );
    }
  ...
  private boolean isModeratedForum() {
    ...
    return (
      node.hasAspect(ModerationModel.ASPECT_MODERATED) &&
      Boolean.TRUE.equals(
        node.getProperties().get(ModerationModel.PROP_IS_MODERATED.toString())
      )
    );
  }

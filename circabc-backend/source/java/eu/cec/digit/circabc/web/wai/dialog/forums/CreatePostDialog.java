...
public class CreatePostDialog extends CreateContentBaseDialog {
  ...
  private static final String MSG_WARN_MODERATION =
    "newsgroups_forum_create_post_moderation_warning";
    ...
    if (content != null && content.length() > 0) {
      ...
      try {
        ...
        if (isAlreadySaved() == false) {
          ...
          final CreateTopicDialog createTopicDialog =
            (CreateTopicDialog) Beans.getBean(CreateTopicDialog.BEAN_NAME);
          final NodeRef topic;
          if (ForumModel.TYPE_TOPIC.equals(getActionNode().getType())) {
            topic = getActionNode().getNodeRef();
          } else {
            topic = getNodeService()

...
public class WebClientHelper {
  ...
  static {
    ...
    exportTypes.add(
      new SelectItem(
        ExportTypeEnum.CSV.toString(),
        ExportTypeEnum.CSV.toString()
      )
    );
    exportTypes.add(
      new SelectItem(
        ExportTypeEnum.XML.toString(),
        ExportTypeEnum.XML.toString()
      )
    );
    exportTypes.add(
      new SelectItem(
        ExportTypeEnum.Excel.toString(),
        ExportTypeEnum.Excel.toString()
      )
    );
   ...
   * @return NodeRef
   */
  public static NodeRef getNodeFromPath(
    final String path,
   ...
   * @return NodeRef
   */
  public static List<NodeRef> getNodesFromPath(
    final String path,
    final SearchService searchService
  ) {
   ...
   * boolean)
   */
  public static String getPathFromSpaceRef(
    final NodeRef ref,
    boolean children
  ) {
    final FacesContext context = FacesContext.getCurrentInstance();
    final Path path = Repository.getServiceRegistry(context)
      .getNodeService()
      .getPath(ref);
    final NamespaceService ns = Repository.getServiceRegistry(
      context
    ).getNamespaceService();
  ...
  public static String toValidFileName(final String fileName) {
    final PropertiesBusinessSrv propertiesBusinessSrv =
      Services.getBusinessRegistry(
        FacesContext.getCurrentInstance()
      ).getPropertiesBusinessSrv();
  ...
  public static String generateUniqueName(
    ...
    if (
      ...
      final String extension = WebClientHelper.getFileNameExtension(
        candidateName
      );
      final String name = WebClientHelper.removeFileNameExtension(
        candidateName
      );
      String uniqueName;
      int tries = 0;
  ...
  public static String getGeneratedWaiRelativeUrl(
    final Node node,
  ...
  public static String getGeneratedWaiFullUrl(
    final Node node,
    final ExtendedURLMode mode
  ) {
  ...
  public static String getGeneratedWaiUrl(
    final NodeRef nodeRef,
    final ExtendedURLMode mode,
  ...
  public static String getGeneratedWaiUrl(
    ...
    switch (mode) {
      case HTTP_WAI_BROWSE:
        if (relative) {
          url = ExternalAccessServlet.generateRelativeWaiExternalURL(
            ExternalAccessServlet.OUTCOME_BROWSE,
            node.getId()
          );
        } else {
          url = ExternalAccessServlet.generateFullWaiExternalURL(
            ExternalAccessServlet.OUTCOME_BROWSE,
            node.getId()
          );
        }
      ...
      case HTTP_USERDETAILS:
        if (relative) {
          url = ExternalAccessServlet.generateRelativeWaiExternalURL(
            ExternalAccessServlet.OUTCOME_USER_DETAILS,
            node.getId()
          );
        } else {
          url = ExternalAccessServlet.generateFullWaiExternalURL(
  ...
  public static String getGeneratedWaiUrl(
    ...
    switch (mode) {
      ...
      case HTTP_INLINE:
        ...
        url += DownloadContentServlet.generateBrowserURL(
          categoryItem.getNodeRef(),
  ...
  private static String generateURLInline(
  ...
  public static String formatLocalizedDate(
    final Date dateToFormat,
  ...
  public static void synchronizeLanguages(Locale requestLocale) {
    ...
    if (
      currentUser == null ||
      currentUser.getUserName().equals(AuthenticationUtil.getGuestUserName())
    ) {
  ...
  private static boolean isFirstAccess(FacesContext fc) {
    HttpSession session = (HttpSession) fc
      .getExternalContext()
      .getSession(false);
    Boolean isFirstAccessPage = (Boolean) session.getAttribute(
      FIRST_ACCESS_PAGE
    );

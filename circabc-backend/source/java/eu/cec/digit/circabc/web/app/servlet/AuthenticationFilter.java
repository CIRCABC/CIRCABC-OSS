...
public class AuthenticationFilter
  extends org.alfresco.web.app.servlet.AuthenticationFilter {
  ...
  public static void redirectToLoginPage(
    ...
    if (uri.contains(req.getContextPath() + BaseServlet.FACES_SERVLET)) {
      ...
      // occured.
      int jspIndex =
        uri.indexOf(BaseServlet.FACES_SERVLET) +
        BaseServlet.FACES_SERVLET.length();

      if (
        uri.length() > jspIndex &&
        (BaseServlet.validRedirectJSP(uri.substring(jspIndex)) ||
          circabcValidRedirectJSPs.contains(uri.substring(jspIndex)))
      ) {
  ...
  public void doFilter(
    ...
    if (!httpReq.getRequestURI().endsWith(getLoginPage(context))) {
      ...
      if (ticket != null && ticket.length() != 0) {
        ...
        final Cookie cookie = AuthenticationHelper.getAuthCookie(httpReq);
        ...
        final String guest = req.getParameter(AuthenticationFilter.ARG_GUEST);
        if (guest != null) {

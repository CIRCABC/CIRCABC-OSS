...
public class LibraryBulkOperationDialog extends BaseWaiDialog {
  ...
  private static final Log logger = LogFactory.getLog(
    LibraryBulkOperationDialog.class
  );
  ...
  protected String finishImpl(FacesContext context, String outcome) {
    ...
    String[] selectedNodes = (String[]) ArrayUtils.addAll(
      this.getSelectedContents(),
      this.getSelectedContainers()
    );
    Set<String> selectedContentSet = new HashSet<>(
      Arrays.asList(this.getSelectedContents())
    );
    Set<String> selectedContainersSet = new HashSet<>(
      Arrays.asList(this.getSelectedContainers())
    );
      ...
      if (ManagedNodes.TRANSLATION.equals(nodeType)) {
        ...
        if (currentNodeRef.equals(pivotTranslation)) {
          if (isAllTrnslationSelected(pivotTranslation, selectedContentSet)) {
            ...
            getMultilingualContentService()
              .deleteTranslationContainer(translationContainer);
          }
        } else {
        ...
        try {
          ...
          if (logger.isWarnEnabled()) {
            logger.warn(
              "Can't delete the node - node is locked:" +
          ...
          Utils.addStatusMessage(
            ...
            translate(
              "msg_error_cant_delete_space_that_contains_checkout_documents"
            )
          );
        }
      }
  ...
  private boolean isAllTrnslationSelected(
  ...
  public List<SelectItem> getOperations() {
    ...
    operations.add(
      new SelectItem("copy", translate("library_bulk_operation_dialog_copy"))
    );
    operations.add(
      new SelectItem("cut", translate("library_bulk_operation_dialog_cut"))
    );
    operations.add(
      new SelectItem(
        "delete",
        translate("library_bulk_operation_dialog_delete")
      )
    );
    return operations;
  }
  ...
  public List<SelectItem> getAllContainers() {
    ...
    for (NavigableNode node : getContainers()) {
      allContainers.add(
        new SelectItem(node.getNodeRefAsString(), node.getName())
      );
    }
  ...
  private void fillBrowseNodes(final Node currentNode) {
    ...
    final RetryingTransactionCallback<Object> callback =
      new RetryingTransactionCallback<Object>() {
        public Object execute() throws Throwable {
          try {
            final List<FileInfo> children = getFileFolderService()
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        "Time to query and build map nodes: " + (endTime - startTime) + "ms"
      );
    }
  }
}

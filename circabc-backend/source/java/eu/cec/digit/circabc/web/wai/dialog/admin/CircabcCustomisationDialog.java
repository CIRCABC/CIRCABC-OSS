...
public class CircabcCustomisationDialog extends BaseWaiDialog {
  ...
  static final Log logger = LogFactory.getLog(CircabcCustomisationDialog.class);
  /**
   *
   */
  ...
  public void uploadTemplate(ActionEvent event) {
    if (template != null) {
      ...
      try {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "error creating temp file when updating the template",
            e
          );
        }
      } finally {
      ...
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      AuthenticationUtil.setRunAsUserSystem();
      applicationCustomisationService.updateTemplate(
        tempFile,
        getSelectedTemplateNodeRef()
      );
      AuthenticationUtil.setRunAsUser(username);
      tempFile.delete();
    }
  ...
  public void uploadDisclaimerLogo(ActionEvent event) {
    if (disclaimer != null) {
      ...
      try {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "error creating temp file when updating the disclaimer logo",
            e
          );
        }
      } finally {
  ...
  public String getPictureUrl() {
    if (pictureUrl == null) {
      pictureUrl = WebClientHelper.getGeneratedWaiUrl(
        applicationCustomisationService.getDefaultLogoNodeRef(),
  ...
  public List<UrlBean> getListOfTemplateUrls() {
    ...
    for (NodeRef nRef : listOfTemplates) {
      if (nRef != null) {
        NodeRef folderRef = getNodeService()
          .getParentAssocs(nRef)
          .get(0)
          .getParentRef();
        UrlBean ub = new UrlBean();
        ub.setName(
          getNodeService()
            .getProperty(folderRef, ContentModel.PROP_NAME)
            .toString()
        );
        ub.setUrl(
   ...
   */
  public void setSelectListOfTemplateUrls(
    List<SelectItem> selectListOfTemplateUrls
  ) {
  ...
  public String getDisclaimerUrl() {
    if (disclaimerUrl == null) {
      disclaimerUrl = WebClientHelper.getGeneratedWaiUrl(
        applicationCustomisationService.getDefaultLogoDisclaimerNodeRef(),
        ExtendedURLMode.HTTP_DOWNLOAD,
        true
      );
    }
  ...
  public void updateContactLink(ActionEvent event) {
    ...
    if (contactLink == null || contactLink.equals("")) {
      ...
      uValid.isValid(contactLink) || contactLink.startsWith("mailto")
    ) {
      applicationCustomisationService.updateContactLink(
        getSecurityService().getCleanHTML(contactLink, true)
      );
    } else {
  ...
  public String getContactLink() {
    return (
      applicationCustomisationService.getContactlink() != null
  ...
  public String getBannerLogoUrl() {
    if (
      bannerLogoUrl == null &&
      applicationCustomisationService.getBannerLogoRef() != null
    ) {
      bannerLogoUrl = WebClientHelper.getGeneratedWaiUrl(
  ...
  public void uploadBannerLogo(ActionEvent event) {
    if (bannerLogo != null) {
      ...
      try {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "error creating temp file when updating the banner logo",
            e
          );
        }
      } finally {
  ...
  public Boolean getLegalLinkDisplayed() {
    ...
    if (applicationCustomisationService.getDisplayLegalLink() != null) {
      legalLinkDisplayed =
        applicationCustomisationService.getDisplayLegalLink();
    }
  ...
  public void updateeLearningLink(ActionEvent event) {
    ...
    if (uValid.isValid(eLearningLink)) {
      applicationCustomisationService.seteLearningLink(
        getSecurityService().getCleanHTML(eLearningLink, true)
      );
    } else {
      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,

...
public class CircabcCCCheckinFileDialog extends CircabcCCUpdateFileDialog {
  ...
  private static final Log logger = LogFactory.getLog(
    CircabcCCCheckinFileDialog.class
  );
  private static final String MSG_ERROR_UNEXPECTED = "checkin_unexpected_error";
  private static final String VALUE_DEFAULT_COPY_LOCATION =
    "__default_copy_location";
  ...
  private static final String CLOSE_AND_LAUNCH_DIALOG =
    DIALOG_CALL +
    CircabcNavigationHandler.OUTCOME_SEPARATOR +
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      ...
      String url = WebClientHelper.getGeneratedWaiFullUrl(
        getActionNode(),
      ...
      getActionNode()
        ...
        .put(
          "workingCopy",
          getActionNode().hasAspect(ContentModel.ASPECT_WORKING_COPY)
        );
      getActionNode()
        .getProperties()
        .put(
          "fileType32",
          FileTypeImageUtils.getFileTypeImage(getActionNode().getName(), false)
        );
    }
  }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    try {
      if (
        VALUE_NEW_COPY_LOCATION.equals(copyLocation) &&
        ...
        final boolean wasAContent = getBusinessRegistry()
          .getNavigationBusinessSrv()
          .isContent(currentRef);

        final File file = getUploadedFile() != null
        ...
        if (file == null) {
          ...
          original = getCociContentBusinessSrv()
            .checkIn(
              ...
              keepCheckedOut,
              file,
              getUploadedFile().isNotificationDisabled(),
              getUploadedFile().getFileName()
            );
        }

        if (keepCheckedOut == false && original != null && wasAContent) {
  ...
  public boolean isHasPermission() {
    ...
    if (getActionNode() != null) {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "For user " + currentAlfrescoUserName + " no current action node"
        );
      }
      throw new RuntimeException("Piracy warning");
    }

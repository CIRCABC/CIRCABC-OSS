...
public class DefineKeywordDialog extends BaseWaiDialog {
  ...
  public static final String MSG_ERROR_TRANSLATION_REQUIRED =
    "add_new_keyword_dialog_error_no_translation";
  public static final String MSG_ERROR_LOCALE_REQUIRED =
    "add_new_keyword_dialog_error_locale_required";
  public static final String MSG_ERROR_VALUE_REQUIRED =
    "add_new_keyword_dialog_error_value_required";
  public static final String MSG_ERROR_LOCALE_DUPLICATED =
    "add_new_keyword_dialog_error_locale_duplicated";
  private static final String COMPONENT_ADD_KEYWORD_LANGUAGE =
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (!NavigableNodeType.IG_ROOT.isNodeFromType(getActionNode())) {
      ...
      throw new IllegalArgumentException(
        "A keyword can only be added under an IG Root"
      );
    }
    ...
    if (getTranslationDataModel().getRowCount() < 1) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Impossible to add a keyword to the Interest Group " +
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Trying to add translation to the Interest Group " +
        getActionNode().getName() +
        "..."
      );
    }
    ...
    if (keyWordToUpdate == null) {
      ...
      key = getKeywordsService()
        .createKeyword(getActionNode().getNodeRef(), key);

      if (logger.isDebugEnabled()) {
        logger.debug(
          "Keyword  successfully created for the Interest Group " +
          getActionNode().getName() +
          "\n	ig: " +
    ...
    // we are in the edit mode, use the keyword defined
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Keyword  successfully updated for the Interest Group " +
          getActionNode().getName() +
          "\n	ig: " +
          getActionNode().getNodeRef() +
          "\n	keywod: " +
  ...
  public void addSelection(ActionEvent event) {
    final UIInput input = (UIInput) event
      .getComponent()
      .findComponent(COMPONENT_ADD_KEYWORD_VALUE);
    ...
    if (!error) {
      keywordTranslations.add(
        new KeywordWrapper(null, selValue, new Locale(selLang))
      );
      this.value = null;
    }
  }
  ...
  public void removeSelection(ActionEvent event) {
    KeywordWrapper wrapper =
      (KeywordWrapper) this.translationDataModel.getRowData();
    if (wrapper != null) {

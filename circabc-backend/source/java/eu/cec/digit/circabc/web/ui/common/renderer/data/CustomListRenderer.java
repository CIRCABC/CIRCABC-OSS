...
public class CustomListRenderer extends RichListRenderer {
  ...
  public static final String EDIT_IN_OFFICE_IMAGE =
    "/images/extension/icons/edit-in-office-action.png";
  ...
  private static final String MSG_DOWLOAD_TOOLTIP =
    "generic_dowload_link_tooltip";
  private static final String MSG_BROWSE_TOOLTIP =
    "generic_browse_link_tooltip";
  private static final String MSG_DATE_TIME_PATTERN =
    WebClientHelper.MSG_DATE_TIME_PATTERN;
  ...
  private static final String MSG_CONTENT_TOOLTIP =
    "generic_content_icon_tooltip";
  ...
  public void encodeChildren(FacesContext fc, UIComponent component)
    ...
    if (component.isRendered() && component.getChildCount() == 0) {
      ...
      final ColumnConfig initialSortColumn =
        configuration.getInitialSortColumn();
      if (initialSortColumn != null) {
        richList.setInitialSortColumn(initialSortColumn.getName());
      } else {
        richList.setInitialSortColumn(
          configuration.getColumns().get(0).getName()
        );
      }
      richList.setInitialSortDescending(
        configuration.isInitialSortDescending()
      );
      ...
      for (final ColumnConfig columnConfig : configuration.getColumns()) {
        ...
        facesColumn
          .getFacets()
          .put(FACET_HEADER, generateSortlink(fc, columnConfig));
        ...
        if (first) {
          ...
          facesColumn
            ...
            .put(
              FACET_SMALL_ICON,
              generateLink(fc, configuration, columnConfig, var, true)
            );
          facesColumn
            .getChildren()
            .add(generateLink(fc, configuration, columnConfig, var, false));
          facesColumn
            .getChildren()
            .add(generateLangLabel(fc, var, configuration));
      ...
      if (configuration.isDisplayActionColumn()) {
        ...
        actionsColumn
          ...
          .put(
            FACET_HEADER,
            generateStaticText(Application.getMessage(fc, MSG_ACTIONS))
          );
      ...
      // add the empty facet
      final HtmlOutputText empty = generateStaticText(
        Application.getMessage(fc, MSG_NO_ITEM)
      );
  ...
  private HtmlOutputText generateText(
    ...
    if (converterId != null && converterId.length() > 0) {
      ...
      if (converter instanceof XMLDateConverter) {
        ...
        dateDonverter.setPattern(
          Application.getMessage(fc, MSG_DATE_TIME_PATTERN)
        );
      }
   ...
   * @return
   */
  private UISortLink generateSortlink(
    final FacesContext fc,
    ...
    sortLink.setTooltipAscending(
      Application.getMessage(fc, MSG_GENERIC_SORT_ASC)
    );
    sortLink.setTooltipDescending(
      Application.getMessage(fc, MSG_GENERIC_SORT_DESC)
    );
  ...
  @SuppressWarnings("unchecked")
  private UIActionLink generateLink(
    final FacesContext fc,
    ...
    final boolean isContent = configuration
      .getService()
      .getType()
      .equalsIgnoreCase(NavigationPreferencesService.CONTENT_TYPE);
    final boolean isDownload = linkTarget.equals(
      NavigationPreference.DOWNLOAD_LINK_ACTION
    );
    ...
    if (previewAction == null && isDownload) { // old IGs & ISs
      ...
      ("custom-action-list-information-content".equals(
            serviceConfig.getActionConfigName()
          ) &&
        (EditNavigationPreference.OPTION_PREVIEW.equals(
            previewAction
          )/* || previewAction == null*/)) ||/* Check previewAction == null to enable preview as default */
      ("custom_action_list_library_content".equals(
            serviceConfig.getActionConfigName()
          ) &&
        (EditNavigationPreference.OPTION_PREVIEW.equals(
            previewAction
    ...
    if (!isDownload) {
      if (!linkTarget.equals(NavigationPreference.BROWSE_LINK_ACTION)) {
        logger.warn(
          linkTarget + " not managed yet... browse link will be generated. "
        );
      }
    ...
    if (isIcon) {
      if (isContent) {
        link.setValueBinding(
          ATTR_IMAGE,
          buildBinding(fc, var, VALUE_FILE_TYPE)
        );
   ...
   * @param var
   */
  private HtmlOutputLabel generateLangLabel(
    final FacesContext fc,
    String var,
    final NavigationPreference configuration
  ) {
    ...
    lang.setValueBinding(
      ATTR_RENDERED,
      buildDifNullBinding(fc, var, VALUE_LANG)
    );
    ...
    if (
      ("custom-action-list-information-content".equals(
        ...
        (EditNavigationPreference.OPTION_PREVIEW.equals(previewAction) ||
          previewAction ==
          null)) ||/* Check previewAction == null to enable preview as default */
      ("custom_action_list_library_content".equals(
            serviceConfig.getActionConfigName()
          ) &&
        (EditNavigationPreference.OPTION_PREVIEW.equals(previewAction) ||
  ...
  private UIParameter generateIdParam(final FacesContext fc, String var) {
    final UIParameter actionParam = (UIParameter) fc
      .getApplication()
   ...
   * @return
   */
  private UIActions buildActions(
    final FacesContext fc,
    final String var,
    final NavigationPreference configuration
  ) {
    final UIActions actions = new UIActions();
    final String mandatoryAction = configuration
      .getService()
      .getMandatoryAction();
    ...
    final List<String> availableActions = configuration
      .getService()
      .getActions();
    ...
    if (
      ...
      !toRenderActions.contains(mandatoryAction)
    ) {
      toRenderActions.add(mandatoryAction);

      if (logger.isWarnEnabled()) {
        logger.warn(
          "Mandatory action forced. Please to add it in the configuration file!:" +
          mandatoryAction
        );
      }
    }
    ...
    for (final String availableAction : availableActions) {
      if (EDIT_IN_OFFICE_ACTION.equals(availableAction)) {
        ...
        actionDef.Onclick = MessageFormat.format(
          COMPOSITE_BINDING,
          var,
          NavigableNode.ONCLICK_EDIT_IN_OFFICE
        );
  ...
  private MethodBinding buildBindingMethod(
    final FacesContext fc,
    final String object,
    final String value,
    ...
    return fc
      .getApplication()
      .createMethodBinding(
        MessageFormat.format(COMPOSITE_BINDING, object, value),
        new Class[] { paramClass }
      );
  }

  private ValueBinding buildSimpleBinding(
    final FacesContext fc,
    final String value
  ) {
    return fc
      .getApplication()
      .createValueBinding(MessageFormat.format(SIMPLE_BINDING, value));
  }

  private ValueBinding buildBinding(
    final FacesContext fc,
    final String object,
    final String value
  ) {
  ...
  private ValueBinding buildDifNullBinding(
    final FacesContext fc,
    final String object,
    final String value
  ) {
    return fc
      .getApplication()
      .createValueBinding(
        MessageFormat.format(BINDING_NOTEQUALS_NULL, object, value)
      );
  }
}

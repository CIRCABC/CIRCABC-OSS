...
public class BulkDownloadServlet extends BaseServlet {
  ...
  private static final String MSG_ERROR_CONTENT_MISSING =
    "error_content_missing";
  private static final String MIMETYPE_OCTET_STREAM =
  ...
  protected void doGet(
    ...
    final String bulkDownloadFile = (String) session.getAttribute(
      CircabcConstant.BULK_DOWNLOAD_FILE
    );
    if (bulkDownloadFile != null) {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Piracy warning non authorized access to BulkDownloadServlet."
        );
      }
      return;
    }
  ...
  protected void processDownloadRequest(
    ...
    if (
      ...
      !zipFilePath.contains("bulk") ||
      !zipFilePath.endsWith(".zip")
    ) {
      if (logger.isErrorEnabled()) {
        logger.error(
          "Piracy warning : zipFilePath  " + zipFilePath + " is invalid."
        );
      }
    ...
    try {
      ...
      if (mimetype == null || mimetype.length() == 0) {
        final MimetypeService mimetypeMap =

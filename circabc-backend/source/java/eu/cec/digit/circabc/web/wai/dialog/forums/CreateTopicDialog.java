...
public class CreateTopicDialog extends CreateContentBaseDialog {
  ...
  /*package*/static final String MSG_EMPTY_MESSAGE =
    "newsgroups_forum_error_no_message";
  /*package*/static final String MSG_WARN_MODERATION =
    "newsgroups_forum_create_topic_moderation_warning";
  ...
  protected String finishImpl(final FacesContext context, final String outcome)
    ...
    if (isValid() == false) {
      ...
      if (isAlreadySaved() == false) {
        ...
        final NodeRef topicNodeRef = createTopicNode(
  ...
  /*package*/NodeRef createTopicNode(final NodeRef forumNodeRef)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Added uifacets aspect with properties: " + uiFacetsPropTopics
      );
    }
    return topicNodeRef;
  }

  /*package*/NodeRef createPost(
    final FacesContext context,
    final NodeRef topicNodeRef,
    ...
    getNodeService()
      .addAspect(postNodeRef, ContentModel.ASPECT_TITLED, titledProps);
  ...
  protected boolean isValid() {
    ...
    if (!isCleanHTML(name, false)) {
      ...
      Utils.addErrorMessage(
        translate(MSG_ERR_INVALID_VALUE_FOR, translate("create_topic_subject"))
      );
    }
  ...
  public final void setModerationService(

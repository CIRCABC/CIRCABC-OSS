 ...
 */
public abstract class NavigableNode
  extends MapNode
  implements Comparable<NavigableNode> {
  ...
  private static final String NAVIGABLE_PARENT_RESOLVER_NAME =
    "navigableParent";
  private static final String NAVIGABLE_CHILDS_RESOLVER_NAME =
    "navigableChilds";
  private static final String HAS_CHILDS_RESOLVER_NAME = "hasChilds";
  private static final String ALTERNATIVE_DOWNLOAD_URL =
  ...
  public NodePropertyResolver resolverPreview = new NodePropertyResolver() {
    ...
    public Object get(Node node) {
      return (
        ...
        "\', \'" +
        SimpleDownloadServlet.generateBrowseURL(
          node.getNodeRef(),
          node.getName()
        ) +
        "\', \'" +
   ...
   */
  public NodePropertyResolver resolverEditInOffice =
    new NodePropertyResolver() {
      private static final long serialVersionUID = 9133669045868847541L;

      private AuthenticationService authenticationService = getServiceRegistry()
      ...
      public Object get(Node node) {
        ...
        int posEndWebdav = webdavUrl.indexOf("/webdav") + "/webdav".length();

        webdavUrl =
          webdavUrl.substring(0, posEndWebdav) +
          NEW_TICKET_PREFIX +
          ticket.substring(
            InMemoryTicketComponentImpl.GRANTED_AUTHORITY_TICKET_PREFIX.length()
          ) +
          webdavUrl.substring(posEndWebdav, webdavUrl.length());
  ...
  public NodePropertyResolver resolverReference = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      final String reference = (String) node
        .getProperties()
   ...
   */
  public NodePropertyResolver resolverVersionLabel =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -5587856279605454785L;
  ...
  public NodePropertyResolver resolverIssueDate = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      final Object value = node
        .getProperties()
        .get(DocumentModel.PROP_ISSUE_DATE.toString());
   ...
   */
  public NodePropertyResolver resolverSecurityRanking =
    new NodePropertyResolver() {
   ...
   */
  public NodePropertyResolver resolverExpirationDate =
    new NodePropertyResolver() {
      ...
      public Object get(final Node node) {
        final Object value = node
          .getProperties()
          .get(DocumentModel.PROP_EXPIRATION_DATE.toString());
  ...
  public NodePropertyResolver resolverKeyword = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      final Serializable value = (Serializable) node
        .getProperties()
      ...
      if (value instanceof Collection) {
        ...
        NodeService nodeService = Services.getAlfrescoServiceRegistry(
          fc
        ).getNodeService();
  ...
  private NodePropertyResolver resolverNavigableParent =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -5583174774838740436L;
  ...
  private NodePropertyResolver resolverNavigableChilds =
    new NodePropertyResolver() {
      private static final long serialVersionUID = 9133660645868747741L;
  ...
  private NodePropertyResolver resolverHasChilds = new NodePropertyResolver() {
    ...
    public Object get(final Node node) {
      final List<NavigableNode> childs =
        ((NavigableNode) node).getNavigableChilds();
  ...
  private void addResolvers() {
    this.addPropertyResolver(
        NAVIGABLE_PARENT_RESOLVER_NAME,
        resolverNavigableParent
      );
    this.addPropertyResolver(
        NAVIGABLE_CHILDS_RESOLVER_NAME,
        resolverNavigableChilds
      );
  ...
  public CircabcServiceRegistry getCircabcServices() {
    if (circabcServiceRegistry == null) {
      circabcServiceRegistry = Services.getCircabcServiceRegistry(

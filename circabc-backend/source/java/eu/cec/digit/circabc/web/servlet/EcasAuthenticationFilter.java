 ...
 * @author atadian
 */
public class EcasAuthenticationFilter
  extends eu.cec.digit.circabc.web.app.servlet.AuthenticationFilter {

  private static final String FACES_JSP_EXTENSION_SESSION_EXPIRED_JSP =
    "/faces/jsp/extension/session_expired.jsp";
  ...
  private static final Log logger = LogFactory.getLog(
    EcasAuthenticationFilter.class
  );
  /**
  ...
  public void doFilter(
      ...
      if (session == null) {
        ...
        if (isSessionInvalid) {
          httpRes.sendRedirect(
            httpReq.getContextPath() + FACES_JSP_EXTENSION_SESSION_EXPIRED_JSP
          );
          return;
        }
      }
    ...
    if (
      requestURI.endsWith("login.jsp") || requestURI.endsWith(ecasLoginPage)
    ) {
    ...
    // Showing CIRCABC interface
      ...
      final AuthenticationStatus status = AuthenticationHelper.authenticate(
        context,
        httpReq,
        httpRes,
        forceGuest,
    ...
    if (
      user != null &&
      AuthenticationUtil.getGuestUserName().equals(user.getUserName()) &&
      session != null
    ) {
      ...
      user == null && session != null && !requestURI.endsWith("login.jsp")
    ) {
      session.invalidate();
    }
  }
   ...
   */
  public void setGuestSessionTimeoutInMinutes(
    int guestSessionTimeoutInMinutes
  ) {

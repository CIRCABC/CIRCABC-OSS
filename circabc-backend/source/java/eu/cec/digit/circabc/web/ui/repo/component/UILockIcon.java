...
public class UILockIcon extends SelfRenderingComponent {
  ...
  private static NodeService getNodeService(FacesContext context) {
    NodeService service = Repository.getServiceRegistry(
      context
    ).getNodeService();
    if (service == null) {
      throw new IllegalStateException(
        "Unable to obtain NodeService bean reference."
      );
    }
  ...
  private static PersonService getPersonService(FacesContext context) {
    PersonService service = Repository.getServiceRegistry(
      context
    ).getPersonService();
    if (service == null) {
      throw new IllegalStateException(
  ...
  public void encodeBegin(FacesContext context) throws IOException {
    ...
    try {
      ...
      if (val instanceof NodeRef) {
        ref = (NodeRef) val;
        if (
          nodeService.exists(ref) &&
          nodeService.hasAspect(ref, ContentModel.ASPECT_LOCKABLE) == true
        ) {
          lockUser = (String) nodeService.getProperty(
            ref,
      ...
      final boolean lockedOwner =
        locked &&
        (lockUser.equals(Application.getCurrentUser(context).getUserName()));
    ...
    if (locked == true) {
      ...
      if (lockedOwner == true) {
        ...
        msg = MessageFormat.format(
          Application.getMessage(context, MSG_LOCKED_USER),
          lockUser.length
        );

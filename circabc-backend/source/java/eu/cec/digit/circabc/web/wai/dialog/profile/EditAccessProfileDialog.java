...
public class EditAccessProfileDialog extends BaseWaiDialog {
  ...
  private static final String COMPONENT_EDIT_PROFILE_LANGUAGE =
    "edit-profile-language";
  private static final String COMPONENT_EDIT_PROFILE_VALUE =
    "edit-profile-value";
  ...
  private static final Log logger = LogFactory.getLog(
    EditAccessProfileDialog.class
  );

  private static final String MSG_CONTAINER_TITLE =
    "edit_access_profile_dialog_page_title";

  private static final String MSG_ERROR_TRANSLATION_REQUIRED =
    "edit_access_profile_dialog_error_no_translation";
  private static final String MSG_ERROR_LOCALE_REQUIRED =
    "edit_access_profile_dialog_error_locale_required";
  private static final String MSG_ERROR_VALUE_REQUIRED =
    "edit_access_profile_dialog_error_value_required";
  private static final String MSG_ERROR_LOCALE_DUPLICATED =
    "edit_access_profile_dialog_error_locale_duplicated";
  private static final String MSG_ERROR_CURRENT_PROFILE =
    "edit_access_profile_dialog_warn_edit_current_profile";
    ...
    if (params != null) {
      ...
      if (profileName != null) {
        ...
        final String currentUserName = getNavigator()
          .getCurrentUser()
          .getUserName();
        final String currentUserProfile = profService.getPersonProfile(
          igNodeRef,
          currentUserName
        );
        selectedProfile = new AccessProfileWrapper(
          profService.getProfileNoCache(igNodeRef, profileName),
          igNodeRef
        );
        if (selectedProfile.getName().equals(currentUserProfile)) {
          ...
          Utils.addStatusMessage(
            ...
            translate(MSG_ERROR_CURRENT_PROFILE)
          );
        }
      } else {
        // creation mode
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (getTranslationDataModel().getRowCount() < 1) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Impossible to add a title to the Profile " +
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Trying to add update / create  the Profile " +
        selectedProfile.getName() +
        "..."
      );
    }
    ...
    final HashMap<String, Set<String>> servicesPermissions =
      webProf.getServicesPermissions();

    final String info;
    if (oldProfileName == null) {
      profileName = webProf.getProfileName();
      info = MessageFormat.format(
        "Profile {0} permissions : {1}",
        webProf.getProfileName(),
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Profile  successfully updated for the Interest Group " +
  ...
  private void createProfile(
    final ProfileManagerService profService,
    final Profile webProf,
    final NodeRef igNodeRef
  ) throws ProfileException {
    final Map<String, Profile> existingProfile = profService.getProfileMap(
      igNodeRef
    );
    ...
    profService.addProfile(
      igNodeRef,
      name.toString(),
      webProf.getServicesPermissions()
    );

    final Profile createdProfile = profService.getProfile(
      igNodeRef,
      name.toString()
    );
    webProf.setProfileName(createdProfile.getProfileName());
    webProf.setAlfrescoGroupName(createdProfile.getAlfrescoGroupName());
    webProf.setPrefixedAlfrescoGroupName(
      createdProfile.getPrefixedAlfrescoGroupName()
    );

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Profile successfully created for the Interest Group " +
        selectedProfile.getName() +
        "\n	ig: " +
        igNodeRef +
        "\n	profile: " +
  ...
  private void applyNewPermissions() throws IllegalStateException {
    final List<ServicePermissionWrapper> wrappers = (List<
        ServicePermissionWrapper
      >) this.permissionDataModel.getWrappedData();
      ...
      switch (wrapper.getService()) {
        ...
        default:
          throw new IllegalStateException(
            "IG Service " + wrapper.getService() + " unknow"
          );
      }
    }
  }
  ...
  public void addSelection(ActionEvent event) {
    final UIInput input = (UIInput) event
      .getComponent()
      .findComponent(COMPONENT_EDIT_PROFILE_VALUE);
    final UISelectOne select = (UISelectOne) event
      .getComponent()
    ...
    for (Map.Entry<Locale, String> entry : selectedProfile
      .getTitles()
      .entrySet()) {
  ...
  public SelectItem[] getLanguages() {
    ...
    final List<String> languages = getContentFilterLanguagesService()
      .getFilterLanguages();
    ...
    for (final String lang : filteredLanguages) {
      final String label = getContentFilterLanguagesService()
        .getLabelByCode(lang);
  ...
  public DataModel getTranslationDataModel() {
    ...
    final Set<Entry<Locale, String>> entrySet = selectedProfile
      .getTitles()
      .entrySet();
    final List<Entry<Locale, String>> entryList = new ArrayList<>(
      entrySet.size()
    );
  ...
  public DataModel getPermissionDataModel() {
    if (this.permissionDataModel == null) {
      ...
      final InformationPermissions[] infPerm = (isSpecial)
        ? InformationPermissions.minimalValues()
        : InformationPermissions.values();
      final LibraryPermissions[] libPerm = (isSpecial)
        ? LibraryPermissions.minimalValues()
        : LibraryPermissions.values();
      final DirectoryPermissions[] dirPerm = (isSpecial)
        ? DirectoryPermissions.minimalValues()
        : DirectoryPermissions.values();
      final EventPermissions[] evePerm = (isSpecial)
        ? EventPermissions.minimalValues()
        : EventPermissions.values();
      final NewsGroupPermissions[] newPerm = (isSpecial)
        ? NewsGroupPermissions.minimalValues()
        : NewsGroupPermissions.values();

      wrappers.add(
        new ServicePermissionWrapper(
          CircabcServices.INFORMATION,
          this.selectedProfile.getInfPermissionValue(),
          infPerm
        )
      );
      wrappers.add(
        new ServicePermissionWrapper(
          CircabcServices.LIBRARY,
          this.selectedProfile.getLibPermissionValue(),
          libPerm
        )
      );
      wrappers.add(
        new ServicePermissionWrapper(
          ...
          this.selectedProfile.getDirPermissionValue(),
          dirPerm,
          editPorfileOfCurrentUser
        )
      );
      wrappers.add(
        new ServicePermissionWrapper(
          CircabcServices.EVENT,
          this.selectedProfile.getEvePermissionValue(),
          evePerm
        )
      );
      wrappers.add(
        new ServicePermissionWrapper(
          ...
          this.selectedProfile.getNewPermissionValue(),
          newPerm
        )
      );
  ...
  protected String doPostCommitProcessing(
    ...
    if (getCircabcService().syncEnabled()) {
      getCircabcService()
        .updateProfileTitles(super.actionNodeDatabaseID, profileName);
    }
    return outcome;
  }
  ...
  public void removeSelection(ActionEvent event) {
    final Map.Entry<Locale, String> wrapper = (Map.Entry<
        Locale,
        String
      >) this.translationDataModel.getRowData();
    if (wrapper != null) {
  ...
  protected final ContentFilterLanguagesService getContentFilterLanguagesService() {
    if (contentFilterLanguagesService == null) {
      contentFilterLanguagesService = Services.getAlfrescoServiceRegistry(

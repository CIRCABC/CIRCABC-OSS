...
public class InterestGroupLogoBean implements Serializable {
  ...
  public boolean isNavigationDisplay() {
    return (
      NavigableNodeType.IG_ROOT.equals(getNavigator().getCurrentNodeType()) ==
        false &&
  ...
  public String getIconTitle() {
    final DefaultLogoConfiguration logoConfig = getLogoConfig();
    final String title = (logoConfig != null && logoConfig.getLogo() != null)
      ? logoConfig.getLogo().getTitle()
      : null;
    if (title != null && title.length() > 0) {
      ...
      return MessageFormat.format(
        Application.getMessage(fc, MSG_LOG_TITLE),
  ...
  public String getIconDesc() {
    final DefaultLogoConfiguration logoConfig = getLogoConfig();
    final String desc = (logoConfig != null && logoConfig.getLogo() != null)
      ? logoConfig.getLogo().getDescription()
      : null;
    if (desc != null && desc.length() > 0) {
      ...
      final FacesContext fc = FacesContext.getCurrentInstance();
      return MessageFormat.format(
        Application.getMessage(fc, MSG_LOG_ALT),
        getNavigator().getCurrentIGRoot().getName()
      );
    }
  }
  ...
  public final void setOtherPagesIconBinding(
    final HtmlOutputText mainPageDescBinding
  ) {}
  ...
  public final HtmlOutputText getMainPageDescBinding() {
    final DefaultLogoConfiguration logoConfig = getLogoConfig();
    final FacesContext fc = FacesContext.getCurrentInstance();
    final HtmlOutputText mainPageDescBinding = (HtmlOutputText) fc
      .getApplication()
  ...
  public final void setMainPageDescBinding(
  ...
  private DefaultLogoConfiguration getLogoConfig() {
    ...
    if (
      currentNodeConfig != null &&
      currentNodeConfig.getFirst() == getNavigator().getCurrentNode()
    ) {
      ...
      try {
        ...
        logger.error(
          "Impossible to retreive logo definition for " +
  ...
  private HtmlGraphicImage getIcon(boolean forMainPage) throws FacesException {
    final FacesContext fc = FacesContext.getCurrentInstance();
    final HtmlGraphicImage iconBinding = (HtmlGraphicImage) fc
      .getApplication()
      .createComponent(HtmlGraphicImage.COMPONENT_TYPE);
    try {
      final DefaultLogoConfiguration logoConfig = getLogoConfig();
      String url = WebClientHelper.getGeneratedWaiUrl(
        logoConfig.getLogo().getReference(),
      ...
      if (!forMainPage) {
        iconBinding.setStyle(
          otherPagesIconCssImgRight + getIconsSizeStyle(logoConfig, false)
        );
      } else {
        if (logoConfig.isMainPageLogoAtLeft()) {
          iconBinding.setStyle(
            mainPageIconCssImgLeft + getIconsSizeStyle(logoConfig, true)
          );
        } else {
          iconBinding.setStyle(
            mainPageIconCssImgRight + getIconsSizeStyle(logoConfig, true)
          );
        }
      }
    } catch (Exception e) {
  ...
  private String getIconsSizeStyle(
    final DefaultLogoConfiguration logoConfig,
    final boolean mainPage
  ) {
    final boolean force = mainPage
      ? logoConfig.isMainPageSizeForced()
      : logoConfig.isOtherPagesSizeForced();
    final int width = mainPage
      ? logoConfig.getMainPageLogoWidth()
      : logoConfig.getOtherPagesLogoWidth();
    final int height = mainPage
      ? logoConfig.getMainPageLogoHeight()
      : logoConfig.getOtherPagesLogoHeight();
  ...
  protected final LogoPreferencesService getLogoPreferencesService() {
    if (logoPreferencesService == null) {
      logoPreferencesService = Services.getCircabcServiceRegistry(
   ...
   */
  public final void setOtherPagesIconCssImgRight(
    String otherPagesIconCssImgRight
  ) {

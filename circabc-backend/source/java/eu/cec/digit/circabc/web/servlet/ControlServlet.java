...
public class ControlServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (!AuthValidator.isAuthorized(authHeader)) {
      logger.error("Unauthorized request: invalid API key from IP " + remoteIp);
      resp.setStatus(HttpServletResponse.SC_FORBIDDEN);
      return;
    }

    if (!IpValidator.isAllowed(remoteIp)) {
      logger.error("Unauthorized request: IP " + remoteIp + " is not allowed");
      resp.setStatus(HttpServletResponse.SC_FORBIDDEN);
      return;
    }
  ...
  public static class AuthValidator {
    ...
    private static final String GLOBAL_PROPS_PATH =
    ...
    static {
      try {
        ...
        props.load(
          AuthValidator.class.getClassLoader()
            .getResourceAsStream(GLOBAL_PROPS_PATH)
        );
        apiKey = props.getProperty(CONFIG_PROPERTY);
      } catch (Exception e) {
        logger.error(
          "Failed to load API key from alfresco-global.properties: " +
          e.getMessage()
        );
        apiKey = null;
      }
  ...
  public static class IpValidator {
    ...
    static {
      try {
        Properties props = new Properties();
        props.load(
          IpValidator.class.getClassLoader()
            .getResourceAsStream(GLOBAL_PROPS_PATH)
        );
        ...
        logger.error(
          "Failed to load allowed IPs from alfresco-global.properties: " +
          e.getMessage()
        );
      }
    }

...
public class AccessProfileWrapper {
  ...
  AccessProfileWrapper(final Profile profile, final NodeRef fromIg) {
    ...
    this.titles = (profile.getTitle() == null)
      ? new MLText()
      : profile.getTitle();
    this.descriptions = (profile.getDescription() == null)
      ? new MLText()
    ...
    final Set<String> visibilityPermissions = profile.getServicePermissions(
  ...
  public AccessProfileWrapper(
    ...
    this.titles = (profile.getTitle() == null)
      ? new MLText()
      : new MLText(profile.getTitle());
    this.descriptions = (profile.getTitle() == null)
      ? new MLText()
      : new MLText(profile.getTitle());
    ...
    if (profile.isImported()) {
      ...
      if (lastIndexOf > -1) {
        this.displayTitle =
          this.name.substring(0, lastIndexOf - 1) + ':' + profile.getTitle();
      } else {
    ...
    this.editActionAvailable = true;
    if (
      (profile.getAlfrescoGroup().equals(CircabcConstant.GUEST_AUTHORITY)) ||
      (profile.getAlfrescoGroup().equals(CircabcConstant.REGISTERED_AUTHORITY))
    ) {
      this.deleteAvailable = false;
    ...
    this.fromIg = profile.isImported()
      ? new NodeRef(profile.getIgFromNodeRef())
      : fromIg;
  }
  ...
  public boolean isSpecialProfile() {
    if (specialProfile == null) {
      specialProfile =
        CircabcConstant.GUEST_AUTHORITY.equals(getName()) ||
  ...
  private void generatePermissions(final Profile prof) {
    ...
    permissions.put(
      CircabcServices.INFORMATION.toString(),
      Collections.singleton(infPermissionValue)
    );
    permissions.put(
      CircabcServices.LIBRARY.toString(),
      Collections.singleton(libPermissionValue)
    );
    permissions.put(
      CircabcServices.DIRECTORY.toString(),
      Collections.singleton(dirPermissionValue)
    );
    permissions.put(
      CircabcServices.NEWSGROUP.toString(),
      Collections.singleton(newPermissionValue)
    );
    permissions.put(
      CircabcServices.EVENT.toString(),
      Collections.singleton(evePermissionValue)
    );

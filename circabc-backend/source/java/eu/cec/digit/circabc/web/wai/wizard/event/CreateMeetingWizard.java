...
public class CreateMeetingWizard extends BaseWaiWizard {
  ...
  public static final String MSG_ERROR_TITLE_MISSING =
    "event_create_meetings_wizard_validation_title_missing";
  /**
  ...
  private static final String MSG_ERROR_START_MEETING_PAST =
    "event_create_meetings_wizard_validation_start_in_past";
  private static final String MSG_ERROR_START_TIME_AFTER_END_TIME =
    "event_create_meetings_wizard_validation_start_after_end";
  private static final String MSG_ERROR_CONTACT_NAME_MISSING =
    "event_create_meetings_wizard_validation_contact_name_missing";
  private static final String MSG_ERROR_CONTACT_EMAIL_MISSING =
    "event_create_meetings_wizard_validation_contact_email_missing";
  private static final String MSG_ERROR_CONTACT_EMAIL_INVALID =
    "event_create_meetings_wizard_validation_contact_email_invalid";
  private static final String MSG_ERROR_CONTACT_PHONE_MISSING =
    "event_create_meetings_wizard_validation_contact_phone_missing";
  private static final String MSG_ERROR_TIME_MISSING =
    "event_create_meetings_wizard_validation_selected_occurence_miss_time";
  private static final String MSG_ERROR_EVERY_MISSING =
    "event_create_meetings_wizard_validation_selected_occurence_miss_every";
  private static final String MSG_ERROR_BAD_OCCURENCE_TIME =
    "event_create_meetings_wizard_validation_bad_occurence_time";
  private static final String MSG_ERROR_BAD_OCCURENCE_EVERY =
    "event_create_meetings_wizard_validation_bad_occurence_every";
  private static final String MSG_ERROR_BAD_ATTENDEES =
    "event_create_meetings_wizard_validation_bad_attendee_list";
  ...
  public SelectItem[] getTimeZones() {
    ...
    for (int x = 0; x < tzCount; ++x) {
      items[x] = new SelectItem(
        EventModel.EVENT_TIME_ZONE_CONSTRAINT_VALUES.get(x)
      );
    }
  ...
  public SelectItem[] pickerCallback(
    ...
    if (filterIndex == PROFILES_IDX) {
      result = ProfileUtils.buildMailableProfileItems(
        getActionNode(),
        contains,
      ...
      result = PermissionUtils.buildInvitedUserItems(
        getActionNode(),
        contains,
        false,
        new ArrayList<String>(),
          ...
          if (
            ...
            internalUsers.add(
              new InternalUser(
                ...
                (String) getNodeService()
                  .getProperty(ref, ContentModel.PROP_EMAIL)
              )
            );
          } else {
            internalUsers.add(
              new InternalUser(
                ...
                profileManagerService
                  .getProfileFromGroup(interestGroup, authority)
                  .getProfileName()
              )
            );
          }
  ...
  protected void checkParams(Map<String, String> parameters) {
    if (getActionNode() == null) {
      ...
      throw new IllegalArgumentException(
        "Aspect " + CircabcModel.ASPECT_EVENT_ROOT + " expected"
      );
    }
  }
  ...
  protected void validateDetails(Appointment appointment) {
    ...
    startCal.set(
      ...
      appointment.getStartTime().getHourOfDay()
    );
    ...
    if (occurenceMain.equals(MainOccurence.Times)) {
      if (timesOneAsString.trim().length() < 1) {
        ...
        Utils.addErrorMessage(
          translate(MSG_ERROR_BAD_OCCURENCE_TIME, maxOccurence)
        );
      } else {
        // all is ok
    ...
    if (isAudienceOpen() == false) {
      ...
      if (internalUsers != null && internalUsers.size() > 0) {
        ...
        for (InternalUser internalUser : internalUsers) {
          ...
          if (
            AuthorityType.getAuthorityType(authority).equals(
              AuthorityType.GROUP
            )
          ) {
            profile = igRootProfileManagerService.getProfileFromGroup(
              igRootRef,
              authority
            );
            usersInProfile = igRootProfileManagerService.getPersonInProfile(
              igRootRef,
  ...
  private Integer safeConvert(
    final String intAsString,
  ...
  protected final EventService getEventService() {
    if (eventService == null) {
      eventService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getEventService();
    }
    return eventService;

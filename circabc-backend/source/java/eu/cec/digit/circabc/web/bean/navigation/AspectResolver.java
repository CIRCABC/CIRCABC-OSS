...
public abstract class AspectResolver {
  ...
  private static final StoreRef SPACE_STORE = new StoreRef(
  ...
  public static boolean isNodeManagedByCircabc(final Node node) {
    if (!SPACE_STORE.equals(node.getNodeRef().getStoreRef())) {
      ...
      CIRCABC_CHILD.isNodeFromType(node) || CATEGORY_HEADER.isNodeFromType(node)
    ) {
      return true;
      ...
      return isNodeManagedByCircabc(
        new Node(mlService.getPivotTranslation(node.getNodeRef()))
      );
    } else if (
      LIBRARY_ML_CONTENT_FORUM.isNodeFromType(node) ||
      LIBRARY_ML_CONTENT_POST.isNodeFromType(node) ||
      LIBRARY_ML_CONTENT_TOPIC.isNodeFromType(node)
    ) {
  ...
  public static NavigableNodeType resolveType(final Node node) {
    ...
    if (LOGGER.isDebugEnabled()) {
      ...
      LOGGER.debug(
        "Time to resolve a circabc node type: " + (endTime - startTime) + "ms. "
      );
      LOGGER.debug(
        "Type returned : " +
        nodeType +
  ...
  private static NavigableNodeType resolveTypeImpl(final Node node) {
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
        "Circabc attempt to manage an No Circabc node or a not managed node " +
        node
      );
    }

...
public class ChangeCircabcAccessRightWizard extends InviteCircabcUsersWizard {
  ...
  private static final Log logger = LogFactory.getLog(
    ChangeCircabcAccessRightWizard.class
  );
  ...
  public SelectItem[] getFilters() {
    ...
    return new SelectItem[] {
      new SelectItem(
        "" + IGROOT_INVITED_USERS_IDX,
        bundle.getString(MSG_USERS)
  ...
  public SelectItem[] pickerCallback(
    ...
    final List<Map> authorities = PermissionUtils.getInterestGroupAuthorities(
    ...
    if (filterIndex == PROFILES_FILTER_IDX) {
      result = ProfileUtils.buildAllProfileItems(
        getActionNode(),
        contains,
        alreadyInvitedProfiles,
        logger
      );
  ...
  public List<SortableSelectItem> getProfiles() {
    final Set<LibraryPermissions> permissions =
      LibraryPermissions.getPermissions();

    final List<SortableSelectItem> permAsList = new ArrayList<>(
      permissions.size()
    );
    ...
    for (final LibraryPermissions perm : permissions) {
      ...
      permAsList.add(
        new SortableSelectItem(permAsString, permAsString, permAsString)
      );
    }
  ...
  public String getBuildTextMessage() {
    ...
    setBody(result);
    setSubject(
      I18NUtil.getMessage("change_permission_group_user_template_mail_subject")
    );

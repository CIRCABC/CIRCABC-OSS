...
public class LoginBean extends org.alfresco.web.bean.LoginBean {
  ...
  private static final String MSG_ERROR_ALREADY_REGISTRED_USER =
    "error_already_registred_user";
  private static final String MSG_ERROR_REGISTRATION_PARAMETERS =
    "error_registration_parameters";
  private static String WEB_ROOT_URL = CircabcConfiguration.getProperty(
  ...
  public String loginCirca() {
    ...
    final Map<Object, Object> session = FacesContext.getCurrentInstance()
      .getExternalContext()
      .getSessionMap();
    ...
    final String redirectURL = (String) session.get(
  ...
  public boolean getInitActivation() {
    ...
    final String paramUserName = (String) params.get(
      SelfRegistrationDialog.USERNAME_URL_PARAM
    );
    ...
    if (paramUserName != null && activationID != null) {
        ...
        AuthenticationUtil.runAs(
          new AuthenticationUtil.RunAsWork<Object>() {
            public Object doWork() {
              ...
              if (
                userNodeRef != null && userNodeRef.getId().equals(activationID)
              ) {
                if (
                  getAuthenticationService()
                    .getAuthenticationEnabled(paramUserName)
                ) {
                  ...
                  if (logger.isDebugEnabled()) {
                    logger.debug(
                      "A person wants to activate a second time its account. " +
                      " Account: " +
                      paramUserName +
                      " Activation id: " +
                      activationID
      ...
      if (logger.isWarnEnabled()) {
        logger.warn(
          "A person wants to activate an inexisting account." +
  ...
  public String logout() {
    ...
    final Map<Object, Object> session = context
      .getExternalContext()
      .getSessionMap();
    final User user = (User) session.get(
    ...
    if (
      ...
      !redirectUrlAfterLogout.isEmpty() &&
      (user != null &&
        !AuthenticationUtil.getAdminUserName().equals(user.getUserName())) &&
      CircabcConfig.ENT
    ) {
      ...
      try {
        externalContext.redirect(
          ...
          "?url=" +
          new URI(WEB_ROOT_URL).toASCIIString()
        );
      } catch (IOException e) {
        if (logger.isErrorEnabled()) {
...
*/
  ...
  public ProfileManagerServiceFactory getProfileManagerServiceFactory() {
    if (profileManagerServiceFactory == null) {
      profileManagerServiceFactory =
        (ProfileManagerServiceFactory) ApplicationContextHelper.getApplicationContext()
          .getBean("ProfileManagerServiceFactory");
    }
  ...
  protected final ManagementService getManagementService() {
    if (managementService == null) {
      managementService =
        (ManagementService) ApplicationContextHelper.getApplicationContext()
  ...
  public String getLoginTextAdapted() {
    ...
    if (CircabcConfig.ENT || CircabcConfig.ECHA) {
      result = WebClientHelper.translate(
        "login_page_text",

...
public class DBManageUserProfilesDialog extends ManageUserProfilesDialog {
  ...
  public List<Map> getFilterUsers() {
    if (shouldOptimize()) {
      ...
      final String currentUserName = getNavigator()
        .getCurrentUser()
        .getUserName();
      final long localeID = getCircabcService()
        .getUserLocaleID(currentUserName);
      ...
      List<UserWithProfile> result = getCircabcService()
        .getFilteredUsers(igNodeRef, localeID, alfrescoGroup, searchText);
      final List<Map> personNodes = new ArrayList<>(result.size());

      int currentNodeAdminCount = getCircabcService()
        .getNumberOfAdmintrators(igNodeRef);
      ...
      for (UserWithProfile userWithProfile : result) {
        ...
        node.put(PermissionUtils.KEY_AUTHORITY, userWithProfile.getUserName());
        node.put(
          PermissionUtils.KEY_DISPLAY_NAME,
          userWithProfile.getUserName()
        );
        if (!CircabcConfig.USE_LDAP) {
          node.put(
            PermissionUtils.KEY_USER_NAME,
            userWithProfile.getUserName()
          );
        } else {
          node.put(
        ...
        if (userWithProfile.isImported()) {
          ...
          if (userWithProfile.getUserName().equals(currentUserName)) {
            ...
            if (currentNodeAdminCount == 1 && userWithProfile.isAdmin()) {
              node.put("canBeRemoved", Boolean.FALSE);
            } else {
  ...
  private boolean shouldOptimize() {
    return (
      getActionNode().hasAspect(CircabcModel.ASPECT_IGROOT) &&
  ...
  public List<SortableSelectItem> getProfiles() {
    if (shouldOptimize()) {
      final String allProfileText = translate(
        ManageUserProfilesDialog.MSG_ALL_PROFILE
      );
      ...
      for (Profile profile : result) {
        if (
          profile.getAlfrescoGroup().equals("guest") ||
          profile.getAlfrescoGroup().equals("EVERYONE")
        ) {
          continue;
        }
        ...
        profiles.add(
          new SortableSelectItem(
            PermissionService.GROUP_PREFIX + profile.getAlfrescoGroup(),
            title,
            title
          )
        );
    ...
    if (shouldOptimize()) {
      ...
      for (Profile profile : result) {
        ...
        if (
          ...
          profile
            .getAlfrescoGroup()
            .equals(CircabcConstant.REGISTERED_AUTHORITY)
        ) {
          continue;
        }

...
public class NodeLinksDialog extends BaseWaiDialog {
  ...
  private static final String LINK_PATTERN =
    "<a href=\"{0}{1}\" target=\"{2}\" title=\"{3}\">{4}{5}</a>";
      ...
      AuthenticationUtil.runAs(
        new AuthenticationUtil.RunAsWork<Object>() {
          public Object doWork() {
            ...
            final NodeRef igLogoRef = Beans.getInterestGroupLogoBean()
              .getLogoRefrence();
            ...
            if (isContent) {
              ...
              smallIconUrl = (String) getBrowseBean()
                .resolverFileType16.get(node);

              if (isLink) {
                downloadUrl = (String) getBrowseBean()
                  .resolverLinkDownload.get(node);
              } else {
                downloadUrl = (String) getBrowseBean()
                  .resolverDownload.get(node);
              }
            } else {
              iconUrl =
                DIRECTORY_ICONS +
                getBrowseBean().resolverSpaceIcon.get(node) +
                ".gif";
              smallIconUrl =
                DIRECTORY_ICONS +
                getBrowseBean().resolverSmallIcon.get(node) +
                ".gif";
            ...
            if (rootLogoRef != null) {
              circabcLogo = (String) getBrowseBean()
                .resolverDownload.get(new Node(rootLogoRef));
            } else {
              circabcLogo = "_";
            }

            if (igLogoRef != null) {
              igLogo = (String) getBrowseBean()
                .resolverDownload.get(new Node(igLogoRef));
            } else {
              igLogo = "_";
            }

            reference = node.getNodeRefAsString();
            browseUrl = WebClientHelper.getGeneratedWaiRelativeUrl(
              node,
            ...
            try {
              path = getManagementService()
                .getNodePath(node.getNodeRef())
                .toString();
            } catch (Exception e) {

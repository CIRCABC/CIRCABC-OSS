...
public class RemoveLogoPreference extends BaseWaiDialog {
  ...
  private static final Log logger = LogFactory.getLog(
    RemoveLogoPreference.class
  );

  private static final String PARAM_LOGONAME = "logoName";

  private static final String MSG_ERROR_DELETE =
    "manage_iglogo_dialog_delete_error";
  private static final String MSG_ERROR_LOCATION_DELETE =
    "manage_iglogo_dialog_delete_error_location";
  private static final String MSG_ERROR_NAME_DELETE =
    "manage_iglogo_dialog_delete_error_name";

  private static final String MSG_UNDEFINED =
    "manage_navigation_dialog_undefined";
  private static final String MSG_LOGO_DELETED =
    "manage_iglogo_dialog_delete_success";
  private static final String MSG_CONFIRMATION =
    "remove_iglogo_dialog_confirmation";
    ...
    if (parameters != null) {
      ...
      if (logoName == null || logoName.trim().length() < 1) {
        throw new IllegalArgumentException(
          "The logo name parameter is mandatory."
        );
      }
    }
  }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    try {
      ...
      if (logoByName == null) {
        ...
        currentLocationRef.equals(logoByName.getDefinedOn()) == false
      ) {
        Utils.addErrorMessage(
          translate(
            MSG_ERROR_LOCATION_DELETE,
            getPath(logoByName.getDefinedOn())
          )
        );

        if (logger.isDebugEnabled()) {
          logger.debug(
            "Delete a logo defined on a parent location is not allowed. Parent: " +
            logoByName.getDefinedOn() +
            " instand of " +
            currentLocationRef
          );
        }
        ...
        getLogoPreferencesService()
          .removeLogo(currentLocationRef, this.logoName);
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Unexpected error during removing logo on " +
          getPath(getActionNode().getNodeRef()),
          t
        );
      }

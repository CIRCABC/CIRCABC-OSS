...
public class EditNavigationPreference extends BaseWaiDialog {
  ...
  private static final String MSG_SELECT_ONE =
    "edit_navigation_dialog_select_one";
  ...
  private static final String BINDING_EDIT_PROPS =
    "#{WaiLeftMenuBean.editPropertiesLabel}";
  ...
  private static final String VALUE_PRIMARY_COL_NAME_TITLE =
    "PRIMARY_COL_NAME_TITLE";
  private static final String VALUE_PRIMARY_COL_TITLE_NAME =
    "PRIMARY_COL_TITLE_NAME";
  ...
  private static final String MSG_LOADED_FROM =
    "edit_navigation_dialog_action_loaded_from";
  private static final String MSG_UNDEFINED =
    "manage_navigation_dialog_undefined";
  /**
   *
   */
  ...
  private void initPreferences(
    ...
    selectedRenderChoice = (preference.getRenderPropertyName() != null)
      ? preference.getRenderPropertyName()
      : navigationListShortNameRender;

    globalPreference = preference.getService();

    contentNode =
      "content".equals(globalPreference.getType()) ||
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    final List<ColumnConfig> newColumns = new ArrayList<>(
      getSelectedColumns().size() + 2
    );
    ...
    if (primaryColumType.equals(VALUE_PRIMARY_BEST_TITLE)) {
      ...
      final ColumnConfig title = getColumnByName(
        COLUMN_TITLE,
        allColumns,
        true
      );
    ...
    for (final SelectItem item : getSelectedColumns()) {
      newColumns.add(
        getColumnByName((String) item.getValue(), allColumns, true)
      );
    }

    //4. Compute actions
    final List<String> newActions = new ArrayList<>(
      getSelectedActions().size()
    );
    ...
    NavigationPreference newPreference = new NavigationPreferenceImpl(
      newColumns,
      newActions,
      listSize,
      safeSortColumn,
  ...
  public void loadDefault(final ActionEvent event) {
    ...
    final ChildAssociationRef parent = getNodeService()
    ...
    String path;
    try {
      final SimplePath simplePath = new SimplePath(
        getNodeService(),
        preference.getCustomizedOn()
      );
    ...
    Utils.addStatusMessage(
      ...
      translate(MSG_LOADED_FROM, path)
    );
  }
  ...
  public String getPrimaryColumn() {
    if (primaryColumType == null) {
      ...
      if (isColumnFromName(columns, 0, COLUMN_TITLE_OR_NAME)) {
        ...
        final boolean isTitleSecond = isColumnFromName(
          columns,
          1,
          COLUMN_TITLE
        );
  ...
  public boolean isNameAndTitleKeys() {
    if (nameAndTitleKeys == null) {
      ...
      if (nameAndTitleKeys) {
        ...
        if (
          col1.equals(col2) ||
          (!col1.equals(COLUMN_NAME) && !col2.equals(COLUMN_NAME)) ||
          (!col1.equals(COLUMN_TITLE) && !col2.equals(COLUMN_TITLE))
        ) {
          throw new IllegalStateException(
            "This kind of double keys columns is not managed by the bean yet: " +
            col1 +
            " - " +
            col2
          );
  ...
  public boolean isBestTitleKey() {
    if (bestTitleKey == null) {
      ...
      if (
        bestTitleKey &&
        !keyColumns.get(0).getName().equals(COLUMN_TITLE_OR_NAME)
      ) {
        throw new IllegalStateException(
          "This kind of single key column is not managed by the bean yet: " +
  ...
  private List<SelectItem> getAllActions() {
    if (allAction == null) {
      ...
      final Config globalConfig = Application.getConfigService(
        FacesContext.getCurrentInstance()
      ).getGlobalConfig();

      //  find the Actions specific config element
      final ActionsConfigElement actionConfig =
        (ActionsConfigElement) globalConfig.getConfigElement(
      ...
      for (final String action : actions) {
        ...
        if (
          BINDING_EDIT_PROPS.equals(actionMsg) ||
          BINDING_EDIT_PROPS.equals(actionDef.Label)
        ) {
  ...
  public int getMaxListSize() {
    return getMaximum(
      globalPreference.getDisplayRowMax(),
      NavigationPreference.MAX_LIST_SIZE
    );
  }

  public int getMaxActionSize() {
    return getMaximum(
      globalPreference.getDisplayActionMax(),
      globalPreference.getActions().size()
    );
  }

  public int getMaxColumnSize() {
    return getMaximum(
      globalPreference.getDisplayColMax(),
      globalPreference.getColumns().size()
    );
  }
    ...
    if (availableColumns == null) {
      ...
      if (allColumns.size() - maximumNumberOfDynamicProperties > 0) {
        availableColumns = new ArrayList<>(
          allColumns.size() -
          maximumNumberOfDynamicProperties +
          actualPropertiesSize
        );
      } else {
      ...
      for (final ColumnConfig col : allColumns) {
        if (!definedColumns.contains(col) && !keyColumns.contains(col)) {
          if (col.isDynamicProperty()) {
            ...
            availableColumns.add(
              new SelectItem(col.getName(), translate(col.getLabel()))
            );
          }
        }
      }
  ...
  private boolean dynamicPropetyExists(
    int index,
    List<DynamicProperty> dynamicProperties
  ) {
    for (DynamicProperty dynamicProperty : dynamicProperties) {
  ...
  public final List<SelectItem> getSelectedPreviewActions() {
    if (selectedPreviewActions == null) {
      ...
      selectedPreviewActions.add(
        new SelectItem(OPTION_DOWNLOAD, translate(SELECT_OPTION_DOWNLOAD))
      );
    }
      ...
      for (final SelectItem action : allActions) {
        ...
        if (!definedActions.contains(value) && !value.equals(mandatory)) {
          ...
          if (
            !(action
                .getLabel()
                .equalsIgnoreCase(CustomListRenderer.EDIT_IN_OFFICE_ACTION) &&
              CircabcConfig.OSS)
          ) {
  ...
  private void move(
    ...
    final String fromPickerId,
    final List<SelectItem> fromList,
    final List<SelectItem> toList
  ) {
    final String[] selected = getSelectedItems(event, fromPickerId);
    if (selected == null || selected.length == 0) {
      Utils.addStatusMessage(
        FacesMessage.SEVERITY_WARN,
        translate(MSG_SELECT_ONE)
      );
    } else {
      final List<String> selectedAsList = Arrays.asList(selected);
      final List<SelectItem> itemsToMove = new ArrayList<>(
        selectedAsList.size()
      );
  ...
  private String[] getSelectedItems(
    ...
    final UISelectMany fromPicker = (UISelectMany) event
      .getComponent()
      .findComponent(fromPickerId);
  ...
  private void sortColumn(
    final ActionEvent event,
    final String fromPickerId,
    final List<SelectItem> listToSort,
    boolean up
  ) {
    final String[] selected = getSelectedItems(event, fromPickerId);
    if (selected == null || selected.length == 0) {
  ...
  private boolean isColumnFromName(
  ...
  private ColumnConfig getColumnByName(
    final String name,
  ...
  protected final NavigationConfigService getNavigationConfigService() {
    if (navigationConfigService == null) {
      navigationConfigService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
   ...
   */
  public final void setNavigationConfigService(
   ...
   */
  public final void setHighlightedAvailableColumns(
    String[] highlightedAvailableColumns
  ) {
   ...
   */
  public final void setHighlightedSelectedColumns(
    String[] highlightedSelectedColumns
  ) {
   ...
   */
  public final void setHighlightedAvailableActions(
    String[] highlightedAvailableActions
  ) {
   ...
   */
  public final void setHighlightedSelectedActions(
    String[] highlightedSelectedActions
  ) {

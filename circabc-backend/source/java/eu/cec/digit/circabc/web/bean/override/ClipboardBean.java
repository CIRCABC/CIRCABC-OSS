...
public class ClipboardBean
  extends org.alfresco.web.bean.clipboard.ClipboardBean
  implements ICircabcContextListener {
  ...
  public void pasteItem(ActionEvent event) {
    if (event instanceof UIClipboardShelfItem.ClipboardEvent) {
      final UIClipboardShelfItem.ClipboardEvent clipEvent =
        (UIClipboardShelfItem.ClipboardEvent) event;
  ...
  private boolean getNotificationStatus(
    ...
    if (igNodeRef != null) {
      if (clipEvent.Action == ACTION_PASTE_ALL) {
        result = getNotificationManagerService()
          .isPasteAllNotificationEnabled(igNodeRef);
      } else if (clipEvent.Action == ACTION_PASTE_ITEM) {
        result = getNotificationManagerService()
          .isPasteNotificationEnabled(igNodeRef);
      }
    }
    return result;
  }

  private void logRecordSetActivity(
    final UIClipboardShelfItem.ClipboardEvent clipEvent
  ) {
    String activity = "";
    if (clipEvent.Action == ACTION_PASTE_ALL) {
  ...
  private NodeRef getTargetNodeRef() {
    ...
    NavigationBean navigator = (NavigationBean) FacesHelper.getManagedBean(
      fc,
    ...
    if (event instanceof UIClipboardShelfItem.ClipboardEvent) {
      ...
      if (index >= getItems().size()) {
        throw new IllegalStateException(
          "Clipboard attempting paste a non existent item index: " + index
        );
      }
  ...
  private void performDownloadAllItems() {
    ...
    File tempZipFile;
    final UserTransaction txn = getTransactionService()
      .getNonPropagatingUserTransaction(false);
    File tempIndexFile;
    try {
      txn.begin();
      tempZipFile = TempFileProvider.createTempFile(
        "bulk",
        ".zip",
        TempFileProvider.getTempDir()
      );
      tempIndexFile = TempFileProvider.createTempFile(
        "bulk",
        ".txt",
        TempFileProvider.getTempDir()
      );
      ...
      final List<IndexRecord> indexRecords = getBulkService()
        .getMetaData(nodes);
      ...
      logRecord.addInfo(
        " Download zip size: " + String.valueOf(length) + " bytes"
      );
      final String viewId =
        BULK_DOWNLOAD + "dummy?" + WebResourcesCircabc.URL_FIX_ISPOSTBACK;
      final UIViewRoot viewRoot = context
        .getApplication()
        .getViewHandler()
      ...
      Utils.addErrorMessage(
        Application.getMessage(context, MSG_ERROR_DOWNLOAD_ALL) +
        err.getMessage(),
        err
      );
    } finally {
  ...
  private void setBulkDownloadFile(final FacesContext context, String file) {
    context
      .getExternalContext()
      .getSessionMap()
      .put(CircabcConstant.BULK_DOWNLOAD_FILE, file);
  }
  ...
  public ZipService getZipService() {
    if (zipService == null) {
      zipService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public NodeService getNodeService() {
    if (nodeService == null) {
      nodeService = Repository.getServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getNodeService();
    }
    return nodeService;
  ...
  public TransactionService getTransactionService() {
    if (transactionService == null) {
      transactionService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public BulkService getBulkService() {
    if (bulkService == null) {
      bulkService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public IndexService getIndexService() {
    if (indexService == null) {
      indexService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public NotificationManagerService getNotificationManagerService() {
    if (notificationManagerService == null) {
      notificationManagerService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public void setNotificationManagerService(

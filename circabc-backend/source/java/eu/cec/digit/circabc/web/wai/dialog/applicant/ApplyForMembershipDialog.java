...
public class ApplyForMembershipDialog extends BaseWaiDialog {
   ...
   */
  public static final String CIRCA_MESSAGE_BUNDLE =
  ...
  private static final Log logger = LogFactory.getLog(
    ApplyForMembershipDialog.class
  );
  ...
  private static final String MESSAGE_ID_PAGE_TITLE_LOG =
    "apply_for_membership_page_title";
  private static final String MESSAGE_ID_PAGE_TITLE_NO_LOG =
    "apply_for_membership_no_log_page_title";
  private static final String MESSAGE_ID_PAGE_DESCRIPTION_LOG =
    "apply_for_membership_page_description";
  private static final String MESSAGE_ID_PAGE_DESCRIPTION_NO_LOG =
    "apply_for_membership_no_log_page_description";
  private static final String MESSAGE_ID_APPLY_FORM_MEMBERSHIP_FORBIDEN =
    "apply_for_membership_forbiden";
  /**
  ...
  public String getContainerDescription() {
    return isGuest()
      ...
      : translate(
        MESSAGE_ID_PAGE_DESCRIPTION_LOG,
        getActionNode()
          .getProperties()
  ...
  public String getInterestGroupDesc() {
    ...
    final Object desc = getActionNode()
      .getProperties()
      .get(ContentModel.PROP_DESCRIPTION.toString());
    if (desc != null) {
    ...
    final Object title = getActionNode()
      .getProperties()
  ...
  public String getContactInfo() {
    final Object contactInfo = getActionNode()
      .getProperties()
  ...
  protected String finishImpl(
    final FacesContext context,
    final String outcome
  ) {
    if (!isJoinAllowed()) {
      Utils.addErrorMessage(
        translate(MESSAGE_ID_APPLY_FORM_MEMBERSHIP_FORBIDEN)
      );
      return null;
    }
      ...
      new RetryingTransactionCallback<String>() {
        public String execute() throws Throwable {
          try {
            ...
            Utils.addErrorMessage(
              translate(
                ManageApplicantDialog.MESSAGE_APPLY_MEMBERSHIP_FAILED,
                e.getMessage()
              ),
              e
            );
  ...
  private void sendMail(final FacesContext context, final Date date) {
    ...
    NodeRef categRef = super
      .getManagementService()
      .getCurrentCategory(this.getActionNode().getNodeRef());
    final ProfileManagerService profileManagerService =
      getProfileManagerServiceFactory().getProfileManagerService(categRef);
    Set<String> categoryAdmins = profileManagerService
      .getInvitedUsersProfiles(categRef)
      .keySet();

    // Eliminate the eventual doublons and construct a list.
    final List<String> receivers = new ArrayList<>(
      dirAdmins.size() + userAdmins.size()
    );
    ...
    for (final String receiver : receivers) {
      ...
      try {
        // Send the message
        getMailService()
          .send(noReply, adminEmail, applicantAdr, subject, body, true, false);
      } catch (Throwable t) {
        // don't stop the action but let admins know email is not getting sent
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Failed to send application email to " +
            adminEmail +
            "\n...with subject:\n" +
            subject +
            "\n...with body:\n" +

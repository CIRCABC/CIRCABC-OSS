...
public class UISchedulerReporter extends BaseDebugComponent {
  ...
  public Map getDebugData() {
    ...
    try {
      final List<TriggerReport> triggers = getServerConfigurationService()
        .getTriggers();
      for (final TriggerReport report : triggers) {
        ...
        properties.put(
          key + " - Previous Fire",
          report.getDisplayPreviousFire()
        );
        properties.put(key + " - Next Fire", report.getDisplayNexTime());
        properties.put(key + " - End Time", report.getDisplayEndTime());
        properties.put(
          key + " - Final Fire Time",
          report.getDisplayFinalFireTime()
        );
        properties.put(
          key + " - Mis Fire Instruction",
          report.getMisFireInstruction()
        );
        ...
        if (jobReport != null) {
          final String jobKey =
            report.getSimpleName() + "[" + jobReport.getSimpleName() + "]";
    ...
    try {
      final List<JobReport> simpleJobs = getServerConfigurationService()
        .getNoTriggeredJobs();
      ...
      for (final JobReport jobReport : simpleJobs) {
        ...
        properties.put(
          "<b>  Not Triggered JOB: " + jobReport.getFullname() + "</b>",
          ""
        );

...
public class CreateSpaceDialog extends BaseWaiDialog {
  ...
  private static final String MSG_TYPE_SPACE =
    "library_create_space_type_space";
  private static final String MSG_TYPE_DOSSIER =
    "library_create_space_type_dossier";
  private static final String MSG_TYPE_SHARED_LINK =
    "library_create_space_type_shared";
  private static final String MSG_TYPE_SPACE_TOOLTIP =
    "library_create_space_type_space_tooltip";
  private static final String MSG_TYPE_DOSSIER_TOOLTIP =
    "library_create_space_type_dossier_tooltip";
  private static final String MSG_TYPE_SHARED_LINK_TOOLTIP =
    "library_create_space_type_shared_tooltip";
  private static final String MSG_NO_SHARE_SPACE_AVAILABLE =
    "library_create_space_no_shared_space";
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      ...
      this.libraryChild = NavigableNodeType.LIBRARY_CHILD.isNodeFromType(
        getActionNode()
      );
    ...
    try {
      ...
      if (TYPE_DOSSIER.endsWith(containerType)) {
        ...
        createdRef = getLinksBusinessSrv()
          .createSharedSpaceLink(
            parent,
            new NodeRef(getSelectedSharedSpace()),
            title,
            description
          );
  ...
  protected String doPostCommitProcessing(FacesContext context, String outc) {
    ...
    if (
      ...
      createdRef != null &&
      isManageShareAfter() &&
      TYPE_SPACE.endsWith(containerType)
    ) {
      ...
      final Map<String, String> parameters = Collections.singletonMap(
        BaseWaiDialog.NODE_ID_PARAMETER,
        createdRef.getId()
      );
      ((ManageShareSpaceDialog) Beans.getBean(
          ManageShareSpaceDialog.BEAN_NAME
        )).init(parameters);
  ...
  public List<SelectItem> getTypes() {
    if (typeSelect == null) {
      ...
      typeSelect.add(
        new SelectItem(
          TYPE_SPACE,
          translate(MSG_TYPE_SPACE),
          translate(MSG_TYPE_SPACE_TOOLTIP)
        )
      );
      ...
      if (libraryChild) {
        typeSelect.add(
          new SelectItem(
            TYPE_SHARED_LINK,
            translate(MSG_TYPE_SHARED_LINK),
      ...
      if (libraryChild == false) {
        ...
        if (sharedSpaces.size() > 0) {
          ...
          for (final ShareSpaceItem sharedSpace : sharedSpaces) {
            availableSharedSpacesItems.add(
              new SelectItem(
                sharedSpace.getNodeRef().toString(),
                sharedSpace.getPath()
              )
            );
          }
          ...
          final SelectItem emptySelect = new SelectItem(
            VALUE_NO_SHARESPACE,
            "<" + translate(MSG_NO_SHARE_SPACE_AVAILABLE) + ">"
          );
          this.availableSharedSpacesItems = Collections.singletonList(
            emptySelect
          );
        }
      }
  ...
  public List<UIListItem> getSpaceIcons() {
    final List<ContainerIcon> containerIcons = getSpaceBusinessSrv()
      .getSpaceIcons();
  ...
  public List<UIListItem> getDossierIcons() {
    final List<ContainerIcon> containerIcons = getDossierBusinessSrv()
      .getDossierIcons();
    final List<UIListItem> convertLogos = JSFUtils.convertLogos(containerIcons);

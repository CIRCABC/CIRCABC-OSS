...
public class NavigationManager implements Serializable {
  ...
  public static boolean areStatesEquals(
  ...
  public void initNavigation(final String navigationBeanName) {
    if (navigationBeanName == null || navigationBeanName.length() < 1) {
      throw new IllegalArgumentException(
        "The navigation bean name is a mandatory parameter."
      );
    }

    // set the bean that will back the navigation
    final WaiNavigator currentBean = Beans.getWaiBrosableBean(
      navigationBeanName
    );
    ...
    if (currentBean instanceof LibraryBean) {
      renderPropertyNameFromBean =
        ((LibraryBean) currentBean).getRenderPropertyNameNavigationPreference();
    } else if (currentBean instanceof NewsGroupBean) {
      renderPropertyNameFromBean =
        ((NewsGroupBean) currentBean).getRenderPropertyNameNavigationPreference();
    } else if (currentBean instanceof InformationBean) {
      renderPropertyNameFromBean =
        ((InformationBean) currentBean).getRenderPropertyNameNavigationPreference();
    } else {
    ...
    if (currentBean == null) {
      throw new IllegalArgumentException(
        ...
        navigationBeanName +
        " must be declared as a managed bean."
      );
    }
  }
  ...
  public String getDescription() {
    return getSecurityService()
      .getCleanHTML(getState().getBean().getPageDescription(), true);
  }
  ...
  public String getTitle() {
    return getSecurityService()
      .getCleanHTML(getState().getBean().getPageTitle(), false);
  }
  ...
  public NavigationState getState() {
    if (state == null) {
      ...
      if (Beans.getWaiNavigator().getCurrentNodeType() == null) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Redirect to the latest requested page:" +
            Beans.getWaiNavigator().getCurrentNodeType().getBeanName()
          );
        }
  ...
  public void restoreState(final NavigationState state) {
    ...
    if (
      !state
        .getNode()
        .getId()
        .equals(Beans.getWaiNavigator().getCurrentNodeId())
    ) {
  ...
  public String toString() {
    return (
      "The navigation being managed: \n\tPage: " +
      getPage() +
      "\n\tBean: " +
      getState().getBean().getManagedNodeType().getBeanName() +
      "\n\tNode Type: " +
      getBean().getManagedNodeType().getComparatorQName()
    );
  }
    ...
    if (Beans.getWaiNavigator().getCurrentIGService() != null) {
      ...
      if (type != null) {
        if (NavigableNodeType.DIRECTORY.equals(type)) {
          ...
          if (
            NavigableNodeType.NEWSGROUP_POST.equals(type) ||
            NavigableNodeType.LIBRARY_POST.equals(type)
          ) {

...
public class UndoCheckoutDialog extends BaseWaiDialog {
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (getActionNode() != null) {
      ...
      if (
        getNavigator()
          .getCurrentNodeType()
          .equals(NavigableNodeType.LIBRARY_CONTENT) ||
        getNavigator()
          .getCurrentNodeType()
          .equals(NavigableNodeType.INFORMATION_CONTENT)
      ) {
      ...
      NodeRef currentInterestGroup = getManagementService()
        .getCurrentInterestGroup(locked);
      logRecord.setIgID(
        (Long) getNodeService()
      ...
      logRecord.setIgName(
        (String) getNodeService()
          .getProperty(currentInterestGroup, ContentModel.PROP_NAME)
      );
      Path path = new Node(locked).getNodePath();
      logRecord.setPath(PathUtils.getCircabcPath(path, true));

      final String resultOutcome = undoCheckoutFileDialog.undoCheckoutFile(
        FacesContext.getCurrentInstance(),
        ""
      );
  ...
  private NodeRef getLockedDocument() {
    return getCheckOutCheckInService()
      .getCheckedOut(getActionNode().getNodeRef());
  }
  ...
  public final void setUndoCheckoutFileDialog(
    CCUndoCheckoutFileDialog undoCheckoutFileDialog
  ) {

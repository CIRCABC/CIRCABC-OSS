...
public class RestContentDownloadServlet extends BaseServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    hasGroupAspect =
      nodeService.hasAspect(contentRef, CircabcModel.ASPECT_INFORMATION) ||
      nodeService.hasAspect(contentRef, CircabcModel.ASPECT_LIBRARY);
    ...
    String filename = nodeService
      .getProperty(contentRef, ContentModel.PROP_NAME)
      .toString();
    String headerContentDispo =
      CommonUtils.generateFilenameContentDispositionHeader(true, filename, req);
    ...
    if (mimetype == null || mimetype.length() == 0) {
      MimetypeService mimetypeMap = getServiceRegistry(
        getServletContext()
      ).getMimetypeService();
  ...
  private boolean processSingleRange(
    ...
    String contentRange =
      "bytes " +
      Long.toString(r.getStart()) +
      "-" +
      Long.toString(r.getEnd()) +
      "/" +
      Long.toString(reader.getSize());
    res.setHeader(HEADER_CONTENT_RANGE, contentRange);
    res.setHeader(
      HEADER_CONTENT_LENGTH,
      Long.toString((r.getEnd() - r.getStart()) + 1L)
    );
  ...
  private LogRecord logDownloadRequest(NodeRef nodeRef) {
    ...
    String filename = serviceRegistry
      .getNodeService()
  ...
  private LogRecord setLogRecord(

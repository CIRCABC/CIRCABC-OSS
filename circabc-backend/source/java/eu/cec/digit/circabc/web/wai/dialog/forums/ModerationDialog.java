...
public class ModerationDialog extends BaseWaiDialog {
  ...
  private static final String MSG_APPROVED_SUCCESS =
    "post_moderation_approve_success";
  private static final String MSG_APPROVED_FAILURE =
    "post_moderation_approve_failure";
  private static final String MSG_REMOVED_SUCCESS =
    "post_moderation_remove_abuse_success";
  private static final String MSG_REMOVED_FAILURE =
    "post_moderation_remove_abuse_failure";
  /**
  ...
  public void acceptDirect(ActionEvent event) {
    ...
    try {
      ...
      Utils.addStatusMessage(
        ...
        translate(MSG_APPROVED_SUCCESS)
      );
  ...
  protected void reject(final String message) throws Exception {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node successfully reject: " + nodeRef + ". With message: " + message
      );
    }

    final String creator = (String) getActionNode()
      .getProperties()
    ...
    if (personService.personExists(creator)) {
      ...
      final String moderator = (String) props.get(
        ModerationModel.PROP_REJECT_BY
      );
      ...
      if (logger.isWarnEnabled()) {
        logger.warn(
          "Impossible to found the nodRef of the creator of the post  " +
          nodeRef +
  ...
  protected void signalAbuse(final String message) throws Exception {
    final NodeRef nodeRef = getActionNode().getNodeRef();
    final AbuseReport report = getModerationService()
      .signalAbuse(nodeRef, message);
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node successfully signaled as abuse: " +
        nodeRef +
        ". With message: " +
        message
      );
    ...
    final String reporterEmail = getUserService()
      .getUserEmail(report.getReporter());
  ...
  protected Set<NodeRef> getModerators() {
    ...
    final NodeRef topicRef = getNodeService()
      .getPrimaryParent(postRef)
      .getParentRef();
    final NodeRef igRef = getManagementService()
      .getCurrentInterestGroup(topicRef);
    ...
    if (getNodeService().hasAspect(topicRef, CircabcModel.ASPECT_LIBRARY)) {
      ...
      getNodeService().hasAspect(topicRef, CircabcModel.ASPECT_NEWSGROUP)
    ) {
    ...
    final List<Map> moderators = PermissionUtils.getInterestGroupAuthorities(
      topicRef,
    ...
    for (Map moderator : moderators) {
      final String authority = (String) moderator.get(
        PermissionUtils.KEY_AUTHORITY
      );
  ...
  public String getCreator() {
    return (String) getActionNode()
      .getProperties()
      .get(ContentModel.PROP_CREATOR.toString());
  }

  public String getCreated() {
    final Date date = (Date) getActionNode()
      .getProperties()
      .get(ContentModel.PROP_CREATED.toString());
    final String formatedDate = WebClientHelper.formatLocalizedDate(
      date,

...
public class EditNodePropertiesDialog extends BaseWaiDialog {
  ...
  protected static final Map<ManagedNodes, String> ICONS_CONFIG = new HashMap<>(
    10
  );
  ...
  private static final Log logger = LogFactory.getLog(
    EditNodePropertiesDialog.class
  );
    ...
    if (parameters != null) {
      ...
      if (actionNodeType == null) {
        throw new IllegalArgumentException(
          "This kind of node is not managed by this dialog. Node Type : " +
          getActionNode().getType().getLocalName() +
          " . Expected: " +
          Arrays.toString(ManagedNodes.values())
        );
      }

      if (
        ManagedNodes.EMPTY_TRANSLATION.equals(actionNodeType) ||
        ManagedNodes.TRANSLATION.equals(actionNodeType)
      ) {
      ...
      final String propertyNotifyAsString = parameters.get(
        NOTIFY_AFTER_SETTING_PROPERTIES
      );
  ...
  private void setMimeTypeAndEncoding() {
    ...
    if (content != null) {
      this.editableNode.getProperties()
        .put(TEMP_PROP_MIMETYPE, content.getMimetype());
      this.editableNode.getProperties()
        .put(TEMP_PROP_ENCODING, content.getEncoding());
    }
  }
  ...
  public BehaviourFilter getPolicyBehaviourFilter() {
    if (policyBehaviourFilter == null) {
      policyBehaviourFilter = Services.getCircabcServiceRegistry(
  ...
  protected String finishImpl(final FacesContext context, final String outcome)
    ...
    boolean isIGRoot = getNodeService()
      .hasAspect(nodeRef, CircabcModel.ASPECT_IGROOT);
    ...
    if (!isIGRoot && !isCleanTitleHTML()) {
      Utils.addErrorMessage(
        translate(MSG_ERR_INVALID_VALUE_FOR, translate("title"))
      );
      this.isFinished = false;
      return null;
    }
    ...
    if (!isIGRoot && !isCleanContactInfoHTML()) {
      Utils.addErrorMessage(
        translate(MSG_ERR_INVALID_VALUE_FOR, translate("contact"))
      );
      this.isFinished = false;
      return null;
    }
  ...
  private boolean isClean(String qnameString) {
    final String value = (String) this.editableNode.getProperties()
      .get(qnameString);
  ...
  protected void editContentProperties() throws Exception {
    ...
    Map<QName, Serializable> repoProps =
      this.getNodeService().getProperties(nodeRef);
    ...
    if (mimetype != null) {
      ...
      ContentData contentData = (ContentData) editedProps.get(
    ...
    if (
      this.getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_AUTHOR) ==
      false
    ) {
      this.getNodeService()
        .addAspect(nodeRef, ContentModel.ASPECT_AUTHOR, null);
    }

    // add the "titled" aspect if required, properties will get set below
    if (
      this.getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_TITLED) ==
      false
    ) {
    ...
    Map<String, Map<String, AssociationRef>> addedAssocs =
      this.editableNode.getAddedAssociations();
    ...
    Map<String, Map<String, AssociationRef>> removedAssocs =
      this.editableNode.getRemovedAssociations();
    ...
    Map<String, Map<String, ChildAssociationRef>> addedChildAssocs =
      this.editableNode.getAddedChildAssociations();
    for (Map<
      String,
      ChildAssociationRef
    > typedAssoc : addedChildAssocs.values()) {
    ...
    Map<String, Map<String, ChildAssociationRef>> removedChildAssocs =
      this.editableNode.getRemovedChildAssociations();
    for (Map<
      String,
      ChildAssociationRef
    > typedAssoc : removedChildAssocs.values()) {
      for (ChildAssociationRef assoc : typedAssoc.values()) {
        this.getNodeService()
          .removeChild(assoc.getParentRef(), assoc.getChildRef());
      }
    }
  }

  private void addRemainingProperties(
    Map<String, Object> editedProps,
    Map<QName, Serializable> repoProps
  ) {
    for (Map.Entry<String, Object> item : editedProps.entrySet()) {
      ...
      if (propValue instanceof String) {
        PropertyDefinition propDef =
          this.getDictionaryService().getProperty(qname);
  ...
  protected void editContainerProperties(boolean isIGRoot) throws Exception {
    ...
    if (name != null) {
      if (
        ManagedNodes.ML_CONTAINER.equals(actionNodeType) ||
        ManagedNodes.CATEGORY_HEADER_NODE.equals(actionNodeType)
      ) {
        // the ml container can bve renamed via file folder service.
        this.getNodeService()
          .setProperty(nodeRef, ContentModel.PROP_NAME, name);
        String oldName = (String) editedProps.get(
        ...
        String oldName =
          this.getFileFolderService().getFileInfo(nodeRef).getName();
        repoProps.put(ContentModel.PROP_NAME, oldName);
    ...
    if (
      this.getNodeService()
        .hasAspect(nodeRef, ApplicationModel.ASPECT_UIFACETS) ==
      false
    ) {
      this.getNodeService()
        .addAspect(nodeRef, ApplicationModel.ASPECT_UIFACETS, null);
    }
    ...
    for (Map.Entry<String, Object> item : editedProps.entrySet()) {
      ...
      if (qname.equals(CircabcModel.PROP_LIGHT_DESCRIPTION)) {
        ...
        if (
          ...
          throw new LightDescriptionSizeExceedException(
            translate("lightDescription_limit_exceed")
          );
        }
      ...
      if (
        ...
        if (propDef != null) {
          ...
          if (
            propDefDataTypeName.equals(DataTypeDefinition.DOUBLE) ||
  ...
  protected boolean isNameValid() {
    ...
    return (
      name != null && name.trim().length() > 0 && !name.matches(FILENAME_REGEX)
    );
  }
  ...
  protected boolean fieldMandatoryIsNotMissing(FacesContext context) {
    ...
    if (name == null || name.length() < 1) {
      Utils.addErrorMessage(
        translate("edit_space_error_required_field_fileName")
      );
      this.isFinished = false;
      return false;
    }
    ...
    if (editableNode.hasAspect(DocumentModel.ASPECT_CPROPERTIES)) {
      ...
      if (status == null || status.length() < 1) {
        Utils.addErrorMessage(
          translate("edit_content_error_required_field_status")
        );
        this.isFinished = false;
        return false;
      }
  ...
  protected String doPostCommitProcessingIntern(
  ...
  public List<UIListItem> getIcons() {
    ...
    Config config = Application.getConfigService(
      FacesContext.getCurrentInstance()
  ...
  public String getBrowserTitle() {
    return translate(
      "edit_" + actionNodeType.toString() + "_properties_dialog_browser_title"
    );
  }

  public String getPageIconAltText() {
    return translate(
      "edit_" + actionNodeType.toString() + "_properties_dialog_icon_tooltip"
    );
  }
  ...
  public String getContainerTitle() {
    return translate(
      "edit_" + actionNodeType.toString() + "_properties_dialog_title"
    );
  }
  ...
  public String getContainerDescription() {
    return translate(
      "edit_" + actionNodeType.toString() + "_properties_dialog_description"
    );
  }
    ...
    switch (actionNodeType) {
      ...
      default:
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Not defined logging activity and service for Action Node Type " +
            actionNodeType.toString()
          );
        }
    }

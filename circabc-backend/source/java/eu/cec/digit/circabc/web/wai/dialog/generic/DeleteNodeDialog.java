...
public class DeleteNodeDialog extends BaseWaiDialog {
  ...
  private static final String MSG_ERROR_CANT_DELETE_SPACE_THAT_CONTAINS_CHECKOUT_DOCUMENTS =
  ...
  protected String finishImpl(final FacesContext context, final String outcome)
    ...
    try {
      ...
      NodeRef igRoot = getManagementService()
      ...
      if (getActionNode() != null && actionNodeType != null) {
        if (ManagedNodes.ML_CONTAINER.equals(actionNodeType)) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Trying to delete multilingual container: " +
              getActionNode().getId() +
          ...
          getMultilingualContentService()
            .deleteTranslationContainer(getActionNode().getNodeRef());
        } else {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Trying to delete " +
          ...
          parent = getNodeService()
            .getPrimaryParent(getActionNode().getNodeRef())
            .getParentRef();
  ...
  public String getConfirmMessage() {
    if (
      getActionNode() != null &&
      getNodeService().exists(getActionNode().getNodeRef()) &&
      actionNodeType != null
    ) {
      return translate(
        "delete_" + actionNodeType.toString() + "_dialog_confirmation",
  ...
  public String getBrowserTitle() {
    return translate(
      "delete_" + actionNodeType.toString() + "_dialog_browser_title"
    );
  }

  public String getPageIconAltText() {
    return translate(
      "delete_" + actionNodeType.toString() + "_dialog_icon_tooltip"
    );
  }
  ...
  public String getContainerDescription() {
    return translate(
      "delete_" + actionNodeType.toString() + "_dialog_description"
    );
  }
}

...
public class UICalendar extends UIComponentBase {
  ...
  public static final String WEEK_START_SUNDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(0);
  public static final String WEEK_START_MONDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(1);
  public static final String WEEK_START_TUESDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(2);
  public static final String WEEK_START_WEDNESDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(3);
  public static final String WEEK_START_THURSDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(4);
  public static final String WEEK_START_FRIDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(5);
  public static final String WEEK_START_SATURDAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(6);
  public static final String WEEK_START_TODAY =
    EventModel.WEEK_START_DAY_CONSTRAINT_VALUES.get(7);
  private static final List<String> VALID_VIEW_MODES =
  ...
  private static final String MSG_PREVIOUS_TOOLTIP =
    "calendar_previous_action_tooltip";
  private static final String MSG_NEXT_TOOLTIP = "calendar_next_action_tooltip";
  private static final String MSG_SELECTED_DATE_TOOLTIP =
    "calendar_selected_date_action_tooltip";
  private static final String MSG_ONE_DAY_TOOLTIP =
    "calendar_one_day_rich_tooltip";
  ...
  private static final ThreadLocal<DateFormat> _DATE_FORMAT = new ThreadLocal<
    DateFormat
  >() {
    @Override
  ...
  private static final String ACTION_LINK_COMPONENT =
    "eu.cec.digit.circabc.faces.ActionLink";
  private static final Class ACTION_CLASS_ARGS[] = {
  ...
  private void writeFiveDaysColumns(
  ...
  private void writeOneDay(final FacesContext context, final int dateDiff)
    throws IOException {
    final ResponseWriter writer = context.getResponseWriter();
    final String eventRootId = Beans.getWaiNavigator().getCurrentNodeId();
    ...
    //start the table
    writer.write(
      "<table border=\"0\" class=\"" + getCssPrefix() + TABLE_STYLE + "\""
    );
    ...
    if (getShowPreviousNextLinks()) {
      ...
      newCalendar.set(
        Calendar.DATE,
        newCalendar.get(Calendar.DATE) - dateDiff - 1
      );
      ...
      UIActionLink previous = buildActionLink(
        context,
        getAction(),
        getActionListener(),
        "<<",
        Application.getMessage(context, MSG_PREVIOUS_TOOLTIP),
        eventRootId
      );
      ...
      writer.write(
        "    <td colspan=\"3\" class=\"" + getCssPrefix() + TITLE_STYLE + "\" >"
      );

      writer.write(
        decoratorObject.getWeekdayTitle(
      ...
      UIActionLink next = buildActionLink(
        context,
        getAction(),
        getActionListener(),
        ">>",
    ...
    final List<AppointmentUnit> appointments =
      decoratorObject.getAppointments();
    ...
    while (currentTime.isBefore(endDayStart)) {
      ...
      writer.write(
        "    <th colspan=\"1\" rowspan=\"4\" class=\"" +
        cssPrefix +
        decoratorObject.getDayStyleClass(true, true) +
        "\" >"
      );
      writer.write(
        padInt(currentTime.getHourOfDay()) +
        ":" +
        padInt(currentTime.getMinuteOfHour()) +
        HTML.NBSP_ENTITY +
        HTML.NBSP_ENTITY
      );
      writer.write("</th>\r\n");
  ...
  private void writeASingleDayQuarter(
    final FacesContext context,
    LocalTime currentTime,
    final List<AppointmentUnit> appointments,
    final int truedayWidth
  ) throws IOException {
    final ResponseWriter writer = context.getResponseWriter();
    ...
    writer.write(
      "    <td colspan=\"6\" class=\"" +
      cssPrefix +
      decoratorObject.getDayStyleClass(false, false) +
      "\" width=\"" +
      truedayWidth +
      "\">"
    );
    ...
    for (AppointmentUnit unit : appointments) {
      if (unit.isInQuarter(currentTime)) {
        ...
        String tooltip = MessageFormat.format(
          ...
          unit.getTitle(),
          unit.getStartAsString(),
          unit.getEndAsString()
        );

        final UIActionLink currentEventAction =
          buildActionLinkForSingleAppointement(
            context,
  ...
  private void writeSevenDaysColumns(final FacesContext context)
    throws IOException {
    final ResponseWriter writer = context.getResponseWriter();
    final String eventRootId = Beans.getWaiNavigator().getCurrentNodeId();
    ...
    if (getShowPreviousNextLinks()) {
      ...
      writer.write(
        "    <td colspan=\"5\" class=\"" + getCssPrefix() + TITLE_STYLE + "\" >"
      );
    ...
    for (int i = 0; i < emptyDays; i++) {
      ...
      if (getBeyond()) {
        ...
        emptyDay.set(
          Calendar.DATE,
          startCalendar.get(Calendar.DATE) - (emptyDays - i)
        );
        decoratorObject.setCalendar(emptyDay);
        currentDayAction = buildActionLinkForDayEvents(
          context,
          convert(emptyDay.getTime()),
          decoratorObject.getEmptyDay(),
          eventRootId
        );
    ...
    while (!iteratingDate.after(endCalendar)) {
      ...
      writer.write(
        ...
        " class=\"" +
        cssPrefix +
        decoratorObject.getDayStyleClass(
          isOddMonth,
      ...
      currentDayAction = buildActionLinkForDayEvents(
        context,
        convert(iteratingDate.getTime()),
        decoratorObject.getDay(null),
        eventRootId
      );
      Utils.encodeRecursive(context, currentDayAction);
      ...
      if (
        iteratingDate.getActualMaximum(Calendar.DATE) ==
        iteratingDate.get(Calendar.DATE)
      ) {
        if (isOddMonth) {
          isOddMonth = false;
        } else {
  ...
  protected void checkDate() {
    ...
    if (startCalendar == null) {
      ...
      if (VIEW_MODE_MONTH.equals(mode)) {
        ...
        startCalendar.set(
          Calendar.MONTH,
          getStartMonthOfTrimester(startCalendar.get(Calendar.MONTH))
        );
        startCalendar.set(Calendar.DAY_OF_MONTH, 1);
      }
    }
  ...
  protected JSFCalendarDecorator initDecorator(
    String calendarDecoratorClassName
  ) {
    try {
      ...
      throw new IllegalArgumentException(
        "Cannot resolve decorator: " + decorator
      );
    }
  }

  protected void initWeekStartDay(
    final Calendar cal,
  ...
  private UIActionLink buildActionLinkForDayEvents(
    final FacesContext context,
    final String date,
    final String value,
    final String valueIdParam
  ) {
    final String tooltip = MessageFormat.format(
      Application.getMessage(context, MSG_SELECTED_DATE_TOOLTIP),
      date
    );
    final UIActionLink actionLink = buildActionLink(
      context,
      getAction(),
      getActionListener(),
      value,
  ...
  private UIActionLink buildActionLinkForSingleAppointement(
    final FacesContext context,
    final String title,
    final String tooltip,
    final AppointmentUnit appointment
  ) {
    final UIActionLink actionLink = buildActionLink(
      context,
      getAction(),
  ...
  private UIActionLink buildActionLink(
    final FacesContext context,
    final String action,
    final String actionListener,
    final String value,
    final String tooltip,
    final String valueIdParam
  ) {
    final javax.faces.application.Application facesApp =
      context.getApplication();
    final UIActionLink actionLink = (UIActionLink) facesApp.createComponent(
    ...
    if (actionListener != null) {
      actionLink.setActionListener(
        context
          .getApplication()
          .createMethodBinding(actionListener, ACTION_CLASS_ARGS)
      );
    }
  ...
  private void addParam(
    final FacesContext context,
    UIActionLink actionLink,
    final String paramName,
    final String paramValue
  ) {
    final javax.faces.application.Application facesApp =
      context.getApplication();
    final UIParameter actionParam = (UIParameter) facesApp.createComponent(
      ComponentConstants.JAVAX_FACES_PARAMETER
    );

...
public class DynamicPropertyGenerator extends BaseComponentGenerator {
  ...
  public UIComponent generateAndAdd(
    ...
    if (igRoot == null) {
      final PropertySheetItem child = getPropertySheetItemByName(
        propertySheet,
        item.getName()
      );
      ...
      if (dynamicProperty == null) {
        ...
        final Object titleCompenent = (child.getChildren() != null &&
  ...
  public UIComponent generate(FacesContext context, String id) {
    ...
    if (dynamicProperty != null) {
      switch (dynamicProperty.getType()) {
        ...
        case MULTI_SELECTION:
          component = context
            .getApplication()
            .createComponent(UICircabcSelectMany.COMPONENT_TYPE);
          FacesHelper.setupComponentId(context, component, id);

          // create the list of choices
          UISelectItems multiSelectItemsComponent = (UISelectItems) context
            .getApplication()
            .createComponent("javax.faces.SelectItems");
          ...
          break;
        case TEXT_AREA:
          component = ((TextAreaGenerator) Beans.getBean(
              "TextAreaGenerator"
            )).generate(context, id);
          break;
      }
    ...
    if (component == null) {
      component = ((TextFieldGenerator) Beans.getBean(
          "TextFieldGenerator"
        )).generate(context, id);
    }
  ...
  protected PropertySheetItem getPropertySheetItemByName(
    final UIPropertySheet propertySheet,
    final String name
  ) {
  ...
  protected DynamicProperty getDynPropertyByName(
    final FacesContext context,
    final String name,
    final NodeRef ref
  ) {
    final String pref = name.substring(0, name.indexOf(':') + 1);
    final String indexStr = name.substring(
      pref.length() + PROPERTY_PREFIX.length()
    );

    final Long index = Long.parseLong(indexStr);

    final DynamicPropertyService propertyService = getDynamicPropertyService(
      context
    );
    final NodeRef igRoot = getManagementService(
      context
    ).getCurrentInterestGroup(ref);
    final List<DynamicProperty> properties =
      propertyService.getDynamicProperties(igRoot);
  ...
  protected DynamicPropertyService getDynamicPropertyService(
    final FacesContext context
  ) {

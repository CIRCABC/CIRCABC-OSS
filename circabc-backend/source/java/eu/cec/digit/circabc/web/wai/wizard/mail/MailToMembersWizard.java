...
public class MailToMembersWizard extends AbstractMailToUsersBean {
  ...
  private static final String ERROR_DURING_QUERY_EXCECUTION =
    "error_during_query_excecution";
  private static final String PLEASE_AFFINE_YOUR_SEARCH_TO_MUCH_RESULTS =
    "please_affine_your_search_to_much_results";
  private static final long serialVersionUID = -6045607817876349903L;
  private static final Log logger = LogFactory.getLog(
    MailToMembersWizard.class
  );
  /**
   * I18N message strings
   */
  private static final String MESSAGE_ID_MAIL_TO_MEMBERS_ERROR =
    "mail_to_members_process_failed";
  ...
  private static final String ALL_CIRCABC_ADMIN_MSG =
    "mail_to_all_circabc_admin";

  private static final String ALL_CATEGORY_ADMINS_VALUE =
    "__ALL_CATEGORY_ADMINS_";
  private static final String ALL_CATEGORY_ADMINS_MSG =
    "mail_to_members_all_cat_admins";

  private static final String ALL_CATEGORY_ADMIN_VALUE =
    "__ALL_CATEGORY_ADMIN_";
  ...
  private static final String ALL_IG_LEADERS_MSG =
    "mail_to_members_all_ig_leaders";

  private static final Map<String, String> CIRCABC_SELECTION = new HashMap<>(1);
  private static final Map<String, String> CATEGORY_SELECTION = new HashMap<>(
    1
  );
   ...
   */
  public final transient Set<SelectedUser> selectedMembers = new HashSet<>();
  ...
  public void init(final Map<String, String> params) {
    ...
    final NavigableNode root = (NavigableNode) getNavigator()
      .getCurrentIGRoot();
   ...
   */
  protected final String finishImpl(
    final FacesContext context,
    final String outcome
  ) throws Exception {
    try {
      if (validateMailData()) {
        final int nbOfUsers = mailToMembers(
          context,
        ...
        Utils.addStatusMessage(
          ...
          translate(MSG_MAIL_SUCCESS, nbOfUsers)
        );
      ...
      Utils.addErrorMessage(
        translate(MESSAGE_ID_MAIL_TO_MEMBERS_ERROR, e.getMessage()),
        e
      );
  ...
  protected int mailToMembers(
    final FacesContext context,
    final NodeRef nodeRef
  ) {
    final User user = Application.getCurrentUser(context);
    String from = (String) getNodeService()
      .getProperty(user.getPerson(), ContentModel.PROP_EMAIL);
    ...
    final ProfileManagerService profileService =
      getProfileManagerServiceFactory().getProfileManagerService(currentNode);
    ...
    for (final SelectedUser selectedMember : this.selectedMembers) {
      ...
      if (getPersonService().personExists(authority) == true) {
        ...
        if (
          NavigableNodeType.CATEGORY.isNodeFromType(getActionNode()) == false
        ) {
        ...
        final CircabcRootProfileManagerService circabcRootProfileService =
        ...
        final Profile circabcAdmin = circabcRootProfileService.getProfile(
          circabcNodeRef,
        ...
        final CategoryProfileManagerService catProfileService =
        ...
        for (final NodeRef category : navigationBusinessSrv.getCategories()) {
          final Profile catAdmin = catProfileService.getProfile(
            category,
    ...
    for (final NodeRef person : persons) {
      ...
      extraModelParams.put(
        AbstractMailToUsersBean.ATTACHED_FILES,
        attachedLinks
      );
   ...
   * @param authority
   */
  private void addPersonsInProfile(
    final Set<NodeRef> persons,
    final NodeRef currentNode,
    final ProfileManagerService profileService,
    final String authority
  ) {
    final Profile profile = profileService.getProfileFromGroup(
      currentNode,
      authority
    );
    for (final String userId : profileService.getPersonInProfile(
      currentNode,
   ...
   */
  public SelectItem[] pickerSendMailToMembersCallback(
    ...
    final RetryingTransactionHelper txnHelper =
      Repository.getRetryingTransactionHelper(fc);
    final RetryingTransactionCallback<List<? extends SelectItem>> callback =
      new RetryingTransactionCallback<List<? extends SelectItem>>() {
        public List<? extends SelectItem> execute() throws Throwable {
          ...
          if (filterIndex == PROFILES_IDX) {
            if (
              ...
              //Add Profiles
              items.addAll(
                ProfileUtils.buildMailableProfileItems(
                  getActionNode(),
                  contains,
                  alreadyInvitedAuthorities,
                  logger
                )
              );
              ...
              items.add(
                new SortableSelectItem(
                  ALL_CATEGORY_ADMINS_VALUE,
                  allAdmins,
                  allAdmins
                )
              );
            } else if (
              NavigableNodeType.CATEGORY.isNodeFromType(getActionNode())
            ) {
              //Add circabc Admin's
              final String allAdmins = translate(ALL_CIRCABC_ADMIN_MSG);
              items.add(
                new SortableSelectItem(
                  ALL_CIRCABC_ADMIN_VALUE,
                  allAdmins,
                  allAdmins
                )
              );
              ...
              for (final Map.Entry<
                String,
                String
              > entry : IG_ROOT_SELECTION.entrySet()) {
              ...
              NavigableNodeType.IG_ROOT.isNodeFromType(getActionNode())
            ) {
              //Add category Admin's if current user is an admin in at least one service
              final AnyIgServicesAdminEvaluator isAdmin =
                new AnyIgServicesAdminEvaluator();
              if (isAdmin.evaluate(getActionNode())) {
                final String allAdmins = translate(ALL_CATEGORY_ADMINS_MSG);
                items.add(
                  new SortableSelectItem(
                    ALL_CATEGORY_ADMIN_VALUE,
                    allAdmins,
                    allAdmins
                  )
                );
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "The Profile search is performed successfully and return " +
                items +
                ". Filter Index: " +
                filterIndex +
                ". Expression: " +
            ...
            if (
              ...
              items.addAll(
                PermissionUtils.buildCircabcUserItems(
                  getActionNode(),
                  contains,
                  getOnlineMembersOnly(),
                  alreadyInvitedAuthorities,
                  logger
                )
              );
              ...
              items.addAll(
                PermissionUtils.buildCategoryUserItems(
                  getActionNode(),
                  contains,
                  getOnlineMembersOnly(),
                  alreadyInvitedAuthorities,
              ...
              items.addAll(
                PermissionUtils.buildInvitedUserItemsRecursivly(
                  getActionNode(),
                  contains,
                  getOnlineMembersOnly(),
                  alreadyInvitedAuthorities,
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "The User search is performed successfully and return " +
                items +
                ". Filter Index: " +
                filterIndex +
                ". Expression: " +
            ...
            logger.error(
              "The picker is called with an invalid index parameter " +
              filterIndex +
          ...
          return items.size() != 0
            ? items
            : Collections.<SortableSelectItem>emptyList();
        }
      };

    List<? extends SelectItem> result = null;
    try {
      result = (List<? extends SelectItem>) AuthenticationUtil.runAs(
      ...
      Utils.addErrorMessage(
        Application.getMessage(fc, PLEASE_AFFINE_YOUR_SEARCH_TO_MUCH_RESULTS),
        ldap
      );
      ...
      Utils.addErrorMessage(
        Application.getMessage(fc, ERROR_DURING_QUERY_EXCECUTION),
        e
      );
    }
    if (result == null) {
  ...
  public DataModel getSelectedUsersDataModel() {
    ...
    this.selectedUsersDataModel.setWrappedData(
        new ArrayList<>(this.selectedMembers)
      );
      ...
      if (foundExisting == false) {
        ...
        if (authority.equals(ALL_CIRCABC_ADMIN_VALUE)) {
          AuthenticationUtil.runAs(
            new RunAsWork<Object>() {
              public Object doWork() throws Exception {
                final NodeRef circabcNodeRef = getNavigationBusinessSrv()
                  .getCircabcRoot();
                final Set<String> circabcInvitedUsers =
                  getProfileManagerServiceFactory()
                    .getCircabcRootProfileManagerService()
                    .getInvitedUsers(circabcNodeRef);
          ...
          AuthenticationUtil.runAs(
            new RunAsWork<Object>() {
              public Object doWork() throws Exception {
                for (final NodeRef category : getNavigationBusinessSrv()
                  .getCategories()) {
                  final Set<String> categoryInvitedUsers =
                    getProfileManagerServiceFactory()
                      .getCategoryProfileManagerService()
                      .getInvitedUsers(category);
              ...
              public Object doWork() throws Exception {
                final Set<String> categoryInvitedUsers =
                  getProfileManagerServiceFactory()
                    .getCategoryProfileManagerService()
                    .getInvitedUsers(
                      getNavigator().getCurrentCategory().getNodeRef()
                    );
                for (final String categoryInvitedUser : categoryInvitedUsers) {
                  addUserToSet(onlineUsers, categoryInvitedUser);
                }
          ...
          final NodeRef categoryRoot = getNavigator()
            .getCurrentNode()
            .getNodeRef();
          for (final NodeRef ig : getNavigationBusinessSrv()
            .getInterestGroups(categoryRoot)) {
            ...
            for (final Profile profile : igProfiles) {
              if (profile.isAdmin()) {
                ...
                if (
                  getAuthorityService()
                    .authorityExists(profile.getPrefixedAlfrescoGroupName())
                ) {
          ...
          final Profile profileFromGroup =
            profileManagerService.getProfileFromGroup(rootNode, authority);
  ...
  public void removeSelection(final ActionEvent event) {
    final SelectedUser wrapper =
      (SelectedUser) this.selectedUsersDataModel.getRowData();
    if (wrapper != null) {
  ...
  public void setAttachLink(NodeRef attachLink) {
    if (attachLink != null) {
      ...
      if (!linkAlreadyAdded(attachedLinks, attachLink)) {
        AttachementWrapper aw = new AttachementWrapper(
          attachLink,
          getNodeService()
            .getProperty(attachLink, ContentModel.PROP_NAME)
            .toString(),
          true
        );
  ...
  private boolean linkAlreadyAdded(
    List<AttachementWrapper> attachedLinks2,
    NodeRef attachLink2
  ) {
  ...
  public static class SelectedUser {
    ...
    public boolean equals(final Object anObject) {
      ...
      if (
        this.authority != null &&
        this.authority.equals(anotherObject.getAuthority())
      ) {
        return true;
      }
      return false;

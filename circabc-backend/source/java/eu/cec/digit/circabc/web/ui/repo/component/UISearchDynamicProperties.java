...
public class UISearchDynamicProperties extends UIComponentBase {
  ...
  public static final String CIRCABC_FACES_DATE_PICKER_RENDERER =
    "eu.cec.digit.circabc.faces.DatePickerGeneratorRenderer";
  ...
  private static final String MSG_NO_PROP =
    "advanced_search_dialog_no_dynamic_properties_defined";
  ...
  protected void createComponents(
    ...
    if (dynamicProperties != null && dynamicProperties.size() > 0) {
      ...
      getChildren()
        .add(
          generateLabel(context, Application.getMessage(context, MSG_NO_PROP))
        );
    }
  }
  ...
  protected void createComponent(
    ...
    super
      .getChildren()
      .add(
        generateLabel(context, property.getLabel().getDefaultValue() + ": ")
      );

    switch (property.getType()) {
      case DATE_FIELD:
        getChildren()
          .add(generateDatePicker(context, property, propertyBinding));
        break;
      case MULTI_SELECTION:
      case SELECTION:
        getChildren()
          .add(generateSelection(context, property, propertyBinding));
        break;
      case TEXT_AREA:
      case TEXT_FIELD:
  ...
  private List<DynamicProperty> getProperties(FacesContext context) {
    final String beanBinding =
      "#{" +
      (String) getAttributes().get("bean") +
      '.' +
      (String) getAttributes().get("var") +
      '}';
    final javax.faces.application.Application facesApp =
      context.getApplication();
    final MethodBinding binding = facesApp.createMethodBinding(
      beanBinding,
      null
    );
      ...
      for (Object obj : (List) dynamicPropertiesObj) {
        if (!(obj instanceof DynamicProperty)) {
          throw new IllegalArgumentException(
            "The list can only contain DynamicProperty."
          );
        }
  ...
  private UIComponent generateLabel(FacesContext context, String displayLabel) {
    UIOutput label = (UIOutput) context
      .getApplication()
  ...
  @SuppressWarnings("unchecked")
  private UIComponent generateDatePicker(
    FacesContext context,
    DynamicProperty property,
    ...
    final ValueBinding startYearBind = facesApp.createValueBinding(
      "#{DatePickerGenerator.startYear}"
    );
    final ValueBinding yearCountBind = facesApp.createValueBinding(
      "#{DatePickerGenerator.yearCount}"
    );
    ...
    component.setValueBinding(
      VALUE,
      facesApp.createValueBinding(propertyBinding)
    );

    return component;
  }

  private UIComponent generateInput(
    FacesContext context,
    DynamicProperty property,
    String propertyBinding
  ) {
    ...
    UIInput component = (UIInput) facesApp.createComponent(
      ComponentConstants.JAVAX_FACES_INPUT
    );
    component.setRendererType(ComponentConstants.JAVAX_FACES_TEXT);
    component.setValueBinding(
      "size",
      facesApp.createValueBinding("#{TextFieldGenerator.size}")
    );
    component.setValueBinding(
      "maxlength",
  ...
  @SuppressWarnings("unchecked")
  private UIComponent generateSelection(
    FacesContext context,
    DynamicProperty property,
    String propertyBinding
  ) {
    ...
    final UISelectOne component = (UISelectOne) facesApp.createComponent(
      UISelectOne.COMPONENT_TYPE
    );
    final UISelectItems itemsComponent =
      (UISelectItems) facesApp.createComponent(
        ComponentConstants.JAVAX_FACES_SELECT_ITEMS
      );

    final List<SelectItem> values = DynamicPropertyGenerator.getMultiValues(
      property
    );
    ...
    items.add(
      new SelectItem(
        "",
        Application.getMessage(context, SearchProperties.ANY_MESSAGE_KEY)
      )
    );
    items.addAll(values);

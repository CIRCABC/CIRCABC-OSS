...
public class UIPropertySheet
  extends org.alfresco.web.ui.repo.component.property.UIPropertySheet {

  public static final String PROPERTY_RENDERER =
    "eu.cec.digit.circabc.faces.PropertyRenderer";
   ...
   */
  private static final String CIRCABC_MESSAGE_BUNDLE =
  ...
  public void encodeBegin(FacesContext context) throws IOException {
    ...
    if (howManyChildren == 0) {
      if (externalConfig != null && externalConfig) {
        ...
        ConfigService configSvc = Application.getConfigService(
        ...
        if (getConfigArea() == null) {
          ...
          configProps = configSvc.getConfig(
            node,
            new ConfigLookupContext(getConfigArea())
          );
        }

        PropertySheetConfigElement itemsToDisplay =
          (PropertySheetConfigElement) configProps.getConfigElement(
            "property-sheet"
          );
        ...
        if (itemsToDisplay != null) {
          ...
          if (this.getMode().equalsIgnoreCase(EDIT_MODE)) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Items to render: " +
                itemsToDisplay.getEditableItemNamesToShow()
              );
            }
          } else {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Items to render: " + itemsToDisplay.getItemNamesToShow()
              );
            }
          }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Put node into session with key '" + getVar() + "': " + node
      );
    }
  ...
  private boolean isNodeNameSecured(Node node) {
    ...
    return (
      type != null &&
      (type.isIGServiceOrAbove() &&
        !type.equals(NavigableNodeType.IG_ROOT) &&
  ...
  private void createComponentsFromNode(FacesContext context, Node node)
    ...
    for (String propertyName : props.keySet()) {
      ...
      UIProperty propComp = (UIProperty) context
        .getApplication()
      ...
      FacesHelper.setupComponentId(
        context,
        propComp,
        PROP_ID_PREFIX + prefixPropName
      );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Created property component " +
          propComp +
          "(" +
          propComp.getClientId(context) +
          ") for '" +
          prefixPropName +
          "' and added it to property sheet " +
          this
        );
      }
    }
    ...
    while (iter.hasNext()) {
      ...
      UIAssociation assocComp = (UIAssociation) context
        .getApplication()
      ...
      FacesHelper.setupComponentId(
        context,
        assocComp,
        ASSOC_ID_PREFIX + prefixAssocName
      );
    ...
    while (iter.hasNext()) {
      String assocName = (String) iter.next();
      UIChildAssociation childAssocComp = (UIChildAssociation) context
        .getApplication()
        .createComponent(RepoConstants.ALFRESCO_FACES_CHILD_ASSOCIATION);
      FacesHelper.setupComponentId(
        context,
        childAssocComp,
        ASSOC_ID_PREFIX + assocName
      );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Created child association component " +
          childAssocComp +
          "(" +
  ...
  private void createComponentsFromConfig(
    FacesContext context,
    Collection<ItemConfig> items,
    ...
    for (ItemConfig item : items) {
      ...
      if (item instanceof PropertyConfig) {
        ...
        propSheetItem = (PropertySheetItem) context
          .getApplication()
          .createComponent(RepoConstants.ALFRESCO_FACES_PROPERTY);
      ...
      if (propSheetItem != null) {
        ...
        final String labelId = item.getDisplayLabelId() == null
          ? shortName
          : item.getDisplayLabelId();
        final ResourceBundle circabcWebBundle = Application.getBundle(
          context
        ...
        if (circabcLabel != null) {
          ...
          String label = Application.getMessage(
            context,
            item.getDisplayLabelId()
          );
          if (label != null) {
        ...
        if (
          isReadOnly() ||
          item.isReadOnly() ||
          (nameReadOnly && item.getName().equals("name"))
        ) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Created property sheet item component " +
            propSheetItem +
            "(" +

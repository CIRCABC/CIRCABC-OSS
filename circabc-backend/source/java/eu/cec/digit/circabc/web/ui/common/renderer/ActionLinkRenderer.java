...
public class ActionLinkRenderer extends BaseRenderer {
  ...
  public static void renderCommandLinkStart(
    ...
    if (link.getImage() != null) {
      String imageName = UtilsCircabc.buildImageNameFromHref(
      ...
      if (link.getValue() == null) {
        ...
        if (link.getEscape() != null) {
          if (!link.getEscape()) {
            ...
            writer.write(
              ESAPI.encoder().encodeForHTML(link.getValue().toString())
            );
          }
        } else {
          writer.write(link.getValue().toString());
   ...
   */
  protected static String renderNonJavaScriptAnchorStart(
    ...
    if (href != null && href.length() > 0) {
      ...
      final FormInfo formInfo = RendererUtils.findNestingForm(
        component,
        facesContext
      );
   ...
   */
  private static void addChildParametersToHref(

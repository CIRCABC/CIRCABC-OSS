...
public class UIActions extends SelfRenderingComponent {
  ...
  public static final String RENDERER_ACTIONLINK =
    "eu.cec.digit.circabc.faces.ActionRenderer";
  ...
  private static String createUniqueId() {
    try {
      return (
        "id_" +
        new BigInteger(GUID.generate().getBytes("8859_1")).toString(
          Character.MAX_RADIX
        )
      );
  ...
  public void encodeBegin(FacesContext context) throws IOException {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "encodeBegin() for <circa:actions/> Id: " +
        getId() +
        " groupId: " +
        getValue()
      );
    ...
    if (actionContext instanceof Node) {
      ...
      if (groups.contains(contextId)) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "---already built component tree for actions contextId: " +
            contextId
          );
        }
        return;
    ...
    if (groupId != null && groupId.length() != 0) {
      ...
      if (config != null) {
        ...
        ActionsConfigElement actionConfig =
          (ActionsConfigElement) config.getConfigElement(
        ...
        if (actionConfig != null) {
          ...
          if (actionGroup != null) {
            ...
            logger.warn(
              "Unable to find specified Action Group config ID: " + groupId
            );
          }
        }
      }
  ...
  public void encodeChildren(FacesContext context) throws IOException {
    ...
    boolean needContainer = ((getAttributes().get(ATTR_STYLE) != null) ||
        (getAttributes().get(ATTR_STYLECLASS) != null))
      ? true
      : false;
    if (needContainer) {
      out.write("<span ");
  ...
  private void buildActionGroup(
    ...
    HtmlPanelGroup wrapper = (HtmlPanelGroup) facesApp.createComponent(
      ComponentConstants.JAVAX_FACES_PANELGROUP
    );
    ...
    for (String actionId : actionGroup) {
      ...
      if (
        hiddenActions.contains(actionId) ||
        (CircabcConfig.ECHA && actionId.equals("alternative_browse_doc"))
      ) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "---ActionDefinition: " +
            actionId +
            " not rendered because it hidden"
          );
        }
      } else {
        ...
        if (allow != null && allow.size() != 0) {
          ...
          permEval = (PermissionEvaluator) facesApp.createComponent(
        ...
        UIActionLink control = (UIActionLink) facesApp.createComponent(
        ...
        if (actionDef.Action != null) {
          if (UIComponentTagUtils.isValueReference(actionDef.Action)) {
            control.setAction(
              facesApp.createMethodBinding(actionDef.Action, null)
            );
          } else {
        ...
        if (actionDef.ActionListener != null) {
          control.setActionListener(
            facesApp.createMethodBinding(
              actionDef.ActionListener,
              ACTION_CLASS_ARGS
            )
          );
        }
        ...
        if (actionDef.Onclick != null) {
          if (UIComponentTagUtils.isValueReference(actionDef.Onclick)) {
            control.setValueBinding(
              "onclick",
              facesApp.createValueBinding(actionDef.Onclick)
            );
          }
        }
        ...
        if (actionDef.Href != null) {
          if (UIComponentTagUtils.isValueReference(actionDef.Href)) {
            control.setValueBinding(
              "href",
              facesApp.createValueBinding(actionDef.Href)
            );
          } else {
          ...
          if (actionDef.Script.charAt(0) == '/') {
            ...
            scriptHref.append(
              Utils.replace(
                URLEncoder.encode(actionDef.Script, "UTF-8"),
                "+",
                "%20"
              )
            );
            ...
            scriptHref
              .append('/')
              .append(ref.getStoreRef().getProtocol())
              .append('/')
              .append(ref.getStoreRef().getIdentifier())
              .append('/')
              .append(ref.getId());
          }
        ...
        if (actionDef.TooltipMsg != null) {
          ...
          if (UIComponentTagUtils.isValueReference(actionDef.Tooltip)) {
            control.setValueBinding(
              "tooltip",
              facesApp.createValueBinding(actionDef.Tooltip)
            );
          } else {
        ...
        if (actionDef.LabelMsg != null) {
          ...
          if (UIComponentTagUtils.isValueReference(actionDef.Label)) {
            control.setValueBinding(
              "value",
              facesApp.createValueBinding(actionDef.Label)
            );
          } else {
        ...
        if (params != null) {
          for (Map.Entry<String, String> entry : params.entrySet()) {
            UIParameter param = (UIParameter) facesApp.createComponent(
            ...
            if (UIComponentTagUtils.isValueReference(value)) {
              param.setValueBinding(
                ATTR_VALUE,
                facesApp.createValueBinding(value)
              );
            } else {

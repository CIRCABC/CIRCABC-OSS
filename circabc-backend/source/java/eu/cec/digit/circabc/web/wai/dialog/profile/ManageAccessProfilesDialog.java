...
public class ManageAccessProfilesDialog extends BaseWaiDialog {
  ...
  /*package*/static final String MSG_NO_LONGER_PERM =
    "no_longer_manage_member_permission";
  private static final long serialVersionUID = -7326739780479536405L;
  private static final Log logger = LogFactory.getLog(
    ManageAccessProfilesDialog.class
  );
  private static final String MSG_IMPORT_EMPTY_SELECTION =
    "import_profile_empty_selection";
  private static final String MSG_IMPORT_ERROR = "import_profile_error";
  private static final String MSG_DEACTIVATE_GUEST_SUCCESS =
    "deactivate_guest_success";
  private static final String MSG_ACTIVATE_GUEST_SUCCESS =
    "activate_guest_success";
  private static final String MSG_ACTIVATE_GUEST_ERRROR =
    "activate_guest_error";
  private static final String MSG_DEACTIVATE_REGISTRED_SUCCESS =
    "deactivate_registred_success";
  private static final String MSG_ACTIVATE_REGISTRED_SUCCESS =
    "activate_registred_success";
  private static final String MSG_ACTIVATE_REGISTRED_ERROR =
    "activate_registred_error";
  private static final String MSG_ACTIVATE_REGISTRED_CAN_APPLY_SUCCESS =
    "activate_registred_can_apply_success";
  private static final String MSG_DEACTIVATE_REGISTRED_CAN_APPLY_SUCCESS =
    "deactivate_registred_can_apply_success";

  private static final String MSG_EXPORT_PROFILE_SUCCESS =
    "export_profile_success";
  private static final String MSG_EXPORT_PROFILE_FAILURE =
    "export_profile_failure";
  private static final String MSG_UNEXPORT_PROFILE_SUCCESS =
    "unexport_profile_success";
  private static final String MSG_UNEXPORT_PROFILE_FAILURE =
    "unexport_profile_failure";

  private static final String IMPORT_SELECT_EMPTY = "__SELECT_ONE__";
  private static final String PARAM_ACTIVATE = "activate";

  private static final HashMap<String, Set<String>> DEFAULT_PERMISSIONS =
    new HashMap<>();
  ...
  static {
    ...
    DEFAULT_PERMISSIONS.put(
      CircabcServices.EVENT.toString(),
  ...
  public void importProfile(final ActionEvent event) {
    if (
      exportedProfile == null || exportedProfile.equals(IMPORT_SELECT_EMPTY)
    ) {
      Utils.addErrorMessage(translate(MSG_IMPORT_EMPTY_SELECTION));
    } else if (
      getCategoryExportedProfiles().containsKey(exportedProfile) == false
    ) {
      Utils.addErrorMessage(translate(MSG_IMPORT_EMPTY_SELECTION));
    } else {
      ...
      try {
        final RetryingTransactionHelper txnHelper =
          Repository.getRetryingTransactionHelper(
            FacesContext.getCurrentInstance()
          );
        final AccessProfileWrapper wrapper = getCategoryExportedProfiles()
          .get(exportedProfile);
        ...
        final String info = MessageFormat.format(
          "Profile {0} has been imported from IG {1}",
          profile,
          igName
        );
  ...
  public void activateGuest(final ActionEvent event) {
    ...
    setLogRecord(
      activateGuestLogRecord,
      "Administration",
      "Modify guest visibility"
    );
    ...
    try {
      ...
      if (valueAsBool) {
        Utils.addStatusMessage(
          ...
          translate(MSG_ACTIVATE_GUEST_SUCCESS)
        );
        ...
        Utils.addStatusMessage(
          FacesMessage.SEVERITY_INFO,
          translate(MSG_DEACTIVATE_GUEST_SUCCESS)
        );
      ...
      Utils.addErrorMessage(
        translate(MSG_ACTIVATE_GUEST_ERRROR, e.getMessage())
      );
  ...
  public void activateRegistered(final ActionEvent event) {
    ...
    setLogRecord(
      activateRegisteredLogRecord,
      "Administration",
      "Modify registred visibility"
    );
    final Boolean valueAsBool = extractActivateParamValue(event);
    ...
    try {
      ...
      Utils.addErrorMessage(
        translate(MSG_ACTIVATE_REGISTRED_ERROR, e.getMessage())
      );
  ...
  public void activateRegisteredApply(final ActionEvent event) {
    ...
    try {
      ...
      if (valueAsBool) {
        ...
        activateRegisteredLogRecord.setInfo(
          "Registred user can apply for membership"
        );
      } else {
        Utils.addStatusMessage(
          FacesMessage.SEVERITY_INFO,
        ...
        activateRegisteredLogRecord.setInfo(
          "Registred user can't apply for membership"
        );
      }
    } catch (final Exception e) {
      if (logger.isErrorEnabled()) {
  ...
  private Profile getRegistredProfile() {
    final NodeRef igNodeRef = getIgNodeRef();
    final String registredGroupName = getCircabcProfileManager()
      .getAllCircaUsersGroupName();
    final Profile profile = getIGRootProfileManager()
      .getProfile(igNodeRef, registredGroupName);
    return profile;
  }
  ...
  public void unexportProfile(final ActionEvent event) {
    ...
    try {
      final AccessProfileWrapper wrapper =
        (AccessProfileWrapper) this.profileDataModel.getRowData();
      getIGRootProfileManager()
        .exportProfile(getIgNodeRef(), wrapper.getName(), false);

      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,
        translate(
          MSG_UNEXPORT_PROFILE_SUCCESS,
      ...
      String igName = (String) getNodeService()
        .getProperty(getIgNodeRef(), ContentModel.PROP_NAME);
      String info = MessageFormat.format(
        "Profile {0} has been exported from IG {1}",
        wrapper.getName(),
        igName
      );
      ...
      Utils.addErrorMessage(
        translate(MSG_UNEXPORT_PROFILE_FAILURE, e.getMessage())
      );
  ...
  public void exportProfile(final ActionEvent event) {
    ...
    try {
      final AccessProfileWrapper wrapper =
        (AccessProfileWrapper) this.profileDataModel.getRowData();
      getIGRootProfileManager()
        .exportProfile(getIgNodeRef(), wrapper.getName(), true);

      String profile = wrapper.getDisplayTitle();
      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,
        translate(
          MSG_EXPORT_PROFILE_SUCCESS,
          profile,
          getNavigator().getCurrentCategory().getName()
        )
      );
      ...
      Utils.addErrorMessage(
        translate(MSG_EXPORT_PROFILE_FAILURE, e.getMessage())
      );
      exportLogRecord.setOK(false);
    } finally {
      getLogService().log(exportLogRecord);
  ...
  private DataModel getDataModel(final boolean onlyImported) {
    ...
    final List<Profile> profiles = getIGRootProfileManager()
      .getProfiles(igNodeRef);
    final List<AccessProfileWrapper> webProfiles = new ArrayList<>(
      profiles.size()
    );
    final String currentUserName = getNavigator()
      .getCurrentUser()
      .getUserName();
    String userProfile = getIGRootProfileManager()
      .getPersonProfile(igNodeRef, currentUserName);
    ...
    if (hasExportedProfiles) {
      if (getCircabcService().readFromDatabase()) {
        final NodeRef category = getManagementService()
          .getCurrentCategory(igNodeRef);
        ...
        for (AccessProfileWrapper accessProfileWrapper : webProfiles) {
          if (
            accessProfileWrapper.isExported() &&
            !prefixedAlfrescoGroupNames.contains(
        ...
        for (final NodeRef ig : getManagementService()
          .getInterestGroups(category)) {
          if (!ig.equals(igNodeRef)) {
            for (final Profile prof : getProfileManagerServiceFactory()
              .getIGRootProfileManagerService()
              .getProfiles(ig)) {
              prefixedAlfrescoGroupNames.add(
                prof.getPrefixedAlfrescoGroupName()
              );
            }
          }
        }
    ...
    if (!onlyImported) {
      ...
      for (AccessProfileWrapper accessProfileWrapper : webProfiles) {
        if (
          !accessProfileWrapper.isSpecialProfile() &&
          getAuthorityService()
            .authorityExists(accessProfileWrapper.getAutority()) &&
          getAuthorityService()
  ...
  public final List<SelectItem> getExportedProfiles() {
    if (getCircabcService().readFromDatabase()) {
      ...
      List<SelectItem> exportedProfiles = AuthenticationUtil.runAs(
        new RunAsWork<List<SelectItem>>() {
          public List<SelectItem> doWork() throws Exception {
            final Map<String, AccessProfileWrapper> exportedProfiles =
              getCategoryExportedProfiles();
            final List<SelectItem> profileItems = new ArrayList<>(
              exportedProfiles.size()
            );
            ...
            for (final Map.Entry<
              String,
              AccessProfileWrapper
            > entry : exportedProfiles.entrySet()) {
              ig = entry.getValue().getFromIg();
              igName = (String) getNodeService()
                .getProperty(ig, ContentModel.PROP_NAME);
  ...
  private Boolean extractActivateParamValue(final ActionEvent event)
    ...
    if (
      ...
      throw new IllegalArgumentException(
        "Param 'activate' accept only true or false value."
      );
    }
  ...
  private boolean computeVisibility(final Profile profile) {
    final Set<String> servicePermissions = profile.getServicePermissions(
      CircabcServices.VISIBILITY.toString()
    );
  ...
  private Map<String, AccessProfileWrapper> getCategoryExportedProfiles() {
    if (getCircabcService().readFromDatabase()) {
      if (this.categoryExportedProfiles == null) {
        final NodeRef category = getNavigator()
          .getCurrentCategory()
          .getNodeRef();
        final NodeRef currentIg = getIgNodeRef();
        final List<ExportedProfileItem> expProfiles = getCircabcService()
        ...
        AuthenticationUtil.runAs(
          new RunAsWork<Object>() {
            public Object doWork() throws Exception {
              ...
              final ProfileManagerService profService =
                getIGRootProfileManager();
              for (ExportedProfileItem exportedProfileItem : expProfiles) {
                NodeRef igNodeRef = new NodeRef(
                  exportedProfileItem.getNodeRef()
                );
                final Profile profile = profService.getProfile(
                  igNodeRef,
                  exportedProfileItem.getName()
                );
                categoryExportedProfiles.put(
        ...
        AuthenticationUtil.runAs(
          new RunAsWork<Object>() {
            public Object doWork() throws Exception {
              ...
              final ProfileManagerService profService =
                getIGRootProfileManager();
              final List<NodeRef> allIg = getManagementService()
                .getInterestGroups(category);
              ...
              for (final NodeRef ig : allIg) {
                if (currentIg.equals(ig) == false) {
                  for (final Profile profile : profService.getExportedProfiles(
                    ig
                  )) {
                    if (
                      importedProfileAuthority.contains(
                        profile.getPrefixedAlfrescoGroupName()
                      ) ==
                      false
                    ) {
  ...
  private void setVisibility(final Boolean valueAsBool, final Profile profile)
    ...
    final InterestGroupNode igRoot = (InterestGroupNode) getNavigator()
      .getCurrentIGRoot();
    ...
    if (newsgroup != null) {
      getNewsGroupProfileManager()
        .updateProfile(
          newsgroup.getNodeRef(),
          profile.getProfileName(),
          profile.getServicesPermissions(),
          true
        );

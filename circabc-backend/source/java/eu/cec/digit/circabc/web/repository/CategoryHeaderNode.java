...
public class CategoryHeaderNode extends NavigableNode {
  ...
  public CategoryHeaderNode(
    ...
    final CircabcRootNode circabcRootNode
  ) {
    super(nodeRef);
    ...
    if (!getNavigableNodeType().equals(NavigableNodeType.CATEGORY_HEADER)) {
      throw new IllegalArgumentException(
        "NodeRef must be a Circabc Header. It's node type must: " +
  ...
  public List<NavigableNode> getNavigableChilds() {
    if (headersNode == null) {
      ...
      final List<NodeRef> categoriesNodeRef = WebClientHelper.getNodesFromPath(
        path,
        getCircabcServices().getNonSecuredSearchService()
      );
      ...
      final RetryingTransactionHelper txnHelper = getServiceRegistry()
            ...
            for (final NodeRef cat : categoriesNodeRef) {
              if (getCircabcServices().getNonSecuredNodeService().exists(cat)) {
                ...
                if (
                  ...
                  externalLinks.add(
                    new ExternalLinkNode(cat, categoryHeaderNode)
                  );
                }
              }
            }
  ...
  public NavigableNode getNavigableParent() {
    if (circabcRootNode == null) {
      circabcRootNode = new CircabcRootNode(
        getManagementService().getCircabcNodeRef()
      );
    }

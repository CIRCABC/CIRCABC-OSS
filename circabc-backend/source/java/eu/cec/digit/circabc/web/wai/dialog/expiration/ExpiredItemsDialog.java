 ...
 * @author makz
 */
public class ExpiredItemsDialog
  extends BaseWaiDialog
  implements IContextListener {

  private static final long serialVersionUID = 1L;

  private static final String MSG_DELETED_ITEMS_SGL =
    "delete_expired_items_dialog_success_single";
  private static final String MSG_DELETED_ITEMS =
    "delete_expired_items_dialog_success";
  ...
  private static final String PROP_NAME = Repository.escapeQName(
  ...
  private static final String QUERY_SEARCH_TEXT =
    QUERY_SEARCH_All + " AND TEXT:\"%s\"";
  ...
  private NodePropertyResolver resolverSmallIcon = new NodePropertyResolver() {
    ...
    public Object get(Node node) {
      QNameNodeMap<String, String> props = (QNameNodeMap<
          String,
          String
        >) node.getProperties();
  ...
  private NodePropertyResolver resolverExpirationDate =
  ...
  private NodePropertyResolver resolverIsMultilingualContainer =
    new NodePropertyResolver() {
      private static final long serialVersionUID = -9181535522349485509L;
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (this.getConfirmationItems().size() > 1) {
      ...
      Utils.addStatusMessage(
        FacesMessage.SEVERITY_INFO,
        translate(MSG_DELETED_ITEMS_SGL)
      );
    }
      ...
      if (results != null && results.length() != 0) {
        ...
        for (final ResultSetRow row : results) {
          ...
          node.addPropertyResolver(
            "isMultilingualContainer",
            resolverIsMultilingualContainer
          );

          QName nodeType = getInternalNodeService().getType(nodeRef);
          if (
            getDictionaryService()
                .isSubClass(nodeType, ContentModel.TYPE_FOLDER) ==
              true &&
            getDictionaryService()
              .isSubClass(nodeType, ContentModel.TYPE_SYSTEM_FOLDER) ==
            false
          ) {
  ...
  private String buildSearchQuery() {
    ...
    if (this.getSearchText() == null || this.getSearchText().length() == 0) {
      ...
      query = String.format(
        QUERY_SEARCH_All,
        actionNode.getNodeRef().toString()
      );
    } else {
  ...
  public String getCurrentSpaceNode() {
    ExternalContext context = FacesContext.getCurrentInstance()
      .getExternalContext();
    this.currentSpaceNode = (String) context
      .getSessionMap()
  ...
  public String getCurrentSpacePath() {
    if (currentSpacePath == null) {
      ExternalContext context = FacesContext.getCurrentInstance()
        .getExternalContext();
      currentSpacePath = (String) context
        .getSessionMap()

...
public class UICircabcContentSelector extends UIContentSelector {
  ...
  private static final Log logger = LogFactory.getLog(
    UICircabcContentSelector.class
  );
  ...
  protected void getAvailableOptions(FacesContext context, String contains) {
    ...
    if (igNodeRefAsString == null) {
      ...
      customGetAvailableOptions(
        context,
        contains,
        new NodeRef(igNodeRefAsString)
      );
    }
  }
  ...
  private void customRenderAvailableOptions(
    ...
    if (itemsPresent) {
      for (NodeRef item : this.availableOptions) {
        ...
        out.write(
          Utils.encode(
            PathUtils.getInterestGroupPath(nodeService.getPath(item), true)
          )
        );
        out.write("/");
  ...
  private void customGetAvailableOptions(
    ...
    if (igNodeRef != null) {
      query
        .append(" AND  PATH:\"")
        .append(WebClientHelper.getPathFromSpaceRef(igNodeRef, true));
      query.append("\"");
    }
    ...
    if (contains != null && contains.length() > 0) {
      ...
      String nameAttr = Repository.escapeQName(
    ...
    int maxResults = Application.getClientConfig(
      context
    ...
    try {
      results = Repository.getServiceRegistry(context)
        .getSearchService()
        .query(searchParams);

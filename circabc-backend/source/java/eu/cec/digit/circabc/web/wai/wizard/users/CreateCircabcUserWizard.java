...
public class CreateCircabcUserWizard extends BaseWaiWizard {
  ...
  protected static final String MSG_ERR_USERNAME_SIZE =
    "err_valid_username_size";
  protected static final String MSG_ERR_USERNAME_DIGIT =
    "err_valid_username_digit";
  protected static final String MSG_ERR_USERNAME_DUPLICATE =
  ...
  protected static final String MSG_ERR_EMAIL_DUPLICATE =
    "err_valid_duplicate_email";
  protected static final String MSG_WRONG_OLD_PASSWORD =
    "change_own_password_error_old_password_incorrect";
  protected static final String MSG_USER_DOES_NOT_EXISTS =
    "resend_page_user_does_not_exists_error";
  protected static final String MSG_VIEW_APPLICANT_USER_DETAILS_PAGE_DESC =
    "view_applicant_details_page_description";
  private static final long serialVersionUID = -7514679421319409121L;
  private static final Log logger = LogFactory.getLog(
    CreateCircabcUserWizard.class
  );
  ...
  public void init(Map<String, String> params) {
    ...
    if (editParam != null && Boolean.valueOf(editParam)) {
      if (getActionNode() == null) {
        // else edit current user
        NodeRef ref = getPersonService()
          .getPerson(getAuthenticationService().getCurrentUserName());
  ...
  public String next() {
    ...
    if (
      setpName.equals("mod-user-step2") || setpName.equals("new-user-step2")
    ) {
    ...
    // only for create user
      ...
      setpName.equals("mod-user-summary") || setpName.equals("new-user-summary")
    ) {
  ...
  public void populate() {
    ...
    this.postalAddress = (String) props.get(
      UserModel.PROP_POSTAL_ADDRESS.toString()
    );
    this.description = (String) props.get(
      UserModel.PROP_DESCRIPTION.toString()
    );
    this.orgDepNumber = (String) props.get(
      UserModel.PROP_ORGDEPNUMBER.toString()
    );
    this.title = (String) props.get(UserModel.PROP_TITLE.toString());
    this.domain = (String) props.get(UserModel.PROP_DOMAIN.toString());

    this.contentFilterLanguage = getUserPreferencesBean()
    ...
    try {
      if (this.editMode) {
        ...
        if (this.oldPassword != null && this.oldPassword.length() != 0) {
          if (
            confirm != null && confirm.length() != 0 && password.equals(confirm)
          ) {
            try {
    ...
    final RetryingTransactionCallback<Object> callback =
      new RetryingTransactionCallback<Object>() {
        public Object execute() throws Throwable {
          ...
          if (
            contentFilterLanguage.equals(
              UserPreferencesBean.MSG_CONTENTALLLANGUAGES
            )
          ) {
      ...
      final RetryingTransactionCallback<Object> callback =
        new RetryingTransactionCallback<Object>() {
          public Object execute() throws Throwable {
            ...
            final NodeRef newPerson = getUserService()
              .createUser(createUserDataBean(false), enabled);
            // remeber the new created user
            createdPerson = newPerson;

            if (logger.isDebugEnabled()) {
              logger.debug(
                "Created User Authentication instance for username: " + userName
              );
            }
            Locale language = null;
            if (
  ...
  public String getContainerDescription() {
    ...
    if (this.readOnly) {
      final ResourceBundle bundle = Application.getBundle(
        FacesContext.getCurrentInstance()
      );
  ...
  protected boolean validateFields(
    ...
    final boolean validateMadatotyProperties,
    final boolean addFacesErrorMessages
  ) {
    final FacesContext context = FacesContext.getCurrentInstance();
    ...
    if (validateUsername) {
      try {
        ...
        try {
          ...
          UserNameValidator.evaluateUnicity(
            getPersonService(),
            completeUserName
          );
          ...
          validationErrors.put(
            USERNAME_FIELD,
            translate(MSG_ERR_USERNAME_DUPLICATE)
          );
        }
      } catch (final Exception ex) {
        if (ex.getMessage().equals(UserNameValidator.getErrorBadCharacters())) {
          validationErrors.put(
            USERNAME_FIELD,
            translate(MSG_ERR_USERNAME_DIGIT)
          );
        } else {
          final String message = translate(
            MSG_ERR_USERNAME_SIZE,
    ...
    if (validatePassword) {
      try {
        if (
          !editMode ||
          (this.oldPassword != null && this.oldPassword.length() != 0)
        ) {
          ...
          if (!getConfirm().equals(getPassword())) {
            validationErrors.put(
              CONFIRM_FIELD,
              translate(UsersDialog.ERROR_PASSWORD_MATCH)
            );
          } else if (editMode) {
            try {
              final RetryingTransactionHelper txnHelper =
              ...
              validationErrors.put(
                PASSWORD_FIELD,
                translate(MSG_WRONG_OLD_PASSWORD)
              );
            }
          }
        }
  ...
  protected final MutableAuthenticationService getAuthenticationService() {
    if (authenticationService == null) {
      authenticationService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getAuthenticationService();
    }

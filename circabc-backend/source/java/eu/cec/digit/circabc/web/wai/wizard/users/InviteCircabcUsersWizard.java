...
public class InviteCircabcUsersWizard extends AbstractMailToUsersBean {
   ...
   */
  protected static final String MESSAGE_ID_INVITATION_ERROR =
    "invitation_process_failed";
  ...
  protected static final String ECASUNAME_REGEX_TINYMCE =
    "[USER_ECAS_USERNAME]";
  ...
  private static final Log logger = LogFactory.getLog(
    InviteCircabcUsersWizard.class
  );
  private static final String CALLBACK_METHOD_NAME =
    "#{InviteCircabcUsersWizard.pickerCallback}";
   ...
   */
  public final transient List<UserGroupProfile> userGroupProfiles =
    new ArrayList<>();
  ...
  public void init(final Map<String, String> params) {
    ...
    if (
      CircabcConfiguration.getProperty(
        CircabcConfiguration.COMPONENT_INVITATION_NOTIFY_DIRECTORY_ADMINS
      ) !=
      null
    ) {
      notifyDirAdmins = CircabcConfiguration.getProperty(
        CircabcConfiguration.COMPONENT_INVITATION_NOTIFY_DIRECTORY_ADMINS
      );
    }
    ...
    if (params != null && (this.filledUsers = params.get("users")) != null) {
      ...
      if (this.filledUsers != null) {
        ...
        this.filledUsersFilterIdx = (searchFilter == null)
          ? (getFilters().length - 1)
          : Integer.parseInt(searchFilter);
      }
    } else {
    ...
    if (
      NavigableNodeType.CIRCABC_ROOT.isNodeFromType(node) ||
      NavigableNodeType.CATEGORY.isNodeFromType(node) ||
      ...
      if (CircabcConfig.ENT) {
        if (
          ...
          List<String> ecordaThemeIds =
            synchronizationService.getEcordaThemeIds(
          ...
          for (String ecordaThemeId : ecordaThemeIds) {
            for (final UserGroupProfile userGroupProfile : userGroupProfiles) {
              synchronizationService.grantThemeRole(
                userGroupProfile.authority,
                ecordaThemeId,
  ...
  public void addSelection(final ActionEvent event) {
    final List<String> guestsPermissions = Collections.unmodifiableList(
      Arrays.asList(
        "LibNoAccess",
        "LibAccess",
        "NwsAccess",
    ...
    final List<String> regiteredPermissions = Collections.unmodifiableList(
      Arrays.asList(
        "LibNoAccess",
        "LibAccess",
        "NwsPost",
    ...
    if (
      picker instanceof
      eu.cec.digit.circabc.web.ui.common.component.UIGenericPicker
    ) {
      results =
        ((eu.cec.digit.circabc.web.ui.common.component.UIGenericPicker) picker).getSelectedResults();
    } else if (
      picker instanceof org.alfresco.web.ui.common.component.UIGenericPicker
    ) {
      results =
        ((org.alfresco.web.ui.common.component.UIGenericPicker) picker).getSelectedResults();
    }
    // test erreur
    ...
    final UISelectOne profilePicker = (UISelectOne) event
      .getComponent()
    ...
    if (profileGroupName == null) {
      FacesContext.getCurrentInstance()
        .addMessage(null, new FacesMessage(translate(PROFILE_NOT_SELECTED)));
      return;
    } else {
      ...
      for (String result : results) {
        ...
        if (foundExisting == false) {
          ...
          if (
            authority.equalsIgnoreCase("guest") &&
            !guestsPermissions.contains(profileGroupName)
          ) {
          ...
          this.userGroupProfiles.add(
              new UserGroupProfile(userName, profileGroupName, label.toString())
            );
  ...
  public void removeSelection(final ActionEvent event) {
    final UserGroupProfile wrapper =
      (UserGroupProfile) this.userProfileDataModel.getRowData();
    if (wrapper != null) {
  ...
  protected List<String> buildInvitedSet() {
    ...
    if (profileManagerService != null) {
      final String nonPrefixedName =
        profileManagerService.getInvitedUsersGroupName(nodeRef);

      // the group of users has the same name
      final String fullGroupName = getAuthorityService()
        .getName(AuthorityType.GROUP, nonPrefixedName);
   ...
   */
  public SelectItem[] pickerCallback(
    ...
    final RetryingTransactionCallback<List<SortableSelectItem>> callback =
      new RetryingTransactionCallback<List<SortableSelectItem>>() {
        ...
        public List<SortableSelectItem> execute() throws Throwable {
          ...
          if (PermissionUtils.FILTER_VALUE_APPLICANT.equals(filterValue)) {
            ...
            final Map<String, Applicant> applicants =
              profileManagerService.getApplicantUsers(
                profileManagerService.getCurrentAspectRoot(currentNodeRef)
              );
            ...
            for (final Map.Entry<
              String,
              Applicant
            > entry : applicants.entrySet()) {
              app = entry.getValue();
              username = app.getUserName();
              firstName = app.getFirstName();
              lastName = app.getLastName();
              login = PermissionUtils.computeUserLogin(
                username,
                getPersonService(),
                getNodeService()
              );
              if (
                contains == null ||
                contains.trim().length() < 1 ||
                contains.trim().equals("*") ||
                username.contains(contains) ||
            ...
            if (
              contains == null ||
              contains.trim().length() <
              PermissionUtils.MIN_CHAR_ALLOWED_FOR_QUERY
            ) {
              if (logger.isDebugEnabled()) {
  ...
  public String getBuildTextMessage() {
    if (topNodeInvitation) {
      ...
      final Map<QName, Serializable> currentProperties = getNodeService()
        .getProperties(current);

      final String fromFullName =
        currentProperties.get(ContentModel.PROP_FIRSTNAME) +
        " " +
        currentProperties.get(ContentModel.PROP_LASTNAME);
      ...
      String buffer =
        I18NUtil.getMessage(
          "mails_common_dear_user",
          FIRSTNAME_REGEX_TINYMCE + " " + LASTNAME_REGEX_TINYMCE
        ) +
        "<br/><br/>" +
        I18NUtil.getMessage(
          "invite_circabc_user_topfolders_template_mail_body",
          fromFullName,
          "[NODE_BEST_TITLE]",
          "[NODE_REF]"
        ) +
      ...
      setSubject(
        I18NUtil.getMessage(
          "invite_circabc_category_user_template_subject",
          "[NODE_BEST_TITLE]",
  ...
  private void mailNotificationToUser(
    ...
    if (to != null && to.length() != 0) {
      String bodyToSend = applyParams(notificationBody, extraBodyParams)
        .replace("\n", "<br />")
        .replace("\r", "<br />");
  ...
  public UIGenericPicker getPrefilledPicker() {
    ...
    genericPicker.setQueryCallback(
      ctx
        .getApplication()
        .createMethodBinding(getCallBackMethodName(), getCallBackMethodArgs())
    );
    ...
    if (filledUsers != null) {
      ...
      final UIGenericPicker.PickerEvent event = new UIGenericPicker.PickerEvent(
        genericPicker,
        PICKER_SEARCH_ACTION,
        filledUsersFilterIdx,
        filledUsers,
        new String[] {}
      );
      // perform the search
  ...
  private List<SortableSelectItem> executeAndFillList(
    final String domain,
    final String criteria,
    final List<String> userToExclude
  ) {
    return PermissionUtils.buildInvitableUserItems(
      domain,
      criteria,
      userToExclude,
      logger
  ...
  public List<SortableSelectItem> getProfiles() {
    List<SortableSelectItem> profiles =
      ProfileUtils.buildAssignableProfileItems(getActionNode(), logger);
  ...
  public boolean isApplicantPresents() {
    if (applicantPresent == null) {
      ...
      if (profileManagerService != null) {
        currentAspectRoot = profileManagerService.getCurrentAspectRoot(
          currentNodeRef
        );
      }
      ...
      if (currentAspectRoot != null) {
        ...
        final Map<String, Applicant> applicants =
          profileManagerService.getApplicantUsers(currentAspectRoot);
  ...
  protected void grantUsers(final FacesContext context, final NodeRef nodeRef) {
    ...
    final boolean isServiceRootNode = nodeRef.equals(
      profileManagerService.getCurrentAspectRoot(nodeRef)
    );

    // get the list of applicants of the current interest group
    final Map<String, Applicant> applicants =
      profileManagerService.getApplicantUsers(nodeRef);
    ...
    for (final UserGroupProfile userGroupProfile : userGroupProfiles) {
      ...
      if (isServiceRootNode) {
        ...
        Profile profile = profileManagerService.getProfileFromGroup(
          nodeRef,
          userGroupProfile.getProfile()
        );
        if (profile == null) {
          profile = profileManagerService.getProfile(
            nodeRef,
            userGroupProfile.getProfile()
          );
        }
        ...
        profileManagerService.addPersonToProfile(
          nodeRef,
          authority,
      ...
      if (NOTIFY_YES.equals(this.notify)) {
        if (authType.equals(AuthorityType.USER)) {
          ...
          final String all_circa_users =
            ((CircabcRootProfileManagerService) getProfileManagerServiceFactory()
      ...
      // if notify = false, send an email to the user if it is an applicant !!!
        ...
        if (
          authType.equals(AuthorityType.USER) &&
        ...
        if (authType.equals(AuthorityType.USER)) {
          if (getPersonService().personExists(authority)) {
            ...
            final Boolean globalNotification = (Boolean) getNodeService()
              .getProperty(person, UserModel.PROP_GLOBAL_NOTIFICATION);
  ...
  protected Map<String, String> buildCustomParams(
    final NodeRef person,
    ...
    params.put(
      FIRSTNAME_REGEX_TINYMCE,
      (String) personProperties.get(ContentModel.PROP_FIRSTNAME)
    );
    params.put(
      LASTNAME_REGEX_TINYMCE,
      (String) personProperties.get(ContentModel.PROP_LASTNAME)
    );
    params.put(
      ECASUNAME_REGEX_TINYMCE,
      (String) personProperties.get(UserModel.PROP_ECAS_USER_NAME)
    );
    ...
    params.put(
      MAIL_REGEX_TINYMCE,
      (String) personProperties.get(ContentModel.PROP_EMAIL)
    );

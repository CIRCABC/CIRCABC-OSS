...
public enum NavigableNodeType {
  ...
  CIRCABC_ROOT(
    ...
    ComparatorType.ASPECT,
    ASPECT_CIRCABC_ROOT,
    WelcomeBean.BEAN_NAME
  ),
  CATEGORY_HEADER(
    ComparatorType.TYPE,
    TYPE_CATEGORY_HEADER,
    CategoryHeadersBean.BEAN_NAME
  ),
  CATEGORY(
    CIRCABC_CHILD,
    ComparatorType.ASPECT,
    ASPECT_CATEGORY,
    CategoryBean.BEAN_NAME
  ),
  IG_ROOT(
    CIRCABC_CHILD,
    ComparatorType.ASPECT,
    ASPECT_IGROOT,
    InterestGroupBean.BEAN_NAME
  ),
  LIBRARY(
    LIBRARY_CHILD,
    ComparatorType.ASPECT,
  ...
  LIBRARY_ML_CONTENT_TOPIC(
    ComparatorType.TYPE,
    ForumModel.TYPE_TOPIC,
    DiscussionTopicBean.BEAN_NAME
  ),
  LIBRARY_ML_CONTENT_FORUM(
    ComparatorType.TYPE,
    ForumModel.TYPE_FORUM,
    DiscussionForumBean.BEAN_NAME
  ),
  LIBRARY_ML_CONTENT_POST(
    ComparatorType.TYPE,
    ForumModel.TYPE_POST,
    DiscussionPostBean.BEAN_NAME
  ),
  ...
  EVENT_APPOINTMENT(
    ...
    ComparatorType.TYPE,
    TYPE_EVENT,
    AppointmentDetailsBean.BEAN_NAME
  );

  private static final String THE_LOGIC_SHOULDN_T_ALLOW_TO_EXECUTE_THIS_CODE =
    "The logic shouldn't allow to execute this code.";
  ...
  NavigableNodeType(
    final ComparatorType comprator,
    final QName comparatorQName,
  ...
  NavigableNodeType(
    final NavigableNodeType requireNodeType,
    final ComparatorType comprator,
    final QName comparatorQName,
    final String beanName
  ) {
  ...
  /*package */boolean isNodeFromType(
    final Node node,
    final boolean resovleParent
  ) {
    if (node == null) {
      return false;
    } else if (
      resovleParent == false ||
      requireNodeType == null ||
      requireNodeType.isNodeFromType(node, resovleParent)
    ) {
      switch (comparatorType) {
        ...
        case TYPE:
          ...
          if (comparatorQName.equals(type)) {
            ...
            if (typeDef != null) {
              if (
                comparatorQName.equals(type) ||
                getDictionaryService().isSubClass(type, comparatorQName)
              ) {
                return true;
              }
            }
        ...
        default:
          throw new IllegalStateException(
            THE_LOGIC_SHOULDN_T_ALLOW_TO_EXECUTE_THIS_CODE
          );
      }
    } else {
      return false;
  ...
  private boolean isEqualsOrUnderImpl(final NavigableNodeType otherType) {
    ...
    if (otherType.equals(CIRCABC_CHILD)) {
      ...
      switch (otherType) {
        ...
        case CATEGORY:
          response =
            !this.equals(CIRCABC_ROOT) && !this.equals(CATEGORY_HEADER);
          break;
        case IG_ROOT:
          response =
            !this.equals(CIRCABC_ROOT) &&
            !this.equals(CATEGORY_HEADER) &&
            !this.equals(CATEGORY);
          break;
        case LIBRARY:
          response =
            this.equals(LIBRARY_CHILD) ||
            (this.getRequireNodeType() != null &&
              this.getRequireNodeType().equals(LIBRARY_CHILD)) ||
            this.equals(LIBRARY_ML_CONTENT) ||
            this.equals(LIBRARY_ML_CONTENT_FORUM) ||
            this.equals(LIBRARY_ML_CONTENT_TOPIC) ||
            this.equals(LIBRARY_ML_CONTENT_POST);
          break;
        case NEWSGROUP:
          response =
            this.equals(NEWSGROUP_CHILD) ||
            (this.getRequireNodeType() != null &&
              this.getRequireNodeType().equals(NEWSGROUP_CHILD));
          break;
        case SURVEY:
          response =
            this.equals(SURVEY_CHILD) ||
            (this.getRequireNodeType() != null &&
              this.getRequireNodeType().equals(SURVEY_CHILD));
          break;
        case INFORMATION:
          response =
            this.equals(INFORMATION_CHILD) ||
            (this.getRequireNodeType() != null &&
              this.getRequireNodeType().equals(INFORMATION_CHILD));
          break;
        case EVENT:
          response =
            this.equals(EVENT_CHILD) ||
            (this.getRequireNodeType() != null &&
              this.getRequireNodeType().equals(EVENT_CHILD));
          break;
        case DIRECTORY:
  ...
  protected DictionaryService getDictionaryService() {
    if (this.dictionaryService == null) {
      ...
      this.dictionaryService = getCircabcServiceRegistry()
        .getNonSecuredDictionaryService();
    }
  ...
  protected CircabcServiceRegistry getCircabcServiceRegistry() {
    if (this.circabcServiceRegistry == null) {
      this.circabcServiceRegistry = Services.getCircabcServiceRegistry(

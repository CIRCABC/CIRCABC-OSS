...
public abstract class NotificationUtils {
  ...
  private static final String MSG_UNSUBSCRIBED =
    "notification_status_unsuscribed";
  private static final String MSG_ENABLED =
    "edit_user_details_global_notification_active";
  private static final String MSG_DISABLED =
    "edit_user_details_global_notification_nonactive";

  private static final String MSG_GLOBAL_NOTIF_ENABLE =
    "edit_user_details_global_notification_active";
  private static final String MSG_GLOBAL_NOTIF_DISABLE =
    "edit_user_details_global_notification_nonactive";

  private static final String GLOBAL_NOTIF_VALUE_ENABLE = "true";
  private static final String GLOBAL_NOTIF_VALUE_DISABLE = "false";

  private static final IGRootProfileManagerService igRootProfileManager =
  ...
  public static NotificationWrapper wrappNotification(
    ...
    switch (authorityNotification.getAuthorityType()) {
      case USER:
        name = PermissionUtils.computeUserLogin(
          authorityNotification.getAuthority()
        );
        break;
      case GROUP:
        name = ProfileUtils.getProfilename(
          node,
          authorityNotification.getAuthority(),
          igRootProfileManager,
          managementService
        );
        break;
      default:
  ...
  public static SelectItem[] getStatusesAsSelectItem(Node currentNode) {
    ...
    final boolean isUnderIg = NavigableNodeType.IG_ROOT.isNodeFromType(
      currentNode
    );

    SelectItem[] notificationStatuses = new SelectItem[isUnderIg
      ? size - 1
      : size];
    int inc = 0;
    ...
    for (final NotificationStatus st : statusEnums) {
      if (isUnderIg && st.equals(NotificationStatus.INHERITED)) {
        ...
        notificationStatuses[inc] = new SelectItem(
          st.toString(),
  ...
  public static SelectItem[] getGlobalStatusesAsSelectItem() {
    return new SelectItem[] {
      new SelectItem(
        GLOBAL_NOTIF_VALUE_ENABLE,
        translate(MSG_GLOBAL_NOTIF_ENABLE)
      ),
      new SelectItem(
        GLOBAL_NOTIF_VALUE_DISABLE,
        translate(MSG_GLOBAL_NOTIF_DISABLE)
      ),
    };
  }

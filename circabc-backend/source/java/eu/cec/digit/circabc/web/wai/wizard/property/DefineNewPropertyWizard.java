...
public class DefineNewPropertyWizard extends BaseWaiWizard {
  ...
  public static final String MSG_ERROR_MULTI_VALUE_REQUIRED =
    "define_property_dialog_error_multi_value_required";
  public static final String MSG_ERROR_TRANSLATION_REQUIRED =
    "define_property_dialog_error_no_translation";
  public static final String MSG_ERROR_LOCALE_REQUIRED =
    "define_property_dialog_error_locale_required";
  public static final String MSG_ERROR_VALUE_REQUIRED =
    "define_property_dialog_error_value_required";
  public static final String MSG_ERROR_LOCALE_DUPLICATED =
    "define_property_dialog_error_locale_duplicated";
  public static final String MSG_SUMMARY_TYPE =
    "define_property_dialog_property_type";
  public static final String MSG_SUMMARY_DESC =
    "define_property_dialog_property_desc";
  public static final String MSG_TYPE_PREFIX =
    "define_property_dialog_property_type_";
  private static final long serialVersionUID = -6962091031742104333L;
  private static final Log logger = LogFactory.getLog(
    DefineNewPropertyWizard.class
  );
    ...
    if (parameters != null) {
      ...
      if (!NavigableNodeType.IG_ROOT.isNodeFromType(getActionNode())) {
        throw new IllegalArgumentException(
          "The node id must be an interest group"
        );
      }
    ...
    if (step == 2) {
      ...
      if (getTranslationDataModel().getRowCount() < 1) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Impossible to add a Dynamic property to the Interest Group " +
      ...
      if (
        type.equals(DynamicPropertyType.SELECTION.name()) ||
        type.equals(DynamicPropertyType.MULTI_SELECTION.name())
      ) {
        ...
        if (trimMultiValue.length() < 1) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Impossible to add a Dynamic property to the Interest Group " +
              getActionNode().getName() +
              " because the available values list is empty."
            );
          }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    final DynamicPropertyImpl dynamicProperty = new DynamicPropertyImpl(
      labels,
      DynamicPropertyType.valueOf(type),
      formatedValues.toString()
    );

    String info = MessageFormat.format(
      "the property {0} has been created - {1}",
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Property " +
        dynamicProperty.getName() +
        " successfully created under the interest group " +
        getActionNode().getName() +
        ". With value: " +
        dynamicProperty.toString()
      );
  ...
  public void addDescription(ActionEvent event) {
    final UIInput input = (UIInput) event
      .getComponent()
      .findComponent("define-property-text");
    ...
    if (!error) {
      propertyTranslations.add(
        new DynPropertyWrapper(
          getSecurityService().getCleanHTML(selValue, false),
          selLocale
        )
      );
      text = null;
  ...
  public void removeSelection(ActionEvent event) {
    DynPropertyWrapper wrapper =
      (DynPropertyWrapper) this.translationDataModel.getRowData();
    if (wrapper != null) {
  ...
  public String getSummary() {
    ...
    String[] values = new String[] { translate(enumType), buff.toString() };

...
public class UIDataPager extends UICommand {
  ...
  public void encodeBegin(FacesContext context) throws IOException {
    ...
    if (currentPage != 0) {
      ...
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_FIRSTPAGE,
          bundle.getString(FIRST_PAGE)
        )
      );
      buf.append("</a>");
    } else {
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_FIRSTPAGE_NONE,
    ...
    if (currentPage != 0) {
      ...
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_PREVIOUSPAGE,
      ...
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_PREVIOUSPAGE_NONE,
          bundle.getString(ACTION_DESACTIVATED)
        )
      );
    }
    ...
    for (int i = 0; i < totalIndex; i++) {
      if (i != currentPage) {
        ...
        buf.append("<a href=\"");
        buf.append(
          UtilsCircabc.generateHrefPart(context, this, id, attributes)
        );
        buf.append("\" title=\"");
        // output Go to Page X
        buf.append(
          MessageFormat.format(
            bundle.getString(MSG_PAGEGOTO),
    ...
    if ((pageCount > 10) && (pageCount % 10 != 0)) {
      if (pageCount - 1 != currentPage) {
        ...
        buf.append(
          MessageFormat.format(
            bundle.getString(MSG_PAGEGOTO),
            new Object[] { Integer.toString(pageCount) }
          )
        );
        buf.append("\">");
    ...
    if ((currentPage < dataContainer.getPageCount() - 1) == true) {
      ...
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_NEXTPAGE,
          bundle.getString(NEXT_PAGE)
        )
      );
      buf.append("</a>");
    } else {
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_NEXTPAGE_NONE,
          bundle.getString(ACTION_DESACTIVATED)
        )
      );
    }
    ...
    if ((currentPage < dataContainer.getPageCount() - 1) == true) {
      ...
      attributes.put(
        "page",
        Integer.valueOf(dataContainer.getPageCount() - 1).toString()
      );
      buf.append("<a href=\"");
      buf.append(UtilsCircabc.generateHrefPart(context, this, id, attributes));
      buf.append("\" title=\"");
      ...
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_LASTPAGE,
          bundle.getString(LAST_PAGE)
        )
      );
      buf.append("</a>");
    } else {
      buf.append(
        UtilsCircabc.buildImageTag(
          context,
          WebResources.IMAGE_LASTPAGE_NONE,
          bundle.getString(ACTION_DESACTIVATED)
        )
      );
    }
  ...
  public void broadcast(FacesEvent event) throws AbortProcessingException {
    if (event instanceof PageEvent == false) {
      ...
      logger.debug(
        "Handling paging event to index: " + ((PageEvent) event).Page
      );
  ...
  private String getHiddenFieldName() {
    UIComponent dataContainer = (UIComponent) Utils.getParentDataContainer(
      getFacesContext(),
      this
    );
    return (
      dataContainer.getClientId(getFacesContext()) +

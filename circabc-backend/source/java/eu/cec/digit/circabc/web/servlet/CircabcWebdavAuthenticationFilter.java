 ...
 *
 * @author Migration 3.1 -> 3.4.6 - 05/12/2011 Class was converted to its 3.4.x
 *         version:
 *         org.alfresco.repo.webdav.auth.AuthenticationFilter
 */
public class CircabcWebdavAuthenticationFilter
  ...
  // WebDAV user-agent header must start with Mozilla for a compatible Mozilla
  // browser
  ...
  private static Log logger = LogFactory.getLog(
    CircabcWebdavAuthenticationFilter.class
  );
  // Allow an authentication ticket to be passed as part of a request to bypass
  // authentication
  ...
  private String guestDisabledForLibrary = "true";
  ...
  public void doFilter(
    ...
    if (
      ...
      if (
        ...
        if (pos == -1) {
          ...
          this.authenticationListener.userAuthenticated(
    ...
    if (ticketdav != null && ticketdav.length() > 0) {
      ...
      this.authenticationListener.userAuthenticated(
          new TicketCredentials(ticketdav)
        );
      chain.doFilter(req, resp);
      return;
    }
    ...
    if (posTK != -1) {
      ...
      String secondPart = webdavUrl.substring(
        posTK + NavigableNode.NEW_TICKET_PREFIX.length()
      );
      ...
      final String newRequestURI =
        firstPart + secondPart.substring(posEndTicket);
    ...
    boolean isBrowser =
      agent != null && agent.trim().startsWith(MOZILLA_WEB_BROWSER);
    boolean isBot =
      agent != null &&
      (agent.trim().endsWith("bot") || agent.trim().endsWith("spider"));
   ...
   */
  private boolean userAllowedOnPath(
    String path,
    String userName,
    ...
    List<String> pathElements = Arrays.asList(
      StringUtils.split((path == null) ? "" : path, '/')
    );
    ...
    if (pathElements.isEmpty()) {
      ...
      try {
        nodeRef = fileFolderService
          .resolveNamePath(rootNodeRef, pathElements)
          .getNodeRef();
    ...
    if (nodeRef != null) {
      ...
      if (
        ...
        this.guestDisabledForLibrary.equals("true")
      ) {
      ...
      return AuthenticationUtil.runAs(
        new AuthenticationUtil.RunAsWork<Boolean>() {
          public Boolean doWork() {
            ...
            for (int idx = 0; idx < permissionNames.length && allowed; idx++) {
    ...
    return AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Boolean>() {
        public Boolean doWork() {
          ...
          result =
            nodeService.hasAspect(finalNodeRef, CircabcModel.ASPECT_LIBRARY) ||
            nodeService.hasAspect(finalNodeRef, CircabcModel.ASPECT_NEWSGROUP);
  ...
  private WebDAVUser checkHttpUser(
    ...
    try {
      status = AuthenticationHelper.authenticate(
        servletContext,
        httpReq,
        httpResp,
        false,
    ...
    if ((userPrincipal != null) || (status == AuthenticationStatus.Success)) {
      ...
      try {
        ...
        final NodeRef homeRef = (NodeRef) nodeService.getProperty(
          personRef,
        ...
        final WebDAVUser webDavUser = new WebDAVUser(
          currentUsername,
          authenticationService.getCurrentTicket(),
          homeRef
        );
        ...
        // Store the User object in the Session - the authentication servlet will then
        // proceed
  ...
  /**
   * Migration 3.1 -> 3.4.6 - 05/12/2011 This method was originally provided by
   * the WebdavServlet of
   * Alfresco 3.1 but in 3.4.x does not exist anymore. So, this implementation was
   * taken from
  ...
  public NodeRef getRootNode() throws ServletException {
    ...
    // Use the system user as the authenticated context for the filesystem
    // initialization
    return AuthenticationUtil.runAs(
      runAsWork,
      AuthenticationUtil.SYSTEM_USER_NAME

...
public class BulkUploadBean extends AddContentBean {
  ...
  private static final String IS_SECURE_TRANSPORT =
    "BulkUploadBean.isSecureTransport";
  private static final String ID_CHECK_DISABLE_INDEX = "check-disable-index";
  private final String CLOSE_AND_BROWSE_OUTCOME =
  ...
  private boolean isSubmitedSkipIndexfile(
    final CircabcUploadedFile uploadedFile
  ) {
    final String value = (uploadedFile == null)
      ? null
      : uploadedFile.getSubmitedProperty(ID_CHECK_DISABLE_INDEX);
      ...
      if (fileBean != null) {
        ...
        try {
          ...
          if (isSubmitedSkipIndexfile(fileBean)) {
            ...
            indexFileEntries = getBulkService()
              .getIndexRecords(uploadedFile.getFile(), messages);
          }

          uploadedEntries = getBulkService()
            .upload(
              getNavigator().getCurrentNode().getNodeRef(),
              fileBean.getFile(),
              indexFileEntries,
              messages
            );
          getBulkService()
            .validateEntries(indexFileEntries, uploadedEntries, messages);
          ...
          if (!foundFatal) {
            messages.add(
              new ValidationMessageImpl(
                ...
                " caused by:" +
                ex.getMessage() +
                " Please contact the Circabc Support Team",
                ErrorType.Fatal
              )
            );
          }
  ...
  public boolean getIsSecureTransport() {
    ...
    if (httpReq.getAttribute(IS_SECURE_TRANSPORT) == null) {
      ...
      this.isSecureTransport = (Boolean) httpReq.getAttribute(
        IS_SECURE_TRANSPORT
      );
    }
  ...
  public BulkService getBulkService() {
    if (this.bulkService == null) {
      ...
      this.bulkService = (BulkService) Services.getCircabcServiceRegistry(
        fc
      ).getBulkService();
    }
    return bulkService;
  ...
  public TransactionService getTransactionService() {
    if (this.transactionService == null) {
      ...
      this.transactionService =
        (TransactionService) Services.getAlfrescoServiceRegistry(
          fc
  ...
  public NotificationService getNotificationService() {
    if (this.notificationService == null) {
      ...
      this.notificationService =
        (NotificationService) Services.getCircabcServiceRegistry(
          fc

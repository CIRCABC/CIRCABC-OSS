...
public class SurveysBean extends BaseWaiDialog implements IContextListener {
  ...
  public void clickSurvey(ActionEvent event) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Click on the survey " + survey + " with the lang " + lang);
    }
  }
  ...
  public void closeSurvey(ActionEvent event) {
    if (logger.isDebugEnabled()) {
      logger.debug("Close the survey " + survey + " with the lang " + lang);
    }
    survey = null;
    lang = null;
  }
  ...
  public List<Survey> getSurveys() {
    if (surveys == null || surveys.size() == 0) {
      ...
      try {
        ...
        surveys = SurveyHelper.transform(
          ipmService.getCircabcSurveys(
            getVirtualCirca(),
            getInterestGroup(),
            uilang
          )
        ...
        logger.error(
          "Unable to get the list of surveys from the IPM server",
          e
        );
      }
    }
  ...
  private Map<String, String[]> getDefaultParameters() {
    ...
    defaultParamsMap.put(
      PARAM_INTEREST_GROUP,
      new String[] { getInterestGroup() }
    );
    defaultParamsMap.put(
      PARAM_VIRTUAL_CIRCABC,

...
public class CategoryBean extends BaseWaiNavigator {
  ...
  private void initAndGroupIgRoot() {
    if (getCircabcService().readFromDatabase()) {
      ...
      final List<InterestGroupItem> interestGroupByCategoryUser =
        getCircabcService()
          .getInterestGroupByCategoryUser(
            getNavigator().getCurrentCategory().getNodeRef(),
            currentUser
          );

      boolean isGuest = getNavigator()
        .getCurrentUser()
        .getUserName()
      ...
      for (InterestGroupItem interestGroupItem : interestGroupByCategoryUser) {
        if (interestGroupItem.isMember()) {
          ...
          interestGroupItem.isRegistered() && !interestGroupItem.isPublic()
        ) {
          if (!isGuest) {
            this.registredIg.add(interestGroupItem);
          }
        } else if (
          interestGroupItem.isPublic() && interestGroupItem.isRegistered()
        ) {
      ...
      final String currentUser = getNavigator().getCurrentUser().getUserName();
      final List<NavigableNode> igs = getNavigator()
        .getCurrentCategory()
        .getNavigableChilds();
      ...
      for (NavigableNode node : igs) {
        try {
          ...
          if (logger.isErrorEnabled()) {
            ...
            logger.error(
              "THE PROBLEM RESOLVE HIMSELF WHEN CLUSTER ELEMENTS FINNISH SYNCHRONISATION"
            );
            logger.error(
      ...
      if (logger.isDebugEnabled()) {
        ...
        logger.debug(
          "Time to query sort and build the list of IgRoot (" +
          memberIg.size() +
          " member ig(s) / " +
          registredIg.size() +
          " registred ig(s) / " +

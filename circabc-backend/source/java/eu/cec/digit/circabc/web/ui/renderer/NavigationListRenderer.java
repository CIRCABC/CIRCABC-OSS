...
public class NavigationListRenderer extends BaseRenderer {
  ...
  public void encodeBegin(
    ...
    String userName = AuthenticationUtil.getRunAsUser() == null
        ...
        final RetryingTransactionCallback<Object> callback =
          new RetryingTransactionCallback<Object>() {
            public Object execute() throws Throwable {
              ...
              Map<String, String> requestParams = context
                .getExternalContext()
                .getRequestParameterMap();
              Boolean isInIgAdminPanel = false;
              if (requestParams != null) {
                if (requestParams.get("FormPrincipal:_idcl") != null) {
                  isInIgAdminPanel = requestParams
                    .get("FormPrincipal:_idcl")
                ...
                for (Object object : value) {
                  ...
                  if (node != null) {
                    ...
                    if (node.hasAspect(CircabcModel.ASPECT_LIBRARY_ROOT)) {
                      ...
                      node.hasAspect(CircabcModel.ASPECT_INFORMATION_ROOT)
                    ) {
                      name = WebClientHelper.translate("information_menu");
                    } else if (
                      node.hasAspect(CircabcModel.ASPECT_NEWSGROUP_ROOT)
                    ) {
                      name = WebClientHelper.translate("newsgroup_menu");
                    } else {
                      if (
                        renderPropertyName != null &&
                        renderPropertyName.equals("title")
                      ) {
                        String title = (String) node
                          .getProperties()
                          .get(ContentModel.PROP_TITLE.toString());
                        name = (title != null && title.length() > 1)
                          ? title
                    ...
                    if (
                      separatorClass != null && bannerStyle.equals("normal")
                    ) {
                      out.write("</span>");
                    }
                  } else {
   ...
   */
  protected void renderNonJavaScriptAnchorStart(
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "NavigationListRenderer : encodeBegin - renderNonJavaScriptAnchorStart - Start"
      );
    }
    ...
    final FormInfo forInfo = RendererUtils.findNestingForm(
      component,
      facesContext
    );
    ...
    hrefBuf.append("&amp;");
    hrefBuf.append(
      RendererUtils.findNestingForm(component, facesContext).getFormName()
    );
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "NavigationListRenderer : encodeBegin - renderNonJavaScriptAnchorStart - End"
      );
    }
  }

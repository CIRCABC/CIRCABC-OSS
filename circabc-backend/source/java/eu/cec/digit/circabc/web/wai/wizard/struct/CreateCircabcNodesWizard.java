...
public class CreateCircabcNodesWizard extends InviteCircabcUsersWizard {
  ...
  private static final String LIGHT_DESCRIPTION_LIMIT_EXCEED =
    "lightDescription_limit_500";
  /**
   * The logger
   */
  private static final Log logger = LogFactory.getLog(
    CreateCircabcNodesWizard.class
  );
  private static final long serialVersionUID = 1634570905315116894L;
  /**
   * generic error message for the create circabc wizard
   */
  private static final String MESSAGE_ID_CREATE_CIRCABC_ERROR =
    "create_new_circabc_error";
  /**
   * generic error message for the create category wizard
   */
  private static final String MESSAGE_ID_CREATE_CATEGORY_ERROR =
    "create_new_category_error";
  /**
   * generic error message for the create interest group wizard
   */
  private static final String MESSAGE_ID_CREATE_IG_ERROR =
    "create_new_interest_group_error";

  private static final String MESSAGE_ID_DUPLICATE_NAME =
    "create_node_duplicate_name";
  private static final String MESSAGE_ID_INVALID_NAME =
    "create_node_invalid_name";
  private static final String MESSAGE_ID_INVALID_NAME_TOO_BIG =
    "create_node_invalid_name_too_big";
  private static final String MESSAGE_ID_INVALID_NAME_CHARACTERS =
    "create_node_invalid_name_characters";
   ...
   */
  private static final String MESSAGE_ID_CREATE_CIRCABC_SELECTED_USERS =
    "create_circabc_wizard_selected_admin";
  /**
   * message 'select admin' for the create category wizard
   */
  private static final String MESSAGE_ID_CREATE_CATEGORY_SELECTED_USERS =
    "create_category_wizard_selected_admin";
  /**
   * message 'select admin' for the create interest group wizard
   */
  private static final String MESSAGE_ID_CREATE_IG_SELECTED_USERS =
    "create_interest_group_wizard_selected_admin";

  /**
   * The profile name for the circabc admin
   */
  private static final String CIRCABC_ADMIN_PROFILE =
    CircabcRootProfileManagerService.Profiles.CIRCABC_ADMIN;
  /**
   * The profile name for the category admin
   */
  private static final String CATEGORY_ADMIN_PROFILE =
    CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN;
  /**
   * The profile name for the interest group leader
   */
  private static final String IG_LEADER_PROFILE =
  ...
  public void init(final Map<String, String> parameters) {
    ...
    newsGroupTitle = CircabcConfiguration.getProperty(
      CircabcConfiguration.NEWSGROUP_NODE_TITLE_PROPERTIES
    );
    newsGroupDescription = CircabcConfiguration.getProperty(
      CircabcConfiguration.NEWSGROUP_NODE_DESCRIPTION_PROPERTIES
    );
    newsGroupIcon = ManagementService.DEFAULT_NEWSGROUP_ICON_NAME;

    libraryTitle = CircabcConfiguration.getProperty(
      CircabcConfiguration.LIBRARY_NODE_TITLE_PROPERTIES
    );
    libraryDescription = CircabcConfiguration.getProperty(
      CircabcConfiguration.LIBRARY_NODE_DESCRIPTION_PROPERTIES
    );
    libraryIcon = ManagementService.DEFAULT_LIBRARY_ICON_NAME;

    surveyTitle = CircabcConfiguration.getProperty(
      CircabcConfiguration.SURVEY_NODE_TITLE_PROPERTIES
    );
    surveyDescription = CircabcConfiguration.getProperty(
      CircabcConfiguration.SURVEY_NODE_DESCRIPTION_PROPERTIES
    );
    surveyIcon = ManagementService.DEFAULT_SURVEY_ICON_NAME;

    informationTitle = CircabcConfiguration.getProperty(
      CircabcConfiguration.INFORMATION_NODE_TITLE_PROPERTIES
    );
    informationDescription = CircabcConfiguration.getProperty(
    ...
    eventTitle = CircabcConfiguration.getProperty(
      CircabcConfiguration.EVENT_NODE_TITLE_PROPERTIES
    );
    eventDescription = CircabcConfiguration.getProperty(
    ...
    // if the current node is the company home
      ...
      getActionNode()
        .getNodeRef()
        .equals(getManagementService().getCompanyHomeNodeRef())
    ) {
      // create a circabc node
      toCreateNodeType = NodeType.CIRCABC;

      // set the default values
      name = CircabcConfiguration.getProperty(
        CircabcConfiguration.CIRCABC_ROOT_NODE_NAME_PROPERTIES
      );
      title = CircabcConfiguration.getProperty(
        CircabcConfiguration.CIRCABC_ROOT_NODE_TITLE_PROPERTIES
      );
      description = CircabcConfiguration.getProperty(
        CircabcConfiguration.CIRCABC_ROOT_NODE_DESCRIPTION_PROPERTIES
      );
    } else {
      toCreateNodeType = null;
    }

    this.isInterestGroupToCreate = NodeType.IGROOT.equals(
      this.toCreateNodeType
    );
  }
        ...
        public String execute() throws Throwable {
          try {
            ...
            try {
              ...
              Utils.addErrorMessage(getErrorMessage(), e);

              switch (toCreateNodeType) {
                case CIRCABC:
                  if (logger.isErrorEnabled()) {
                    logger.error(
                      "Impossible to create a Circabc root folder named " +
                      getName() +
                      " under the space " +
                ...
                case CATEGORY:
                  if (logger.isErrorEnabled()) {
                    logger.error(
                      "Impossible to create a Category named " +
                      getName() +
                      " under the Circabc root node " +
                      getActionNode().getName(),
                      e
                    );
                  }
                ...
                case IGROOT:
                  if (logger.isErrorEnabled()) {
                    logger.error(
                      "Impossible to create an Interest group named " +
                      getName() +
                      " under the Category " +
                      getActionNode().getName(),
                      e
            ...
            Utils.addErrorMessage(
              translate(MESSAGE_ID_INVITATION_ERROR, pe.getExplanation()),
              pe
            );
            return null;
          } catch (final Throwable e) {
            ...
            Utils.addErrorMessage(
              translate(MESSAGE_ID_INVITATION_ERROR, e.getMessage()),
              e
            );
            return null;
          }
  ...
  @Override
  protected String doPostCommitProcessing(
    ...
    if (step == 2) {
      ...
      if (this.name == null || this.name.length() < 1) {
        FacesContext.getCurrentInstance()
          .addMessage(
            null,
            new FacesMessage(getBundle().getString(MESSAGE_ID_INVALID_NAME))
          );
        ...
        FacesContext.getCurrentInstance()
          .addMessage(
            null,
            new FacesMessage(
              getBundle().getString(MESSAGE_ID_INVALID_NAME_TOO_BIG)
            )
          );
        ...
        FacesContext.getCurrentInstance()
          .addMessage(
            null,
            new FacesMessage(
              getBundle().getString(MESSAGE_ID_INVALID_NAME_CHARACTERS)
            )
          );
        ...
        FacesContext.getCurrentInstance()
          .addMessage(
            null,
            new FacesMessage(getBundle().getString(MESSAGE_ID_DUPLICATE_NAME))
          );
      ...
      if (
        getNodeService()
          .hasAspect(getActionNode().getNodeRef(), CircabcModel.ASPECT_CATEGORY)
      ) {
        if (
          this.lightDescription.length() >
          ...
          FacesContext.getCurrentInstance()
            .addMessage(
              null,
              new FacesMessage(
                getBundle().getString(LIGHT_DESCRIPTION_LIMIT_EXCEED)
              )
            );
  ...
  protected void createCategory(final FacesContext context) {
    ...
    final NodeRef categoryNodeRef = getManagementService()
      .createCategory(
    ...
    uiFacetsProps.put(
      ...
      getSecurityService()
        .getCleanHTML(
          (this.title == null || this.title.trim().length() < 1)
            ? getName()
            : this.title,
          false
        )
    ...
    //reset cache
    final CategoryHeadersBeanData categoryHeadersBeanData =
      (CategoryHeadersBeanData) Beans.getBean("CategoryHeadersBeanData");
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        ...
        categoryHeaderId +
        "\n  Category Admin(s):" +
        super.userGroupProfiles
      );
    }
  }

  protected void createIGRoot(final FacesContext context) {
    igRootNodeRef = getManagementService()
      .createIGRoot(this.getActionNode().getNodeRef(), getName(), getContact());
    catNodeRef = this.getActionNode().getNodeRef();
    {
      ...
      if (
        ...
        throw new LightDescriptionSizeExceedException(
          translate(LIGHT_DESCRIPTION_LIMIT_EXCEED)
        );
      }

      uiFacetsProps.put(
        CircabcModel.PROP_LIGHT_DESCRIPTION,
  ...
  protected NodeRef createLibrary(final NodeRef igRootNodeRef) {
    ...
    for (String lang : languages) {
      ResourceBundle bundle = ResourceBundleWrapper.getResourceBundle(
        "alfresco.messages.webclient",
        new Locale(lang.toLowerCase())
      );
    ...
    getNodeService()
      .addAspect(lib, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);
  ...
  protected NodeRef createNewsGroup(final NodeRef igRootNodeRef) {
    final NodeRef circabcNewsGroupNodeRef = getManagementService()
      .createNewsGroup(igRootNodeRef);
    //apply the uifacets aspect - icon, title and description props
    final Map<QName, Serializable> uiFacetsProps = new HashMap<>(5);
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "NewsGroup created under " +
        getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME) +
        "\n  Title: " +
        this.newsGroupTitle +
        "\n  Description: " +
        this.newsGroupDescription
      );
    }
  ...
  protected NodeRef createSurvey(final NodeRef igRootNodeRef) {
    ...
    getNodeService()
      .addAspect(survey, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Survey created under " +
        getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME) +
        "\n  Title: " +
        this.surveyTitle +
        "\n  Description: " +
  ...
  protected NodeRef createInformation(final NodeRef igRootNodeRef) {
    final NodeRef inf = getManagementService()
      .createInformationService(igRootNodeRef);
    ...
    for (String lang : languages) {
      ...
      titleValue.addValue(
        new Locale(lang),
        bundle.getString("information_menu")
      );
    }
    ...
    getNodeService()
      .addAspect(inf, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Information created under " +
        getNodeService().getProperty(igRootNodeRef, ContentModel.PROP_NAME) +
        "\n  Title: " +
  ...
  protected NodeRef createEvent(final NodeRef igRootNodeRef) {
    final NodeRef event = getManagementService()
      .createEventService(igRootNodeRef);
    ...
    getNodeService()
      .addAspect(event, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);
  ...
  protected NodeRef createDirectory(final NodeRef igRootNodeRef) {
    final NodeRef directory = getManagementService()
      .createDirectory(igRootNodeRef);
  ...
  public DataModel getCategoryListDataModel() {
    ...
    final List<NodeRef> categories = getManagementService()
      .getExistingCategoryHeaders();

    // construct the list of wrappers of categories
    final List<CategoryWrapper> categoriesWrapper = new ArrayList<>(
      categories.size()
    );
    for (final NodeRef ref : categories) {
      ...
      final Serializable name = getNodeService()
        .getProperty(ref, ContentModel.PROP_NAME);
      final Serializable desc = getNodeService()
        .getProperty(ref, ContentModel.PROP_DESCRIPTION);

      categoriesWrapper.add(
        new CategoryWrapper(
          ref,
  ...
  public void addSelectedUsers(final ActionEvent event) {
    final UIGenericPicker picker = (UIGenericPicker) event
      .getComponent()
    ...
    if (results != null) {
      ...
      for (final String authority : results) {
        ...
        if (foundExisting == false) {
          ...
          if (
            authType == AuthorityType.GUEST || authType == AuthorityType.USER
          ) {
            userName = authority;
          ...
          this.userGroupProfiles.add(
              new UserGroupProfile(userName, profile, label.toString())
            );
          ...
          FacesContext.getCurrentInstance()
            .addMessage(
              ...
              new FacesMessage(
                getBundle()
                  .getString(InviteCircabcUsersWizard.USER_SPECIFIED_TWICE)
              )
            );
        }
      }
  ...
  public boolean getNextButtonDisabled() {
    ...
    final int currentStep = Application.getWizardManager()
      .getState()
      .getCurrentStep();
  ...
  public List<UIListItem> getIcons() {
    ...
    final Config config = Application.getConfigService(
      FacesContext.getCurrentInstance()
  ...
  protected final NodeService getInternalNodeService() {
    if (internalNodeService == null) {
      internalNodeService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  public final void setInternalNodeService(
    final NodeService internalNodeService
  ) {
  ...
  public CircabcService getCircabcService() {
    if (circabcService == null) {
      circabcService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getCircabcService();
    }

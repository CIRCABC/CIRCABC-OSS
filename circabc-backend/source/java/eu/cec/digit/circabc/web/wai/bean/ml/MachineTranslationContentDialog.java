...
public class MachineTranslationContentDialog extends BaseWaiDialog {
  ...
  private static final String MSG_ERROR_INVALID_FILE_TYPE =
    "machine_translate_doc_err_invalid_file_type";
  private static final String MSG_ERROR_DOCUMENT_IS_TOO_BIG =
    "machine_translate_doc_err_too_big";
  private static final String MSG_ERROR_INVALID_SOURCE_LANGUAGE =
    "machine_translate_common_invalid_source_lang";
  private static final String MSG_ERROR_EMPTY_TARGET_LANGUAGE =
    "machine_translate_common_empty_target_langs";

  private static final Log logger = LogFactory.getLog(
    MachineTranslationContentDialog.class
  );
    ...
    if (parameters != null) {
      ...
      for (Entry<Locale, NodeRef> item : translations.entrySet()) {
        ...
        existingTranslations.add(
          new SelectItem(lang, labelByCode, nodeRefValue.toString())
        );
      ...
      Set<String> languages = new HashSet<>(
        getTranslationService().getAvailableLanguages()
      );
      targetTranslations = new ArrayList<>();
      ...
      Collections.sort(targetTranslations, new SelectItemLabelComparator());
    }
  }
    ...
    if (FacesContext.getCurrentInstance().getMessages().hasNext()) {
      ...
      try {
        ...
        Long id = (Long) getNodeService()
          .getProperty(selectedNodeRef, ContentModel.PROP_NODE_DBID);
        ...
        getNodeService()
          .setProperty(copyOfDocument, ContentModel.PROP_NAME, newName);

        isFinished = true;
        return CircabcNavigationHandler.CLOSE_WAI_DIALOG_OUTCOME;
      } catch (Exception e) {
        logger.error(
          "Error when copy document that need to be machine translated ",
          e
        );
        Utils.addErrorMessage(
          translate(Repository.ERROR_GENERIC, e.getMessage()),
          e
        );
        isFinished = false;
        return null;
  ...
  private void validate() {
    ...
    final Set<String> availableLanguagesSet = getTranslationService()
      .getAvailableLanguages();
    if (!availableLanguagesSet.contains(sourceLanguage.toUpperCase())) {
      final String availableLanguages = StringUtils.join(
        availableLanguagesSet.iterator(),
        ","
      );
      Utils.addErrorMessage(
        translate(MSG_ERROR_INVALID_SOURCE_LANGUAGE, availableLanguages)
      );
    }
    ...
    if (
      !getTranslationService()
        .canBeTranslated(
          getNodeService()
            .getProperty(selectedNodeRef, ContentModel.PROP_NAME)
            .toString()
        )
    ) {
      final String fileExtensions = StringUtils.join(
        getTranslationService().getAvailableFileExtensions().iterator(),
        ","
      );
      Utils.addErrorMessage(
        translate(MSG_ERROR_INVALID_FILE_TYPE, fileExtensions)
      );
    }
    // check file size
    ...
    if (content != null && content.getSize() > maxFileSizeInBytes) {
      final String maxFileSize = FileUtils.byteCountToDisplaySize(
        maxFileSizeInBytes
      );
      Utils.addErrorMessage(
        translate(MSG_ERROR_DOCUMENT_IS_TOO_BIG, maxFileSize)
      );
    }
  }
    ...
    if (isFinished) {
      ...
      getLogRecord()
        .addInfo(
          ...
          sourceLanguage.toUpperCase() +
          " to " +
          StringUtils.join(selectedTargetTranslations.iterator(), ",")
        );
      return outcome;
    } else {
      return null;
  ...
  public TranslationService getTranslationService() {
    if (translationService == null) {
      translationService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getNonSecuredTranslationService();
    }

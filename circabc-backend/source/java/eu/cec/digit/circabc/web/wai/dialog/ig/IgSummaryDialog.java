...
public class IgSummaryDialog extends BaseWaiDialog {
  ...
  public void init(Map<String, String> parameters) {
    ...
    SelectItem optionGBS = new SelectItem(
      GLOBAL_STATISTICS,
      translate("ig_summary_statistic_to_display_global")
    );
    SelectItem optionTS = new SelectItem(
      TIMELINE_STATISTICS,
  ...
  private void buildIGData() {
    ...
    IgStatisticsParameter statistics =
      this.statsServ.buildStatsData(currentNode);

    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_created_date"),
          statistics.getCreationDate()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_number_of_users"),
          statistics.getNbUsers()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_library_folder_count"),
          statistics.getLibraryFolderCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_library_document_count"),
          statistics.getLibraryDocumentCount()
        )
      );
    Long libSize = statistics.getLibrarySize();
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_library_size"),
          CircabcUploadedFile.humanReadableByteCount(libSize, false)
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_information_folder_count"),
          statistics.getInformationFolderCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_information_document_count"),
          statistics.getInformationDocumentCount()
        )
      );
    Long infSize = statistics.getInformationSize();
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_information_size"),
          CircabcUploadedFile.humanReadableByteCount(infSize, false)
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_version_count"),
    ...
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_version_size"),
          CircabcUploadedFile.humanReadableByteCount(verSize + custSize, false)
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_total_size"),
          CircabcUploadedFile.humanReadableByteCount(
            (libSize + infSize + verSize + custSize),
            false
          )
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_event_count"),
          statistics.getEventCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_meeting_count"),
          statistics.getMeetingCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_forum_count"),
          statistics.getForumCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_topic_count"),
          statistics.getTopicCount()
        )
      );
    this.igData.add(
        new StatData(
          translate("ig_summary_statistic_post_count"),
  ...
  private void builIGStructure() {
    ServiceTreeRepresentation informationRepresent =
      this.statsServ.getInformationStructure(currentNode);
    ServiceTreeRepresentation libraryRepresent =
      this.statsServ.getLibraryStructure(currentNode);
    ServiceTreeRepresentation newgroupsRepresent =
      this.statsServ.getNewsgroupsStructure(currentNode);

    HtmlServiceTree = buildUlLiTreeFolder(
      informationRepresent.getChild(),
      true
    );
  ...
  private String buildUlLiTreeFolder(Child child, Boolean firstChild) {
    ...
    if (firstChild) {
      html
        .append("<span class=\"recordSetHeader\">")
  ...
  private void builIGtimeline() {
    this.igTimeLineActivity = this.getStatsServ()
      .getListOfActivityCount(currentNode);
  }
  ...
  public boolean getIgStructureChoosen() {
    ...
    if (this.selectedStatisticChoices != null) {
      if (
        this.selectedStatisticChoices.equals(
            INTEREST_GROUP_STRUCTURE_STATISTICS
          )
      ) {
        ok = true;
  ...
  public String export(ActionEvent event) {
    ...
    try {
      ...
      if (
        getSelectedStatisticExport().equals(EXPORT_CSV) &&
        getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)
      ) {
        response.setHeader(
          "Content-Disposition",
        ...
        getSelectedStatisticExport().equals(EXPORT_XML) &&
        getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)
      ) {
        response.setHeader(
          "Content-Disposition",
        ...
        getSelectedStatisticExport().equals(EXPORT_XLS) &&
        getSelectedStatisticChoices().equals(GLOBAL_STATISTICS)
      ) {
        response.setHeader(
          "Content-Disposition",
        ...
        getSelectedStatisticExport().equals(EXPORT_CSV) &&
        getSelectedStatisticChoices().equals(TIMELINE_STATISTICS)
      ) {

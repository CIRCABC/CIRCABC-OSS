...
public class AuditSearchDialog extends BaseWaiDialog {
  ...
  private static final String MESSAGE_ID_DESCRIPTION_IG =
    "audit_search_page_description_for_ig";
  private static final String MESSAGE_ID_DESCRIPTION_CIRCABC =
    "audit_search_page_description_for_circabc";
  ...
  private static final String MESSAGE_ID_SPACES =
    "library_panel_container_label";
  private static final String MESSAGE_ID_ERROR_FOUND_SOURCES =
    "error_db_populate_lists";
  private static final String MESSAGE_ID_ERROR_DATE_MANDATORY =
    "audit_search_dates_mandatory";
  private static final String MESSAGE_ID_WARN_DATES_INVERSED =
    "audit_search_dates_limit_interval_inversed";
  private static final String MESSAGE_ID_WARN_DATE_LIMIT_EXCEEDED =
    "audit_search_dates_limit_exceeded_circabc";
  private static final String MESSAGE_ID_WARN_TO_DATES_IN_FUTUR =
    "audit_search_dates_to_date_in_futur";
  /**
  ...
  private void initUsers() {
    ...
    NodeRef circabcContainer = getManagementService()
    ...
    final Set<String> invitedUsers =
      circabcProfileManagerService.getInvitedUsers(circabcContainer);
    final ArrayList<SortableSelectItem> userList = new ArrayList<>(
      invitedUsers.size()
    );
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    // get the audit list
    auditList = getLogService()
      .search(igID, user, service, method, fromDate, toDate);
  ...
  private Long getContainerID() {
    ...
    NodeRef circabcContainer = getManagementService()
      .getCurrentInterestGroup(spaceToInspect);
    if (circabcContainer == null) {
      circabcContainer = getManagementService()
        .getCurrentCategory(spaceToInspect);
      if (circabcContainer == null) {
        circabcContainer = getManagementService().getCircabcNodeRef();
      }
    }
    Long igID = (Long) getNodeService()
      .getProperty(circabcContainer, ContentModel.PROP_NODE_DBID);
    return igID;
  }
  ...
  protected void checkDates() throws IllegalArgumentException {
    if (fromDate == null || toDate == null) {
      throw new IllegalArgumentException(
        bundle.getString(MESSAGE_ID_ERROR_DATE_MANDATORY)
      );
    }
  ...
  public void export() {
    ...
    String extension = exportType.equals(ExportTypeEnum.Excel)
      ? "xls"
    ...
    try {
      ...
      response.setHeader(
        "Content-Disposition",
        "attachment;filename=circabc_log." + extension
      );
      switch (exportType) {
        case CSV:
  ...
  private void writeXML(
    ...
    for (LogSearchResultDAO item : list) {
      ...
      xtw.writeAttribute(
        "path",
        WebClientHelper.emptyStringIfNull(item.getPath())
      );
      xtw.writeAttribute(
        "information",
        WebClientHelper.emptyStringIfNull(item.getInfo())
      );
      xtw.writeEndElement();
    }
    xtw.writeCharacters("\n");
  ...
  private void writeXLS(
    List<LogSearchResultDAO> logSearchResults,
    ServletOutputStream outStream
  ) throws IOException {
    ...
    for (LogSearchResultDAO logSearchResult : logSearchResults) {
      ...
      row
        .createCell(0)
        .setCellValue(simpleDateFormat.format(logSearchResult.getLogDate()));
      ...
      row
        .createCell(5)
        .setCellValue(
          WebClientHelper.emptyStringIfNull(logSearchResult.getPath())
        );
      row
        .createCell(6)
        .setCellValue(
          WebClientHelper.emptyStringIfNull(logSearchResult.getInfo())
        );
      idx++;
    }
  ...
  public String getFinishButtonLabel() {
    ...
    return (auditList == null)
      ? bundle.getString(MESSAGE_ID_SEARCH)
  ...
  public SelectItem[] getServices() {
    ...
    List<SelectItem> servicesAsItemList = new ArrayList<>(
      servicesAsList.size()
    );
    ...
    return servicesAsItemList.toArray(
      new SelectItem[servicesAsItemList.size()]
    );
  }

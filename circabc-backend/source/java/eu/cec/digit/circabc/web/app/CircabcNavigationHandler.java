...
public class CircabcNavigationHandler extends AlfrescoNavigationHandler {
  ...
  public static final String CLOSE_WAI_DIALOG_OUTCOME =
    WAI_PREFIX + CLOSE_DIALOG_OUTCOME;
  public static final String CLOSE_WAI_WIZARD_OUTCOME =
    WAI_PREFIX + CLOSE_WIZARD_OUTCOME;
  ...
  public static final String WAI_NAVIGATION_CONTAINER_PAGE =
    "/jsp/extension/wai/navigation/container.jsp";
  public static final String WAI_DIALOG_CONTAINER_PAGE =
    "/jsp/extension/wai/dialog/container.jsp";
  public static final String WAI_WIZARD_CONTAINER_PAGE =
    "/jsp/extension/wai/wizard/container.jsp";
  public static final String VIEW_STACK = "_alfViewStack";
  private static final Log logger = LogFactory.getLog(
    CircabcNavigationHandler.class
  );
    ...
    if (outcome != null) {
      if (isWai(outcome)) {
        ...
        if (isNavigation(outcome)) {
          ...
          if (
            "dialog:updateContentWai".equals(outcome) ||
  ...
  @Override
  protected String getViewIdFromStackObject(
    ...
    if (
      viewId.equals("/jsp/close.jsp") ||
      (topOfStack instanceof String &&
        ((String) topOfStack).equals("/jsp/close.jsp"))
    ) {
      return WAI_NAVIGATION_CONTAINER_PAGE;
    }
      ...
      if (stateObject instanceof NavigationState) {
        ...
        if (!Beans.getWaiBrowseBean().isNodeSecure(state.getNode())) {
          ...
          if (stack.isEmpty()) {
            ...
            return getViewIdFromStackObject(
              context,
              getViewStack(context).pop()
            );
          }
        }
        ...
        // restore the dialog state and get the dialog container viewId
        WaiApplication.getDialogManager()
          .restoreState((DialogState) stateObject);
      } else if (stateObject instanceof WizardState) {
        // restore the wizard state and get the wizard container viewId
        WaiApplication.getWizardManager()
          .restoreState((WizardState) stateObject);
      }
  ...
  protected void addCurrentViewToStack(final FacesContext context) {
    ...
    if (viewId.equals(getNavigationContainer(context))) {
      ...
      newEqualsTop = NavigationManager.areStatesEquals(
        navMgr.getState(),
        topOfStack instanceof StackElement
          ? ((StackElement) topOfStack).getState()
          : topOfStack
      );
  ...
  protected boolean isViewAnyWizardContainer(
    FacesContext context,
    String viewId
  ) {
    if (viewId == null) {
      ...
      return (
        viewId.equals(getWizardContainer(context)) ||
        WAI_WIZARD_CONTAINER_PAGE.equals(viewId)
      );
    }
  }

  protected boolean isViewAnyDialogContainer(
    FacesContext context,
    String viewId
  ) {
    if (viewId == null) {
      ...
      return (
        viewId.equals(getDialogContainer(context)) ||
        WAI_DIALOG_CONTAINER_PAGE.equals(viewId)
      );
    }
  }

  @Override
  protected void handleDialogOrWizardClose(
    FacesContext context,
    String fromAction,
    String outcome,
    boolean dialog
    ...
    if (
      getViewStack(context).empty() == true &&
      (DialogManager.CANCEL_FROM_ACTION.equals(fromAction) ||
        WizardManager.CANCEL_FROM_ACTION.equals(fromAction))
    ) {
      ...
      if (getViewStack(context).empty() == false) {
        ...
        if (overriddenOutcome == null) {
          ...
          if (numberToClose == 1) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Popped item from the top of the view stack: " + stackObject
              );
            }
          } else {
            ...
            if (itemsOnStack < numberToClose) {
              if (logger.isDebugEnabled()) {
                logger.debug(
                  "Returning to first item on the view stack as there aren't " +
                  numberToClose +
                  " containers to close!"
                );
              }
          ...
          if (
            stackObject.equals("/jsp/browse/browse.jsp") &&
            !context
              .getViewRoot()
              .getViewId()
          ...
          String previousViewId = getViewIdFromStackObject(
            context,
            getViewStack(context).peek()
          );
  ...
  private void navigate(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Passing outcome '" + outcome + "' to original navigation handler"
      );
    }
  ...
  protected boolean isWizardStep(String fromAction) {
    ...
    if (
      fromAction != null &&
      (fromAction.equals("#{WaiWizardManager.next}") ||
        fromAction.equals("#{WaiWizardManager.back}"))
    ) {
      wizardStep = true;
    } else {
  ...
  public void useCircabcContainer(FacesContext context) {
    context
      .getExternalContext()
      .getSessionMap()
      .put(WAI_CONTAINER_SESSION, Boolean.TRUE);
  }

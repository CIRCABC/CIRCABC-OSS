...
public class AddTranslationWithoutContentDialog
  extends org.alfresco.web.bean.ml.AddTranslationWithoutContentDialog
  implements WaiDialog {

  public static final String BEAN_NAME =
    "CircabcAddTranslationWithoutContentDialog";
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (id != null && id.length() > 0) {
      try {
        ...
        if (
          ApplicationModel.TYPE_FOLDERLINK.equals(
            getNodeService().getType(this.newTranslation)
          )
        ) {
    ...
    if (
      getNodeService()
        .hasAspect(this.newTranslation, ContentModel.ASPECT_AUTHOR) &&
      getNodeService()
        .getProperty(this.newTranslation, ContentModel.PROP_AUTHOR) !=
      null
    ) {
      setAuthor(
    ...
    if (
      getNodeService()
        .hasAspect(this.newTranslation, ContentModel.ASPECT_LOCALIZED) &&
      getNodeService()
        .getProperty(this.newTranslation, ContentModel.PROP_LOCALE) !=
      null
    ) {
      setLanguage(
        ((Locale) getNodeService()
            .getProperty(
              this.newTranslation,
    ...
    this.navigator.setCurrentNodeId(
        getNodeService()
          .getPrimaryParent(this.newTranslation)
          .getParentRef()
          .getId()
      );
  ...
  protected boolean validate() {
    ...
    if (getLanguage().equalsIgnoreCase("null")) {
      Utils.addErrorMessage(
        Application.getMessage(
          FacesContext.getCurrentInstance(),
          "make_multilingual_error_language"
        )
      );
      valid = false;
    }
    ...
    if (getAuthor() == null || getAuthor().trim().length() < 1) {
      Utils.addErrorMessage(
        Application.getMessage(
          FacesContext.getCurrentInstance(),
          "make_multilingual_error_author"
        )
      );
      valid = false;
    }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (!validate()) {
      ...
      getNodeService()
        .addAspect(newTranslation, ContentModel.ASPECT_TITLED, null);
      getNodeService()
        .addAspect(
          newTranslation,
      ...
      getNodeService()
        .addAspect(newTranslation, CircabcModel.ASPECT_LIBRARY, null);
  ...
  protected String doPostCommitProcessing(
    ...
    logRecord.setInfo(
      " Only added content properties in" + super.getLanguage()
    );

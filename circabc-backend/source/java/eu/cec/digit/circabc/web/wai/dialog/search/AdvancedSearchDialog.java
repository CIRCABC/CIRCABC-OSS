...
public class AdvancedSearchDialog
  extends org.alfresco.web.bean.search.AdvancedSearchDialog
  ...
  private static final String SPACE_STRORE_STRING =
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      reset = parameters.containsKey(RESET)
        ? Boolean.parseBoolean(parameters.get(RESET))
        : true;
    } else {
      reset = false;
    }
    ...
    if (properties.getLookin().equals(SearchProperties.LOOKIN_INTEREST_GROUP)) {
      location = ((CircabcNavigationBean) navigator).getCurrentIGRoot()
        .getNodeRef();
    } else if (
      properties.getLookin().equals(SearchProperties.LOOKIN_CURRENT_SERVICE)
    ) {
      location = getCurrentServiceRoot();
    } else {
      location = ((CircabcNavigationBean) navigator).getCurrentNode()
        .getNodeRef();
    }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    final SearchProperties circabcProperties =
      (SearchProperties) super.properties;

    search.setForceAndTerms(forceAnd);

    if (
      circabcProperties.getStatus() != null &&
      !circabcProperties.getStatus().equals(SearchProperties.ANY_VALUE)
    ) {
      search.addFixedValueQuery(PROP_STATUS, circabcProperties.getStatus());
    }
    if (
      circabcProperties.getSecurityRanking() != null &&
      !circabcProperties.getSecurityRanking().equals(SearchProperties.ANY_VALUE)
    ) {
      search.addFixedValueQuery(
        PROP_SECURITY_RANKING,
        circabcProperties.getSecurityRanking()
      );
    }

    if (
      circabcProperties.getReference() != null &&
      circabcProperties.getReference().length() != 0
    ) {
      search.addAttributeQuery(
        PROP_REFERENCE,
        circabcProperties.getReference()
      );
    }

    if (
      circabcProperties.getUrl() != null &&
      circabcProperties.getUrl().length() != 0
    ) {
    ...
    if (circabcProperties.isIssueDateChecked()) {
      ...
      final String strIssueDateFrom = df.format(
        circabcProperties.getIssueDateFrom()
      );
      final String strIssueDateTo = df.format(
        circabcProperties.getIssueDateTo()
      );
      search.addRangeQuery(
        PROP_ISSUE_DATE,
        strIssueDateFrom,
        strIssueDateTo,
        true
    ...
    if (circabcProperties.isExpirationDateChecked()) {
      final SimpleDateFormat df = CachingDateFormat.getDateFormat();
      final String strExpDateFrom = df.format(
        circabcProperties.getExpirationDateFrom()
      );
      final String strExpDateTo = df.format(
        circabcProperties.getExpirationDateTo()
      );
      search.addRangeQuery(
    ...
    if (
      circabcProperties.getKeywords() != null &&
      circabcProperties.getKeywords().size() > 0
    ) {
      ...
      for (int x = 0; x < keySize; ++x) {
        ...
        uglyWorkaround
          ...
          .append(":\"\\")
          .append(QueryParser.escape(keywordRef.toString()))
          .append("\\*\"");
    ...
    if (
      circabcProperties.getSelectedLanguageOption() != null &&
      !circabcProperties
        .getSelectedLanguageOption()
        .equals(SearchProperties.LANGUAGE_ALL)
    ) {
      if (
        circabcProperties
          .getSelectedLanguageOption()
          .equals(SearchProperties.LANGUAGE_CURRENT)
      ) {
        final String userLang = getUserPreferencesBean()
          .getContentFilterLanguage();

        if (
          UserPreferencesBean.MSG_CONTENTALLLANGUAGES.equalsIgnoreCase(userLang)
        ) {
      ...
      if (circabcProperties.getLanguage() != null) {
        search.addAttributeQuery(
          PROP_LOCALE,
          '*' + circabcProperties.getLanguage() + '*'
        );
      }
    }

    final List<DynamicProperty> interestGroupDynamicProperties =
      getInterestGroupDynamicProperties();
    final Map<QName, DynamicProperty> map = new HashMap<>(
    ...
    for (QName qName : DocumentModel.ALL_DYN_PROPS) {
      Serializable dynamicProperty =
        (Serializable) circabcProperties.getDynamicProperty(i);
    ...
    if (
      circabcProperties.getSelectedDeepOption() != null &&
      !circabcProperties
        .getSelectedDeepOption()
        .equals(SearchProperties.DEEP_OPTION_LATEST_VERSION)
    ) {
    ...
    if (
      circabcProperties
        .getLookin()
        .equals(SearchProperties.LOOKIN_INTEREST_GROUP)
    ) {
      location = ((CircabcNavigationBean) navigator).getCurrentIGRoot()
        .getNodeRef();
    } else if (
      circabcProperties
        .getLookin()
        .equals(SearchProperties.LOOKIN_CURRENT_SERVICE)
    ) {
      location = getCurrentServiceRoot();
    } else {
      location = ((CircabcNavigationBean) navigator).getCurrentNode()
  ...
  public List<SelectItem> getSavedSearches() {
    ...
    try {
      ...
      if (locationRef != null) {
        ...
        List<SelectItem> userSavedSearches = getOptimizedSavedSearches(
          locationRef
        );

        properties.getCachedSavedSearches().put(null);
        properties.setSavedSearchMode(SAVED_SEARCHES_GLOBAL);
        List<SelectItem> globalSavedSearches = getOptimizedSavedSearches(
          locationRef
        );
  ...
  private List<SelectItem> getOptimizedSavedSearches(String locationRef) {
    ...
    if (savedSearches == null) {
      ...
      if (SAVED_SEARCHES_USER.equals(properties.getSavedSearchMode())) {
        ...
        SAVED_SEARCHES_GLOBAL.equals(properties.getSavedSearchMode())
      ) {
        ...
        if (childRefs.size() != 0) {
          for (ChildAssociationRef ref : childRefs) {
            ...
            if (dd.isSubClass(childNode.getType(), ContentModel.TYPE_CONTENT)) {
              savedSearches.add(
                new SelectItem(childNode.getId(), childNode.getName())
              );
            }
          }
      ...
      if (globalRef != null) {
        ...
        try {
          results = getSearchService()
            .selectNodes(globalRef, xpath, null, getNamespaceService(), false);
  ...
  private void resetFields() {
    ...
    circabcProperties.setLanguage(null);
    circabcProperties.setSelectedDeepOption(
      SearchProperties.DEEP_OPTION_LATEST_VERSION
    );
  ...
  private NodeRef getCurrentServiceRoot() {
    final NavigableNode service =
      ((CircabcNavigationBean) navigator).getCurrentIGService();
  ...
  public List<SelectItem> getSecurityRankings() {
    ...
    if (
      (circabcProperties.getSecurityRankings() == null) ||
      ...
      for (final String securityRanking : SECURITY_RANKINGS) {
        circabcProperties
          ...
          .add(
            new SelectItem(
              securityRanking,
              translate(securityRanking.toLowerCase())
            )
          );
      }
    }
  ...
  public String getDisplayKeywords() {
    return KeywordConverter.getDisplayString(
      FacesContext.getCurrentInstance(),
      getKeywords(),
      Boolean.TRUE
    );
  ...
  public void updateSavedSearch(ValueChangeEvent event) {
    ...
    if (
      ...
      circabcProperties.setExpirationDateFrom(
        (Date) additionalProperties.get(
          UISearchCustomProperties.PREFIX_DATE_FROM +
          PROP_EXPIRATION_DATE.toString()
        )
      );
      circabcProperties.setExpirationDateTo(
        (Date) additionalProperties.get(
          UISearchCustomProperties.PREFIX_DATE_TO +
          PROP_EXPIRATION_DATE.toString()
        )
      );
    }
    ...
    if (
      (Boolean) additionalProperties.get(PROP_ISSUE_DATE.toString()) != null
    ) {
      circabcProperties.setIssueDateChecked(
        (Boolean) additionalProperties.get(PROP_ISSUE_DATE.toString())
      );
      circabcProperties.setIssueDateFrom(
        (Date) additionalProperties.get(
          UISearchCustomProperties.PREFIX_DATE_FROM + PROP_ISSUE_DATE.toString()
        )
      );
      circabcProperties.setIssueDateTo(
        (Date) additionalProperties.get(
          UISearchCustomProperties.PREFIX_DATE_TO + PROP_ISSUE_DATE.toString()
        )
      );
    }

    if (additionalProperties.get(PROP_LOCALE.toString()) != null) {
      final String lang = (String) additionalProperties.get(
        PROP_LOCALE.toString()
      );
      final int length = lang.length();

      if (length > 2) {
        circabcProperties.setSelectedLanguageOption(
          SearchProperties.LANGUAGE_SPECIFY
        );
        circabcProperties.setLanguage(lang.substring(1, length - 1));
      }
    }

    circabcProperties.setSecurityRanking(
      (String) additionalProperties.get(
        UISearchCustomProperties.PREFIX_LOV_ITEM +
        PROP_SECURITY_RANKING.toString()
      )
    );
    circabcProperties.setStatus(
      (String) additionalProperties.get(
        UISearchCustomProperties.PREFIX_LOV_ITEM + PROP_STATUS.toString()
      )
    );
    circabcProperties.setReference(
      (String) additionalProperties.get(PROP_REFERENCE.toString())
    );
    circabcProperties.setUrl(
      (String) additionalProperties.get(PROP_URL.toString())
    );
    circabcProperties.setKeywords(
      parseKeywordQuery(additionalProperties.get(PROP_KEYWORD.toString()))
    );
    ...
    int i = 1;
    for (QName qName : DocumentModel.ALL_DYN_PROPS) {
      Serializable dynamicProperty = (Serializable) additionalProperties.get(
        qName.toString()
      );
  ...
  public boolean isLookinCurrentLocationDisable() {
    final NavigableNodeType nodeType =
      ((CircabcNavigationBean) navigator).getCurrentNodeType();

    return (
      nodeType == null || !nodeType.isStrictlyUnder(NavigableNodeType.IG_ROOT)
    );
  }
  ...
  public List<SelectItem> getStatuses() {
    ...
    if (
      (circabcProperties.getStatuses() == null) ||
      (Application.isDynamicConfig(FacesContext.getCurrentInstance()))
    ) {
  ...
  public List<DynamicProperty> getInterestGroupDynamicProperties() {
    final NavigableNode interestGroup =
      ((CircabcNavigationBean) navigator).getCurrentIGRoot();

    if (interestGroup == null) {
      return null;
    } else {
      return getDynamicPropertyService()
        .getDynamicProperties(interestGroup.getNodeRef());
    }
  }
  ...
  public List<SelectItem> getLanguages() {
    ...
    if (
      (circabcProperties.getLanguages() == null) ||
      (Application.isDynamicConfig(FacesContext.getCurrentInstance()))
    ) {
      final SelectItem[] languagesAsArray = getUserPreferencesBean()
        .getContentFilterLanguages(false);
      circabcProperties.setLanguages(
        new ArrayList<SelectItem>(languagesAsArray.length)
      );
  ...
  private void setDynamicProperty(
    ...
    if (value != null && value.toString().trim().length() > 0) {
      if (value instanceof Date) {
        ...
        search.addRangeQuery(
          propQname,
          df.format(calendarFrom.getTime()),
          df.format(calendarTo.getTime()),
          true
        );
      } else {
   ...
   */
  public final void setUserPreferencesBean(
    CircabcUserPreferencesBean userPreferencesBean
  ) {
  ...
  protected final KeywordsService getKeywordsService() {
    if (keywordsService == null) {
      keywordsService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()
      ).getKeywordsService();
    }
  ...
  protected final DynamicPropertyService getDynamicPropertyService() {
    if (dynamicPropertyService == null) {
      dynamicPropertyService = Services.getCircabcServiceRegistry(
        FacesContext.getCurrentInstance()

...
public class BulkInviteCircabcUsersWizard extends BaseWaiWizard {
  ...
  public static final String BLANK_PROFILE_DETECTED =
    "bulk_invite_user_blank_profile_detected";
  static final Log logger = LogFactory.getLog(
    BulkInviteCircabcUsersWizard.class
  );
  /**
   *
   */
  ...
  public void init(Map<String, String> parameters) {
    ...
    ecasDepartmentNumberEnabled = Boolean.valueOf(
      CircabcConfiguration.getProperty(
        CircabcConfiguration.BULK_USER_IMPORT_DEPARTMENT_NUMBER_STATUS
      )
    );
    ...
    for (Profile profile : bulkUserImportService.listGroupProfiles(
    ...
    if (toContinue) {
      ...
      if (
        profileManagerService
          .getInvitedUsers(categRefs.get(0))
          .contains(this.getNavigator().getCurrentAlfrescoUserName())
      ) {
        for (NodeRef igEntry : getManagementService()
          .getInterestGroups(categRefs.get(0))) {
          if (
            !knownIg.contains(igEntry.getId()) &&
            ...
            this.availableInterestGroups.add(
                new SelectItem(
                  igEntry.getId(),
                  getNodeService()
                    .getProperty(igEntry, ContentModel.PROP_TITLE)
                    .toString(),
        ...
        for (UserIGMembershipRecord igEntry : getUserService()
          .getInterestGroups(
            this.getNavigator().getCurrentUser().getUserName(),
            categRefs
          )) {
          if (
            ...
            if (
              !knownIg.contains(igEntry.getInterestGroupNodeId()) &&
    ...
    for (String item : selectedAvailableInterestGroups) {
      if (!item.equals(this.getActionNode().getNodeRef().getId())) {
        ...
        bulkUserImportService.addAll(
          model,
          bulkUserImportService.listMembers(
            new NodeRef(this.getActionNode().getNodeRef().getStoreRef(), item),
            createIgProfileHelper
          ),
          this.getActionNode().getNodeRef()
        );
      }
  ...
  public void removeSelection(ActionEvent event) {
    for (String item : selectedChosedInterestGroups) {
      for (Entry<String, List<SelectItem>> entry : chosenGroups.entrySet()) {
        for (
          Iterator<SelectItem> tmp = entry.getValue().iterator();
          tmp.hasNext();
        ) {
  ...
  protected String finishImpl(FacesContext context, final String outcome)
    ...
    if (!containsProfileBlankName()) {
      ...
      Utils.addStatusMessage(
        ...
        translate(BLANK_PROFILE_DETECTED)
      );
      isFinished = false;
      return null;
    }
  ...
  public void uploadTemplate(ActionEvent event) {
    if (submittedFile != null) {
      if (
        allowedFileTypes.contains(submittedFile.getContentType()) ||
        submittedFile.getName().matches(".*xls") ||
        submittedFile.getName().contains(".*xlsx")
      ) {
        try {
          ByteArrayInputStream is = new ByteArrayInputStream(
            submittedFile.getBytes()
          );
          HSSFWorkbook wb = new HSSFWorkbook(is);

          bulkUserImportService.addAll(
            model,
            bulkUserImportService.loadWork(wb, submittedFile.getName()),
            this.getActionNode().getNodeRef()
          );
  ...
  public void saveWorkTemplate(ActionEvent event) {
    ...
    response.setContentType(allowedFileTypes.get(0)); // Check http://www.w3schools.com/media/media_mimeref.asp for all types. Use if necessary ServletContext#getMimeType() for auto-detection based on filename.
    response.setHeader(
      "Content-disposition",
      "attachment; filename=\"bulkUserImportWorkingCopy.xls\""
  ...
  @Override
  public String next() {
    bulkUserImportService.parseProfilesToBeCreated(
      model,
      createIgProfileHelper,
  ...
  public void refreshDepartmentNumberProfileHelper(ValueChangeEvent event) {
    createDepartmentNumberProfileHelper = Boolean.valueOf(
      event.getNewValue().toString()
    );
   ...
   */
  public void setAvailableInterestGroups(
    List<SelectItem> availableInterestGroups
  ) {
   ...
   */
  public void setSelectedAvailableInterestGroups(
    List<String> selectedAvailableInterestGroups
  ) {
  ...
  public List<SelectItem> getChosenConvertedGroups() {
    ...
    for (String key : getChosenGroups().keySet()) {
      for (SelectItem chosenGroup : getChosenGroups().get(key)) {
        result.add(
          new SelectItem(
            chosenGroup.getValue(),
            key + " / " + chosenGroup.getLabel()
          )
        );
      }
    }
  ...
  public void setCreateDepartmentNumberProfileHelper(
    ...
    this.createDepartmentNumberProfileHelper =
      createDepartmentNumberProfileHelper;
  }
  ...
  private void initProfilesSelectItems() {
    ...
    for (String profile : igProfiles.keySet()) {
      ...
      if (
        !(profile.equalsIgnoreCase("Guest") ||
          profile.equalsIgnoreCase("Registered") ||
          profile.contains(":"))
      ) {

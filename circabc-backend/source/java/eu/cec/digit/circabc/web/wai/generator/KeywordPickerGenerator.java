...
public class KeywordPickerGenerator extends BaseDialogLauncherGenerator {
  ...
  private static final String MSG_PICK_TOOLTIP =
    "keywords_property_pick_tooltip";
  ...
  protected String getTextContent(FacesContext context, String id) {
    final KeywordsService keywordService = Services.getCircabcServiceRegistry(
      context
    ).getKeywordsService();
    final List<Keyword> keywords = keywordService.getKeywordsForNode(
      getNode().getNodeRef()
    );

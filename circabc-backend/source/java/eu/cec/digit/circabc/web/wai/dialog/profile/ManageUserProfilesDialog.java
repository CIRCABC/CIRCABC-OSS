...
public class ManageUserProfilesDialog extends BaseWaiDialog {
  ...
  protected static final String MSG_ALL_PROFILE =
    "members_home_disable_profile_filter";
  ...
  private static final Log logger = LogFactory.getLog(
    ManageUserProfilesDialog.class
  );
  ...
  public List<SortableSelectItem> getProfiles() {
    ...
    profiles.addAll(
      ProfileUtils.buildMailableProfileItems(getActionNode(), logger)
    );
    return profiles;
  }
      ...
      new RetryingTransactionCallback<Object>() {
        public Object execute() throws Throwable {
          ...
          for (final String authority : allInvited) {
            ...
            Profile profile;
            personProfile = profileManagerService.getPersonProfile(
              activeNode,
              authority
            );
          ...
          for (final String authority : allInvited) {
            ...
            final String firstName = (String) node.get(
              ContentModel.PROP_FIRSTNAME.toString()
            );
            final String lastName = (String) node.get(
              ContentModel.PROP_LASTNAME.toString()
            );
            final String email = (String) node.get(
              ContentModel.PROP_EMAIL.toString()
            );
            ...
            node.put(
              PermissionUtils.KEY_USER_FULL_NAME,
              firstName + " " + lastName
            );
            ...
            node.put(
              PermissionUtils.KEY_USER_PROFILE,
            ...
            if (profile.isImported()) {
              ...
              authority.equals(getAuthenticationService().getCurrentUserName())
            ) {
              node.put("canBeRemoved", Boolean.FALSE);
            } else {
  ...
  private Set<String> buildInvitedSet() {
    ...
    final RetryingTransactionHelper txnHelper =
      Repository.getRetryingTransactionHelper(context);
    final RetryingTransactionCallback<Set<String>> callback =
      new RetryingTransactionCallback<Set<String>>() {
        public Set<String> execute() throws Throwable {
          ...
          if (
            selectedProf == null ||
            selectedProf.length() < 1 ||
            VALUE_ALL_PROFILES.equals(selectedProf)
          ) {
            invitedUsers = profileManagerService
              .getInvitedUsersProfiles(nodeRef)
              .keySet();
          } else {
            if (authorityService.authorityExists(selectedProf)) {
              invitedUsers = getAuthorityService()
  ...
  public String applyNewProfile() {
    try {
      ...
      final ProfileManagerService profileManagerService =
        getProfileManagerServiceFactory().getProfileManagerService(IGNodeRef);
      for (Map map : getCachedUsers()) {
        ...
        if (node != null) {
          ...
          final Profile newProfile = profileManagerService.getProfileFromGroup(
            IGNodeRef,
            this.newProfile
          );
  ...
  public String getContainerTitle() {
    return translate(
      "manage_invited_users_dialog_title",
    ...
    if (this.selectedProfile != null) {
      if (!VALUE_ALL_PROFILES.equalsIgnoreCase(this.selectedProfile)) {
        ...
        final Profile selectedProfile =
          profileManagerService.getProfileFromGroup(
            IGNodeRef,
  ...
  public Boolean getDisableApplyNewProfileButton() {
    boolean result =
      ...
      selectedProfile.endsWith(VALUE_ALL_PROFILES) ||
      (selectedProfile.length() == 0) ||
      getCachedUsers() == null ||
      getCachedUsers().isEmpty() ||
  ...
  public void export() {
    ...
    for (Map props : filteredUsers) {
      ...
      if (!CircabcConfig.USE_LDAP) {
        ...
        username = props.get(
            "{http://www.cc.cec/circabc/model/user/1.0}ecasUserName"
          ) ==
          null
          ? ""
          : props
            .get("{http://www.cc.cec/circabc/model/user/1.0}ecasUserName")
            .toString();
      }
      String firstName = props.get("firstName") == null
        ? ""
        : props.get("firstName").toString();
      String lastName = props.get("lastName") == null
        ? ""
        : props.get("lastName").toString();
      String email = props.get("email") == null
        ? ""
        : props.get("email").toString();
      String currentProfile = props.get("userProfile") == null
        ? ""
        : props.get("userProfile").toString();
    ...
    try {
      ...
      response.setHeader(
        "Content-Disposition",
        "attachment;filename=MemberList." + extension
      );
      switch (exportType) {
        case CSV:

...
public class ExternalAccessServlet
  extends org.alfresco.web.app.servlet.ExternalAccessServlet {
  ...
  public static final String MSG_LOGIN_BUT_NOT_ALLOWED =
   ...
   * @return URL
   */
  public static String generateRelativeWaiExternalURL(
   ...
   * @return URL
   */
  public static String generateFullWaiExternalURL(
    final String outcome,
    final String args
  ) {
   ...
   * @return URL
   */
  public static String generateFullExternalURL(
    final String outcome,
    final String args
  ) {
  ...
  protected void service(
    ...
    // logged into ECAS
      ...
      Authentication authentication =
        AuthenticationUtil.setFullyAuthenticatedUser(userPrincipal.getName());
    ...
    final FacesContext fc = FacesHelper.getFacesContext(
      req,
      res,
      getServletContext()
    );
    ...
    final NodeService nodeService = Services.getAlfrescoServiceRegistry(
      getServletContext()
    ).getNodeService();
    ...
    if (
      OUTCOME_DOCDETAILS.equals(outcome) ||
      OUTCOME_BROWSE.equals(outcome) ||
      OUTCOME_USER_DETAILS.equals(outcome)
    ) {
      ...
      if (isDialog) {
        ...
        fc
          .getViewRoot()
          .setViewId(CircabcNavigationHandler.WAI_DIALOG_CONTAINER_PAGE);

        // init the return location being to the current node
        CircabcNavigationHandler.getViewStack(fc).push(
          new CircabcNavigationHandler.StackElement(
        ...
        ((ViewUserDetailsDialog) Beans.getBean(
            ViewUserDetailsDialog.BEAN_NAME
          )).init(parameters);
        dialogManager.setCurrentDialog(
          getDialogConfig(fc, ViewUserDetailsDialog.DIALOG_NAME)
        );
      }
      ...
      // authentication to know if we must authenticate
        ...
        Utils.addErrorMessage(
          Application.getMessage(fc, MSG_LOGIN_BUT_NOT_ALLOWED)
        );
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "The Guest user have permissions to READ NodeRef: " +
            nodeRef.toString()
          );
        }
        ...
        if (isGuest()) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "The user is not connected. Redirect him to the login page."
            );
          }
          ...
          Utils.addErrorMessage(
            Application.getMessage(fc, MSG_GUEST_NOT_ALLOWED)
          );
  ...
  private String grantAccess(
    ...
    final CircabcBrowseBean browser,
    NodeRef nodeRef,
    String redirectPage
  ) {
    // set the external container session flag so that a plain container gets used
    fc
      .getExternalContext()
      .getSessionMap()
      .put(AlfrescoNavigationHandler.EXTERNAL_CONTAINER_SESSION, Boolean.TRUE);
  ...
  public AuthenticationStatus servletAuthenticate(
    ...
    if (ticket != null && ticket.length() != 0) {
      ...
      if (cookie != null && cookie.getValue() != null) {
        wasNotGuest = !cookie
          .getValue()
          .equals(CircabcConstant.GUEST_AUTHORITY);
      }
  ...
  protected DialogConfig getDialogConfig(FacesContext context, String name) {
    ...
    if (config != null) {
      DialogsConfigElement dialogsCfg =
        (DialogsConfigElement) config.getConfigElement(

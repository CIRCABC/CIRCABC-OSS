...
public class UploadFileServlet extends BaseServlet {
  ...
  private static String validJavaScript =
    "javascript:window.parent.upload_complete_helper('',{error: '${_UPLOAD_ERROR}', fileTypeImage: '${_FILE_TYPE_IMAGE}'})";
  private static Set<String> circabcValidPages = new HashSet<>();

  static {
    circabcValidPages.add("/jsp/extension/wai/dialog/actions/import.jsp");
    circabcValidPages.add(
      "/jsp/extension/wai/dialog/admin/uploadCircaIndexFile.jsp"
    );
  ...
  protected void service(
    ...
    try {
      ...
      final int maxSizeInBytes =
        this.config.getMaxSizeInMegaBytes() * 1024 * 1024;
      if (request.getContentLength() > maxSizeInBytes) {
        throw new CircabcRuntimeException(
          ...
          String.valueOf(maxSizeInBytes) +
          " bytes."
        );
      }

      final AuthenticationStatus status = servletAuthenticate(
        request,
        response
      );
      ...
      for (final FileItem item : fileItems) {
        if (item.isFormField()) {
          ...
          if (fieldName.equals("return-page")) {
            ...
            fieldName.length() > 0 && fieldName.equals("submitfile") == false
          ) {
            ...
            if (allowZeroByteFiles() == true || item.getSize() > 0) {
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                  ...
                  " size " +
                  tempFile.length() +
                  " bytes created from upload filename: " +
                  filename
                );
              }
            } else {
      ...
      if (Application.inPortalServer() == false) {
        ...
        while (enumNames.hasMoreElements()) {
          ...
          if (
            name.startsWith("javax.portlet.p") &&
            name.endsWith(AuthenticationHelper.AUTHENTICATION_USER)
          ) {
            final String key = name.substring(
              0,
              name.lastIndexOf(AuthenticationHelper.AUTHENTICATION_USER)
            );
      ...
      if (submitCallback != null) {
        final FacesContext ctx = FacesHelper.getFacesContext(
          request,
          response,
          servletConfig.getServletContext()
        );

        final MethodBinding callback = ctx
          .getApplication()
          .createMethodBinding(
            "#{" + submitCallback + "}",
      ...
      if (
        bean.getFile() == null && uploadId != null && logger.isWarnEnabled()
      ) {
      ...
      if (returnPage == null || returnPage.length() == 0) {
        throw new AlfrescoRuntimeException(
          "return-page parameter has not been supplied"
        );
      }
      ...
      if (returnPage.startsWith("javascript:")) {
        ...
        String cleanReturnPage = returnPage.replace(
      ...
      Application.handleServletError(
        getServletContext(),
  ...
  private void handleUploadException(
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "An error has occurred. Sending back response for redirecting to error page: " +
          errorPage
        );
      }
  ...
  private boolean allowZeroByteFiles() {
    final ClientConfigElement clientConfig = (ClientConfigElement) configService
      .getGlobalConfig()

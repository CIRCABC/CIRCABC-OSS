...
public class DefineOthersNotification extends BaseWaiDialog {
  ...
  protected static final String USER_SPECIFIED_TWICE =
    "notification_define_other_dialog_user_specified_twice";
  /**
   *
   */
  ...
  private static final Log logger = LogFactory.getLog(
    DefineOthersNotification.class
  );
  /**
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Start to update notification status on the node " +
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "-- 1.  Start to remove authorities that the notification status is removed ..."
      );
    }

    final Set<AuthorityNotification> oldNotifications =
      getNotificationSubscriptionService()
        .getNotifications(getActionNode().getNodeRef());
    for (
      Iterator<AuthorityNotification> iterator = oldNotifications.iterator();
      iterator.hasNext();
    ) {
      AuthorityNotification authorityNotification =
        (AuthorityNotification) iterator.next();
      ...
      for (NotificationWrapper wrapper : userNotifications) {
        newNotifications.add(
          new AuthorityNotificationImpl(
            wrapper.getStatusValue(),
            wrapper.getAuthority()
          )
        );
      }
    }

    SetView<AuthorityNotification> differenceOldNew = Sets.difference(
      oldNotifications,
      newNotifications
    );
    SetView<AuthorityNotification> differenceNewOld = Sets.difference(
      newNotifications,
      oldNotifications
    );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Notifications successfull update for the node " +
        getActionNode().getNodeRef() +
        "\n\tBefore:   " +
        oldNotifications +
        "\n\tAfter:    " +
  ...
  public SelectItem[] getStatuses() {
    if (notificationStatuses == null) {
      notificationStatuses = NotificationUtils.getStatusesAsSelectItem(
        getActionNode()
      );
    }
  ...
  public void addSelection(ActionEvent event) {
    final IGRootProfileManagerService profileManager =
          ...
          if (foundExisting == false) {
            ...
            if (authType == AuthorityType.USER) {
              ...
              final Profile profileFromGroup =
                profileManager.getProfileFromGroup(igRoot, authority);
  ...
  public void removeSelection(ActionEvent event) {
    NotificationWrapper wrapper =
      (NotificationWrapper) this.userNotificationDataModel.getRowData();
    if (wrapper != null) {
  ...
  private List<String> getAlreadySettedAuthorities() {
    final List<String> invitedList = new ArrayList<>(
      this.userNotifications.size()
    );
  ...
  private void fillDatas() {
    ...
    for (final AuthorityNotification notification : notifications) {
      wrapper = NotificationUtils.wrappNotification(
        notification,
        getActionNode().getNodeRef()
      );
  ...
  public String getContainerTitle() {
    return translate(
      "notification_define_other_dialog_title",

...
public class UICircabcContextService implements Serializable {
   ...
   */
  private Map<Class, ICircabcContextListener> registeredBeans = new HashMap<>(
    7,
    1.0f
  );
  ...
  public static UICircabcContextService getInstance(FacesContext fc) {
    ...
    UICircabcContextService service = (UICircabcContextService) session.get(
      CONTEXT_KEY
    );
    if (service == null) {
  ...
  public void registerBean(ICircabcContextListener bean) {
    if (bean == null) {
      throw new IllegalArgumentException(
        "Bean reference specified cannot be null!"
      );
    }

 ...
 */
public class CircabcNavigationBean
  extends NavigationBean
  implements Serializable, IContextListener {

  private static final Log logger = LogFactory.getLog(
    CircabcNavigationBean.class
  );
  ...
  private static final String ECAS_REDIRECTION_PAGE_BUTTON_CAPTION =
    "ecas_redirection_page_button_caption";

  private static final String FACES_JSP_EXTENSION_WAI_NAVIGATION_CONTAINER_JSP =
  ...
  private MultifactorConfigurationService multifactorConfigurationService =
    null;
  ...
  public void activateIGService(final String service) {
    if (getCurrentIGRoot() != null) {
      if (isIgServiceValid(service)) {
        ...
        this.currentIgService = (IGServicesNode) getCurrentIGRoot()
          .get(service);
        ...
        if (
          memIgServiceId != null &&
          getCurrentIGService() != null &&
          !currentIgService.getId().equals(memIgServiceId)
        ) {
          UICircabcContextService.getInstance(
            FacesContext.getCurrentInstance()
          ).igServiceChanged();
        }
      } else {
        throw new IllegalArgumentException(
          "The service specified doesn't exist: " +
          service +
          ". It must be contains in " +
          Arrays.toString(InterestGroupNode.VALID_SERVICES.toArray())
        );
      }
    } else {
  ...
  public String getCircabcUrl() {
    ...
    final String caption = WebClientHelper.translate(
      ECAS_REDIRECTION_PAGE_BUTTON_CAPTION,
      getAppName()
    );
    final String tooltip = WebClientHelper.translate(
      ECAS_REDIRECTION_PAGE_BUTTON_CAPTION,
      getAppName()
    );
    ...
    final String containerUrl =
      FACES_JSP_EXTENSION_WAI_NAVIGATION_CONTAINER_JSP;

    final String circabcUrl =
      A_HREF +
      WebClientHelper.getCircabcUrl() +
      containerUrl +
      TOOLTIP +
      tooltip +
      A_HREF_CLOSE +
  ...
  public String getBannerContactLink() {
    return CircabcConfiguration.getProperty(
      CircabcConfiguration.BANNER_CONTACT_LINK
    );
  }
  ...
  public NavigableNode getCurrentCategoryHeader() {
    if (currentCategoryHeader == null && getCurrentCategory() != null) {
      currentCategoryHeader =
        (CategoryHeaderNode) currentCategory.getNavigableParent();
    }
  ...
  public String getCurrentUserName() {
    if (userName == null) {
      userName = PermissionUtils.computeUserLogin(
        super.getCurrentUser().getPerson(),
        getNodeService()
      );
    }
    return userName;
  ...
  public List<IBreadcrumbHandler> getLocation() {
    ...
    if (getCurrentNode() != null) {
      parents = getManagementService()
        .getAllParents(getCurrentNode().getNodeRef(), false);
    }
  ...
  public void updateCircabcNavigationContext() {
    ...
    memCatHeaderId = (this.getCurrentCategoryHeader() == null)
      ? memCatHeaderId
      : getCurrentCategoryHeader().getId();
    memCategoryId = (this.getCurrentCategory() == null)
      ? memCategoryId
      : getCurrentCategory().getId();
    memIgRootId = (this.getCurrentIGRoot() == null)
      ? memIgRootId
      : getCurrentIGRoot().getId();
    memIgServiceId = (this.getCurrentIGService() == null)
      ? memIgServiceId
      : getCurrentIGService().getId();

    final boolean wasInCircabc = NavigableNodeType.NOT_CIRCABC_CHILD.equals(
      currentNodeType
    );
    ...
    if (currentNode instanceof NavigableNode) {
      ...
      this.currentNodeType =
        ((NavigableNode) currentNode).getNavigableNodeType();
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "The requested node type for the browsing " + currentNodeType
        );
      }

      if (
        currentNodeType == null ||
        NavigableNodeType.NOT_CIRCABC_CHILD.equals(currentNodeType) ||
        ...
        switch (this.currentNodeType) {
          case CATEGORY_HEADER:
            this.currentCategoryHeader = new CategoryHeaderNode(
          ...
          case CATEGORY:
            ...
            break;
          case IG_ROOT:
            this.currentIGRoot = new InterestGroupNode(
              currentNode.getNodeRef()
            );
            this.currentCategory =
              (CategoryNode) this.currentIGRoot.getNavigableParent();
            break;
          default:
              ...
              if (igNodeRef != null) {
                if (
                  getPermissionService()
                    .hasPermission(igNodeRef, PermissionService.READ)
                ...
                if (currentNodeType.isEqualsOrUnder(DIRECTORY)) {
                  ...
                  this.currentIgService = new IGServicesNode(
                    currentNode.getNodeRef(),
                    currentIGRoot
                  );
                }
    ...
    if (
      memCatHeaderId != null &&
      getCurrentCategoryHeader() != null &&
      !currentCategoryHeader.getId().equals(memCatHeaderId)
    ) {
      // we are just leaving
      UICircabcContextService.getInstance(
        FacesContext.getCurrentInstance()
    ...
    if (
      ...
      !currentCategory.getId().equals(memCategoryId)
    ) {
      UICircabcContextService.getInstance(
        FacesContext.getCurrentInstance()
      ).categoryChanged();
    ...
    if (
      memIgRootId != null &&
      getCurrentIGRoot() != null &&
      !currentIGRoot.getId().equals(memIgRootId)
    ) {
      UICircabcContextService.getInstance(
        FacesContext.getCurrentInstance()
      ).igRootChanged();
  ...
  protected final DictionaryService getDictionaryService() {
    if (dictionaryService == null) {
      dictionaryService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
  ...
  protected final MultilingualContentService getMultilingualContentService() {
    if (multilingualContentService == null) {
      multilingualContentService = Services.getAlfrescoServiceRegistry(
        FacesContext.getCurrentInstance()
      ...
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          if (applicationCustomisationService.getBannerLogoRef() != null) {
            customBannerLink = WebClientHelper.getGeneratedWaiUrl(
              applicationCustomisationService.getBannerLogoRef(),
  ...
  public Boolean getSearchLinkDisplayed() {
    searchLinkDisplayed =
      applicationCustomisationService.getDisplaySearchLink();

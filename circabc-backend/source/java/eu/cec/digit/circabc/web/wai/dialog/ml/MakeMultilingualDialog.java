...
public class MakeMultilingualDialog extends BaseWaiDialog {
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (this.parameters != null) {
      ...
      if (
        getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_AUTHOR) ==
          true &&
        getNodeService().getProperty(nodeRef, ContentModel.PROP_AUTHOR) != null
      ) {
        setAuthor(
          (String) getNodeService()
            .getProperty(nodeRef, ContentModel.PROP_AUTHOR)
        );
      } else {
        setAuthor("");
      }
      ...
      if (
        getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_LOCALIZED) ==
          true &&
        getNodeService().getProperty(nodeRef, ContentModel.PROP_LOCALE) != null
      ) {
        setLanguage(
          ((Locale) getNodeService()
              .getProperty(nodeRef, ContentModel.PROP_LOCALE)).toString()
        );
      } else {
        setLanguage(null);
      }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (
      getLanguage() == null ||
      getLanguage().equalsIgnoreCase(NO_LANGUAGE_SELECTED)
    ) {
    ...
    if (isError) {
      ...
      final NodeRef mlContainer = getMultilingualContentService()
        .getTranslationContainer(nodeRef);
      ...
      if (nodeAuthor == null || nodeAuthor.length() < 1) {
        getNodeService()
          .setProperty(nodeRef, ContentModel.PROP_AUTHOR, getAuthor());
      }

      // set properties of the ml container
      getNodeService()
        .setProperty(mlContainer, ContentModel.PROP_AUTHOR, getAuthor());

      //https://webgate.ec.europa.eu/CITnet/jira/browse/DIGIT-CIRCABC-2290
      getNodeService()
        .addAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE, null);
      return outcome;
    }
  }
  ...
  public String getPageIconAltText() {
    return Application.getMessage(
      FacesContext.getCurrentInstance(),
      "make_multilingual_icon_tooltip"
    );
  }

...
public class UIMemoryReporter extends BaseDebugComponent {
   ...
   */
  static final Log logger = LogFactory.getLog(UIMemoryReporter.class);
  ...
  public Map getDebugData() {
    ...
    try {
      final List<CacheReport> caches = getServerConfigurationService()
        .getCaches();
      ...
      for (final CacheReport cache : caches) {
        ...
        estimatedMaxSize += (Double.isNaN(cacheEstimatedMaxSize) ||
            Double.isInfinite(cacheEstimatedMaxSize))
          ? 0.0
        ...
        if (!hitRatio.endsWith("NaN percent")) {
          hitRatio = hitRatio.substring(
            0,
            hitRatio.length() - "percent".length()
          );
        ...
        if (hitRatioValue != null) {
          percentageFull = percentageFull.substring(
            0,
            percentageFull.length() - "percent".length()
          );
        ...
        properties.put(
          hitRationColorBegin + name + " - Hit Ratio" + hitRationColorEnd,
        ...
        properties.put(
          name + " - Dirty Entry Count",
          cache.getDirtyEntryCount()
        );
        properties.put(
          percentageFullBegin + name + " - Percentage used" + percentageFullEnd,
        ...
        properties.put(
          name + " - Estimated maximum size",
          cache.getEstMaxSize()
        );
        properties.put(
          name + " - Memory Store Eviction Policy",
          cache.getMemoryStoreEvictionPolicy()
        );
        properties.put(
          name + " - Eviction Percentage",
          cache.getEvictionPercentage()
        );
        properties.put(name + " - Merge Policy", cache.getMergePolicy());
        properties.put(
          name + " - Time To Live Seconds",
          cache.getTimeToLiveSeconds()
        );
        properties.put(
          name + " - Time To Idle Seconds",
      ...
      propertiesFull.put(
        "<b>Caches with Hit Ratio >= 80% FOUND</b>",
      ...
      propertiesFull.put(STARS, STARS);
      propertiesFull.put(
        "<b>Caches with 80% > Hit Ratio >= 60% FOUND</b>",
      ...
      propertiesFull.put(STARS, STARS);
      propertiesFull.put(
        "<b>Caches with Hit Ratio < 60% FOUND</b>",
      ...
      double sizePercentage =
        ((double) allCachesTotalSize / (double) maxHeapSize) * 100.0;
      double maxSizePercentage =
        (estimatedMaxSize / (double) maxHeapSize) * 100.0;

      // fill the missing values
      propertiesFull.put(
        FIRST_LINES_KEYS[0],
        String.format("%5.2f MB", maxHeapSize / 1024.0 / 1024.0)
      );
      propertiesFull.put(
        FIRST_LINES_KEYS[1],
        String.format("%5.2f MB", freeMemory / 1024.0 / 1024.0)
      );
      propertiesFull.put(
        FIRST_LINES_KEYS[2],
        String.format("%5.2f MB", totalMemory / 1024.0 / 1024.0)
      );
      ...
      propertiesFull.put(
        FIRST_LINES_KEYS[5],
        String.format("%5.2f MB", estimatedMaxSize / 1024.0 / 1024.0)
      );
      propertiesFull.put(
        FIRST_LINES_KEYS[6],
      ...
      propertiesFull.put(
        FIRST_LINES_KEYS[7],
        String.format("%3.2f percent of VM size", sizePercentage)
      );
      propertiesFull.put(
        FIRST_LINES_KEYS[8],
        String.format("%5.2f MB", estimatedMaxSize / 1024.0 / 1024.0)
      ...
      propertiesFull.put(
        FIRST_LINES_KEYS[9],
        String.format("%3.2f percent of VM size", maxSizePercentage)
      );

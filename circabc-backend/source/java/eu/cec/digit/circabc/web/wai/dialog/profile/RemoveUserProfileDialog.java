...
public class RemoveUserProfileDialog extends BaseWaiDialog {
  ...
  private static final String CAN_NOT_DELETE_LAST_CIRCABC_ADMIN =
    "can_not_delete_last_circabc_admin";
  private static final long serialVersionUID = 222220868117140631L;

  @SuppressWarnings("unused")
  private static final Log logger = LogFactory.getLog(
    RemoveUserProfileDialog.class
  );
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      if (getActionNode() == null) {
        ...
        throw new IllegalArgumentException(
          "The user name is a mandatory parameter "
        );
      }
  ...
  public String getContainerTitle() {
    ...
    if (
      isSelfRemoveUserProfile && containerType == ContainerType.INTEREST_GROUP
    ) {
      message = translate("self_uninvite_user_title_ig", name);
    } else if (
      isSelfRemoveUserProfile && containerType == ContainerType.CATEGORY
    ) {
  ...
  protected String finishImpl(final FacesContext context, String outcome)
    ...
    try {
      ...
      final ProfileManagerService profileManagerService =
        getProfileManagerServiceFactory().getProfileManagerService(nodeRef);
      final String personProfile = profileManagerService.getPersonProfile(
        nodeRef,
        userName
      );
      ...
      if (
        profile.isAdmin() &&
        getNumberofAdmins(nodeRef, profileManagerService) == 1
      ) {
      ...
      if (CircabcConfig.ENT) {
        if (interestGroup || (containerType == ContainerType.INTEREST_GROUP)) {
          ...
          for (String ecordaThemeId : ecordaThemeIds) {
            synchronizationService.revokeThemeRole(
              userName,
              ecordaThemeId,
      ...
      if (NOTIFY_YES.equals(this.notifyDirAdmins)) {
        ...
        for (String dirAdmin : dirAdmins) {
          if (getPersonService().personExists(dirAdmin)) {
            final NodeRef adminPerson = getPersonService().getPerson(dirAdmin);
            final Boolean globalNotification = (Boolean) getNodeService()
              .getProperty(adminPerson, UserModel.PROP_GLOBAL_NOTIFICATION);
            if (globalNotification != null && globalNotification) {
              ...
              model.putAll(
                buildTemplateModel(
                  person,
                  profile.getTitle().getDefaultValue(),
                  nodeRef
                )
              );
              ...
              final String to = (String) personProperties.get(
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Unexpected error:" + profileException.getExplanation(),
          profileException
        );
      }

      Utils.addErrorMessage(
        translate(Repository.ERROR_GENERIC, profileException.getExplanation()),
        profileException
      );

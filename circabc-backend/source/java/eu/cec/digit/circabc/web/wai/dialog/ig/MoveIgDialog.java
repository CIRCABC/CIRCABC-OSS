...
public class MoveIgDialog extends BaseWaiDialog {
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (this.getTargetCategory() != null && this.getSourceIg() != null) {
      ...
      if (
        hasSharedSpaces(igRef) ||
        hasExportedOrImportedProfiles(igRef, igRootProfileManagerService)
      ) {
        this.isFinished = false;
        Utils.addErrorMessage(translate("move_ig_dialog_msg_invalid"));
      } else {
        try {
          CategoryProfileManagerService categoryProfileManagerService =
            getProfileManagerServiceFactory()
              .getCategoryProfileManagerService();

          String masterInvitedGroupName =
            igRootProfileManagerService.getMasterInvitedGroupName(igRef);
          String newSubsGroupName =
            categoryProfileManagerService.getSubsGroupName(newCategoryRef);
          String oldSubsGroupName =
            categoryProfileManagerService.getSubsGroupName(oldCategoryRef);

          Profile newCategoryProfile = categoryProfileManagerService.getProfile(
            newCategoryRef,
            CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN
          );
          Profile oldCategoryProfile = categoryProfileManagerService.getProfile(
            oldCategoryRef,
            CategoryProfileManagerService.Profiles.CIRCA_CATEGORY_ADMIN
          );
          ...
          changeCategoryAdminPermissionsReqursivly(
            igRef,
  ...
  private boolean hasExportedOrImportedProfiles(
    NodeRef igRef,
    IGRootProfileManagerService igRootProfileManagerService
  ) {
  ...
  private void changeCategoryAdminPermissionsReqursivly(
    NodeRef nodeRef,
    String oldPrefixedGroupName,
    String newPrefixedGroupName
  ) {
    movePermissionRecursively(
      nodeRef,
      oldPrefixedGroupName,
      newPrefixedGroupName
    );
  }

  private void movePermissionRecursively(
    final NodeRef nodeRef,
    final String oldAuthority,
  ...
  private void moveUserGroups(
    String oldParentName,
    String newParentName,
    String childName
  ) {
    String newParentGroupName = authorityService.getName(
      AuthorityType.GROUP,
      newParentName
    );
    String oldParentGroupName = authorityService.getName(
      AuthorityType.GROUP,
      oldParentName
    );
    String childGroupName = authorityService.getName(
      AuthorityType.GROUP,
      childName
    );
  ...
  private List getAllCategoryHeaders() {
    ...
    for (CategoryHeader h : this.getCategoryHeadersBean()
      .getCategoryHeaders()) {
      CategoryItem header = h.getCategoryHeaderItem();
      result.add(
        new SelectItem(header.getNodeRef().toString(), header.getName())
      );
    }
    return result;
  }
  ...
  private List getCategoriesOfHeader(String headerRefStr) {
    ...
    if (isNotEmpty(headerRefStr)) {
      ...
      for (CategoryItem category : header.getCategories()) {
        ...
        result.add(
          new SelectItem(categoryRef.toString(), getName(categoryRef))
        );
      }
    }
    return result;
  ...
  public void export() {
    if (this.getSourceIg() != null) {
      ...
      try {
        ...
        response.setHeader(
          "Content-Disposition",
          "attachment;filename=" + interestGroupName + ".xls"
        );
  ...
  private List<UserRecord> getUserList() {
    ...
    Map<String, Profile> invitedUsersProfiles =
      igRootProfileManagerService.getInvitedUsersProfiles(igRef);
    for (Map.Entry<String, Profile> entry : invitedUsersProfiles.entrySet()) {
      ...
      String email = (String) getNodeService()
        .getProperty(person, ContentModel.PROP_EMAIL);
      String ecasDomain = (String) getNodeService()
        .getProperty(person, UserModel.PROP_DOMAIN);
      String moniker = (String) getNodeService()
        .getProperty(person, UserModel.PROP_ECAS_USER_NAME);

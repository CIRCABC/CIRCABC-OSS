...
public class EditPropertyDialog extends DefineNewPropertyWizard {
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      ...
      for (Map.Entry<Locale, String> entry : propertyToEdit
        .getLabel()
        .entrySet()) {
        propertyTranslations.add(
          new DynPropertyWrapper(entry.getValue(), entry.getKey())
        );
      }
    }
  }
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    String info = MessageFormat.format(
      "the property {0} has been edited - description {1}",
      propertyToEdit.getName(),
      labels.toString()
    );
    logRecord.setInfo(info);

    this.getPropertiesService()
      .updateDynamicPropertyLabel(propertyToEdit, labels);

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Property " +
        propertyToEdit.getName() +
        " successfully updated under the interest group " +
        getActionNode().getName() +
        ". New value: " +
        propertyToEdit.toString()
      );
    }

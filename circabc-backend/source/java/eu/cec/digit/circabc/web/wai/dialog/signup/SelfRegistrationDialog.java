...
public class SelfRegistrationDialog extends CreateCircabcUserWizard {
   ...
   */
  public static final String CONGRATULATION_DIALOG =
    "wai:dialog:selfRegisterSuccessWai";
  ...
  public static final String SESSION_KEY_NAME =
    "SelfRegistrationCaptchaKeyName";
  /**
   * The url of the activation page
   */
  protected static final String REGISTRATION_PAGE =
    "/faces/jsp/extension/wai/activation.jsp";
  private static final long serialVersionUID = 626669531664749789L;
  private static final Log logger = LogFactory.getLog(
    SelfRegistrationDialog.class
  );
  ...
  private static final String CAPTCA_VALIDATION_ERROR =
    "self_registration_page_captcha_error";
  /**
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    if (getNavigator().getCurrentNodeId() == null) {
      getNavigator()
        .setCurrentNodeId(getManagementService().getCircabcNodeRef().getId());
    }
      ...
      final RetryingTransactionCallback<String> callback =
        new RetryingTransactionCallback<String>() {
          public String execute() throws Throwable {
            ...
            try {
              ...
              if (logger.isErrorEnabled()) {
                logger.error(
                  "The user " +
                  getUserName() +
                  " can't register itself due to an error: " +
                  e.getMessage(),
                  e
                );
              }
            ...
            String activationURL = generateActivationURL(
              getUserName(),
              createdPerson.getId()
            );
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "The user " +
                getUserName() +
                " has just registred itself. Its activation is " +
                activationURL
              );
            }
            ...
            if (notifyUser(activationURL)) {
              ...
              logger.warn(
                "Failed to send the self registration email to " + getEmail()
              );
              addErrorMessage(
                FacesContext.getCurrentInstance(),
                translate(MAIL_ERROR)
              );
            }
            return outcome;
          }
  ...
  private String getCaptchaGeneratedValue() {
    String captchaValue =
      (String) ((HttpSession) FacesContext.getCurrentInstance()
          .getExternalContext()
          .getSession(true)).getAttribute(SESSION_KEY_NAME);
    return captchaValue;
  }
  ...
  protected boolean validateFields(
    ...
    boolean validateMadatotyProperties,
    boolean addFacesErrorMessages
  ) {
    super.validateFields(
      validateUsername,
      validatePassword,
      validateMadatotyProperties,
  ...
  public void init(Map<String, String> params) {
    final String requireRefreshAll = (params != null)
      ? params.get("new")
      : null;
    ...
    if (
      requireRefreshAll != null && requireRefreshAll.equalsIgnoreCase("true")
    ) {
      editMode = false;
    }
  ...
  public void validateCaptcha(
    ...
    if (isResponseCorrect == false) {
      throw new ValidatorException(
        new FacesMessage(translate(CAPTCA_VALIDATION_ERROR))
      );
    }
  }
  ...
  protected boolean notifyUser(String URL) throws Exception {
    ...
    if (to != null && to.length() != 0) {
      ...
      final Map model = getMailPreferencesService()
        .buildDefaultModel(circabcRoot, createdPerson, null);

      model.put(KEY_ACTIVATION_URL, URL);
      model.put(
        KEY_EXPIRATION,
        "" + getUserService().getAccountExpirationDays()
      );
  ...
  protected String generateActivationURL(String userName, String activationKey)
    ...
    final String urlRoot = CircabcConfiguration.getProperty(

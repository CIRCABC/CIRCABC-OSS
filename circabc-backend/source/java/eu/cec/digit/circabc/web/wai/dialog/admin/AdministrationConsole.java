...
public class AdministrationConsole extends BaseWaiDialog {
  ...
  private static final String ADMIN_CONSOLE_DIALOG_ICON_TOOLTIP =
    "admin_console_dialog_icon_tooltip";
  private static final String ADMIN_CONSOLE_DIALOG_BROWSER_TITLE =
    "admin_console_dialog_browser_title";
  ...
  public void init(final Map<String, String> parameters) {
    ...
    final CategoriesDialog categoriesDialog = (CategoriesDialog) Beans.getBean(
      "CategoriesDialog"
    );
    ...
    if (parameters != null) {
      ...
      if (
        getNavigator().getCurrentNodeId() == null ||
        !getNavigator().getCurrentNodeId().equals(id)
      ) {
  ...
  public boolean isDisplaySurveyMenu() {
    if (viewSurveyConsole == null) {
      if (!isSuperAdmin() && isCurrentNodeSurvey()) {
        ...
        viewSurveyConsole = survey.hasPermission(
  ...
  public boolean isDisplayOtherServiceConsole() {
    if (viewOtherServiceConsole == null) {
      ...
      final InterestGroupNode ig = (InterestGroupNode) getNavigator()
        .getCurrentIGRoot();
      if (!isSuperAdmin() && ig != null) {
        if (
          ig.getDirectory() != null &&
          ig.hasPermission(DirectoryPermissions.DIRMANAGEMEMBERS.toString())
        ) {
          isAdmin = true;
        } else if (
          ig.getLibrary() != null &&
          ig.getLibrary().hasPermission(LibraryPermissions.LIBADMIN.toString())
        ) {
          isAdmin = true;
        } else if (
          ig.getNewsgroup() != null &&
          ig
            .getNewsgroup()
          ...
          ig.getEvent() != null &&
          ig.getEvent().hasPermission(EventPermissions.EVEADMIN.toString())
        ) {
          isAdmin = true;
        } else if (
          ...
          ig.getSurvey() != null &&
          ig.getSurvey().hasPermission(SurveyPermissions.SURADMIN.toString())
        ) {
          isAdmin = true;
        }
      }
  ...
  public boolean isDisplayLibraryChildMenu() {
    if (viewLibChildConsole == null) {
      if (isCurrentNodeLibraryChild()) {
        viewLibChildConsole = getNavigator()
          .getCurrentNode()
          .hasPermission(LibraryPermissions.LIBADMIN.toString());
      } else {
  ...
  public boolean isDisplayForumChildMenu() {
    if (viewForumChildConsole == null) {
      if (isCurrentNodeForumChild()) {
        viewForumChildConsole = getNavigator()
          .getCurrentNode()
          .hasPermission(NewsGroupPermissions.NWSADMIN.toString());
      } else {
  ...
  public boolean isDisplaySurveyChildSpaceMenu() {
    if (viewSurveyChildConsole == null) {
      if (isCurrentNodeSurveyChild()) {
        viewSurveyChildConsole = getNavigator()
          .getCurrentNode()
  ...
  public boolean isDisplayInformationChildSpaceMenu() {
    if (viewInformationChildConsole == null) {
      if (isCurrentNodeInformationChild()) {
        viewInformationChildConsole = getNavigator()
          .getCurrentNode()
          .hasPermission(InformationPermissions.INFADMIN.toString());
      } else {
  ...
  public boolean isDisplayLibraryMenu() {
    if (viewLibraryConsole == null) {
      if (isCurrentNodeLibraryRoot()) {
        viewLibraryConsole = getNavigator()
          .getCurrentNode()
          .hasPermission(LibraryPermissions.LIBADMIN.toString());
      } else {
  ...
  public boolean isDisplayEventsMenu() {
    if (viewEventConsole == null) {
      if (!isSuperAdmin() && isCurrentNodeEventsRoot()) {
        ...
        viewEventConsole = eve.hasPermission(
  ...
  private boolean isCurrentNodeLibraryChild() {
    return isCurrentNodeOfSubtypeType(
      NavigableNodeType.LIBRARY,
      NavigableNodeType.LIBRARY_CHILD
    );
  }

  private boolean isCurrentNodeForumChild() {
    return isCurrentNodeOfSubtypeType(
      NavigableNodeType.NEWSGROUP,
  ...
  private boolean isCurrentNodeSurveyChild() {
    return isCurrentNodeOfSubtypeType(
      NavigableNodeType.SURVEY,
      NavigableNodeType.SURVEY_CHILD
    );
  }

  private boolean isCurrentNodeInformationChild() {
    return isCurrentNodeOfSubtypeType(
      NavigableNodeType.INFORMATION,
  ...
  private boolean isCurrentNodeOfSubtypeType(
    final NavigableNodeType rootType,
    ...
    return (
      type != null &&
      (subType.equals(type) ||
        (type.getRequireNodeType() != null &&
          type.getRequireNodeType().equals(subType))) &&

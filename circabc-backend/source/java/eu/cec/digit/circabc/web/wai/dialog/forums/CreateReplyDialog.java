...
public class CreateReplyDialog extends CreatePostDialog {
  ...
  private static final String CITATION_OF =
    "newsgroups_topic_subject_citation_of";
  private static final String BLOCKQUOTE =
    "<table style=\"width:90%;padding:5px;\" border=\"0\" bgcolor=\"#C0D0F0\"><tbody><tr><td><p><strong>{0}</strong></p><blockquote>{1}</blockquote></td></tr></tbody></table><br /><br />";
  ...
  protected String finishImpl(FacesContext context, String outcome)
    ...
    try {
      ...
      if (
        outcome != null &&
        getNodeService()
          .hasAspect(this.postRef, ContentModel.ASPECT_REFERENCING) ==
        false
      ) {
        //setup the referencing aspect with the references association
        // between the new post and the one being replied to
        getNodeService()
          .addAspect(this.postRef, ContentModel.ASPECT_REFERENCING, null);
        getNodeService()
          .createAssociation(
            this.postRef,
  ...
  public void init(Map<String, String> parameters) {
    ...
    if (parameters != null) {
      ...
      final String creator = (String) getBrowseBean()
        .resolverCreatorUserName.get(node);

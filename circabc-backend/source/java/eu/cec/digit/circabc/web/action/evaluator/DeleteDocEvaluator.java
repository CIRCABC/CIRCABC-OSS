  ...
  public boolean evaluate(Node node) {
    ...
    if (node.getAspects().contains(ContentModel.ASPECT_MULTILINGUAL_DOCUMENT)) {
      MultilingualContentService mlservice =
        (MultilingualContentService) FacesHelper.getManagedBean(
          fc,
      ...
      // Else if the node is the pivot language, user can't delete it
        ...
        mlservice
          .getPivotTranslation(node.getNodeRef())
          .getId()
    ...
    NodeService nodeService = (NodeService) FacesHelper.getManagedBean(
      fc,
      "NodeService"
    );
    AuthenticationService authenticationService =
      (AuthenticationService) FacesHelper.getManagedBean(
        fc,
    ...
    isOwner = nodeService
      .getProperty(node.getNodeRef(), ContentModel.PROP_OWNER)
      .toString()
    ...
    return (
      (node.isLocked() == false &&
        ...
        isPivot == false) &&
      (node.hasPermission("Delete") || isOwner)
    );
  }
}

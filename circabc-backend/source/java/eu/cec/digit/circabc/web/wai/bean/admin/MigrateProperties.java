...
public class MigrateProperties extends AddContentBean {
  ...
  protected String finishImpl(final FacesContext context, String outcome)
    ...
    SimplePath igPath;
    try {
      igPath = getManagementService()
        .getNodePath(safeString(getInterestGroupPath()));
    } catch (PathNotFoundException e) {
      throw new IllegalArgumentException(e);
    }

    if (
      !NavigableNodeType.IG_ROOT.isNodeFromType(new Node(igPath.getNodeRef()))
    ) {
      throw new IllegalArgumentException(
        "The path doens't point to an interest group"
      );
    }
    ...
    for (int x = 0; x < EXPECTED_COL_TITLES.length; ++x) {
      if (!EXPECTED_COL_TITLES[x].equals(nextLine[x])) {
        throw new IllegalArgumentException(
          "Invalid column name at position " +
          (x + 1) +
          "Expected: " +
          EXPECTED_COL_TITLES[x]
        );
      }
    }
    ...
    while ((nextLine = reader.readNext()) != null) {
      ...
      properties.put(
        ...
        I18NUtil.parseLocale(safeString(nextLine[1]))
      );
      ...
      try {
        ...
        NodeRef document = getManagementService()
          .getNodePath(documentPath)
          .getNodeRef();
        // set the properties
  ...
  public final String getInterestGroupPath() {
    if (interestGroupPath == null) {
      try {
        interestGroupPath =
          getManagementService().getRootSimplePath().toString() + "/?/?";

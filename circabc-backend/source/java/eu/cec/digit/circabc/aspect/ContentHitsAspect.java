 ...
 */
public class ContentHitsAspect
  implements
   ...
   */
  private static final String KEY_CONTENT_HITS_READS =
    ContentHitsAspect.class.getName() + ".reads";
  /**
   * A key that keeps track of nodes that need write count increments
   */
  private static final String KEY_CONTENT_HITS_WRITES =
    ContentHitsAspect.class.getName() + ".writes";
  ...
  private class ContentHitsTransactionListener
    ...
    public void afterCommit() {
      ...
      @SuppressWarnings("unchecked")
      final Set<NodeRef> readNodeRefs = (Set<
          NodeRef
        >) AlfrescoTransactionSupport.getResource(KEY_CONTENT_HITS_READS);
      if (readNodeRefs != null) {
        Runnable runnable;
        for (final NodeRef nodeRef : readNodeRefs) {
          runnable = new ContentHitsReadCountIncrementer(nodeRef);
          threadExecuter.execute(runnable);
        }
      }
      @SuppressWarnings("unchecked")
      final Set<NodeRef> writeNodeRefs = (Set<
          NodeRef
        >) AlfrescoTransactionSupport.getResource(KEY_CONTENT_HITS_WRITES);
      ...
      if (writeNodeRefs != null) {
        ...
        for (final NodeRef nodeRef : writeNodeRefs) {
          runnable = new ContentHitsWriteCountIncrementer(nodeRef);
          threadExecuter.execute(runnable);
        }
      }
    }
  ...
  private class ContentHitsReadCountIncrementer implements Runnable {
    ...
    public void run() {
      ...
      final RetryingTransactionCallback<Integer> callback =
      ...
      try {
        ...
        final Integer newCount = txnHelper.doInTransaction(
          callback,
          false,
          true
        );
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Unable to increment content read count on missing node: " + nodeRef
          );
        }
        ...
        logger.error(
          "Failed to increment content read count on node: " + nodeRef
        );
  ...
  private class ContentHitsWriteCountIncrementer implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Unable to increment content write count on missing node: " +
            nodeRef
          );
        }
      } catch (final Throwable e) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "Failed to increment content write count on node: " + nodeRef
          );
        }
        // We are the last call on the thread
      } finally {

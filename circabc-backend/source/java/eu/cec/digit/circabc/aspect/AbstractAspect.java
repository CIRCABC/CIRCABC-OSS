...
public abstract class AbstractAspect {
  ...
  public abstract void onAddAspect(
  ...
  public void beforeDeleteNode(final NodeRef nodeRef) {
    ...
    if (profileManagerService != null) {
      ...
      final Map<String, Profile> profiles = profileManagerService.getProfileMap(
        nodeRef
      );
      ...
      for (final Profile profile : profiles.values()) {
        if (!profile.isImported()) {
          ...
          try {
            if (
              ...
              if (authorityService.authorityExists(profileGroupName)) {
                ...
                if (logger.isTraceEnabled()) {
                  logger.trace(
                    "Tried to delete a non existing Authority (allready deleted):" +
                    profileGroupName
                  );
                }
              }
          ...
          if (logger.isWarnEnabled()) {
            logger.warn(
              "Don't delete the Authority linked to a profile Imported from another node"
            );
          }
        }
      }
      ...
      String authorityName = authorityService.getName(
        AuthorityType.GROUP,
        profileManagerService.getMasterInvitedGroupName(nodeRef)
      );
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "There is no redefinition of herited permission defined on this node:" +
          nodeRef
        );
      }
    }

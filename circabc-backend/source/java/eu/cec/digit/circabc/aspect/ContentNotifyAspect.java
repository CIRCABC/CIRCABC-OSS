...
public class ContentNotifyAspect
  ...
  public void onAddAspect(final NodeRef nodeRef, final QName aspectTypeQName) {
    ...
    Set<NodeRef> createNodeRefs = (Set<
        NodeRef
      >) AlfrescoTransactionSupport.getResource(KEY_CONTENT_NOTIFY_CREATE);
  ...
  public void onContentUpdate(final NodeRef nodeRef, final boolean newContent) {
    ...
    Set<NodeRef> updateNodeRefs = (Set<
        NodeRef
      >) AlfrescoTransactionSupport.getResource(KEY_CONTENT_NOTIFY_UPDATE);
  ...
  private final class ContentNotifyer implements Runnable {
    ...
    public void run() {
      ...
      final RetryingTransactionCallback<Object> callback =
        new RetryingTransactionCallback<Object>() {
          public Object execute() throws Throwable {
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Trying to notify the creation/update of the node " + nodeRef
              );
            }
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Unable send notification content on missing node: " + nodeRef
          );
        }

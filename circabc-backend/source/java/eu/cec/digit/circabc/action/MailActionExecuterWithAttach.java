...
public class MailActionExecuterWithAttach extends ActionExecuterAbstractBase {
  ...
  protected void executeImpl(
    ...
    try {
      ...
      if (to != null && to.length() != 0) {
        ...
        final List<String> authorities = (List<
            String
          ...
          for (final String authority : authorities) {
            ...
            if (authType.equals(AuthorityType.USER)) {
              if (this.personService.personExists(authority)) {
                ...
                address = (String) this.nodeService.getProperty(
                    person,
              ...
              if (this.authorityService.authorityExists(authority)) {
                ...
                users = this.authorityService.getContainedAuthorities(
                    AuthorityType.USER,
                    authority,
                    false
                  );
      ...
      if (templateRef != null && !templateRef.toString().equals("none")) {
        ...
        final Map<String, Object> model = createEmailTemplateModel(
          actionedUponNodeRef
        );

        // process the template against the model
        text = templateService.processTemplate(
          "freemarker",
      ...
      final Boolean senddocument = (Boolean) ruleAction.getParameterValue(
        PARAM_SENDDOCUMENT
      );

      if (senddocument) {
        getMailService()
          .sendNode(actionedUponNodeRef, from, tos, null, subject, text, true);
      } else {
      ...
      if (logger.isErrorEnabled()) {
        ...
        logger.error(
          ...
          (String) ruleAction.getParameterValue(PARAM_TO),
          e
        );
      }
    }
  ...
  private Map<String, Object> createEmailTemplateModel(final NodeRef ref) {
    ...
    final NodeRef person = personService.getPerson(
    ...
    final NodeRef parent = serviceRegistry
      .getNodeService()
  ...
  protected void addParameterDefinitions(
    final List<ParameterDefinition> paramList
  ) {
    paramList.add(

...
public class AdvancedImporterActionExecuter extends ActionExecuterAbstractBase {
  ...
  private static final Log logger = LogFactory.getLog(
    AdvancedImporterActionExecuter.class
  );
    ...
    try {
      for (Enumeration e = archive.getEntries(); e.hasMoreElements();) {
        ...
        if (!entry.isDirectory()) {
          ...
          boolean esapiValidFileName = ESAPI.validator()
            .isValidFileName("import", esapiFileName, false);
          if (!esapiValidFileName) {
            throw new IOException(
              "Filename '" +
              destFileName +
              "' is invalid. Please check its name."
            );
          }
      ...
      if (reader != null) {
        ...
        if (MimetypeMap.MIMETYPE_ACP.equals(reader.getMimetype())) {
          ...
          try {
            ...
            String bindingString = (String) ruleAction.getParameterValue(
              PARAM_BINDING
            );
  ...
  protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
    ...
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        DataTypeDefinition.TEXT,
        false,
        getParamDisplayLabel(PARAM_BINDING)
      )
    );
  }
}

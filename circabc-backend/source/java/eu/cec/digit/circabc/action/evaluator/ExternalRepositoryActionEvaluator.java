...
public class ExternalRepositoryActionEvaluator extends BaseActionEvaluator {
  ...
  private static final Log logger = LogFactory.getLog(
    ExternalRepositoryActionEvaluator.class
  );
  ...
  public boolean evaluate(Node node) {
    ...
    if (
      nodeService.hasAspect(documentNodeRef, ContentModel.ASPECT_WORKING_COPY)
    ) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Aspect WORKING_COPY present. Current Username: " +
          userName +
          ", IG NodeRef: " +
          igNodeRef +
          ", " +
    ...
    if (
      !externalRepositoriesManagementService.isUserAuthorizedToPublish(userName)
    ) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "User not authorized to publish/register. Current Username: " +
          userName +
          ", IG NodeRef: " +
          igNodeRef +
          ", " +
    ...
    if (
      externalRepositoriesManagementService.wasPublishedTo(
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Document already published. Current Username: " +
          userName +
          ", IG NodeRef: " +
          igNodeRef +
          ", " +
    ...
    if (igNodeRef != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Current Username: " + userName + ", IG NodeRef: " + igNodeRef
        );
      }

      NodeRef externalRepositoriesNodeRef = nodeService.getChildByName(
        igNodeRef,
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "External repositories configuration NodeRef: " +
          externalRepositoriesNodeRef
        );
      }

      boolean result =
        externalRepositoriesNodeRef != null &&
        nodeService.getChildAssocs(externalRepositoriesNodeRef).size() > 0;

      if (logger.isDebugEnabled()) {
        logger.debug(
          "Show external repositories register link on right menu: " + result
        );
      }

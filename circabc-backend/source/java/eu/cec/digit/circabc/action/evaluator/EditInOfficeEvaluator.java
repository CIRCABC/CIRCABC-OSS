...
public class EditInOfficeEvaluator extends BaseActionEvaluator {
  ...
  public boolean evaluate(Node node) {
    ...
    DictionaryService dictionaryService = Services.getAlfrescoServiceRegistry(
      fc
    ).getDictionaryService();
    NodeService nodeService = Services.getAlfrescoServiceRegistry(
      fc
    ).getNodeService();
    ContentService contentService = Services.getAlfrescoServiceRegistry(
      fc
    ).getContentService();
    PermissionService permissionService = Services.getAlfrescoServiceRegistry(
      fc
      ...
      if (propertyDefinitions.containsKey(contentQName)) {
        ...
        if (reader != null && officeMimetypes.contains(reader.getMimetype())) {
          ...
          return (
            ...
            !node.isLocked() &&
            checkOutCheckInService.getWorkingCopy(node.getNodeRef()) == null &&

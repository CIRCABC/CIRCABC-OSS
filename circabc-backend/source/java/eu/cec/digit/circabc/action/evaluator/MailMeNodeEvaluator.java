...
public class MailMeNodeEvaluator extends BaseActionEvaluator {
  ...
  public boolean evaluate(final Node node) {
    ...
    if (
      node.hasAspect(
        org.alfresco.model.ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION
      )
    ) {
      return false;
    }
    ...
    if (
      node.hasAspect(
        org.alfresco.model.ContentModel.ASPECT_MULTILINGUAL_DOCUMENT
      )
    ) {
      ...
      NodeRef containerNodeRef =
        multilingualContentService.getTranslationContainer(node.getNodeRef());
      containerNode = new Node(containerNodeRef);
    } else {
      containerNode = node;
    }
    Object securityRanking = containerNode
      .getProperties()

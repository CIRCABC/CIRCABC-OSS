...
public class CircabcReplicationActionExecutor
  extends ReplicationActionExecutor {

  /**
   * The logger
   */
  private static Log logger = LogFactory.getLog(
    CircabcReplicationActionExecutor.class
  );
  ...
  protected Set<NodeRef> expandPayload(ReplicationDefinition replicationDef) {
    ...
    final Collection<QName> allAssociations =
      dictionaryService.getAllAssociations();
    crawler.setNodeFinders(
      new ChildAssociatedNodeFinder(new HashSet<>(allAssociations))
    );
    final Collection<QName> allTypes = dictionaryService.getAllTypes();
    crawler.setNodeFilters(
      new ContentClassFilter(allTypes.toArray(new QName[allTypes.size()]))
    );
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    ...
    if (
      ...
      throw new ReplicationServiceException(
        "The target is required but wasn't given"
      );
    }
    ...
    if (!replicationDef.isEnabled()) {
      throw new ReplicationServiceException(
        "Unable to execute a disabled replication definition"
      );
    }
    ...
    try {
      ...
      lock.close();
      throw new ReplicationServiceException(
        "Error processing payload list - " + e.getMessage(),
        e
      );
    }
    ...
    try {
      ...
      if (e instanceof TransferFailureException) {
        ...
        throw new ReplicationServiceException(
          "Error executing transfer - " + cause.getMessage(),
          cause
        );
      }

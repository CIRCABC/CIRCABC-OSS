...
public class CircabcImporterActionExecuter extends ImporterActionExecuter {
  ...
  public void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef) {
    ...
    try {
      Boolean deleteFile = (Boolean) ruleAction.getParameterValue(
        PARAM_DELETE_FILE
      );
      ...
      if (reader != null) {
        if (MimetypeMap.MIMETYPE_ZIP.equals(reader.getMimetype())) {
          ...
          try {
            ...
            List<IndexRecord> indexRecords = getBulkService()
              .getIndexRecords(tempFile, messages);
            ...
            if (tempFile != null) {
              ...
              if (!isDeleted && logger.isWarnEnabled()) {
                logger.warn(
                  "Unable to delete file : " + tempFile.getCanonicalPath()
                );
              }
            }
          }
  ...
  private void setLogRecord(
    ...
    final NodeRef igNodeRef = getManagementService()
      .getCurrentInterestGroup(importDest);
    logRecord.setIgID(
    ...
    logRecord.addInfo(
      " file : " +
      (String) myNodeService.getProperty(
        actionedUponNodeRef,
  ...
  private void notifyUser(String userToNotify) {
    ...
    String to = (String) this.myNodeService.getProperty(
        person,
      ...
      if (reader != null) {
        ...
        if (MimetypeMap.MIMETYPE_ACP.equals(reader.getMimetype())) {
          ...
          try {
            ...
            this.myImporterService.importView(
                importHandler,
            ...
            if (zipFile != null) {
              ...
              if (!isDeleted && logger.isWarnEnabled()) {
                try {
                  logger.warn(
                    "Unable to delete file : " + zipFile.getCanonicalPath()
                  );
                } catch (IOException e) {
                  logger.warn(
                    "Unable to get getCanonicalPath for : " + zipFile.getPath(),
                    e
                  );
                }
              }
    ...
    for (File file : topdir.listFiles()) {
      try {
        ...
        if (file.isFile()) {
          ...
          //Special case for OLAF:
          if (CircabcConfig.OLAF) {
            //Expiration date is mandatory and should be set with today's date + 1 month at 23:59:59.999
            Date expirationDate = DateUtils.inOneMonth23h59M59S();
            myNodeService.setProperty(
              targetRef,
              DocumentModel.PROP_EXPIRATION_DATE,
              expirationDate
            );
            //All documents imported should be marked as SNC by default
            myNodeService.setProperty(
              targetRef,

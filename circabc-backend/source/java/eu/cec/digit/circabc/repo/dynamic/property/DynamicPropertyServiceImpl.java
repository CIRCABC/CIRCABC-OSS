...
public class DynamicPropertyServiceImpl implements DynamicPropertyService {
  ...
  public DynamicProperty addDynamicProperty(
    ...
    if (
      ...
      properties.put(
        PROP_DYNAMIC_PROPERTY_VALID_VALUES,
  ...
  public List<DynamicProperty> getDynamicProperties(NodeRef dp) {
    ...
    if (!nodeService.hasAspect(dp, CircabcModel.ASPECT_IGROOT)) {
      ...
      if (igFromDynProp == null) {
        logger.warn(
          "The model seems corrupted, no IG found for the dynamic property " +
          dp
        );
    ...
    try {
      ...
      for (final ChildAssociationRef ref : dynamicPropertiesAssoc) {
        ...
        if (nodeService.getType(noderef).equals(TYPE_DYNAMIC_PROPERTY)) {
          final MLText label = (MLText) nodeService.getProperty(
            noderef,
            PROP_DYNAMIC_PROPERTY_LABEL
          );
          final Long index = (Long) nodeService.getProperty(
            noderef,
            PROP_DYNAMIC_PROPERTY_INDEX
          );
          final String typeAsString = (String) nodeService.getProperty(
            noderef,
            PROP_DYNAMIC_PROPERTY_TYPE
          );
          final DynamicPropertyType type = DynamicPropertyType.valueOf(
            typeAsString
          );
          ...
          dynamicProperties.add(
            new DynamicPropertyImpl(index, noderef, label, type, validValues)
          );
        } else {
  ...
  public void deleteDynamicProperty(DynamicProperty dp) {
    if (
      dp == null ||
      !TYPE_DYNAMIC_PROPERTY.isMatch(nodeService.getType(dp.getId()))
    ) {
      throw new IllegalArgumentException(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Dynamic property successfully dropped from the ig: " +
        "\n  Property: " +
        dp
      );
    }
  ...
  private List<NodeRef> getNodesForProperty(
    ...
    query
      .append(" AND  +@")
      .append(QueryParser.escape(propName.toString()))
      .append(":");
    if (like) {
      query.append("*");
    }

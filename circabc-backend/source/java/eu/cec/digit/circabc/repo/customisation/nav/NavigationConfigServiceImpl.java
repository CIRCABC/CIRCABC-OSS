...
public class NavigationConfigServiceImpl implements NavigationConfigService {
  ...
  private static final String GLOBAL_CONFIG_CONDITION =
    "Global Navigation Preference";
  private static final String CUSTOM_CONFIG_CONDITION =
    "Custom Navigation Preference";
  ...
  private static final String ELEMENT_INITIAL_SORT_DESC =
    "initialSortDescending";
  private static final String ELEMENT_INITIAL_SORT_POSTS_DESC =
    "initialSortPostsDescending";
  ...
  private static final String ELEMENT_RENDER_PROPERTY_NAME =
    ...
    if (!init) {
      try {
        ...
        final Config configConditions = configService.getConfig(
          GLOBAL_CONFIG_CONDITION,
          clContext
        );

        final Map<String, ConfigElement> configElements =
          configConditions.getConfigElements();

        if (
          configElements.containsKey(ELEMENT_COLUMNS) == false ||
        ...
        for (final ConfigElement columnElement : configElements
          .get(ELEMENT_COLUMNS)
          .getChildren()) {
        ...
        for (final ConfigElement serviceElement : configElements
          .get(ELEMENT_SERVICES)
          .getChildren()) {
          ...
          final Pair<Integer, Integer> displayRow = getMinMax(
            serviceElement.getChild(ELEMENT_DISPLAY_ROW)
          );
          final Pair<Integer, Integer> displayCol = getMinMax(
            serviceElement.getChild(ELEMENT_DISPLAY_COL)
          );
          final Pair<Integer, Integer> displayAct = getMinMax(
            serviceElement.getChild(ELEMENT_DISPLAY_ACTION)
          );
          final String mandatoryAction = serviceElement.getChildValue(
            ELEMENT_MANDATORY_ACTION
          );
          ...
          service.setActionConfigName(
            serviceElement.getChildValue(ELEMENT_ACTION_CONFIG)
          );
          ...
          final ConfigElement columnsElement = serviceElement.getChild(
            ELEMENT_COLUMNS
          );
          if (columnsElement != null) {
            for (final ConfigElement columnElement : columnsElement.getChildren()) {
              ...
              if (columns.containsKey(colName)) {
                ...
                if (
                  id != null && id.equalsIgnoreCase(Boolean.TRUE.toString())
                ) {
        ...
        throw new ConfigException(
          "Impossible to apply default navigation customization settings: " +
          e.getMessage(),
          e
        );
      }
  ...
  public NavigationPreference buildPreference(
    ...
    final RepoRefConfigSource repoRefConfigSource = new RepoRefConfigSource(
      ref,
      contentService,
      nodeService
    );
    final XMLConfigService refConfigService = new XMLConfigService(
      repoRefConfigSource
    );

    refConfigService.initConfig();
    final Config configConditions = refConfigService.getConfig(
      CUSTOM_CONFIG_CONDITION,
    ...
    if (configElements.containsKey(ELEMENT_PREFERENCE) == false) {
      throw new ConfigException(
        "The element " + ELEMENT_PREFERENCE + " is mandatory"
      );
    }
    ...
    navigationOption.setDisplayActionColumnStr(
      preference.getChildValue(ELEMENT_DISPLAY_ACTIONS)
    );
    navigationOption.setLinkTarget(
      preference.getChildValue(ELEMENT_LINK_DESTINATION)
    );
    ...
    navigationOption.setPreviewAction(
      preference.getChildValue(ELEMENT_PREVIEWACTION)
    );
    navigationOption.setInitialSortColumnStr(
      preference.getChildValue(ELEMENT_INITIAL_SORT_COLUMN)
    );
    ...
    navigationOption.setRenderPropertyName(
      preference.getChildValue(ELEMENT_RENDER_PROPERTY_NAME)
    );
   ...
   */
  public String buildPreferenceXML(
  ...
  private List<String> extractActions(final ConfigElement parentElement) {
    final String actionConfigName = parentElement.getChildValue(
      ELEMENT_ACTION_CONFIG
    );
    final ConfigElement actionsElement = parentElement.getChild(
      ELEMENT_ACTIONS
    );
    ...
    if (actionConfigName != null && actionConfigName.length() > 0) {
      ...
      if (actionConfig != null) {
        ...
        final ActionGroup actionGroup = actionConfig.getActionGroup(
          actionConfigName
        );
    ...
    if (actionsElement != null) {
      for (final ConfigElement actionElement : actionsElement.getChildren(
        ELEMENT_ACTION
      )) {
  ...
  private List<ColumnConfig> extractColumns(final ConfigElement parentElement) {
    final ConfigElement columnElements = parentElement.getChild(
      ELEMENT_COLUMNS
    );
    final List<ColumnConfig> columnConfigs = new ArrayList<>();

    if (columnElements != null) {
      for (final ConfigElement columnElement : columnElements.getChildren(
        ELEMENT_COLUMN
      )) {
        ...
        if (value != null && value.length() > 0) {
          ...
          if (
            ...
            columnConfigs.contains(columnConfig) == false
          ) {
            columnConfigs.add(columnConfig);
          } else {
            throw new ConfigException(
              "The column " + value + " not found in the global config."
            );
          }
        }
      }
   ...
   */
  public final void setWebClientConfig(
    final RepoXMLConfigService webClientConfig
  ) {

...
public class DBLogServiceImpl implements LogService {
  ...
  public List<LogSearchResultDAO> searchPage(
    ...
    try {
      logSearchResult = logDaoService.searchPage(
        igID,
        user,
        service,
        activity,
  ...
  public void logBatch(List<LogRecord> logRecords) {
    ...
    for (LogRecord logRecord : logRecords) {
      ...
      if ((activity == null) || (service == null)) {
        if (logger.isErrorEnabled()) {
          logger.error(
            "Invalid activity " + activity + " or service " + service
          );
        }
        continue;
      }
  ...
  public void deleteInterestgroupLog(long igID) {
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Error performing deletion of interest group :" +
          String.valueOf(igID),
          e
        );
      }
  ...
  public Date getLastUpdateDateOnInterestGroupForStatistics(long igID) {
    try {
      return logDaoService.getLastUpdateDateOnInterestGroupForStatistics(igID);
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
        logger.error(
          "Error during getting the last update log for group id:" + igID,

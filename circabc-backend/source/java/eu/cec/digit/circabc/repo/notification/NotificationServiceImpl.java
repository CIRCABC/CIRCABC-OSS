...
public class NotificationServiceImpl implements NotificationService {
  ...
  public void notify(final NodeRef nodeRef, final Set<NotifiableUser> users)
    ...
    if (getNodeService().exists(nodeRef)) {
      if (
        getNodeService()
          .hasAspect(nodeRef, RenditionModel.ASPECT_HIDDEN_RENDITION)
      ) {
  ...
  public void notifyNewEdition(
    ...
    if (getNodeService().exists(nodeRef)) {
      if (
        getNodeService()
          .hasAspect(nodeRef, ContentModel.ASPECT_MULTILINGUAL_DOCUMENT)
      ) {
  ...
  protected String getBestTitle(final NodeRef nodeRef) {
    ...
    final Serializable titleObj = getNodeService()
  ...
  private void notifyImplForNewEdition(
    ...
    for (final NotifiableUser user : users) {
      ...
      model = getMailPreferencesService()
        .buildDefaultModel(nodeRef, user.getPerson(), null);
      mail = getMailPreferencesService()
  ...
  private void notifyImplForContent(
    ...
    for (final NotifiableUser user : users) {
      ...
      model = getMailPreferencesService()
        .buildDefaultModel(nodeRef, user.getPerson(), null);
      mail = getMailPreferencesService()
        .getDefaultMailTemplate(nodeRef, MailTemplate.NOTIFY_DOC);
  ...
  private void notifyImplForNews(
    ...
    for (final NotifiableUser user : users) {
      ...
      model = getMailPreferencesService()
        .buildDefaultModel(nodeRef, user.getPerson(), null);
      mail = getMailPreferencesService()
        .getDefaultMailTemplate(nodeRef, MailTemplate.NOTIFY_NEWS);
  ...
  private void notifyImplForPost(
    ...
    for (final NotifiableUser user : users) {
      ...
      model = getMailPreferencesService()
        .buildDefaultModel(nodeRef, user.getPerson(), null);
      mail = getMailPreferencesService()
        .getDefaultMailTemplate(nodeRef, MailTemplate.NOTIFY_POST);
  ...
  private String getCurrentIgTitle(NodeRef nodeRef) {
    ...
    Serializable title = getNodeService()
      .getProperty(igRef, ContentModel.PROP_TITLE);
    String result = getNodeService()
      .getProperty(igRef, ContentModel.PROP_NAME)
      .toString();
    if (title instanceof MLText) {
      String defaultTitle = ((MLText) title).getDefaultValue();
      String mailTitle = ((MLText) title).get(DEFAULT_MAIL_LOCALE);
  ...
  private String getSafeValue(Locale locale, final Object value) {
    if (value == null) {
      ...
      return getSafeProperty(
        ...
        getNodeService().getProperties((NodeRef) value)
      );
    } else {
  ...
  private void notifyImplForContent(
    ...
    for (final String email : mails) {
      model = getMailPreferencesService()
        .buildDefaultModel(nodeRef, null, null);
      mail = getMailPreferencesService()
        .getDefaultMailTemplate(nodeRef, templateType);
  ...
  private void notifyImplInviteUser(
    ...
    Map<String, Object> model = getMailPreferencesService()
      .buildDefaultModel(nodeRef, nUser.getPerson(), null);
    MailWrapper mail = getMailPreferencesService()
      .getDefaultMailTemplate(nodeRef, MailTemplate.INVITE_USER);

    String profile = igRootProfileManagerService.getPersonProfile(
      nodeRef,
      nUser.getUserName()
    );
  ...
  public void notify(
    ...
    MailWrapper mail = getMailPreferencesService()
      .getDefaultMailTemplate(currentNodeRef, mailTemplate);
    ...
    Map<String, Object> model = getMailPreferencesService()
      .buildDefaultModel(currentNodeRef, userRef, null);
  ...
  private List<TemplatableNode> convertToTemplateNodes(
    ...
    for (NodeRef nodeRef : nodeRefs) {
      ...
      Object descObj = nodeService.getProperty(
        nodeRef,
  ...
  public void notify(
    ...
    MailWrapper mail = getMailPreferencesService()
      .getDefaultMailTemplate(nodeRef, notifyDocBulk);

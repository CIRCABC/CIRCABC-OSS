...
public class GroupsBulkKeywordsPost extends CircabcDeclarativeWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    try {
      if (
        !this.currentUserPermissionCheckerService.isInterestGroupLibAdmin(id)
      ) {
  ...
  private void importKeywords(
    ...
    List<KeywordDefinition> igKeywords =
      this.keywordsApi.groupsIdKeywordsGet(id);
      ...
      if (keywordToUpdate != null) {
        for (String key : keywordEntry.getTitle().keySet()) {
          if (!keywordToUpdate.getTitle().containsKey(key)) {
            keywordToUpdate
              .getTitle()
              .put(key, keywordEntry.getTitle().get(key));
            this.keywordsApi.keywordsKeywordIdPut(
                keywordToUpdate.getId(),
                keywordToUpdate
              );
          }
        }

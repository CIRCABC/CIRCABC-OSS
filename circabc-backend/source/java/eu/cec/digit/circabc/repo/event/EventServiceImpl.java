...
public class EventServiceImpl implements EventService {
  ...
  private static final String PROP_EVENT_DATE_OF_EVENT_ESCAPED =
    EventModel.PROP_EVENT_DATE.toString()
      .replaceAll(":", ESCAPE4)
      .replaceAll("\\{", "\\\\{")
      .replaceAll("\\}", "\\\\}");
  ...
  private List<String> getUsersEmails(NodeRef eventRoot) {
    ...
    users.addAll(
      getUserService()
        .getUsersWithPermission(eventRoot, EventPermissions.EVEADMIN.toString())
    );
  ...
  private void sendEventNotificationMessage(
    ...
    MailWrapper bodyReminder = getMailPreferencesService()
      .getDefaultMailTemplate(eventRoot, mailTemplate);
  ...
  private Map<String, Object> buildDefaultTemplateModel(
    ...
    final Map<String, Object> model = getMailPreferencesService()
      .buildDefaultModel(eventService, null, null);
  ...
  public final void deleteAppointment(
    ...
    if (mode == UpdateMode.Single) {
      eventDate = (Date) getNodeService()
        .getProperty(appointmentNodeRef, EventModel.PROP_EVENT_DATE);
    }
    if (mode == UpdateMode.FuturOccurences) {
      ...
      for (NodeRef ref : futureEvents) {
        if (isFirst) {
          eventDate = (Date) getNodeService()
            .getProperty(ref, EventModel.PROP_EVENT_DATE);
          break;
        }
      }
    }
  ...
  public final List<EventItem> getAllOccurences(
    ...
    final NodeRef interestGroup = managementService.getCurrentInterestGroup(
      appointmentNodeRef
    );
  ...
  private List<NodeRef> getAllAppointmens(NodeRef appointment) {
    ...
    NodeRef appointmentDateContainer = getNodeService()
      .getPrimaryParent(appointment)
      .getParentRef();
    String luceneQuery = null;

    boolean directStoreAccess = Boolean.parseBoolean(
      CircabcConfiguration.getProperty("events.direct.store.access")
    );
  ...
  private NodeRef getEventsDatesContainer(NodeRef nodeRef) {
    ...
    if (
      !(TYPE_EVENT_DEFINITION.isMatch(type) ||
        TYPE_EVENT_MEETING_DEFINITION.isMatch(type))
    ) {
      throw new IllegalArgumentException(EVENT_NODE_REF_WRONG_TYPE_OR_NULL);
    }
    ...
    try {
      ...
      for (final ResultSetRow row : resultSet) {
        ...
        if (nodeService.exists(nodeRef)) {
          ...
          MeetingRequestStatus meetingStatus = getMeetingStatus(
            nodeRef,
            userName
          );
          String status = "";
    ...
    for (ChildAssociationRef childRef : children) {
      ...
      if (nodeService.getType(childNodeRef).equals(EventModel.TYPE_EVENT)) {
        ...
        if (
          ...
          boolean shouldAddTheAppointment = AuthenticationUtil.runAs(
              ...
              public Boolean doWork() {
                ...
                if (
                  ...
                  if ("Closed".equals(audience)) {
                    ...
                    if (
                      invitedUsers.contains(finalUserName) ||
  ...
  public final void deleteAllEvents(NodeRef eventRoot) {
    ...
    for (ChildAssociationRef ref : cars) {
      if (
        isEventDefinition(ref.getChildRef()) ||
        isMeetingDefinition(ref.getChildRef())
      ) {
  ...
  private String getInvitedUserWithSameEmail(
    ...
    if (!userName.contains("@")) {
      ...
      final String email = (String) getNodeService()
      ...
      for (String user : users) {
        ...
        final String userEmail = (String) getNodeService()
          .getProperty(userNodeRef, ContentModel.PROP_EMAIL);
  ...
  public final List<EventItem> getEventsBetweenDates(
    ...
    Date beginDate = setStartTime(
      AppointmentUtils.convertDateValueToDate(dateFrom)
    );
    ...
    switch (mode) {
      ...
      case Single:
        ...
        if (
          ...
          getNodeService()
            .setProperty(appointmentNodeRef, PROP_EVENT_DATE, javaDate);
        }
        break;
      default:
        break;
  ...
  private Integer incrementSequence(NodeRef appointmentNodeRef) {
    ...
    Serializable property = getNodeService()
      .getProperty(appointmentDefinition, PROP_SEQUENCE);
    Integer sequence = 0;
    if (property != null) {
    ...
    sequence = sequence + 1;
    getNodeService()
      .setProperty(appointmentDefinition, PROP_SEQUENCE, sequence);
  ...
  private void updateAppointment(
    ...
    Map<QName, Serializable> oldProperties = getNodeService()
      .getProperties(appointmentNodeRef);
  ...
  private void updateAllAppointments(
    ...
    PropertyMap newProperties = appointment.getProperties(updateInfo);
    Map<QName, Serializable> oldProperties = getNodeService()
      .getProperties(appointmentDefinition);
    oldProperties.putAll(newProperties);
  ...
  private List<EventItem> getExactEventsOnDate(
    ...
    Date dateFromRaw = AppointmentUtils.convertDateValueToDate(date);
    Date dateFromMin = setStartTime(dateFromRaw);
    Date dateFromMax = setEndTime(dateFromRaw);
    return getEvents(userName, eventRoot, dateFromMin, dateFromMax);
  }
  ...
  public final List<Appointment> getAllAppointments(NodeRef eventRoot) {
    ...
    for (ChildAssociationRef ref : childAssocs) {
      ...
      if (nodeService.hasAspect(nodeRef, CircabcModel.ASPECT_EVENT)) {
        ...
        if (
          nodeService.getType(nodeRef).equals(EventModel.TYPE_EVENT_DEFINITION)
        ) {
          item = new EventImpl();
        }
        if (
          nodeService
            .getType(nodeRef)
            .equals(EventModel.TYPE_EVENT_MEETING_DEFINITION)
        ) {
  ...
  public NodeRef getIGsEventRoot(String id) {
    ...
    if (eventRoot == null) {
      throw new RuntimeException(
        "The Event root for IG with id '" + id + "' does not exist."
      );
    }
    return eventRoot;
  }

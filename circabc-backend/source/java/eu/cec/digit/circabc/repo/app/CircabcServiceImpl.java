...
public class CircabcServiceImpl implements CircabcService {
  ...
  public static final String ERROR_GET_ALL_ALFRESCO_LOCALE =
  ...
  private void loadInterestGroups(NodeRef category) {
    ...
    final long categoryID = (Long) getNodeService()
      .getProperty(category, ContentModel.PROP_NODE_DBID);
    for (NodeRef interestGroup : interestGroups) {
      final long interestGroupID = insertInterestGroup(
        categoryID,
        interestGroup
      );
      final List<Profile> profiles = igRootProfileManagerService.getProfiles(
        interestGroup
      );
  ...
  private void insertProfile(
    ...
    eu.cec.digit.circabc.repo.app.model.Profile prof = loadProfileInfo(
      interestGroupID,
      profile
    );
    if (prof != null) {
      ...
      final Set<String> personInProfile =
        igRootProfileManagerService.getPersonInProfile(
          interestGroup,
      ...
      if (!prof.isImported()) {
        for (String userName : personInProfile) {
          if (
            !profile.getAlfrescoGroupName().equalsIgnoreCase("ALL_CIRCA_USERS")
          ) {
  ...
  private eu.cec.digit.circabc.repo.app.model.Profile loadProfileInfo(
    ...
    try {
      ...
      final HashMap<String, Set<String>> servicesPermissions =
        profile.getServicesPermissions();

      String newsgroupPermission = servicesPermissions
        .get("NEWSGROUP")
        .iterator()
        .next();
      String eventPermission = servicesPermissions
        .get("EVENT")
        .iterator()
        .next();
      String informationPermission = servicesPermissions
        .get("INFORMATION")
        .iterator()
        .next();
      String libraryPermission = servicesPermissions
        .get("LIBRARY")
        .iterator()
        .next();
      String directoryPermission = servicesPermissions
        .get("DIRECTORY")
        .iterator()
        .next();
      ...
      if (!visibilityPerms.isEmpty()) {
        String visibilityPermission = servicesPermissions
          .get(VISIBILITY_UPPER)
          .iterator()
          .next();
      ...
      prof = new eu.cec.digit.circabc.repo.app.model.Profile(
        interestGroupID,
        id,
        alfrescoGroup,
        name,
  ...
  private long insertInterestGroup(long categoryID, NodeRef interestGroup) {
    ...
    try {
      isRegistered = managementService.hasAllCircabcUsersVisibility(
        interestGroup
      );
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
  ...
  private Set<NodeRef> loadCategoriesAndHeaders() {
    ...
    final Map<NodeRef, List<NodeRef>> categoryMap =
    ...
    for (Entry<NodeRef, List<NodeRef>> item : categoryMap.entrySet()) {
      ...
      if (!value.isEmpty()) {
        result.add(key);
        final Map<QName, Serializable> headerProperties =
          nodeService.getProperties(value.get(0));
        final long headerID = (Long) headerProperties.get(
          ContentModel.PROP_NODE_DBID
        );
  ...
  public void addCategoryAdmin(NodeRef categoryNodeRef, String userName) {
    ...
    long categoryID = (Long) getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_NODE_DBID);

    Long id = getUserIDByUserName(userName);
    if (id == 0L) {
      Map<String, Long> allAlfrescoLocale =
        circabcDaoService.getAllAlfrescoLocale();
  ...
  public void addPersonToProfile(
    ...
    long interestGroupID = (Long) getNodeService()
  ...
  public void uninvitePerson(NodeRef igNodeRef, String userName) {
    ...
    if (result != 1 && logger.isErrorEnabled()) {
      logger.error(
        "Error uninvitePerson expected one row to delete but deleted " +
        result +
        " rows"
      );
    }
  ...
  public void importProfile(
    ...
    long fromInterestGroupID = (Long) getNodeService()
      .getProperty(fromIgNoderef, ContentModel.PROP_NODE_DBID);

    final eu.cec.digit.circabc.repo.app.model.Profile sourceProfile =
      getProfileByInterestGroupIDAndProfileName(
        fromInterestGroupID,
        fromProfileName
      );

    long toInterestGroupID = (Long) getNodeService()
      .getProperty(toIgNoderef, ContentModel.PROP_NODE_DBID);
    final List<Profile> profiles = igRootProfileManagerService.getProfiles(
      toIgNoderef
    );
  ...
  public void addCategoryNode(NodeRef headerNodeRef, NodeRef categoryNodeRef) {
    ...
    long id = (Long) getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_NODE_DBID);
    String name = (String) getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_NAME);
    String title = null;
    Serializable titleObj = getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_TITLE);
    ...
    long headerID = (Long) getNodeService()
      .getProperty(headerNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public void addInterestGroupNode(NodeRef catNodeRef, NodeRef igRootNodeRef) {
    ...
    long categoryID = (Long) getNodeService()
      .getProperty(catNodeRef, ContentModel.PROP_NODE_DBID);
    long interestGroupID = (Long) getNodeService()
      .getProperty(igRootNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public void updateIntestGroupProperties(NodeRef igNodeRef) {
    ...
    Object titleObj = getNodeService()
      .getProperty(igNodeRef, ContentModel.PROP_TITLE);
    String title = "";
    if (titleObj instanceof String) {
      title = (String) getNodeService()
        .getProperty(igNodeRef, ContentModel.PROP_TITLE);
    } else if (titleObj instanceof MLText) {
      title = ((MLText) getNodeService()
          .getProperty(igNodeRef, ContentModel.PROP_TITLE)).getDefaultValue();
    }
  ...
  public void updateCategoryProperties(NodeRef catNodeRef) {
    ...
    long categoryID = (Long) getNodeService()
      .getProperty(catNodeRef, ContentModel.PROP_NODE_DBID);
    String name = (String) getNodeService()
      .getProperty(catNodeRef, ContentModel.PROP_NAME);
    String title = null;
    Serializable titleObj = getNodeService()
      .getProperty(catNodeRef, ContentModel.PROP_TITLE);
    if (titleObj instanceof String) {
      title = (String) titleObj;
    } else if (titleObj instanceof MLText) {
      title = ((MLText) titleObj).getDefaultValue();
    ...
    try {
      ...
      if (titleObj instanceof String) {
        ...
        circabcDaoService.updateCategoryTitles(
          categoryID,
          Converter.toI18NProperty((MLText) titleObj)
        );
      }
    } catch (Exception e) {
      if (logger.isErrorEnabled()) {
  ...
  public void deleteCategory(NodeRef catNodeRef) {
    ...
    long categoryID = (Long) circabcDaoService.selectCategoryIDByNodeRef(
      catNodeRef.toString()
    );
    try {
  ...
  @Override
  public void moveIG(
    NodeRef oldCategoryRef,
    NodeRef newCategoryRef,
    NodeRef igRef
  ) {
    if (!isSyncEnabled) {
      return;
    }
    long oldCategoryID = (Long) getNodeService()
      .getProperty(oldCategoryRef, ContentModel.PROP_NODE_DBID);
    long newCategoryID = (Long) getNodeService()
      .getProperty(newCategoryRef, ContentModel.PROP_NODE_DBID);
    long interestGroupID = (Long) getNodeService()
      .getProperty(igRef, ContentModel.PROP_NODE_DBID);

    try {
      circabcDaoService.moveInterestGroup(
        interestGroupID,
        oldCategoryID,
        newCategoryID
      );
  ...
  public List<InterestGroupItem> getInterestGroupByCategoryUser(
    ...
    long categoryID = (Long) getNodeService()
      .getProperty(categoryRef, ContentModel.PROP_NODE_DBID);
    try {
  ...
  @Override
  public List<ExportedProfileItem> getExportedProfiles(
    NodeRef categoryRef,
    NodeRef igRef
  ) {
    if (!isReadFromDatabase) {
    ...
    try {
      result = circabcDaoService.selectExpProfilesByCategoryIDInterestGroupID(
        categoryID,
        interestGroupID
      );
      ...
      for (
        Iterator<ExportedProfileItem> iterator = result.iterator();
        iterator.hasNext();
      ) {
        ExportedProfileItem exportedProfileItem = iterator.next();
        if (
          selectImporetedAlfGroups.contains(
            exportedProfileItem.getPrefixedAlfrescoGroup()
          )
        ) {
          iterator.remove();
        }
  ...
  @Override
  public Set<String> getCategoryAlfrescoGroupsExceptCurrentIG(
    NodeRef category,
    NodeRef igNodeRef
  ) {
    if (!isReadFromDatabase) {
    ...
    try {
      result = circabcDaoService.selectAlfrescoGroupNotInIneterstGroup(
        categoryID,
        interestGroupID
      );
    } catch (Exception e) {
  ...
  public List<UserIGMembershipRecord> getInterestGroups(String userName) {
    ...
    try {
      ...
      for (UserIGMembership userIGMembership : list) {
        ...
        final String category = capitalize(
        ...
        Serializable propTitle = nodeService.getProperty(
          categoNodeRef,
        ...
        final String interesGroup = capitalize(
          (String) nodeService.getProperty(
            igRootNodeRef,
            ContentModel.PROP_NAME
          )
        ...
        Serializable propIgTitle = nodeService.getProperty(
          igRootNodeRef,
          ContentModel.PROP_TITLE
        );
        ...
        if (isReadFromDatabase) {
          final List<TranslationEntry> profileTitles =
            circabcDaoService.selectProfileTitles(
              userIGMembership.getProfileId()
            );
          if (profileTitles.size() == 1) {
            ...
            for (TranslationEntry translation : profileTitles) {
              trans.put(
                translation.getAlfLocaleId(),
                translation.getTranslation()
              );
            }
            if (
              allLocales.containsKey(EN_US_UNDERSCORE) &&
              trans.containsKey(allLocales.get(EN_US_UNDERSCORE))
            ) {
              profileDisplayName = trans.get(allLocales.get(EN_US_UNDERSCORE));
            } else if (
              allLocales.containsKey("en_") &&
              trans.containsKey(allLocales.get("en_"))
            ) {
              profileDisplayName = trans.get(allLocales.get("en_"));
            } else if (
              allLocales.containsKey(EN_GB_UNDERSCORE) &&
          ...
          final Profile profile = getIgRootProfileManagerService()
            .getProfile(igRootNodeRef, profileName);
  ...
  public List<UserCategoryMembershipRecord> getCategories(String userName) {
    ...
    try {
      ...
      for (String item : list) {
        ...
        final String profile = getCategoryProfileManagerService()
          .getPersonProfile(categoNodeRef, userName);
        if (
          (profile != null) &&
          !profile.equals(
            CircabcRootProfileManagerService.ALL_CIRCA_USERS_PROFILE_NAME
          )
        ) {
          result.add(
            new UserCategoryMembershipRecord(
              category,
              profile,
              categoNodeRef.getId()
            )
  ...
  public void resyncInterestGroup(NodeRef nodeRef) {
    if (getNodeService().hasAspect(nodeRef, CircabcModel.ASPECT_IGROOT)) {
      ...
      final NodeRef categoryNodeRef = getNodeService()
      ...
      long interestGroupNodeDBID = (Long) getNodeService()
      ...
      try {
        ...
        if (logoDefinition != null && logoDefinition.getLogo() != null) {
          ...
          if (logoRef != null && nodeService.exists(logoRef)) logoId =
            logoRef.toString();
        }
        circabcDaoService.updateLogoIdForGroup(interestGroupNodeDBID, logoId);
      } catch (CustomizationException e) {
        logger.error(
          "Impossible to update the logo ref in the group resync",
          e
        );
      }
  ...
  public void resyncUsers() {
    ...
    if (allAlfrescoLocale != null) {
      final List<String> selectAllUserNodeRef =
        circabcDaoService.selectAllUserNodeRef();
  ...
  public List<CategoryHeader> getHeadersAndCategories() {
    ...
    final List<HeaderCategory> headerCategoryList =
      circabcDaoService.selectHeadersCategories();
    ...
    for (HeaderCategory headerCategory : headerCategoryList) {
      if (
        Objects.equals(headerCategory.getHeaderNodeRef(), currentHeaderNodeRef)
      ) {
        String categoryTitle = headerCategory.getCategoryTitle();
        String categoryName = headerCategory.getCategoryName();
        NodeRef categoryNodeRef = new NodeRef(
          headerCategory.getCategoryNodeRef()
        );
        ...
        CategoryItem headerItem = new CategoryItem(
          headerID,
          headerTitle,
          false,
          headerName,
  ...
  @Override
  public List<ProfileWithUsersCount> getProfiles(
    NodeRef nodeRef,
    long localeID
  ) {
  ...
  public List<eu.cec.digit.circabc.repo.app.model.Profile> getProfilesForIg(
    ...
    if (getNodeService().hasAspect(igNodeRef, CircabcModel.ASPECT_IGROOT)) {
      long igID = (Long) getNodeService()
        .getProperty(igNodeRef, ContentModel.PROP_NODE_DBID);
      result = circabcDaoService.selectProfileTitleAlfGroupByIgId(
        igID,
        localeID,
        searchQuery
      );
  ...
  public void postOrUpdateProfile(io.swagger.model.Profile profile) {
    ...
    String alfrescoGroup = nodeService
    ...
    String igFromNodeRef = null;
    if (isImported) {
      igFromNodeRef = nodeService
        .getProperty(profileRef, ProfileModel.PROP_PROFILE_IMPORTED_REF)
        .toString();
    }
  ...
  public boolean isCategoryAdminOfInterestGroup(
    ...
    long interestGroupID = (Long) getNodeService()
      .getProperty(interestGroupNodeRef, ContentModel.PROP_NODE_DBID);
    return (
  ...
  @Override
  public boolean isIgToBeDeleted(NodeRef interestGroupNodeRef) {
    long igID = (Long) getNodeService()
      .getProperty(interestGroupNodeRef, ContentModel.PROP_NODE_DBID);
    return circabcDaoService.isIgToBeDeleted(igID);
  }
  ...
  public List<String> getCategoryAdmins(NodeRef categoryNodeRef) {
    long categID = (Long) getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public void deletePersonFromGroup(NodeRef igNodeRef, String userName) {
    Long groupId = (Long) getNodeService()
      .getProperty(igNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public void updateLogoRefProperty(NodeRef categoryNodeRef) {
    long categID = (Long) getNodeService()
      .getProperty(categoryNodeRef, ContentModel.PROP_NODE_DBID);
    NodeRef logoRef = (NodeRef) getNodeService()
      .getProperty(categoryNodeRef, CircabcModel.PROP_LOGO_REF);
    String logoId = "";
    if (logoRef != null) {
  ...
  public Map<String, String> getProfileTitle(NodeRef profileNodeRef) {
    long profileId = (Long) getNodeService()
      .getProperty(profileNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public Map<String, String> getCategoryTitle(NodeRef categNodeRef) {
    long categId = (Long) getNodeService()
      .getProperty(categNodeRef, ContentModel.PROP_NODE_DBID);
  ...
  public void setLogoPreferencesService(
    LogoPreferencesService logoPreferencesService
  ) {
  ...
  public void syncGroupLogos() {
    if (!lockService.isLocked(RELOAD_GROUP_LOGOS)) {
      ...
      List<InterestGroupResult> allGroups =
        circabcDaoService.getAllInterestGroups();
      for (InterestGroupResult group : allGroups) {
        ...
        if (nodeService.exists(nodeRef)) {
          ...
          try {
            ...
            circabcDaoService.updateLogoIdForGroup(
              interestGroupNodeDBID,
              logoId
            );
          } catch (CustomizationException e) {
            logger.error(
              "Impossible to update the logo ref in the ALL group resync logos",
              e
            );
          }
        }

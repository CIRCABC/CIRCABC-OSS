...
public class KeywordsServiceImpl implements KeywordsService {
  ...
  public void removeKeyword(final Keyword keyword) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Keyword successfully dropped from the ig: " + "\n  Keyword: " + keyword
      );
    }
  }
  ...
  protected void setKeywordsToNodeImpl(
    ...
    if (replace) {
      ...
      for (final NodeRef kwRef : newKeywords) {
        if (existingKeywords.contains(kwRef)) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Keyword " + kwRef + " already setted to the document " + document
            );
          }
        } else {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Keywords " +
        newKeywords +
        " successfully added to the document " +
        document
      );
    }
  }
  ...
  protected void setKeywordTranslationsNoderef(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "The keyword " +
        keyword +
        " translations are correctly setted " +
        allValues
      );
    }
  }
  ...
  private NodeRef makeKeywordContainer(final NodeRef nodeRef) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Keyword container successfully created under the node " + nodeRef
      );
    }

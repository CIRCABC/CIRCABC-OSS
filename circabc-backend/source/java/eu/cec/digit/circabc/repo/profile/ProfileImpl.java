...
public class ProfileImpl implements Profile, Serializable {
  ...
  public ProfileImpl(
    ...
    final QName authorithyService = QName.createQName(
    ...
    if (CircabcConstant.GUEST_AUTHORITY.equals(profileName)) {
      ...
      if (
        ...
        alfrescoGroupName = profileName;
        prefixedAlfrescoGroupName = authorityService.getName(
          AuthorityType.GROUP,
          profileName
        );
      } else {
    ...
    if (
      !CircabcConstant.GUEST_AUTHORITY.equals(this.getAlfrescoGroupName()) &&
      !circabcRootProfileManagerService
        .getAllCircaUsersGroupName()
        .equals(this.getAlfrescoGroupName())
    ) {
      final String prefixedParentGroup = authorityService.getName(
        AuthorityType.GROUP,
        parentGroup
      );
      ...
      if (authorityService.authorityExists(prefixedParentGroup)) {
        ...
        if (this.getAlfrescoGroupName() != null) {
          final String newGroup = authorityService.createAuthority(
            AuthorityType.GROUP,
        ...
        throw new CircabcRuntimeException(
          "Parent Authority doesn't exist:" + prefixedParentGroup
        );
      }
    }
  }
  ...
  public HashMap<String, Set<String>> getServicesPermissions() {
    final HashMap<String, ServicePermissions> map =
      profilePermissions.getServicePermissions();
   ...
   */
  public final void setPrefixedAlfrescoGroupName(
    final String prefixedAlfrescoGroupName
  ) {
  ...
  public boolean isAdmin() {
    ...
    for (Entry<
      String,
      Set<String>
    > servicePermission : servicesPermissions.entrySet()) {
      ...
      if (key.equalsIgnoreCase(CircabcServices.CATEGORY.toString())) {
        isCategoryAdmin = value.contains(
          CategoryPermissions.CIRCACATEGORYADMIN.toString()
        );
      }
      if (key.equalsIgnoreCase(CircabcServices.CIRCABC.toString())) {
        isCircabcAdmin = value.contains(
          CircabcRootPermissions.CIRCABCADMIN.toString()
        );
      }
    }
    result = ((isLibAdmin &&

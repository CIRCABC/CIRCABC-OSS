...
public class BulkUserImportServiceImpl implements BulkUserImportService {
      ...
      for (int iSheet = 0; iSheet < book.getNumberOfSheets(); iSheet++) {
        ...
        for (int i = iRow + 1; i < nbRows; i++) {
          ...
          if (r != null) {
            ...
            String username = (r.getCell(iCeel) != null
                ? r.getCell(iCeel).getStringCellValue()
                : "");
            String lastname = (r.getCell(iCeel + 2) != null
                ? r.getCell(iCeel + 2).getStringCellValue()
                : "");
            String email = (r.getCell(iCeel + 3) != null
                ? r.getCell(iCeel + 3).getStringCellValue()
                : "");
            String profile = (r.getCell(iCeel + 4) != null
                ? r.getCell(iCeel + 4).getStringCellValue()
                : "");
    ...
    if (r.getPhysicalNumberOfCells() >= 5) {
      ...
      if (r.getCell(iCellFirst) != null) {
        if (!r
            .getCell(iCellFirst)
            .getStringCellValue()
            .equals(xlsDefinedColumns.get(0))) {
          result = false;
        }
      } else {
        result = false;
      ...
      if (r.getCell(iCellFirst + 1) != null) {
        if (!r
            .getCell(iCellFirst + 1)
            .getStringCellValue()
            .equals(xlsDefinedColumns.get(1))) {
          result = false;
        }
      } else {
        result = false;
      ...
      if (r.getCell(iCellFirst + 2) != null) {
        if (!r
            .getCell(iCellFirst + 2)
            .getStringCellValue()
            .equals(xlsDefinedColumns.get(2))) {
          result = false;
        }
      } else {
        result = false;
      ...
      if (r.getCell(iCellFirst + 3) != null) {
        if (!r
            .getCell(iCellFirst + 3)
            .getStringCellValue()
            .equals(xlsDefinedColumns.get(3))) {
          result = false;
        }
      } else {
        result = false;
      ...
      if (r.getCell(iCellFirst) != null) {
        if (!r
            .getCell(iCellFirst + 4)
            .getStringCellValue()
            .equals(xlsDefinedColumns.get(4))) {
          result = false;
        }
      } else {
        result = false;
  ...
  public void addAll(
    ...
    for (BulkImportUserData tmpUser : newValues) {
      if (tmpUser.getUser() != null) {
        Boolean found = false;
        String tmpUserEmail = tmpUser.getUser().getEmail();

        for (BulkImportUserData existingUser : model) {
          if (existingUser.getUser() != null) {
            if (tmpUserEmail != null && tmpUserEmail != "" && existingUser.getUser().getEmail() != null
                    && tmpUserEmail.toLowerCase()
  ...
  private void filterAlreadyMember(
      List<BulkImportUserData> newValues,
      NodeRef currentIgRef) {
    Map<String, Profile> members = igRootProfileManagerService.getInvitedUsersProfiles(currentIgRef);
  ...
  public void parseProfilesToBeCreated(
    ...
    for (BulkImportUserData tmpUser : model) {
      ...
      if (createDepartmentNumberProfileHelper &&
          tmpUser.getDepartmentNumber() != null) {
      ...
      } else if (!createDepartmentNumberProfileHelper &&
          tmpUser.getDepartmentNumber() != null) {
        tmpUser.setProfile("");
  ...
  public void inviteUsers(
    ...
    for (BulkImportUserData user : model) {
      ...
      if (user.getUser() != null &&
        ...
        if (igRootProfileManagerService.getPersonProfile(
            igRef,
            user.getUser().getUserName()) == null) {
  ...
  private void inviteUserToGroup(
    ...
    igRootProfileManagerService.addPersonToProfile(
        igRef,
        user.getUser().getUserName(),
        igProfiles.get(user.getProfile()));

    if (notify) {
      Set<NotifiableUser> users = new HashSet<>();

      NodeRef person = getPersonService()
          .getPerson(user.getUser().getUserName());
  ...
  private Map<String, String> createNewAccessProfile(
    ...
    apw.withDescription(
        Locale.ENGLISH,
        "Profile automatically created from Bulk User Import tool");
    ...
    igRootProfileManagerService.addProfile(
        igRef,
        name,
        p.getServicesPermissions());
    ...
    igRootProfileManagerService.updateProfile(
        igRef,
        name,
        p.getServicesPermissions(),
        false);

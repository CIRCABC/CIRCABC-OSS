...
public class SupportServiceImpl implements SupportService {
  ...
  public boolean isUserFromSupport(final String user) {
    ...
    if (
      containingAuthoritiesForUser != null &&
    ...
    if (fileRef != null) {
      ...
      try {
        ...
        for (Object key : prop.keySet()) {
          ...
          String tmpKey = keyString.substring(
            keyString.indexOf('.') + 1,
            keyString.lastIndexOf('.')
          );
        ...
        for (String key : listOfKeys) {
          ...
          contact.setId(key);
          contact.setEmail(
            prop.getProperty(contactKey + "." + key + "." + emailKey)
          );
          contact.setPhone(
            prop.getProperty(contactKey + "." + key + "." + phoneKey)
          );
          contact.setTitle(
            prop.getProperty(contactKey + "." + key + "." + titleKey)
          );
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "Problem during reading contact file from datadictionnary",
            e
          );
        }
      }
  ...
  private NodeRef getContactFileRef() {
    ...
    NodeRef helpdeskFolderNodeRef = nodeService.getChildByName(
      circabcFolderNodeRef,
      ContentModel.ASSOC_CONTAINS,
      helpdeskFolderName
    );
    NodeRef contactFolderNodeRef = nodeService.getChildByName(
      helpdeskFolderNodeRef,
      ContentModel.ASSOC_CONTAINS,
      contactFolderName
    );
    NodeRef fileRef = nodeService.getChildByName(
      contactFolderNodeRef,
      ContentModel.ASSOC_CONTAINS,
      contactFile
    );
  ...
  public Boolean sendSupportRequest(
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "User does not exist impossible to send support request to contact mail",
          e
        );
      }
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Impossible to send support request to contact by email",
          e
        );
      }
    }
  ...
  public SupportContact getContactById(String id) {
    ...
    if (fileRef != null) {
      ...
      try {
        ...
        result.setId(id);
        result.setEmail(
          prop.getProperty(contactKey + "." + id + "." + emailKey)
        );
        result.setPhone(
          prop.getProperty(contactKey + "." + id + "." + phoneKey)
        );
        result.setTitle(
          prop.getProperty(contactKey + "." + id + "." + titleKey)
        );
      } catch (IOException e) {
        if (logger.isErrorEnabled()) {
          logger.error(

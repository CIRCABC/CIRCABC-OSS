...
public class KeywordsDelete extends CircabcDeclarativeWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    try {
      ...
      NodeRef containerId =
        this.nodeService.getPrimaryParent(keywordId).getParentRef();
      NodeRef igId =
        this.nodeService.getPrimaryParent(containerId).getParentRef();
      ...
      if (logger.isWarnEnabled()) {
        logger.warn(
          "Read-only mode prevented keyword delete: " + roe.getMessage()
        );
      }
      return null;

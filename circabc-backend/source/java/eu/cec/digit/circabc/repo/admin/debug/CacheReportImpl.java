...
public class CacheReportImpl implements CacheReport {
  ...
  public CacheReportImpl(
    ...
    if (Proxy.isProxyClass(cache.getClass())) {
      ...
      backingCache = ((ClusteredObjectProxyInvoker<
            Object
          >) handler).getBackingObject();
    } else {
      backingCache = cache;
    }
  ...
  private synchronized void calculate(SimpleCache<Serializable, Object> cache) {
    ...
    for (Serializable key : keys) {
      ...
      if (
        element != null &&
        Serializable.class.isAssignableFrom(element.getClass())
      ) {
    ...
    size = count > 0
      ? (long) ((double) size * ((double) keys.size() / (double) count))
      : 0L;
    ...
    memoryStoreEvictionPolicy = getProperty(
      name,
      "eviction-policy",
      DEFAULT_NOT_AVAILABLE
    );

    timeToLiveSeconds = getProperty(
      name,
      "timeToLiveSeconds",
      DEFAULT_NOT_AVAILABLE
    );
    timeToIdleSeconds = getProperty(
      name,
      "maxIdleSeconds",
      DEFAULT_NOT_AVAILABLE
    );
    ...
    evictionPercentage = getProperty(
      name,
      "eviction-percentage",
      DEFAULT_NOT_AVAILABLE
    );

 ...
 */
public class NavigationPreferencesServiceImpl
  implements NavigationPreferencesService {
  ...
  private static final Log logger = LogFactory.getLog(
    NavigationPreferencesServiceImpl.class
  );
  ...
  public NavigationPreference getServicePreference(
    ...
    try {
      ...
      final NodeRef file = nodePreferencesService.getDefaultConfigurationFile(
        nodeRef,
      ...
      final Date fileDate = (Date) nodeService.getProperty(
        file,
      ...
      if (config == null) {
        // cache not found
        config =
          (NavigationPreferenceImpl) navigationConfigService.buildPreference(
            serviceName,
            nodeType,
            fileRef
          );
        config.setModifiedDate(fileDate);
        config.setCustomizedOn(
          nodePreferencesService.getCustomizationFromNode(fileRef)
        );
  ...
  public void removeServicePreference(
    ...
    final NodeRef customizationRef = nodePreferencesService.getCustomization(
      nodeRef,
      CUSTOMIZATION_ROOT_FOLDER,
      serviceName,
      nodeType,

...
public class ExternalRepositoriesManagementServiceImpl
  ...
  private static final Log logger = LogFactory.getLog(
    ExternalRepositoriesManagementServiceImpl.class
  );
  ...
  public void init() {
    // Init evaluators
    ExternalRepositoryActionEvaluator.setExternalRepositoriesManagementService(
      this
    );
    ManageExternalRepositoriesActionEvaluator.setExternalRepositoriesManagementService(
      this
    );
    ...
    ((EntityWebServiceLocator) entityWebService).setEntityServiceEndpointAddress(
        endpointBaseAddress + "/1.16/EntityWebServicePS"
      );
    ((DocumentWebServiceLocator) documentWebService).setDocumentServiceEndpointAddress(
        endpointBaseAddress + "/1.16/DocumentWebServicePS"
      );
    ((WorkflowWebServiceLocator) workflowWebService).setWorkflowServiceEndpointAddress(
        endpointBaseAddress + "/1.16/WorkflowWebServicePS"
      );
    ((UserWebServiceLocator) userWebService).setUserServiceEndpointAddress(
        endpointBaseAddress + "/1.16/UserWebServicePS"
      );
  }
   ...
   */
  public NodeRef createOrGetRepositoryConfigurationFolder(
    String parentNodeId,
    boolean create
  ) {
  ...
  @Override
  public Map<String, Map<String, String>> getRepositoryDataForDocument(
    String documentId
  ) {
    ...
    if (
      ...
      final HashMap<String, HashMap<String, String>> repositoriesInfo =
        (HashMap<String, HashMap<String, String>>) nodeService.getProperty(
          nodeRef,
          CircabcModel.PROP_REPOSITORIES_INFO
        );

      HashMap<String, String> data = repositoriesInfo.get(
  ...
  public PublishResponse publishDocument(
    ...
    final RetryingTransactionCallback<PublishResponse> callback =
      new RetryingTransactionCallback<PublishResponse>() {
        public PublishResponse execute() throws Throwable {
          ...
          // Publish
          RegistrationSummary registrationSummary = publishToHermes(
            nodeRef,
            registrationRequest
          );
          // Error
          ...
          data.put(
            PROPERTY_DIRECT_ARES_LINK,
            aresLinkPrefix + registrationSummary.getDocumentId()
          );
          ...
          if (workflow.getWorkflowId() == 0) {
            ...
            publishResponse.setMessage(
              PUBLISH_SUCCESS_WORKFLOW_FAILURE + " " + workflowId
            );
            return publishResponse;
          }
    ...
    PublishResponse publishResponse = AuthenticationUtil.runAs(
  ...
  private CurrentInternalEntity[] getInternalEntitiesIntern(
    ...
    try {
      EntityService_PortType entityService =
        entityWebService.getEntityService();
      ...
      if (ecasUserName == null || ecasUserName.isEmpty()) {
        ...
        searchByExpressionRequest = new EntitySearchByExpressionRequest(
          searchExpression,
          sortOptions
        );
      }
  ...
  public List<String> getInternalEntities(
    ...
    CurrentInternalEntity[] currentInternalEntities = getInternalEntitiesIntern(
      firstName,
      lastName,
      ecasUserName
    );
  ...
  public String getInternalEntityName(String ecasUserName) {
    ...
    if (currentInternalEntities != null && currentInternalEntities.length > 0) {
      CurrentInternalPerson person =
        currentInternalEntities[0].getCurrentInternalPerson();
  ...
  public List<String> getExternalEntities(
    ...
    // Build search expression
    String searchExpression = buildSearchExpression(
      firstName,
      lastName,
      organization
    );
  ...
  private String buildSearchExpression(
    ...
    if (firstName != null) {
      searchExpression
        .append("personFirstName startswith '")
        .append(firstName)
        .append("'");
    }
    if (lastName != null) {
      ...
      searchExpression
        .append("personLastName startswith '")
        .append(lastName)
        .append("'");
    }
    if (organization != null) {
      ...
      searchExpression
        .append("organization startswith '")
        .append(organization)
        .append("'");
    }
  ...
  public boolean isUserAuthorizedToPublish(String ecasUserName) {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Got profile " + userProfile.getValue() + " for user " + ecasUserName
        );
      }
  ...
  private RegistrationSummary publishToHermes(
    ...
    try {
      ...
      String uploadResponse = uploadDocument(
        registrationRequest.getRegistrationUserName(),
        content
      );
      ...
      boolean success = Boolean.parseBoolean(
        (String) evaluateXPath(
          uploadResponse,
          "/hrs:response/hrs:success/text()",
          XPathConstants.STRING
        )
      );
      if (!success) {
        String error = (String) evaluateXPath(
          uploadResponse,
          "/hrs:response/hrs:error/text()",
          XPathConstants.STRING
        );
        return new RegistrationSummary(
          ERROR_UPLOAD_CONTENT + " " + error,
          null,
          null,
          null,
          null
      ...
      String uploadedContentId = (String) evaluateXPath(
        uploadResponse,
        "/hrs:response/hrs:content-id/text()",
        XPathConstants.STRING
      );
   ...
   * the document.
   */
  private String uploadDocument(String userName, InputStream content)
    throws Exception {
    ...
    try {
      ...
      postMethod.setRequestHeader(
        new Header("X-HRS-TICKET", proxyTicketResolver.getProxyTicket())
      );
      postMethod.setRequestHeader(
        new Header("X-HRS-APPLICATION", applicationId)
      );
  ...
  private String buildError(String message) {
    return (
      "<response xmlns=\"http://ec.europa.eu/sg/hrs/dts/1.0\" " +
   ...
   */
  private RegistrationSummary registerDocument(
    RegistrationRequest registrationRequest
  ) {
    try {
      DocumentService_PortType documentService =
      ...
      return documentService.registerDocument(
        createHeader(registrationRequest.getRegistrationUserName()),
        request
      );
    } catch (Exception e) {
      logger.error("Error registering document.", e);
      return new RegistrationSummary(
        ERROR_REGISTER_DOCUMENT + " " + e.getMessage(),
        null,
        null,
        null,
        null
  ...
  private AssignmentWorkflow addAssignmentTask(AssignmentTaskRequest request) {
    ...
    try {
      ...
      AddAssignmentsRequest assignmentsRequest = new AddAssignmentsRequest(
        request.getDocumentId(),
        tasks
      );

      workflow = workflowService.addAssignments(
        createHeader(request.getAssignmentUserName()),
        assignmentsRequest
      );
    } catch (Exception e) {
      logger.error("Error while assigning workflow.", e);
      workflow = new AssignmentWorkflow(
        0,
   ...
   */
  private eu.cec.digit.circabc.repo.hrs.ws.Header createHeader(
    String userName
  ) {
   ...
   */
  public void setHrsInternalEntitiesCache(
    SimpleCache<String, Object> hrsInternalEntitiesCache
  ) {

...
public class MailServiceImpl implements MailService {
  ...
  public boolean send(
    ...
    return sendImpl(from, to, others, replyTo, subject, body, html, useCC);
  }
    ...
    final MimeMessagePreparator mailPreparer = new MimeMessagePreparator() {
      public void prepare(final MimeMessage mimeMessage)
        ...
        if (environmentNameEnabled) {
          ...
          StringBuilder environmentStringBuffer = new StringBuilder(
            containerStart
          );
        ...
        try {
          ...
          final NodeRef logoNodeRef = mailPreferencesService.getDisclamerLogo(
            circabcRootRef
          );
          ...
          final ContentReader logoContent = contentService.getReader(
            logoNodeRef,
            ContentModel.PROP_CONTENT
          );
          final ContentReader headerLogoContent = contentService.getReader(
            headerLogoNodeRef,
            ContentModel.PROP_CONTENT
          );
          final ContentReader headerEULogoContent = contentService.getReader(
            headerEULogoNodeRef,
            ContentModel.PROP_CONTENT
          );
  ...
  public boolean sendMeetingRequest(
    ...
    if (mode == null) {
      ...
      final boolean isDateChanged = !oldMeeting
        .getDate()
        .equals(meeting.getDate());
  ...
  public String getOrganizer() {
    if (organizer == null) {
      organizer = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_ADDRESS
      );
    }
    return organizer;
  }
  ...
  public Boolean getIsListenerActive() {
    if (isListenerActive == null) {
      isListenerActive = CircabcConfiguration.getProperty(

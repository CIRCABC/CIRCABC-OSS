    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        try {
            if (!(currentUserPermissionCheckerService.isAlfrescoAdmin()
                    ...
                    || currentUserPermissionCheckerService.isCurrentUserEqualTo(templateVars.get("userId")))) {
                throw new AccessDeniedException("");
            }

            model.put("email", appMessageApi.getSubscribedDistributionEmail(templateVars.get("userId")));

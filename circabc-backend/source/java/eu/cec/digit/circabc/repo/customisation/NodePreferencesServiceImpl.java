...
public class NodePreferencesServiceImpl implements NodePreferencesService {
  ...
  public NodeRef getCustomization(
    ...
    if (file == null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Customization file '" + fileName + "' successfully found"
        );
      }
  ...
  private List<NodeRef> getConfigFilesImpl(
    ...
    if (folders.containsKey(key)) {
      ...
      final QName childAssoc = (isRootContainer(container))
        ? ASSOC_CONTAINS
        : ASSOC_CHILDREN;

      final NodeRef configTypeRootRef = getChildByName(
        container,
        childAssoc,
        configTypeRoot
      );
      if (configTypeRootRef != null) {
        final NodeRef configSubTypeRef = getChildByName(
          configTypeRootRef,
          childAssoc,
          configSubType
        );
        ...
        if (configSubTypeRef != null) {
          configElementRef = getChildByName(
            configSubTypeRef,
            childAssoc,
            configElement
          );
  ...
  private NodeRef getConfigurationContainer(
    ...
    if (getCircabcRootNodeRef().equals(recursionRef)) {
      ...
      final List<ChildAssociationRef> associations = nodeService.getChildAssocs(
        recursionRef,
        ASSOC_CUSTOMIZE,
        TYPE_CUSTOMIZATION_CONTAINER
      );
      ...
      if (associations == null || associations.size() < 1) {
        ...
        if (associations.size() > 1) {
          logger.warn(
            "More than one Preference Root Node found for the node " +
            recursionRef
          );
        }
  ...
  private NodeRef getCircabcDDNodeRef() {
    if (circabcDictionaryNodeRef == null) {
      circabcDictionaryNodeRef =
        managementService.getCircabcDictionaryNodeRef();
    }

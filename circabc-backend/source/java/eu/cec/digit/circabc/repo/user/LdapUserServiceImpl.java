...
public class LdapUserServiceImpl implements LdapUserService {
    ...
    try {
      ...
      while (results.hasMore()) {
        ...
        if (attributes.get(PHYSICAL_DELIVERY_OFFICE_NAME) != null) {
          final String postalAddress = (String) attributes
            .get(PHYSICAL_DELIVERY_OFFICE_NAME)
            .get();
        ...
        if (attributes.get(MODIFICATION_DATE) != null) {
          final String modificationDateSting = (String) attributes
          ...
          try {
            final Date modificationDate = formatter.parse(
        ...
        if (attributes.get(SOURCE_ORGANISATION) != null) {
          final String sourceOrganisation = (String) attributes
            .get(SOURCE_ORGANISATION)
            .get();
      ...
      // don't ignore this... accessing by uid should never get a
  ...
  private String getDomainFilter(final String search) {
    ...
    if (
      ...
      final String sourceOrganisationFilter = getSourceOrganisationFilter(
        search
      );
      ...
      final Set<String> allEcasDomains =
        ldapEcasDomainService.getAllEcasDomains();
    ...
    try {
      ...
      while (results.hasMore()) {
        ...
        if (
          ...
          users.add(
            new SearchResultRecord(uid, moniker, firstName, lastName, email)
          );
        }
      }
    } catch (final LimitExceededException e) {
  ...
  private List<String> getUsersWithoutMail(
    ...
    while (results.hasMore()) {
      ...
      if (
        ...
        final String user =
          firstName + BLANK_DELIM + lastName + " (" + uid + ")";
  ...
  private String getReturnAttributesValues(final Attributes attributes) {
    ...
    final NamingEnumeration<? extends Attribute> namingEnumaration =
      attributes.getAll();
  ...
  public void setLdapEcasDomainService(
    LdapEcasDomainService ldapEcasDomainService
  ) {
  ...
  public Boolean userExists(String uid) {
    ...
    try {
      ...
      if (results.hasMore()) {
        ...
        attributes = searchResult.getAttributes();
        if (
          !(attributes.get(UID) == null || attributes.get(RECORDSTATUS) == null)
        ) {
          uid = (String) attributes.get(UID).get();

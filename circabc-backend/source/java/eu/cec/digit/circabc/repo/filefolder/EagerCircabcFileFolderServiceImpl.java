...
public class EagerCircabcFileFolderServiceImpl
  implements CircabcFileFolderService {
  ...
  public List<FileInfo> list(NodeRef contextNodeRef) {
    ...
    final String key =
      AuthenticationUtil.getRunAsUser() + libraryNodeRef.getId();

    // reset library node
    if (
      nodeService.hasAspect(contextNodeRef, CircabcModel.ASPECT_LIBRARY_ROOT)
    ) {
  ...
  private NodeRef getLibraryNodeRef(NodeRef contextNodeRef) {
    ...
    if (
      ...
      while (
        !getNodeService()
          .hasAspect(currentNodeRef, CircabcModel.ASPECT_LIBRARY_ROOT)
      ) {
        currentNodeRef = getNodeService()
          .getPrimaryParent(currentNodeRef)
          .getParentRef();
      }
   ...
   */
  private List<FileInfo> toFileInfo(Set<NodeRef> nodeRefs)
  ...
  public void setTreeCache(
    SimpleCache<String, Map<NodeRef, Set<NodeRef>>> treeCache
  ) {
  ...
  public SimpleCache<
    String,
    Map<NodeRef, Set<NodeRef>>
  > getCircabcFileFolderServiceCache() {
    return circabcFileFolderServiceCache;
  }

...
public class TranslationServiceImpl implements TranslationService {
  ...
  private String webRootUrl = CircabcConfiguration.getProperty(
    CircabcConfiguration.WEB_ROOT_URL
  );
  private String callbackUrl = CircabcConfiguration.getProperty(
    CircabcConfiguration.MT_CALLBACK_URL
  );
  private String mtUserName = CircabcConfiguration.getProperty(
    CircabcConfiguration.MT_USER
  );
  private String mtPassword = CircabcConfiguration.getProperty(
    CircabcConfiguration.MT_PASSWORD
  );

  private String applicationName = CircabcConfiguration.getProperty(
    CircabcConfiguration.MT_APPLICATION_NAME
  );
  ...
  public void translateProperty(
    ...
    CircabcUserDataBean circabcUserDataBean = getUserService()
      .getCircabcUserDataBean(username);

    Serializable propertyValue = getNodeService()
      .getProperty(nodeRef, property);
  ...
  public void translateDocument(
    ...
    final NodeRef parentFolder = (NodeRef) AuthenticationUtil.runAs(
    ...
    final String documentToTranslate = (String) AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          String sourceName = (String) getNodeService()
            .getProperty(source, ContentModel.PROP_NAME);
    ...
    final String targetTranslationPath = (String) AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
      ...
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          return getNodeService()
            .getProperty(copyOfDocument, ContentModel.PROP_NAME);
        }
      },
      AuthenticationUtil.getSystemUserName()
    );
  ...
  private NodeRef getFolder(String spaceName, NodeRef parentNodeRef) {
    NodeRef result = getNodeService()
      .getChildByName(parentNodeRef, ContentModel.ASSOC_CONTAINS, spaceName);
    if (result == null) {
      final ChildAssociationRef childRef = getNodeService()
        .createNode(
          parentNodeRef,
  ...
  private NodeRef copyDocument(NodeRef parentFolder, NodeRef originalDocument) {
    String name = (String) getNodeService()
      .getProperty(originalDocument, ContentModel.PROP_NAME);
    ...
    final List<ChildAssociationRef> childAssocs = getNodeService()
      .getChildAssocs(
        parentFolder,
        ContentModel.ASSOC_CONTAINS,
        qname,
  ...
  public NodeRef copyDocumentToBeTranslated(final NodeRef nodeRef) {
    org.alfresco.service.namespace.QName type = getNodeService()
      .getType(nodeRef);
    ...
    final NodeRef result = (NodeRef) AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
  ...
  private void processUserNotification(Date fromDate) {
    try {
      ...
      for (final SearchResultNotify searchResult : translationsToProcess) {
        if (searchResult.getTargetPath().startsWith(ftpUrl)) { // process only rows from server that init machine translation
  ...
  private void sendNotificationMessage(
    ...
    NodeRef otherPerson = getPersonService()
      .getPerson(searchNotifyItem.getUsername());
    Map<String, Object> model = getMailPreferencesService()
      .buildDefaultModel(currentRef, otherPerson, null);
    ...
    MailWrapper mail = getMailPreferencesService()
      .getDefaultMailTemplate(currentRef, mailTemplate);

    String from = mailService.getNoReplyEmailAddress();
    String to = (String) getNodeService()
      .getProperty(otherPerson, ContentModel.PROP_EMAIL);
  ...
  private void updateProperty(SearchResult searchResult) {
    ...
    PropertyDefinition propDef =
      this.getDictionaryService().getProperty(propQname);
    ...
    if (
      ...
      mlText.addValue(
        new Locale(searchResult.getTargetLang()),
  ...
  private MLText getMLPropertyValue(
    ...
    try {
      ...
      properties = (MLText) getNodeService()
        .getProperty(nodeRef, propertyQname);
    } finally {
  ...
  private void updateMLDocument(SearchResult searchResult) {
    ...
    if (multilingualContentService.isTranslation(docNodeRef)) {
      String documentName = (String) getNodeService()
        .getProperty(docNodeRef, ContentModel.PROP_NAME);
      ...
      Long id = (Long) getNodeService()
        .getProperty(docNodeRef, ContentModel.PROP_NODE_DBID);
      ...
      if (translationNodeRef != null) {
        ...
        writer.putContent(new ByteArrayInputStream(binaryData));
        getNodeService()
          .addAspect(translationNodeRef, ContentModel.ASPECT_TEMPORARY, null);
      ...
      Action action =
        this.getActionService()
          .createAction(ExecuteAllRulesActionExecuter.NAME);
  ...
  private NodeRef getTranslationNodeRef(
    String ftpPath,
    String targetLang,
    String name
  ) {
    ...
    for (String space : spaces) {
      if (!space.equals("")) {
        currentNode = getNodeService()
          .getChildByName(currentNode, ContentModel.ASSOC_CONTAINS, space);
        if (currentNode == null) {
          break;
        }
      }
    ...
    if (currentNode != null) {
      List<ChildAssociationRef> children = getNodeService()
        .getChildAssocs(currentNode);
      for (ChildAssociationRef childAssoc : children) {
        NodeRef childNodeRef = childAssoc.getChildRef();
        String childName = (String) getNodeService()
  ...
  public void cleanTempSpace(int year, int month) {
    ...
    final NodeRef yearFolder = getNodeService()
      .getChildByName(
        mtRoNodeRef,
        ContentModel.ASSOC_CONTAINS,
        String.valueOf(year)
    ...
    if (yearFolder != null) {
      final NodeRef monthFolder = getNodeService()
        .getChildByName(
          yearFolder,
          ContentModel.ASSOC_CONTAINS,

      ...
      switch (elementName) {
        ...
        case ELEM_FILE: {
          ...
          if (isNameValid(name)) {
            ...
            final Integer revision = getInteger(
              element.getAttribute(ATTR_REVISION)
            );
            boolean forceUpdate = isTrue(
              element.getAttribute(ATTR_FORCEUPDATE)
            );
            ...
            if (
              newContent || forceUpdate || updateContent(contentRef, revision)
            ) {
              final String guestPerm = element.getAttribute(ATTR_GUEST_PERM);
              final String registredPerm = element.getAttribute(
                ATTR_REGISTRED_PERM
              );

              final InputStream inputStream = getInpuStreamFromResource(
                element.getValue()
              );
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                  "Content '" + name + "' doesn't need to be updated"
                );
              }
            }

...
public class UserServiceImpl implements UserService {
  ...
  public void updateMissingLastNamePersons() {
    ...
    try {
      ...
      for (final ResultSetRow row : resultSet) {
        ...
        if (
          !(name.equalsIgnoreCase(CircabcConstant.GUEST_AUTHORITY) ||
  ...
  private synchronized NodeRef getUserPreferencesRefCreate(
  ...
  @Deprecated // see eu.cec.digit.circabc.business.helper.UserManager
  public Serializable getPreference(
    final NodeRef person,
  ...
  public NodeRef createUser(final CircabcUserDataBean pCircabcUser) {
    ...
    final NodeRef newPerson =
      this.personService.createPerson(pCircabcUser.getAttributesAsMap());
    ...
    try {
      ...
      throw new RuntimeException(
        "Can not generate password Algoritam does not exists"
      );
    }

    final String password = (pCircabcUser.getPassword() == null)
      ? new String(newPassword)
      : pCircabcUser.getPassword();

    if (
      !authenticationService.authenticationExists(pCircabcUser.getUserName())
    ) {
    ...
    nodeService.addAspect(
      ...
      pCircabcUser.getAspectAttributesInMap()
    );
  ...
  public void updateUser(
    ...
    final NodeRef person =
      this.personService.getPerson(pCircabcUser.getUserName());
   ...
   */
  public void setConfigurableService(
    final ConfigurableService configurableService
  ) {
  ...
  private List<UserCategoryMembershipRecord> getCategoriesOldWay(
    final String pUserName
  ) {
    ...
    for (final String authority : authorities) {
      if (
        ...
        if (categoNodeRef != null) {
          String category = upFirstLetterInString(
            nodeService.getProperty(categoNodeRef, ContentModel.PROP_NAME)
          );
          final String categoryTitle = (String) nodeService.getProperty(
            categoNodeRef,
          ...
          final String profile = getProfileManagerServiceFactory()
      ...
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          final Set<String> authorities =
            authorityService.getAuthoritiesForUser(pUserName);
          ...
          for (final String authority : authorities) {
            if (
              ...
              if (igRootNodeRef != null) {
                final NodeRef categoNodeRef = nodeService
                  .getPrimaryParent(igRootNodeRef)
                  .getParentRef();
                ...
                if (isFilterMatch) {
                  ...
                  final String category = upFirstLetterInString(
                    nodeService.getProperty(
                      categoNodeRef,
                      ContentModel.PROP_NAME
                    )
                  );
                  final String categoryTitle = upFirstLetterInString(
                    nodeService.getProperty(
                      categoNodeRef,
                      ContentModel.PROP_TITLE
                    )
                  );
                  final String interesGroup = upFirstLetterInString(
                    nodeService.getProperty(
                      igRootNodeRef,
                      ContentModel.PROP_NAME
                    )
                  );
                  final String interesGroupTitle = upFirstLetterInString(
                    nodeService.getProperty(
                      igRootNodeRef,
                      ContentModel.PROP_TITLE
                    )
                  ...
                  final String profileName =
                    rootProfileManagerService.getPersonProfile(
                      igRootNodeRef,
                      pUserName
                    );
                  final Profile profile = rootProfileManagerService.getProfile(
                    igRootNodeRef,
                    profileName
                  );
                  ...
                  if (profile.isExported()) {
                    ...
                    for (AssociationRef item : targetAssocs) {
                      ...
                      if (parentAssocs.size() > 0) {
                        final NodeRef importedIgNodeRef = parentAssocs
                          .get(0)
                          .getParentRef();
  ...
  public Set<String> getOnlineUsers(final NodeRef igNodeRef) {
    ...
    final Set<String> igOnlineUser = getProfileManagerServiceFactory()
  ...
  public String getUserNameByEmail(final String email) {
    ...
    try {
      ...
      if (rs.length() != 0) {
        result = nodeService
          .getProperty(rs.getRow(0).getNodeRef(), PROP_USERNAME)
          .toString();
      }
    } catch (final Exception e) {
      if (logger.isErrorEnabled()) {
  ...
  public List<String> getLDAPUserIDByMailDomain(
    final String mail,
    final String domain
  ) {
  ...
  public List<SearchResultRecord> getUsersByMailDomain(
    ...
    //DIGITCIRCABC-5063 if we do a global search, search also the users in Alfresco Repo to retrieve non EULogin/Ldap users
    // It is only necessary when we have access to the LDAP (CircabcConfig.USE_LDAP is true) because otherwise we already used luceneUserService to search the users before
    if (CircabcConfig.USE_LDAP && !filter) {
  ...
  public void copyUser(
    ...
    if (!oldPersonExists) {
      throw new IllegalStateException(
        "Person does not exists : " + oldUserName
      );
    }
    ...
    if (!newPersonExists) {
      CircabcUserDataBean circabcUserDataBean = getCircabcUserDataBean(
        oldUserName
      );
  ...
  private void copyGroupMebership(String oldUserName, String newUserName) {
    IGRootProfileManagerService igs = getProfileManagerServiceFactory()
      .getIGRootProfileManagerService();
    CategoryProfileManagerService categs = getProfileManagerServiceFactory()
      .getCategoryProfileManagerService();

    List<UserIGMembershipRecord> igListOld = getInterestGroups(oldUserName);
    List<UserCategoryMembershipRecord> categListOld = getCategories(
      oldUserName
    );

    List<UserIGMembershipRecord> igListNew = getInterestGroups(newUserName);
    List<UserCategoryMembershipRecord> categListNew = getCategories(
      newUserName
    );
        ...
        try {
          igs.addPersonToProfile(
            new NodeRef(
              ...
              igMembershipOld.getInterestGroupNodeId()
            ),
            newUserName,
          ...
          if (logger.isErrorEnabled()) {
            logger.error(
              "Problem during the copy of user from" +
              oldUserName +
              " to " +
              newUserName,
              e
    ...
    for (UserCategoryMembershipRecord categMembershipOld : categListOld) {
      ...
      for (UserCategoryMembershipRecord categMembershipNew : categListNew) {
        if (
          categMembershipNew
            .getCategoryNodeId()
            .equals(categMembershipOld.getCategoryNodeId())
        ) {
          alreadyMember = true;
          break;
        }
      ...
      if (!alreadyMember) {
        try {
          categs.addPersonToProfile(
            new NodeRef(
              "workspace://SpacesStore/" +
              categMembershipOld.getCategoryNodeId()
            ),
            newUserName,

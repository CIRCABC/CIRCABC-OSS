...
public class NotificationSubscriptionServiceImpl
   ...
   */
  public final NotificationStatus DEFAULT_TOP_LEVEL_STATUS =
   ...
   * Group
   */
  public final NotificationStatus DEFAULT_STATUS_ANY_OTHER_NODE =
  ...
  public Set<AuthorityNotification> getNotifications(final NodeRef nodeRef) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Trying to retive the notification status for the node " +
        nodeRef +
        " ..."
      );
    }
    ...
    final boolean inheritParentPermissions = getPermissionService()
    ...
    final Set<AuthorityNotification> notifications = new HashSet<>(
      permissions.size()
    );
    ...
    switch (status) {
      case SUBSCRIBED:
        getPermissionService()
          .setPermission(nodeRef, authority, NOTIFICATION_AS_PERMISSION, true);
        break;
      case UNSUBSCRIBED:
        getPermissionService()
          .setPermission(nodeRef, authority, NOTIFICATION_AS_PERMISSION, false);
        break;
      case INHERITED:
  ...
  public AuthorityNotification getAuthorityNotificationStatus(
    ...
    final NodeRef interestGroup = getManagementService()
  ...
  public UserNotificationReport getUserNotificationReport(
    ...
    final UserNotificationReport report = getReportUserProfileImpl(
      nodeRef,
      igNoderef,
      userAuthority,
      profile
  ...
  public Set<NotifiableUser> getNotifiableUsers(final NodeRef nodeRef)
    ...
    final Map<String, LeveledNotificationStatus> profileNotif = new HashMap<>(
      80
    );
    ...
    > entry : profileNotif.entrySet()) {
      profile = getIGProfileManagerService()
        .getProfileFromGroup(igNoderef, entry.getKey());
      if (profile != null) {
        usersInProfile = getIGProfileManagerService()
          .getPersonInProfile(igNoderef, profile.getProfileName());
        for (final String user : usersInProfile) {
          userLeveledNotificationStatus = userNotif.get(user);
          if (
            userLeveledNotificationStatus == null ||
    ...
    boolean isMultilingualDocument = nodeService.hasAspect(
      nodeRef,
      ContentModel.ASPECT_MULTILINGUAL_DOCUMENT
    );
    if (isMultilingualDocument) {
      NodeRef pivot = getMultilingualContentService()
        .getPivotTranslation(nodeRef);
      pivotlocale = (Locale) getNodeService()
        .getProperty(pivot, ContentModel.PROP_LOCALE);
      if (
        getNodeService()
          .hasAspect(
            nodeRef,
        ...
        documentLocale = (Locale) getNodeService()
    ...
    > entry : userNotif.entrySet()) {
      ...
      if (
        NotificationStatus.SUBSCRIBED.equals(
          userLeveledNotificationStatus.status
        )
      ) {
        ...
        if (globalNotif != null && globalNotif == true) {
          ...
          final Boolean hasReadPermision = (Boolean) AuthenticationUtil.runAs(
          ...
          if (hasReadPermision) {
            ...
            if (isMultilingualDocument) {
              ...
              if (
                ...
                usersToNotify.add(
                  new NotifiableUserImpl(person, locale, properties)
                );
              }
            } else {
              usersToNotify.add(
  ...
  private void fillNotifiableAuthoritiesImpl(
    ...
    if (!from.equals(to)) {
      final ChildAssociationRef parentAssoc = getNodeService()
        .getPrimaryParent(from);
  ...
  private String getPersonProfile(NodeRef nodeRef, String userAutority) {
    final String profileName = getIGProfileManagerService()
      .getPersonProfile(nodeRef, userAutority);
    final Profile pofile = getIGProfileManagerService()
      .getProfile(nodeRef, profileName);
  ...
  private void removeNotificationImpl(
    ...
    if (getDefinedAuthorityStatus(nodeRef, authority) != null) {
      getPermissionService()
        .deletePermission(nodeRef, authority, NOTIFICATION_AS_PERMISSION);
    }
  }
    ...
    if (globalNotification != null && globalNotification == true) {
      ...
      while (willUserReceive == null) {
        ...
        if (willUserReceive == null) {
          ...
          boolean topFolder =
            (tempNodeRef == null) || (tempNodeRef.equals(igRef));
  ...
  protected final IGRootProfileManagerService getIGProfileManagerService() {
    if (igProfileManagerService == null) {
      igProfileManagerService = getProfileManagerServiceFactory()

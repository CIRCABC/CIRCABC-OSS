...
public class GlobalStatisticsServiceImpl implements GlobalStatisticsService {
  ...
  private HSSFWorkbook builtExcelFromData(Map<String, Object> lData) {
      ...
      if (!key.matches("list.*")) {
        switch (key) {
          ...
          case "actionCountForYesterDay": {
            ...
            for (LogCountResultDAO logEntry : (List<
                LogCountResultDAO
              >) entry.getValue()) {
              HSSFRow row2 = sheetNumbers.createRow(iRow);
    ...
    cell6.setCellValue(new HSSFRichTextString("Size (MB)"));

    HSSFCell cell7 = row.createCell((iCol + 6));
    cell7.setCellValue(new HSSFRichTextString("Nb Members"));

    HSSFCell cell8 = row.createCell((iCol + 7));
    cell8.setCellValue(new HSSFRichTextString("Nb IGs"));

    iRow++;

    String rootUrl = CircabcConfiguration.getProperty(
      CircabcConfiguration.WEB_ROOT_URL
    );
    ...
    for (CategoryDescriptor c : (List<CategoryDescriptor>) lData.get(
      ...
      cCateg1.setCellValue(
        new HSSFRichTextString(
          (c.getTitle().equals("") ? c.getName() : c.getTitle())
        )
      );
      ...
      // parse list of IG for the current category
      for (IgDescriptor ig : c.getListOfIgs()) {
        ...
        nbMembers += statistics.getNbUsers();
        ...
        double libraryDocSize = 0;
        double informationDocSize = 0;

        if (ig.getLibraryDocSize() != null) {
          libraryDocSize = ig.getLibraryDocSize();
        }
        if (ig.getInformationInfoSize() != null) {
          informationDocSize = ig.getInformationInfoSize();
        }

        size += libraryDocSize + informationDocSize;
      }

      HSSFCell cCateg6 = rowCateg.createCell(
        (iCol + 5),
        HSSFCell.CELL_TYPE_NUMERIC
      );
      cCateg6.setCellValue(size);
      cCateg6.setCellStyle(numberCellStyle);

      HSSFCell cCateg7 = rowCateg.createCell((iCol + 6));
      cCateg7.setCellValue(nbMembers);

      HSSFCell cCateg8 = rowCateg.createCell((iCol + 7));
      cCateg8.setCellValue(nbIgs);

      iRow++;

      //The second time we parse the list of IGs all the value are alreadyset
      for (IgDescriptor ig : c.getListOfIgs()) {
        size = 0;
        ...
        c2.setCellValue(
          new HSSFRichTextString(
            (ig.getTitle().equals("") ? ig.getName() : ig.getTitle())
          )
        );
        ...
        HSSFCell c6 = row2.createCell((iCol + 5), HSSFCell.CELL_TYPE_NUMERIC);
        c6.setCellValue(size);
        c6.setCellStyle(numberCellStyle);

        HSSFCell c7 = row2.createCell((iCol + 6));
        c7.setCellValue(ig.getNbMembers());

        HSSFCell c8 = row2.createCell((iCol + 7));
      ...
      for (IgDescriptor ig : categ.getListOfIgs()) {
        if (ig.getPublicVisibility()) {
          ...
          cellOwner.setCellValue(
            new HSSFRichTextString(ig.getSetOfLeaders().toString())
          );
          ...
          cellTitle.setCellValue(
            !ig.getTitle().isEmpty() ? ig.getTitle() : ig.getName()
          );
          ...
          cellFunctionalities.setCellValue(
            ig.getAvailableServices().toString()
          );
  ...
  private Set<String> getListOfLeadersForInterestGroup(NodeRef igNodeRef) {
    ...
    List<Profile> lProfiles = igRootProfileManagerService.getProfiles(
      igNodeRef
    );
    for (Profile p : lProfiles) {
      if (
        p.isAdmin() ||
        p
          .getServicePermissions(CircabcServices.DIRECTORY.name())
    ...
    for (NodeRef nCateg : getListOfCircabcCategories()) {
      ...
      c.setTitle(
        (nodeService.getProperties(nCateg).containsKey(ContentModel.PROP_TITLE)
            ? nodeService
              .getProperty(nCateg, ContentModel.PROP_TITLE)
              .toString()
            : "")
      );
      ...
      for (NodeRef nIg : getListOfCircabcInterestGroupsForCategory(nCateg)) {
        ...
        if (detailed) {
          ig.setDescription(
            (nodeService
                  .getProperties(nIg)
                  .containsKey(ContentModel.PROP_DESCRIPTION)
                ? nodeService
                  .getProperty(nIg, ContentModel.PROP_DESCRIPTION)
                  .toString()
                : "")
          );
          ig.setLightDescription(
            (nodeService
                  .getProperties(nIg)
                  .containsKey(CircabcModel.PROP_LIGHT_DESCRIPTION)
                ? nodeService
                  .getProperty(nIg, CircabcModel.PROP_LIGHT_DESCRIPTION)
                  .toString()
                : "")
          );
          ...
          if (
            nodeService
              .getProperties(nIg)
              .containsKey(ContentModel.PROP_CREATED)
          ) {
          ...
          Profile profileGuest = igRootProfileManagerService.getProfile(
            nIg,
            CircabcConstant.GUEST_AUTHORITY
          );
          Profile profileRegistered = igRootProfileManagerService.getProfile(
            nIg,
            CircabcConstant.REGISTERED_AUTHORITY
          );
          ig.setPublicVisibility(
            computeVisibility(profileGuest) ||
            computeVisibility(profileRegistered)
          );
          ...
          for (Profile profile : igRootProfileManagerService.getProfiles(nIg)) {
            ...
            final Set<String> infServicePermissions = permissions.get(
              CircabcServices.INFORMATION.toString()
            );
            final Set<String> libServicePermissions = permissions.get(
              CircabcServices.LIBRARY.toString()
            );
            final Set<String> dirServicePermissions = permissions.get(
              CircabcServices.DIRECTORY.toString()
            );
            final Set<String> newServicePermissions = permissions.get(
              CircabcServices.NEWSGROUP.toString()
            );
            final Set<String> eveServicePermissions = permissions.get(
  ...
  private Set<String> getListOfAdminsForCategory(NodeRef categoryNodeRef) {
    ...
    Set<String> invitedUsers = profileManagerService.getInvitedUsers(
      categoryNodeRef
    );
  ...
  public void cleanAndZipPreviousReportFiles() {
    ...
    if (!isDeleted && logger.isWarnEnabled()) {
      try {
        logger.warn(
          "Unable to delete file : " + tempZipFile.getCanonicalPath()
        );
      } catch (IOException e) {
        logger.warn(
          "Unable to get getCanonicalPath for : " + tempZipFile.getPath(),
          e
        );
      }
    }
    ...
    if (node == null) {
      ...
      if (!isDeleted && logger.isWarnEnabled()) {
        try {
          ...
          logger.warn(
            "Unable to get getCanonicalPath for ." + tempFile.getPath(),
            e
          );
        }
      }
  ...
  private HSSFWorkbook builtCategoryExcelFromData(
    ...
    HSSFRow row = sheetNumbers.createRow(iRow);
    row.createCell(0).setCellValue("IG Title");
    row.createCell(1).setCellValue("IG Name");
    row.createCell(2).setCellValue("Link");
    row.createCell(3).setCellValue("Creation date");
    row.createCell(4).setCellValue("Last accessed");
    row.createCell(5).setCellValue("Last updated");
    row.createCell(6).setCellValue("Nb documents");
    row.createCell(7).setCellValue("Size of documents (Mb)");
    row.createCell(8).setCellValue("Size of information files (Mb)");
    row.createCell(9).setCellValue("Nb events/meetings");
    row.createCell(10).setCellValue("Nb posts");
    row.createCell(11).setCellValue("Deepness");
    row.createCell(12).setCellValue("Nb Members");
    row.createCell(13).setCellValue("Leaders");
    row.createCell(14).setCellValue("Contact Information");
    ...
    for (IgDescriptor ig : computedCategoryGroupStatistics) {
      ...
      rowTmp
        .createCell(7, HSSFCell.CELL_TYPE_NUMERIC)
        .setCellValue(ig.getLibraryDocSize());
      rowTmp.getCell(7).setCellStyle(numberCellStyle);
      rowTmp
        .createCell(8, HSSFCell.CELL_TYPE_NUMERIC)
        .setCellValue(ig.getInformationInfoSize());
    ...
    rowTmp.createCell(2).setCellValue("");
    rowTmp.createCell(3).setCellValue("");
    rowTmp.createCell(4).setCellValue("");
    rowTmp.createCell(5).setCellValue("");
    rowTmp.createCell(11).setCellValue("");
    rowTmp.createCell(13).setCellValue("");
    rowTmp.createCell(14).setCellValue("");

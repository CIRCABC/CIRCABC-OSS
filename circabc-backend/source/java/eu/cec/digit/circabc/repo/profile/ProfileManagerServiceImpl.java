...
public abstract class ProfileManagerServiceImpl
  ...
  private static final Log logger = LogFactory.getLog(
    ProfileManagerServiceImpl.class
  );
  ...
  private QName getProfileAssocQName() {
    final QName profileAssocQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "ProfileAssoc"
    );
  ...
  private QName getProfileQName() {
    ...
    final QName profileQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "Profile"
    );
    return profileQName;
  }
  ...
  private QName getProfileGroupNameQName() {
    ...
    final QName profileGroupNameQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "ProfileGroupName"
    );
  ...
  private QName getServiceAssocQName() {
    final QName serviceAssocQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "ServiceAssoc"
    );
  ...
  private QName getServiceQName() {
    /** node type for the profile */
    final QName serviceQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "Service"
    );
    return serviceQName;
  }
  ...
  private QName getPermissionSetQName() {
    ...
    final QName servicePermissionSetQName = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "PermissionSet"
    );
    return servicePermissionSetQName;
  }

  // Profile
  private NodeRef createProfileNode(
    ...
    if (
      !CircabcConstant.GUEST_AUTHORITY.equalsIgnoreCase(
      ...
      if (profile.getPrefixedAlfrescoGroupName() == null) {
        prefixedProfileGroupName = authorityService.getName(
          AuthorityType.GROUP,
          profile.getAlfrescoGroupName()
        );
      } else {
    ...
    final ChildAssociationRef profileAssociation = nodeService.createNode(
      nodeRef,
  ...
  private NodeRef updateProfileNode(
    final NodeRef nodeRef,
    final Profile profile
  ) {
    final NodeRef profileNodeRef = getProfileNode(
      nodeRef,
      profile.getProfileName()
    );
    if (profileNodeRef == null) {
      ...
      try {
        ...
        if (
          curentPrefixedProfileGroupName != null &&
  ...
  private void deleteProfileNode(
    final NodeRef nodeRef,
  ...
  private NodeRef getProfileNode(
    final NodeRef nodeRef,
    final String profileName
  ) {
    ...
    for (final ChildAssociationRef profileAssoc : listOfProfilesAssoc) {
      ...
      currentProfileName = (String) nodeService.getProperty(
        profileNodeRef,
        getProfileNameQName()
      );
  ...
  private Map<String, NodeRef> getProfileNodesMap(final NodeRef nodeRef) {
    ...
    for (final ChildAssociationRef profileAssoc : listOfProfilesAssoc) {
      ...
      profileName = (String) nodeService.getProperty(
        profileNodeRef,
        getProfileNameQName()
      );
  ...
  private NodeRef createServiceNode(
    ...
    final ChildAssociationRef serviceAssociation = nodeService.createNode(
      profileNodeRef,
    ...
    if (result != null) {
      if (result.size() != 0 && result.size() == profileNodes.size()) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "WORK ARROUND: REMOVE EMPTY CACHE PROFILE MAP for node:" + key
          );
        }
  ...
  public Profile updateProfile(
    ...
    if (!getProfileMap(nodeRef).containsKey(profileName)) {
      ...
      profile.clearNodeServicesPermissions(
        nodeRef,
        filteredServicesPermissions.keySet(),
        serviceRegistry
      );
      profile.setServicesPermissions(filteredServicesPermissions);
      profile.setNodeServicesPermissions(
        nodeRef,
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "updateProfile(profile:" + profileName + ", node:" + nodeRef + ")"
      );
    }
    return profile;
  }
  ...
  public void updateProfileMap(
    ...
    NodeRef serviceNodeRef;
    final Map<String, Set<String>> servicesPermissions =
      newProfile.getServicesPermissions();
    for (final Map.Entry<
      String,
      Set<String>
  ...
  private Profile addProfileImpl(
    ...
    if (
      getProfileMap(nodeRef) != null &&
      getProfileMap(nodeRef).containsKey(profileName)
    ) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "addProfile(profile:" + profileName + ", node:" + nodeRef + ")"
      );
    }
  ...
  public void deleteProfile(
      ...
      if (authorityService.authorityExists(alfrescoGroupName)) {
        ...
        if (!profile.isImported()) {
          if (!authorithyContainedInProfile.isEmpty()) {
            throw new ProfileException(
              profileName,
              "Tried to delete non empty profile!!"
            );
          }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "deleteProfile(profile:" + profileName + ", node:" + nodeRef
      );
    }
  }
  ...
  public Profile getProfile(final NodeRef nodeRef, final String profileName) {
    ...
    if (profile == null) {
      throw new ProfileException(
        profileName,
        "Profile not exist " + " for profiles: " + profileMap
      );
    } else {
      return profile;
    }
  ...
  public Profile getProfileNoCache(
    final NodeRef nodeRef,
    final String profileName
  ) {
    profileMapCache.remove(nodeRef.toString());
  ...
  private Profile createProfile(final NodeRef profileNodeRef) {
    ...
    try {
      ...
      if (
        !CircabcConstant.GUEST_AUTHORITY.equals(curentPrefixedProfileGroupName)
      ) {
        curentProfileGroupName = curentPrefixedProfileGroupName.substring(
          GROUP_UNDESCORE.length(),
      ...
      profile.setDescriptions(
        (MLText) properties.get(ContentModel.PROP_DESCRIPTION)
      );
  ...
  public void renameProfile(
    ...
    if (oldProfileName.contains(":") || newProfileName.contains(":")) {
      throw new ProfileException(
        "renameProfile",
        "profileName should not contains semicolon"
      );
    }
    // Remark: even if the associated profile is renamed, the associated
    // group name is left unchanged
    ...
    if (profile.isImported()) {
      throw new ProfileException(
        oldProfileName,
        "Impossible rename an imported profile"
      );
    } else {
   ...
   */
  private void clearPermissionRecursively(
    final NodeRef nodeRef,
  ...
  public String getRegistredGroupName(final NodeRef nodeRef) {
    if (hasParentSubsGroup()) {
      ...
      final String subsGroupName = profileManagerService.getSubsGroupName(
        parentNodeRef
      );
  ...
  public void addApplicantPerson(
    ...
    if (hasApplicantFeature()) {
      ...
      Map<String, Applicant> applicantMap = (Map<
          String,
          Applicant
        >) nodeService.getProperty(nodeRef, getApplicantUsersMapQName());
      if (applicantMap == null) {
        applicantMap = new HashMap<>();
      }
      applicantMap.put(
        userID,
        new Applicant(userID, date, message, firstName, lastName)
      );
      nodeService.setProperty(
        nodeRef,
        getApplicantUsersMapQName(),
        (Serializable) applicantMap
      );

      if (logger.isDebugEnabled()) {
        final String igName = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_NAME
        );
  ...
  public void removeApplicantPerson(final NodeRef nodeRef, final String userID)
    ...
    if (hasApplicantFeature()) {
      ...
      final Map<String, Applicant> applicantMap = (Map<
          String,
          Applicant
        >) nodeService.getProperty(nodeRef, getApplicantUsersMapQName());
  ...
  public final void addPersonToProfile(
    ...
    if (
      ...
      throw new ProfileException(
        profileName,
        "User " + userID + " already has a profile in the group"
      );
    }
  ...
  public final void changePersonProfile(
    ...
    if (profile != null) {
      ...
      if (originalProfileGroupName == null) {
        ...
        throw new ProfileException(
          targetProfileName,
          "movePerson failed because original group wasn't found"
        );
      }
  ...
  public void uninvitePerson(
    ...
    if (userProfileGroup == null) {
      throw new ProfileException(
        userID,
        "User doesn't belong to the user group"
      );
    }
  ...
  public final String getPersonProfile(
    final NodeRef nodeRef,
    final String userID
  ) {
    if (getProfilePrefix() == null) {
    ...
    for (final Profile profile : profileMap.values()) {
      if (profile.getAlfrescoGroupName().equals(allCircaUsersGroupName)) {
        ...
        containingAuthoritiesForUser.contains(
  ...
  public final String getPersonProfileGroup(
    final NodeRef nodeRef,
    final String userID
  ) {
    if (getProfilePrefix() == null) {
    ...
    containingAuthoritiesForUser.remove(
      GROUP_UNDESCORE + ALL_CIRCA_USERS_PROFILE_NAME
    );
  ...
  public final Set<String> getPersonInProfile(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "getPersonInProfile(profileName:" + profileName + ", node:" + nodeRef
      );
    }
    final Map<String, Profile> profileMap = getProfileMap(nodeRef);
    ...
    for (final Profile profile : profileMap.values()) {
      if (profileName.equals(profile.getProfileName())) {
        ...
        if (
          ...
          CircabcConstant.GUEST_AUTHORITY.equals(
            profile.getPrefixedAlfrescoGroupName()
          )
        ) {
          break;
        ...
        if (
          ...
          personsInProfile = authorityService.getContainedAuthorities(
            AuthorityType.USER,
            profile.getPrefixedAlfrescoGroupName(),
            true
          );
          ...
          if (logger.isWarnEnabled()) {
            logger.warn(
              "This group don't exist anymore:" +
              profile.getPrefixedAlfrescoGroupName()
            );
          }
        }
  ...
  public String createMasterGroup(final NodeRef nodeRef) {
    ...
    if (hasMasterParentGroup()) {
      ...
      parentSubsGroupName = parentProfileManagerService.getSubsGroupName(
        parentNodeRef
      );
    ...
    String createdAuthority = authorityService.createAuthority(
      AuthorityType.GROUP,
      masterGroupName,
      masterGroupName,
  ...
  public final Set<String> getInvitedUsers(final NodeRef nodeRef) {
    ...
    if (cachedUserSet == null) {
      ...
      if (authorityService.authorityExists(prefixedUserGroupName)) {
        ...
        if (logger.isErrorEnabled()) {
          logger.error(
            "Error get contained authorities for group " + prefixedUserGroupName
          );
        }
      }
    } else {
  ...
  public Map<String, Profile> getInvitedUsersProfiles(NodeRef rootNode) {
    ...
    if (authorityService.authorityExists(prefixedUserGroupName)) {
      groupSet = authorityService.getContainedAuthorities(
        AuthorityType.GROUP,
        prefixedUserGroupName,
        true
      );
    ...
    for (String group : groupSet) {
      if (authorityService.authorityExists(prefixedUserGroupName)) {
        Set<String> usersSet = authorityService.getContainedAuthorities(
          AuthorityType.USER,
          group,
          true
        );
        for (String user : usersSet) {
          final String profileKey = getProfileFromAlfrescoPrefixedGroupName(
            group
          );
    ...
    final List<Profile> importedProfiles = getImportedProfilesRecursivly(
      rootNode
    );
  ...
  public final Set<String> getAllSubUsers(final NodeRef nodeRef) {
    ...
    final String userGroupName = parentProfileManagerService.getSubsGroupName(
      parentNodeRef
    );
    ...
    if (authorityService.authorityExists(prefixedUserGroupName)) {
      parentSubsGroupUsersSet = authorityService.getContainedAuthorities(
        AuthorityType.USER,
        prefixedUserGroupName,
        false
      );
  ...
  public String getMasterInvitedGroupName(final NodeRef nodeRef) {
    final String allUsersGroupName = (String) nodeService.getProperty(
      nodeRef,
      getMasterGroupQName()
    );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "return:" +
        allUsersGroupName +
        " + getMasterInvitedGroupName(node:" +
        nodeRef
      );
    }
  ...
  public final String getInvitedUsersGroupName(final NodeRef nodeRef) {
    final String invitedUsersGroupName = (String) nodeService.getProperty(
      nodeRef,
      getInvitedUsersGroupQName()
    );
  ...
  public String getSubsGroupName(final NodeRef nodeRef) {
    final String subsGroupName = (String) nodeService.getProperty(
      nodeRef,
      getSubsGroupQName()
    );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "return:" + subsGroupName + " + getSubsGroupName(node:" + nodeRef
      );
    }
    return subsGroupName;
  }
  ...
  public String getApplicantUsersMapName(final NodeRef nodeRef) {
    final String applicantGroupName = (String) nodeService.getProperty(
      nodeRef,
      getApplicantUsersMapQName()
    );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "return:" +
        applicantGroupName +
        " + getApplicantUsersGroupName(node:" +
        nodeRef
      );
    }
  ...
  public final QName getMasterGroupQName() {
    ...
    final QName allUsersGroup = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "MasterGroup"
    );
  ...
  public final QName getInvitedUsersGroupQName() {
    ...
    final QName invitedUsersGroup = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "InvitedUsersGroup"
    );
  ...
  public final QName getSubsGroupQName() {
    ...
    final QName subsGroup = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "SubsGroup"
    );
    return subsGroup;
  }
  ...
  public final QName getApplicantUsersMapQName() {
    ...
    final QName applicantUsersGroup = QName.createQName(
      CIRCABC_CONTENT_MODEL_1_0_URI,
      getProfilePrefix() + "ApplicantUsersProperty"
    );
  ...
  public Profile getProfileFromGroup(
    final NodeRef nodeRef,
  ...
  public void importProfile(
    ...
    if (targetProfile.isExported() == false) {
      throw new ProfileException(
        fromProfileName,
        "Impossible to import an Profile not exported"
      );
    }

    final Serializable fromIgName = nodeService.getProperty(
      fromIgNoderef,
      ContentModel.PROP_NAME
    );
    ...
    final Profile prof = addProfileImpl(
      toIgNoderef,
      newProfileName,
    ...
    for (final ChildAssociationRef assoc : nodeService.getChildAssocs(
      ...
      childService = profileManagerServiceFactory.getProfileManagerService(
        childRef
      );

      if (childService != null) {
        childService.updateProfile(
          childRef,
          prof.getProfileName(),
          servicesPermissions,
          true
    ...
    NodeRef impotedNodeRef = getProfileNodesMap(toIgNoderef).get(
      newProfileName
    );
  ...
  public void exportProfile(
    ...
    if (profile != null) {
      if (profile.isImported()) {
        ...
        throw new ProfileException(
          profileName,
          "Impossible to export an imported Profile"
        );
      } else {
        if (export) {
          ...
          if (profile.isExported() == true) {
            ...
            final List<Profile> profiles = getImportedProfilesRecursivly(
              parentNode
            );
            ...
            if (profiles.contains(profile)) {
              throw new ProfileException(
                profileName,
                "Impossible to unexport: the Profile is imported"
              );
            } else {
              final NodeRef profileNodeRef = getProfileNode(
                nodeRef,
   ...
   * @return
   */
  public boolean isProfileDeletable(
    final NodeRef nodeRef,
    final String profileName
  ) {
   ...
   * @return
   */
  public List<NodeRef> getRefInExportedProfile(
    final NodeRef nodeRef,
    final String profileName
  ) {
    final Profile profile = getProfile(nodeRef, profileName);
    ...
    for (final NodeRef ig : managementService.getInterestGroups(category)) {
      if (ig.equals(nodeRef) == false) {
        for (final Profile prof : getProfileManagerServiceFactory()
          ...
          if (
            prof.isImported() &&
            prof.getPrefixedAlfrescoGroupName().equals(authority)
          ) {
            refs.add(ig);
          }
        }
  ...
  protected void resetCache() {
    /*PermissionUtils.resetCache(
      circabcDynamicAuthorityCache,
      logger,
      PermissionUtils.CIRCABC_CACHE_NAME
    );*/
  }
  ...
  public void setInvitedUsersCache(
    SimpleCache<String, Set<String>> invitedUsersCache
  ) {

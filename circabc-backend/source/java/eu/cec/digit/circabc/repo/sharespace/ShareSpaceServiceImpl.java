...
public class ShareSpaceServiceImpl implements ShareSpaceService {
  ...
  private void validateInviteParameters(
    ...
    final NodeRef currentInterestGroup =
      managementService.getCurrentInterestGroup(shareSpace);
    ...
    final NodeRef shareSpaceCategory = managementService.getCurrentCategory(
      shareSpace
    );
    final NodeRef interestGroupCategory = managementService.getCurrentCategory(
      interestGroup
    );
  ...
  public List<NodeRef> getAvailableShareSpaces(NodeRef space) {
    ...
    try {
      if (useLucene) {
        String luceneQuery = buildLuceneQueryAvailableShareSpaces(
          interestGroup,
        ...
        for (final ResultSetRow row : resultSet) {
          ...
          final ChildAssociationRef primaryParent =

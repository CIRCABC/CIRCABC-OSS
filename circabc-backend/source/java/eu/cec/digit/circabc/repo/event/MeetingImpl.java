  ...
  public void init(Map<QName, Serializable> properties) {
    ...
    if (availability != null) {
      this.setAvailability(
          MeetingAvailability.valueOf(availability.toString())
        );
    }
    ...
    if (this.getAudienceStatus() == AudienceStatus.Closed) {
      Serializable propertyAcceptedUsers = properties.get(
      ...
      if (propertyAcceptedUsers != null) {
        ...
        if (acceptedUsersList.length() > 1) {
          acceptedUsersList = acceptedUsersList.substring(
            1,
            acceptedUsersList.length() - 1
          );
      ...
      Serializable propertyRejectedUsers = properties.get(
      ...
      if (propertyRejectedUsers != null) {
        ...
        if (rejectedUsersList.length() > 1) {
          rejectedUsersList = rejectedUsersList.substring(
            1,
            rejectedUsersList.length() - 1
          );

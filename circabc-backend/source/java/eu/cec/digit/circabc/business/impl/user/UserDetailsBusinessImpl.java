...
public class UserDetailsBusinessImpl implements UserDetailsBusinessSrv {
  ...
  private static final String MSG_NOT_EXISTS_USER =
    "business_validation_not_a_existing_user";
  private static final String MSG_NO_USERNAME =
    "business_validation_no_username";
  private static final String MSG_CANNOT_LOAD =
    "business_error_cannot_load_user_details";
  ...
  public UserDetails getUserDetails(String userName) {
    ...
    if (userName == null || userName.length() < 1) {
      ...
      (userInRepository = userManager.personExists(userName)) == false &&
  ...
  public void updateUserDetails(
    ...
    final Map<QName, Serializable> propertiesToUpdate =
      userDetails.getUpdatedProperties();
    final Map<QName, Serializable> preferencesToUpdate =
    ...
    if (propertiesToUpdate != null && propertiesToUpdate.size() > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          propertiesToUpdate.size() +
          " properties setted for user: " +
          userDetails.getUserName()
        );
      }
    } else if (logger.isDebugEnabled()) {
      logger.debug(
        "No properties to update for user: " + userDetails.getUserName()
      );
    }

    if (preferencesToUpdate != null && preferencesToUpdate.size() > 0) {
      final NodeRef prefRef = userManager.createUserPreferencesRef(
        userDetails.getNodeRef()
      );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          preferencesToUpdate.size() +
          " preferences setted for user: " +
          userDetails.getUserName()
        );
      }
    } else if (logger.isDebugEnabled()) {
      logger.debug(
        "No preferences to update for user: " + userDetails.getUserName()
      );
    }
  }
  ...
  private NodeRef updateAvatar(
    ...
    if (
      nodeService.hasAspect(person, ContentModel.ASPECT_PREFERENCES) == false
    ) {
    ...
    if (imageRef == null) {
      imageToLink = contentManager.createContent(
        person,
        avatarFileName,
  ...
  private void removeChildAssociations(
    final NodeRef person,
   ...
   */
  public final void setValidationManager(
  ...
  public final void setRemoteUserBusinessSrv(
    RemoteUserBusinessSrv remoteUserBusinessSrv
  ) {

...
public class CociContentBusinessImpl implements CociContentBusinessSrv {
  ...
  public NodeRef checkOut(final NodeRef nodeRef) {
    ValidationUtils.assertDocument(
      nodeRef,
    ...
    final NodeRef workingCopyRef = getCheckOutCheckInService()
      .checkout(nodeRef);

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node successfully locked and checked out. Created working copy: " +
        workingCopyRef
      );
    }
   ...
   */
  public NodeRef checkOutForWorkflow(
    ...
    if (task != null) {
      ...
      if (workflowPackage != null) {
        ...
        final QName validQname = QName.createQName(
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Added working copy to workflow package: " + workflowPackage
          );
        }
      } else {
        logger.error(
          "No workflow package found for the task " +
          workflowTaskId +
      ...
      throw new CheckOutCheckInServiceException(
        "Invalid workflow task ID: " + workflowTaskId
      );
    }
  ...
  public NodeRef cancelCheckOut(final NodeRef workingCopy) {
    ValidationUtils.assertWorkingCopyDocument(
      workingCopy,
      getValidationManager(),
      logger
    );

    final NodeRef originalRef = getCheckOutCheckInService()
      .cancelCheckout(workingCopy);

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node " +
        originalRef +
        " not longer locked. The working copy is deleted."
      );
    }
  ...
  public NodeRef checkIn(
    ...
    final String contentUrl =
      (contentData == null ? null : contentData.getContentUrl());
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Document checked in with properties: " +
        "\n\t Version Desc: " +
        props.get(Version.PROP_DESCRIPTION) +
        "\n\t Version type: " +
        props.get(VersionModel.PROP_VERSION_TYPE)
      );
  ...
  public void update(
    ...
    NodeRef parentRef = getNodeService()
      .getPrimaryParent(document)
      .getParentRef();

    NodeRef existingNodeRef = getNodeService()
      .getChildByName(parentRef, ContentModel.ASSOC_CONTAINS, uploadedfilename);

    String newUploadedfilename = uploadedfilename;

    if (existingNodeRef != null) {
      newUploadedfilename = metadataManager.getValidUniqueName(
        parentRef,
        uploadedfilename
      );
    }
    ...
    getNodeService()
      .setProperty(document, ContentModel.PROP_NAME, newUploadedfilename);

    // also update the mime type in case a different type of file is uploaded
    final String mimeType = getMetadataManager()
      .guessMimetype(uploadedfilename);
  ...
  public NodeRef getWorkingCopy(final NodeRef lockedRef) {
    ValidationUtils.assertLockedDocument(
      lockedRef,
      getValidationManager(),
      logger
    );

    final NodeRef workingCopy = getCheckOutCheckInService()
      .getWorkingCopy(lockedRef);
  ...
  public NodeRef getWorkingCopyOf(final NodeRef workingCopyRef) {
    ...
    final NodeRef lockedRef = checkOutCheckInService.getCheckedOut(
      workingCopyRef
    );
   ...
   */
  public final void setCheckOutCheckInService(

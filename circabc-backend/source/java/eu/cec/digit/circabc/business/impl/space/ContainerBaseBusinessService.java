...
public abstract class ContainerBaseBusinessService {
  ...
  private final Log logger = LogFactory.getLog(
    ContainerBaseBusinessService.class
  );
  ...
  protected NodeRef createContainerImpl(
    final NodeRef parent,
    final QName type,
    final String title,
    final String description,
    final ContainerIcon icon,
    ...
    final String validName = getMetadataManager()
      .getValidUniqueName(parent, title);

    final FileInfo fileInfo = getFileFolderService()
      .create(parent, validName, type);
    final NodeRef nodeRef = fileInfo.getNodeRef();

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Created " + type.getLocalName() + " node with name: " + validName
      );
    }
    ...
    if (expirationDate != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Added expiration property (Without associated aspect): " +
          expirationDate
        );
      }
    }

...
public abstract class ExceptionHelper {
  ...
  private static final Pattern SPLIT_UPPER_LOWER_CASE = Pattern.compile(
    "[A-Z]+[a-z]*"
  );
      ...
      if (navigationBusinessSrv != null) {
        ...
        if (args != null && args.length > 0 && args[0] != null) {
          ...
          if (firstArg instanceof NodeRef) {
            relevantArgString = navigationBusinessSrv.getNodePathString(
              (NodeRef) firstArg
            );
          } else if (firstArg instanceof ChildAssociationRef) {
            relevantArgString = navigationBusinessSrv.getNodePathString(
              ((ChildAssociationRef) firstArg).getChildRef()
            );
          } else if (firstArg instanceof NavNode) {
            relevantArgString = navigationBusinessSrv.getNodePathString(
              ((NavNode) firstArg).getNodeRef()
            );
          } else {
  ...
  public static String getFriendlyCause(final Throwable error) {
    ...
    final Matcher matcher = SPLIT_UPPER_LOWER_CASE.matcher(
    ...
    final String cause = error.getCause() == null
      ? null

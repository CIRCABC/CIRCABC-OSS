...
public class LinksBusinessImpl implements LinksBusinessSrv {
  ...
  public NodeRef createLink(final NodeRef parent, final NodeRef target) {
    ...
    final String title =
      LINK_TO_PREFIX +
      getNodeService().getProperty(target, ContentModel.PROP_TITLE);
    final ChildAssociationRef assocRef = getNodeService()
      .getPrimaryParent(target);
    ...
    if (getNodeTypeManager().isContent(target)) {
      ...
      applyUIFacet(
        ...
        title,
        title,
        getSpaceLinkIcons().get(0).getIconName()
      );

      if (logger.isDebugEnabled()) {
        logger.debug(
          "Link to content created with name: " + name + " and title" + title
        );
      }
    } else {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Link to space created with name: " + name + " and title" + title
        );
      }
    }
  ...
  public NodeRef createSharedSpaceLink(
    ...
    final NodeRef targetSharedSpace,
    final String title,
    ...
    final NodeRef link = getShareSpaceService()
      .linkSharedSpace(parent, targetSharedSpace, name);

    applyUIFacet(link, title, name, getSpaceLinkIcons().get(0).getIconName());

    if (logger.isDebugEnabled()) {
      logger.debug(
        "Link to share space created with name: " + name + " and title" + title
      );
    }
  ...
  public List<ShareSpaceItem> getAvailableSharedSpaces(final NodeRef nodeRef) {
    ...
    final List<NodeRef> allSharedSpaces = getShareSpaceService()
      .getAvailableShareSpaces(nodeRef);
   ...
   */
  public List<InterestGroupItem> getInterestGroupForSharing(
    ...
    final List<NodeRef> interestGroups = getShareSpaceService()
      .getAvailableInterestGroups(nodeRef);

    if (interestGroups == null || interestGroups.size() < 1) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "All interest group of the category have been invited: " + nodeRef
        );
      }

      return Collections.emptyList();
    } else {
      final List<InterestGroupItem> items = new ArrayList<>(
        interestGroups.size()
      );
   ...
   */
  public List<InterestGroupItem> getInvitationsForSharing(
    final NodeRef nodeRef
  ) {
    ValidationUtils.assertLibrarySpace(nodeRef, getValidationManager(), logger);
  ...
  public List<ContainerIcon> getSpaceLinkIcons() {
    final List<ContainerIcon> icons = getConfigManager()
      .getIcons(ApplicationModel.TYPE_FOLDERLINK);
  ...
  private List<ShareSpaceItem> buildSharedSpaceItems(
    ...
    for (final NodeRef ref : allSharedSpaces) {
      if (checkIfOneInvitedInterestGroupExists(ref)) {
        ...
        wrappers.add(
          new ShareSpaceItem(ref, PathUtils.getCategoryPath(path, true))
        );
      }
    }
  ...
  private boolean checkIfOneInvitedInterestGroupExists(NodeRef shareSpace) {
    ...
    if (
      getNodeService().hasAspect(shareSpace, CircabcModel.ASPECT_SHARED_SPACE)
    ) {
  ...
  private void applyUIFacet(
    ...
    getNodeService()
      .addAspect(ref, ApplicationModel.ASPECT_UIFACETS, titledProps);
  }

  private void assertLinkCreation(
    final NodeRef parent,
  ...
  private void assertInvitation(
    ...
    try {
      ValidationUtils.assertLibrarySpace(
        sharedSpace,
  ...
  private InterestGroupItem buildInterestGroupItem(
    final NodeRef interestGroup,
    final String permission
  ) {
    final Map<QName, Serializable> props = getNodeService()
      .getProperties(interestGroup);
  ...
  private String getLinkName(final NodeRef parent, final NodeRef target) {
    final String targetName = (String) getNodeService()
      .getProperty(target, ContentModel.PROP_NAME);

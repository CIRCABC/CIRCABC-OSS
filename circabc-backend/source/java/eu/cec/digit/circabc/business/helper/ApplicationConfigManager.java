...
public class ApplicationConfigManager {
  ...
  private Map<Set<QName>, Map<String, ItemConfig>> propertyConfigs =
  ...
  public String getEncoding() {
    ...
    if (this.encoding == null || isDynamicConfig()) {
      this.encoding = extractConfigValue(
        CONFIG_CONTENT_WIZARDS,
        ELEM_DEFAULT_ENCODING
      );
      if (this.encoding == null || this.encoding.length() == 0) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Default encoding not configurend in config '" +
            CONFIG_CONTENT_WIZARDS +
            "', element '" +
            ELEM_DEFAULT_ENCODING +
            "'"
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Application configured default encoding: " + this.encoding
          );
        }
      }
    }
  ...
  public List<String> getInlineEditableMimeTypes() {
    ...
    if (this.inlineEditableMimeTypes == null || isDynamicConfig()) {
      this.inlineEditableMimeTypes = extractConfigAttributes(
        CONFIG_CONTENT_WIZARDS,
        ELEM_CREATE_MIME_TYPES,
        ATTR_NAME
      );
      ...
      if (this.inlineEditableMimeTypes == null) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Default inline editables mimetypes not configurend in config '" +
            CONFIG_CONTENT_WIZARDS +
            "', element '" +
            ELEM_CREATE_MIME_TYPES +
            "', attributes '" +
      ...
      if (this.inlineEditableMimeTypes.size() == 0) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Application configured inline editable mimetypes: " +
            this.inlineEditableMimeTypes
          );
        }
      }
    }
  ...
  public List<ContainerIcon> getIcons(final QName nodeTypeQname) {
    ...
    if (!this.icons.containsKey(nodeTypeQname) || isDynamicConfig()) {
      final String typePrefixForm = nodeTypeQname.toPrefixString(
        getNamespaceService()
      );
      ...
      final List<Pair<String, String>> iconConfigs = extractConfigAttributes(
        configName,
        ELEM_ICON,
        ATTR_NAME,
        ATTR_PATH
      ...
      if (iconConfigs == null) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "No icon config found for node type " +
            nodeTypeQname.toString() +
            ". Please to fill '" +
            configName +
            "' configuration name."
          );
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Icons for node type " + nodeTypeQname + ": " + wrappers
          );
        }
      }
   ...
   */
  public Map<String, ItemConfig> getPropertiesDefinition(
    ...
    if (
      !this.propertyConfigs.containsKey(typesAndAspects) || isDynamicConfig()
    ) {
      final PropertySheetConfigElement configElement =
        (PropertySheetConfigElement) getConfigElement(
          node,
          ELEM_PROPERTY_SHEET
        );
      ...
      if (itemConfigs.size() > 0) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Property definition found for node (" +
            nodeRef +
            "): " +
            itemConfigs.keySet()
          );
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "No property sheet configuration found for node (" +
            nodeRef +
            ")" +
            "\n\tWith type:   " +
            node.getType() +
  ...
  private String extractConfigValue(
    final Object configId,
    final String configElementName
  ) {
    final ConfigElement configElement = getConfigElement(
      configId,
      configElementName
    );
    if (configElement != null) {
  ...
  private List<String> extractConfigAttributes(
    final Object configId,
    final String configElementName,
  ...
  private List<Pair<String, String>> extractConfigAttributes(
    final Object configId,
    final String configElementName,
    final String firstAttributeName,
  ...
  private ConfigElement getConfigElement(
    final Object configId,
    final String configElementName
  ) {

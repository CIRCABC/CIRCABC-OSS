...
public class MailMeContentBusinessImpl implements MailMeContentBusinessSrv {
  ...
  public boolean send(
    ...
    try {
        ...
        if (attachContent) {
          final MailWrapper mail =
            mailPreferencesService.getDefaultMailTemplate(
              targetIfLink,
              MailTemplate.MAIL_ME_CONTENT
            );
          return mailService.sendNode(
            targetIfLink,
            getFromAddress(),
            to,
            null,
          ...
          final MailWrapper mail =
            mailPreferencesService.getDefaultMailTemplate(
              targetIfLink,
              MailTemplate.MAIL_ME_CONTENT_URL
            );
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "impossible to send node by email: " + silentError.getMessage(),
          silentError
        );
      }
  ...
  private String getTo(NodeRef person) {
    final String address = (String) userManager.getUserProperty(
      person,
  ...
  private Locale getMailLanguage(NodeRef person) {
    Serializable langObject = userManager.getUserPreference(
      person,

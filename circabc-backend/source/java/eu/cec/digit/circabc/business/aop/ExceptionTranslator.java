...
public class ExceptionTranslator implements MethodInterceptor {
  ...
  public void registerHandler(final ExceptionHandler handler, Class clazz) {
    if (clazz == null) {
      logger.error(
        "Handler not registered beacause it doens't provide managed error type: " +
        handler.getClass()
      );
    } else {
    ...
    try {
      ...
      if (handler != null) {
        stack = new BusinessStackError(
          new BusinessRuntimeExpection(
            handler.getMessageKey(error),
        ...
        try {
          ...
          logger.error(
            "Impossible to perform " + handler.getClass() + ".onThrows()",
            t
          );
        }
      } else {
        ...
        stack = new BusinessStackError(
          new BusinessRuntimeExpection(DEFAULT_MESSAGE)
        );
      }
  ...
  private ExceptionHandler getHandler(
    ...
    if (handler == null) {
      for (final Class<
        ? extends Throwable
      > clazz : exceptionHandlers.keySet()) {

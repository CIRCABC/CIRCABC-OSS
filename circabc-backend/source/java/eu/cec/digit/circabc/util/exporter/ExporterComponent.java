...
public class ExporterComponent implements CircabcExporterService {
  ...
  private class DefaultCrawler implements ExporterCrawler {
    ...
    public void export(
      ...
      while (context.canRetrieve()) {
        ...
        for (NodeRef nodeWithAssociations : nodesWithSecondaryLinks.keySet()) {
          String authorityName = (String) nodeService.getProperty(
            nodeWithAssociations,
      ...
      if (logger.isInfoEnabled()) {
        logger.info(
          "Number of exported entities per type :\n" + displayEntitiesPerType()
        );
      }
    }
    ...
    private void walkNode(
      ...
      if (
        !(parameters.getCrawlOnlyGivenUsers() == null &&
        ...
        if (path.size() > 1) {
          ...
          if (
            (ContentModel.TYPE_USER.equals(type) ||
              ContentModel.TYPE_PERSON.equals(type)) &&
            !isIncludedUser(
              parameters.getCrawlOnlyGivenUsers(),
            ...
            ContentModel.TYPE_AUTHORITY_CONTAINER.equals(type) &&
            !isIncludedGroup(
              parameters.getCrawlOnlyGivenGroups(),
              childQName.getLocalName()
            )
          ) {
            return;
        ...
        for (ChildAssociationRef childAssoc : childAssocs) {
          ...
          if (
            (!parameters.isCrawlChildNodes() &&
              ...
              CircabcModel.ASSOC_CIRCA_CATEGORY_PROFILE.equals(
                childAssocType
              )) ||
            parameters.isCrawlChildNodes()
          ) {
      ...
      if (
        ...
        NodeRef versionedContainerNodeRef = nodeService.getChildByName(
          versionStoreRootNoderef,
      ...
      if (hasMLlDocumentAspect) {
        List<ChildAssociationRef> mlContainers = nodeService.getParentAssocs(
          nodeRef,
    ...
    private String getNodeInfo(NodeRef nodeRef, QName type, String nodeName) {
      String log =
        ...
        (nodeName != null
            ? nodeName
            : "<No cm:name was defined for this node>") +
        ", type=" +
        type.getLocalName() +
        "]";
    ...
    private void walkNodeSecondaryLinks(
      ...
      if (assocTypes.size() > 0) {
        ...
        String assocName = (String) nodeService.getProperty(
          nodeRef,
    ...
    private void walkNodeAssociations(
      ...
      for (AssociationRef assoc : assocs) {
        ...
        if (
          !isWithinExport(assoc.getTargetRef(), parameters) &&
    ...
    private boolean isIncludedGroup(
  ...
  private class ExporterContextImpl implements ExporterContext {
    ...
    public ExporterContextImpl(ExporterCrawlerParameters parameters) {
      ...
      exportList = (parameters.getExportFrom() == null)
        ? null

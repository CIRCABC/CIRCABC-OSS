...
public class ExportActionExecuter extends ActionExecuterAbstractBase {
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    ...
    boolean exportAuthorities = (Boolean) action.getParameterValue(
      PARAM_EXPORT_AUTHORITIES
    );
    boolean exportStructure = (Boolean) action.getParameterValue(
      PARAM_EXPORT_STRUCTURE
    );
    boolean exportHeaders = (Boolean) action.getParameterValue(
      PARAM_EXPORT_HEADERS
    );
    boolean exportOnlyRoot = (Boolean) action.getParameterValue(
      PARAM_EXPORT_ONLY_ROOT
    );

    List<ExportItemData> itemsToExport = (List<
        ExportItemData
    ...
    for (ExportItemData exportItemData : itemsToExport) {
      logger.info(
        "Starting export in background. Name: " +
        exportItemData.getName() +
        ", NodeRef: " +
      ...
      try {
        circabcExporter.export(
          exportItemData.getNodeRef(),
          exportItemData.getName(),
          exportDirectory,
          exportAuthorities,
          exportStructure,
        ...
        action.setParameterValue(
          PARAM_CURRENT_STATUS,
          ExportActionStatus.FAILED.toString() + " (" + e.getMessage() + ")"
        );
        continue;
      }
      ...
      action.setParameterValue(
        PARAM_EXE_TIME,
        action.getParameterValue(PARAM_EXE_TIME) + " " + executionTime
      );

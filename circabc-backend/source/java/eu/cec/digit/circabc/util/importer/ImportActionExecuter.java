...
public class ImportActionExecuter extends ActionExecuterAbstractBase {
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    ...
    Date importScheduledStart = (Date) action.getParameterValue(
      PARAM_IMPORT_START
    );
    ...
    if (importScheduledStart != null) {
      action.setParameterValue(
        PARAM_CURRENT_STATUS,
        ExportActionStatus.WAITING.toString() +
        " - Import of '" +
        importName +
        "' scheduled to start on: " +
        importScheduledStart
      );
    ...
    String importDirectory = (String) action.getParameterValue(
      PARAM_IMPORT_DIRECTORY
    );
    boolean importAuthorities = (Boolean) action.getParameterValue(
      PARAM_IMPORT_AUTHORITIES
    );
    boolean importStructure = (Boolean) action.getParameterValue(
      PARAM_IMPORT_STRUCTURE
    );
    boolean importHeaders = (Boolean) action.getParameterValue(
      PARAM_IMPORT_HEADERS
    );
    boolean keepUUIDs = (Boolean) action.getParameterValue(
      PARAM_IMPORT_KEEP_UUIDS
    );
    List<String> packages = (List<String>) action.getParameterValue(
      PARAM_IMPORT_PACKAGES
    );

    if (logger.isInfoEnabled()) {
      logger.info(
        "Starting import in background '" +
        importName +
        "' in NodeRef: " +
    ...
    for (String packagesAsText : packages) {
      ...
      try {
        ...
        circabcImporter.importTo(
          actionedUponNodeRef,
          getPackages(packagesAsText),
          importDirectory,
          importAuthorities,
          importStructure,
          importHeaders,
    ...
    action.setParameterValue(
      PARAM_CURRENT_STATUS,
      buildStatusMessage(logs, errorLogs)
    );
    String completeExecutionTime = getExecutionTime(
      new Date(),
      importStartDate
    );

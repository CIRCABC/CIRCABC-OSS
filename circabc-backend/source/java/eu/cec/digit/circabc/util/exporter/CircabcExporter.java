...
public class CircabcExporter {
  ...
  public void export(
    final NodeRef nodeRefToExport,
    String exportFileName,
    String outputFolderPath,
    boolean exportAuthorities,
    ...
    try {
      ...
      String masterGroupName = (String) nodeService.getProperty(
        nodeRefToExport,
        PROP_CIRCA_CATEGORY_MASTER_GROUP
      );

      String exportNamePrefix = CATEGORY_PREFIX;

      if (masterGroupName == null) {
        masterGroupName = (String) nodeService.getProperty(
          nodeRefToExport,
      ...
      if (exportHeaders && CATEGORY_PREFIX.equals(exportNamePrefix)) {
        List<NodeRef> alfrescoCategories = (List<
            NodeRef
        ...
        if (alfrescoCategories != null && alfrescoCategories.size() == 1) {
          alfrescoCategoryName =
            "/cm:" +
            ISO9075.encode(
              (String) nodeService.getProperty(
                alfrescoCategories.get(0),
  ...
  protected NodeRef findMasterGroupNodeRef(String masterGroupName) {
    ...
    try {
      ...
      if (children.size() != 1) {
        throw new ExporterException(
          "The number of authorities with " +
          "the given name is not one. Given name: " +
          "GROUP_" +
          masterGroupName +
  ...
  private void exportFrom(
    ...
    final boolean exportRootNode,
    final boolean exportChildNodes,
    final boolean createAcp,
    ...
    AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          ...
          parameters.setExcludeAspects(
            new QName[] { CircabcModel.ASPECT_LOCKED_FOR_ACCESS }
          );

          if (userNames != null) {
            // For users export (will be fed in by
            // groups/users/authorities export results from the SpacesStore)
            parameters.setCrawlOnlyGivenUsers(
          ...
          if (groupNames != null) {
            // For group export
            parameters.setCrawlOnlyGivenGroups(
              groupNames.toArray(new String[groupNames.size()])
            );
          }

          try {
            circabcExporterService.exportView(
              exportHandler,
              parameters,
              new ExportProgress()
            );
   ...
   */
  private void getAuthorityNamesTree(
    NodeRef rootNodeRef,
    ...
    List<ChildAssociationRef> childrenRefs = nodeService.getChildAssocs(
      rootNodeRef
    );

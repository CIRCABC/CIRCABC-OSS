...
public class ViewParser implements Parser {
  ...
  private static final QName VIEW_VERSIONS = QName.createQName(
    NamespaceService.REPOSITORY_VIEW_1_0_URI,
    "versions"
  );
  private static final QName VIEW_MLTRANSLATIONS = QName.createQName(
    NamespaceService.REPOSITORY_VIEW_1_0_URI,
    "mlTranslations"
  );
  ...
  // for logging purpose
  private static final List<String> ENTITY_NAMES_TO_LOG = Arrays.asList(
    "cm:Library",
    "cm:Newsgroups",
    "cm:Information",
    "cm:Events"
  );
  private static final List<String> ENTITY_TYPES_TO_LOG = Arrays.asList(
    "folder",
    "forums"
  );
  ...
  private void logEntityStartTag(
    XmlPullParser xpp,
    ...
    if (
      ENTITY_TYPES_TO_LOG.contains((startTag.getLocalName())) &&
      ENTITY_NAMES_TO_LOG.contains((childName))
    ) {
  ...
  private void logEntityEndTag(ParserContext parserContext) {
    ...
    if (element instanceof NodeContext) {
      ...
      if (
        ENTITY_TYPES_TO_LOG.contains(
          (node.getTypeDefinition().getName().getLocalName())
        ) &&
  ...
  private void processMetaData(
    ...
    MetaDataContext metaData =
      (MetaDataContext) parserContext.elementStack.peek();
  ...
  private void processProperty(
    ...
    propertyName = QName.createQName(
  ...
  private void processMultilingual(
    XmlPullParser xpp,
    QName refName,
    ParserContext parserContext
  ) throws XmlPullParserException, IOException {
  ...
  public String displayIncompleteReferences() {
    ...
    for (Entry<
      NodeRef,
      ImportNode
    > reference : incompleteReferences.entrySet()) {
      log
        .append("*** Node[ref=")
        .append(reference.getKey())
        .append("] doesn't exist. Cannot create reference '")
        .append(
          reference.getValue().getParentContext().getAssocType().getLocalName()
        )
        .append("' with parent Node[ref=")
        .append(reference.getValue().getParentContext().getParentRef())
        .append("]\n");
    }

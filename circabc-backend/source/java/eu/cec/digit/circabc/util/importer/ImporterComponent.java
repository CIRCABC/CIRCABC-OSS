 ...
 */
public class ImporterComponent implements CircabcImporterService {
  ...
  private static final List<String> EXCLUDED_NODE_NAMES = Arrays.asList(
    "sys:people",
    "sys:workflow",
    "cm:packages",
    "sys:authorities",
  ...
  public void parserImport(
    ...
    try {
      ...
      if (logger.isInfoEnabled()) {
        logger.info(
          "Number of imported entities per type :\n" +
          ((NodeImporter) nodeImporter).displayEntitiesPerType()
        );
      }
      if (
        viewParser instanceof ViewParser &&
        ((ViewParser) viewParser).hasIncompleteReferences()
      ) {
        logger.warn(
          "Incomplete(s) reference(s):\n" +
          ((ViewParser) viewParser).displayIncompleteReferences()
        );
      }
  ...
  public class NodeImporter implements Importer {
    ...
    private NodeImporter(
      ...
      if (binding == null || binding.getExcludedClasses() == null) {
        this.excludedClasses = new QName[] {
    ...
    private NodeRef linkNode(ImportNode context) {
      ...
      if (
        Version2Model.TYPE_QNAME_VERSION_HISTORY.equals(
          context.getTypeDefinition().getName()
        )
      ) {
        // node from the version2Store - it has been previously created so we return it directly
        referencedRef = new NodeRef(
          getVersion2StoreNodeRef().getStoreRef(),
          uuid
        );
        return referencedRef;
      } else if (
        Version2Model.ASSOC_ROOT_VERSION.equals(
          context.getParentContext().getAssocType()
        )
      ) {
        ...
        if (assocDef.isChild()) {
          ...
          if (ContentModel.ASSOC_MULTILINGUAL_CHILD.equals(assocType)) {
            Map<Locale, NodeRef> translations =
              multilingualContentService.getTranslations(parentRef);
            ...
            if (
              nodeService.getChildByName(parentRef, assocType, nodeName) != null
            ) {
              found = true;
            }
          }
          ...
          if (!found) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                ...
                getAssociationInfo(assocType, true, parentRef, referencedRef)
              );
            }
          } else {
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Association already created : " +
                getAssociationInfo(assocType, true, parentRef, referencedRef)
              );
            }
          }
          ...
          // check if the association already exists
          List<AssociationRef> children = nodeService.getTargetAssocs(
            parentRef,
            new QNamePattern() {
              @Override
          ...
          if (!found) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Association has been created : " +
                getAssociationInfo(assocType, false, parentRef, referencedRef)
              );
            }
          } else {
            if (logger.isDebugEnabled()) {
    ...
    private String getAssociationInfo(
      QName assocType,
      boolean isChildAssoc,
      NodeRef parentRef,
      NodeRef referencedRef
    ) {
      StringBuilder log = new StringBuilder("Association[type=").append(
        assocType.getLocalName()
      );
      if (isChildAssoc) {
    ...
    private void importContent(
      ...
      if (importContentData != null && importContentData.length() > 0) {
        ...
        ContentData contentData =
          (ContentData) DefaultTypeConverter.INSTANCE.convert(
            dataTypeDef,
            importContentData
          );
    ...
    public NodeRef resolvePath(String path) {
      ...
      if (path != null && path.length() > 0) {
        if (path.startsWith(VER2_PREFIX)) {
          referencedRef = resolveImportedNodeRef(
            getVersion2StoreNodeRef(),
            path
          );
        } else {
      ...
      for (ImportedNodeRef importedRef : nodeRefs) {
        ...
        if (importedRef.value != null) {
          if (importedRef.value instanceof Collection) {
            Collection<String> unresolvedRefs = (Collection<
                String
              >) importedRef.value;
            ...
            refProperty = resolveImportedNodeRef(
              importedRef.context.getNodeRef(),
        ...
        Set<QName> disabledBehaviours = getDisabledBehaviours(
          importedRef.context
        );
        try {
    ...
    private QName getChildName(ImportNode context) {
      ...
      if (childName != null) {
        ...
        if (
          ContentModel.TYPE_PERSON.equals(context.getTypeDefinition().getName())
        ) {
    ...
    private Map<QName, Serializable> bindProperties(ImportNode context) {
      ...
      Map<QName, Serializable> boundProperties = new HashMap<>(
        properties.size()
      );
    ...
    private List<AccessPermission> bindPermissions(
      ...
      List<AccessPermission> boundPermissions = new ArrayList<>(
        permissions.size()
      );
    ...
    private Serializable bindValue(
      ...
      if (
        (ModerationModel.PROP_ABUSE_MESSAGES.equals(property) ||
          CircabcModel.PROP_APPLICANTS.equals(property)) &&
        isNotEmpty(value)
      ) {
        try {
          objValue = (Serializable) CommonUtils.fromBase64String(
            (String) value
          );
        } catch (ClassNotFoundException | IOException e) {
          logger.error("Cannot convert value for property " + property, e);
          throw new ImporterException(
            "Cannot convert value for property " + property,
            e
          );
        }
        ...
        if (
          ...
          objValue = (Serializable) DefaultTypeConverter.INSTANCE.convert(
            valueType,
            value
          );
        }
    ...
    private String getNodeInfo(
      ...
      String log =
        "Node[ref=" +
        (node.getNodeRef() != null ? node.getNodeRef() : nodeRef) +
        ", name=" +
        childQName.getLocalName() +
        ", type=" +
        nodeType.getName().getLocalName() +
        ", assocType=" +
        assocType.getLocalName() +
        ", parentRef=" +
        parentRef +
    ...
    private class CreateNewNodeImporterStrategy
      ...
      private boolean isUserOrPerson(ImportNode node) {
        return (
          ContentModel.TYPE_USER.equals(node.getTypeDefinition().getName()) ||
          isPerson(node)
        );
      }

      private boolean isPerson(ImportNode node) {
        return ContentModel.TYPE_PERSON.equals(
          node.getTypeDefinition().getName()
        );
      }

      private boolean isConfigurations(ImportNode node) {
        return ApplicationModel.TYPE_CONFIGURATIONS.equals(
          node.getTypeDefinition().getName()
        );
      }
      ...
      private boolean isPreferences(ImportNode node, QName childQName) {
        return (
          ContentModel.TYPE_CMOBJECT.equals(
            node.getTypeDefinition().getName()
          ) &&
      ...
      private NodeRef doesChildAlreadyExist(
        NodeRef parentRef,
        ...
        if (children != null && children.size() == 1) {
          ...
          throw new RuntimeException(
            new ImporterException(
              "There is more than one child node matching " +
              childQName +
      ...
      public NodeRef importNode(ImportNode node) {
        ...
        if (childQName == null) {
          ...
          isNodeExcluded(childQName.getPrefixString(), EXCLUDED_NODE_NAMES) ||
          // a node already created by the system
          isUserOrPerson(node)
        ) {
          NodeRef childNodeRef = doesChildAlreadyExist(
            parentRef,
            childQName,
          ...
          if (childNodeRef != null) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                ...
                getNodeInfo(node, childNodeRef, childQName)
              );
            }
            return childNodeRef;
          }
        } else if (
          isConfigurations(node) && existingPersons.contains(parentRef)
        ) {
          NodeRef childNodeRef = doesChildAlreadyExist(
            parentRef,
            childQName,
          ...
          isPreferences(node, childQName) &&
          existingConfigurations.contains(parentRef)
        ) {
          NodeRef childNodeRef = doesChildAlreadyExist(
            parentRef,
            childQName,
          ...
          if (uuid != null && uuid.length() > 0) {
            if (
              ...
              node.getProperties().get(ContentModel.PROP_STORE_IDENTIFIER) !=
              null
            ) {
              String protocol = (String) node
                .getProperties()
                .get(ContentModel.PROP_STORE_PROTOCOL);
              String identifier = (String) node
                .getProperties()
                .get(ContentModel.PROP_STORE_IDENTIFIER);
              NodeRef existingNodeRef = new NodeRef(
                getStoreRef(protocol, identifier),
                uuid
              );
              if (nodeService.exists(existingNodeRef)) {
                if (logger.isDebugEnabled()) {
                  logger.debug(
                    "Node already created : " +
                    getNodeInfo(node, existingNodeRef, childQName)
                  );
                }
        ...
        for (QName disabledBehaviour : disabledBehaviours) {
          boolean alreadyDisabled = !behaviourFilter.isEnabled(
            disabledBehaviour
          );
        ...
        if (
          ContentModel.TYPE_PERSON.equals(node.getTypeDefinition().getName())
        ) {
        ...
        if (
          authenticationContext.isCurrentUserTheSystemUser() ||
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Node has been created : " + getNodeInfo(node, nodeRef, childQName)
          );
        }
        return nodeRef;
      }
      ...
      private boolean isNodeExcluded(
        String childQName,
        List<String> excludedNodesNames
      ) {

...
public class ExtensionVariableResolver extends DelegatingVariableResolver {
  ...
  public Object resolveVariable(FacesContext facesContext, String name)
    ...
    if (mbc != null && mbc.getManagedBeanClassName().startsWith("Spring")) {
      ...
      if (wac.containsBean(name)) {
        ...
        List beansUnderConstruction = (List) requestMap.get(
        ...
        try {
          ...
          initializeProperties(
            facesContext,
            mbc.getManagedProperties(),
            mbc.getManagedBeanScope(),
            o
          );
        } finally {
  ...
  protected RuntimeConfig getRuntimeConfig(FacesContext facesContext) {
    if (_runtimeConfig == null) {
      _runtimeConfig = RuntimeConfig.getCurrentInstance(
  ...
  public Object buildManagedBean(
    ...
    switch (beanConfiguration.getInitMode()) {
      case ManagedBean.INIT_MODE_PROPERTIES:
        try {
          initializeProperties(
            facesContext,
            beanConfiguration.getManagedProperties(),
      ...
      case ManagedBean.INIT_MODE_MAP:
        ...
        initializeMap(
          facesContext,
          beanConfiguration.getMapEntries(),
          (Map) bean
        );
        break;
      case ManagedBean.INIT_MODE_LIST:
        ...
        initializeList(
          facesContext,
          beanConfiguration.getListEntries(),
          (List) bean
        );
        break;
  ...
  private void initializeProperties(
    ...
    while (managedProperties.hasNext()) {
      ...
      if (property.getPropertyClass() == null) {
        propertyClass = propertyResolver.getType(
          bean,
          property.getPropertyName()
        );
      } else {
        propertyClass = ClassUtils.simpleJavaTypeToClass(
  ...
  private void initializeMap(
    ...
    for (Iterator iterator = mapEntries.getMapEntries(); iterator.hasNext();) {
      ...
      if (entry.isNullValue()) {
        ...
        map.put(
          ClassUtils.convertToType(key, keyClass),
          ClassUtils.convertToType(value, valueClass)
        );
      }
    }
  }
  ...
  private boolean isInValidScope(
    ...
    String[] expressions = extractExpressions(
    ...
    for (String expression : expressions) {
      ...
      if (targetScope.equalsIgnoreCase("application")) {
        if (valueScope != null) {
          if (
            valueScope.equalsIgnoreCase("request") ||

...
public class CircabcImporter {
  ...
  public void importTo(
    ...
    final boolean importAuthorities,
    final boolean importStructure,
    final boolean importHeaders,
    ...
    try {
      ...
      logger.error(
        "Failed to import using RetryingTransactionHelper : " + e.getMessage()
      );
      //throw e;
      ...
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          ...
          if (importAuthorities) {
            StoreRef alfrescoUserStore = new StoreRef(
              "user",
              "alfrescoUserStore"
            );
            NodeRef userStoreRootNodeRef = nodeService.getRootNode(
              alfrescoUserStore
            );
            ...
            // users
            doImport(
              userStoreRootNodeRef,
              "/sys:system/sys:people",
              null,
              importFolderPath,
              getPackage(packages, "Authorities_alfrescoUserStore"),
              keepUUIDs,
              true
            );
            ...
            doImport(
              ...
              null,
              importFolderPath,
              getPackage(packages, "Authorities_SpacesStore"),
              keepUUIDs,
              true
            );
          }
          ...
          if (importStructure) {
            ...
            String entityMasterGroupName = getEntityMasterGroupName(
              importFolderPath,
              getPackage(packages, "Nodes_SpacesStore")
            );
  ...
  private String getEntityMasterGroupName(
    ...
    FileImportPackageHandler importHandler = new FileImportPackageHandler(
      sourceDir,
      dataFile,
      "UTF-8"
    );
      ...
      try {
        for (
          ...
          if (eventType == XmlPullParser.START_TAG) {
            if (
              CAT_MASTERGROUP.equals(xpp.getName()) ||
              IG_MASTERGROUP.equals(xpp.getName())
            ) {
              eventType = xpp.next();
    ...
    if (entityMasterGroup == null) {
      throw new ImporterException(
        "The MasterGroup authority of the exported entity was not found in " +
        packageFileName
      );
    }
  ...
  private void addAuthority(
    NodeRef parentNode,
    final String childNodeMasterGoupAuthName
  ) {
    final String parentNodeSubsGroupAuthName;
    if (nodeService.getProperty(parentNode, PROP_CIRCABC_SUBGROUP) != null) {
      parentNodeSubsGroupAuthName = (String) nodeService.getProperty(
        parentNode,
        PROP_CIRCABC_SUBGROUP
      );
    } else if (
      nodeService.getProperty(parentNode, PROP_CIRCA_CATEGORY_SUBGROUP) != null
    ) {
      parentNodeSubsGroupAuthName = (String) nodeService.getProperty(
        parentNode,
        PROP_CIRCA_CATEGORY_SUBGROUP
      );
    } else {
      throw new ImporterException(
        "The SubsGroup authority of " + parentNode + " was not found."
      );
    }
    ...
    try {
      ...
      List<ChildAssociationRef> children = nodeService.getChildAssocs(
        ...
        new QNamePattern() {
          ...
          public boolean isMatch(QName qName) {
            return qName
              .getLocalName()
              .startsWith(GROUP_PREFIX + parentNodeSubsGroupAuthName);
          }
        },
        true
      );
      ...
      if (children != null && children.size() == 1) {
        ...
        throw new ImporterException(
          parentNodeSubsGroupAuthName +
          " doesn't exist or it has been found more than once."
        );
      }

      // find the MasterGroup authority child node
      children = nodeService.getChildAssocs(
        authoritiesNodeRef,
        ContentModel.ASSOC_CHILDREN,
        new QNamePattern() {
          @Override
          public boolean isMatch(QName qName) {
            return qName
              .getLocalName()
              .startsWith(GROUP_PREFIX + childNodeMasterGoupAuthName);
          }
        },
        true
      );
    ...
    String childNodeMasterGoupName = (String) nodeService.getProperty(
      childNodeMasterGroup,
    ...
    if (
      nodeService.getChildByName(
        ...
        childNodeMasterGoupName
      ) ==
      null
    ) {
      authorityService.addAuthority(
        GROUP_PREFIX + parentNodeSubsGroupAuthName,
  ...
  private String getAuthorityInfo(
    QName assocType,
    NodeRef parentRef,
    NodeRef childRef
  ) {
    StringBuilder log = new StringBuilder("Authority[type=").append(
  ...
  private void doImport(
    ...
    if (useACP) {
      importHandler = new ACPImportPackageHandler(
        new File(importFolderPath + "/" + packageFileName),
        "UTF-8"
      );
    } else {
      File sourceDir = new File(importFolderPath + "/" + packageFileName);
      File dataFile = new File(packageFileName + ".xml");
      importHandler = new FileImportPackageHandler(
        sourceDir,
        dataFile,
        "UTF-8"
      );
    ...
    try {
      circabcImporterService.importView(
        importHandler,
        location,
        new CircabcImportBinding(keepUUIDs),
        null
      );

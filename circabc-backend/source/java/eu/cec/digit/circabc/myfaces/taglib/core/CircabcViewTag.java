    ...
    try {
      ...
      if (bodyContent != null) {
        ...
        final StateManager stateManager = facesContext
          .getApplication()
          .getStateManager();
        final StateManager.SerializedView serializedView =
        ...
        final ResponseWriter realWriter = bufferWriter.cloneWithWriter(
          getPreviousOut()
        );
        ...
        final String viewState = StateUtils.construct(
          savedState,
        ...
        while (form_marker != -1 || url_marker != -1) {
          if (
            url_marker == -1 || (form_marker != -1 && form_marker < url_marker)
          ) {
            // replace form_marker
            realWriter.write(
              bodyStr,
            ...
            lastMarkerEnd =
              form_marker + JspViewHandlerImpl.FORM_STATE_MARKER_LEN;
            form_marker = bodyStr.indexOf(
              JspViewHandlerImpl.FORM_STATE_MARKER,
              lastMarkerEnd
            );
          } else {
            // replace url_marker
            ...
            //
            realWriter.write(
              HtmlResponseStateManager.STANDARD_STATE_SAVING_PARAM
            );
            realWriter.write("=");
            realWriter.write(URLEncoder.encode(viewState, "UTF-8"));
            lastMarkerEnd =
              url_marker + HtmlLinkRendererBase.URL_STATE_MARKER_LEN;
            url_marker = bodyStr.indexOf(
              HtmlLinkRendererBase.URL_STATE_MARKER,
              lastMarkerEnd
            );
          }
        }
  ...
  protected void setProperties(final UIComponent component) {
    ...
    if (_locale != null) {
      ...
      if (UIComponentTag.isValueReference(_locale)) {
        ...
        final ValueBinding vb = context
          .getApplication()
          .createValueBinding(_locale);
        ...
        if (localeValue instanceof Locale) {
          ...
          if (localeValue != null) {
            throw new IllegalArgumentException(
              "Locale or String class expected. Expression: " +
              _locale +
              ". Return class: " +
      ...
      Config.set(
        (ServletRequest) getFacesContext().getExternalContext().getRequest(),
        Config.FMT_LOCALE,
        locale
      );
    } else {
      ...
      if (facesContext != null) {
        if (
          facesContext.getExternalContext().getSessionMap().containsKey(LOCALE)
        ) {

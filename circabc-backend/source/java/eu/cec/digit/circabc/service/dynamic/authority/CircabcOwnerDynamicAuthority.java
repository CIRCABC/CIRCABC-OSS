    ...
    return AuthenticationUtil.runAs(
      new RunAsWork<Boolean>() {
        public Boolean doWork() throws Exception {
          ...
          boolean isMangedByCircabc = nodeService.hasAspect(
            nodeRef,
        ...
        private boolean isInvitedUser(NodeRef nodeRef, String userName) {
          try {
            ...
            if (
              nodeService.hasAspect(container, CircabcModel.ASPECT_IGROOT) &&
        ...
        private Set<String> getInvitedUsers(NodeRef container) {
          ...
          if (nodeService.hasAspect(container, CircabcModel.ASPECT_IGROOT)) {
            invitedUsersGroupQName = QName.createQName(
              CIRCABC_CONTENT_MODEL_1_0_URI,
              "circaIGRoot" + "InvitedUsersGroup"
            );
          } else if (
            nodeService.hasAspect(container, CircabcModel.ASPECT_CATEGORY)
          ) {
            invitedUsersGroupQName = QName.createQName(
              CIRCABC_CONTENT_MODEL_1_0_URI,
              "circaCategory" + "InvitedUsersGroup"
            );
          } else if (
            nodeService.hasAspect(container, CircabcModel.ASPECT_CIRCABC_ROOT)
          ) {
            invitedUsersGroupQName = QName.createQName(
              CIRCABC_CONTENT_MODEL_1_0_URI,
          ...
          if (invitedUsersGroupQName != null) {
            ...
            if (property != null) {
              ...
              if (authorityService.authorityExists(prefixedUserGroupName)) {
                try {
                  result = authorityService.getContainedAuthorities(
                    AuthorityType.USER,
                  ...
                  if (logger.isErrorEnabled()) {
                    logger.error(
                      "Try to check authority for a deleted node / profile.",
                      e
                    );
                  }
                }
                ...
                if (logger.isErrorEnabled()) {
                  logger.error(
                    "Authority does bot exists: " + prefixedUserGroupName
                  );
                }
              }
            } else {

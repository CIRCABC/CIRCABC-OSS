...
public class BulkServiceImpl implements BulkService {
  ...
  public List<CompressedEntry> getCompressedEntries(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "upload file:" + compressedFile.getPath() + " to:" + containerNodeRef
      );
    }
    ...
    try {
      ...
      if (temporaryIndexFile != null) {
        ...
        if (!isDeleted && logger.isWarnEnabled()) {
          try {
            logger.warn(
              "Unable to delete file : " + temporaryIndexFile.getCanonicalPath()
            );
          } catch (IOException e) {
            logger.warn(
              "Unable to get getCanonicalPath for : " +
              temporaryIndexFile.getPath(),
              e
            );
          }
        }
  ...
  public List<UploadedEntry> upload(
    ...
    final NodeRef libraryNodeRef = managementService.getCurrentLibrary(
      containerNodeRef
    );
    ...
    for (final Map.Entry<String, NodeRef> entry : extractedFiles.entrySet()) {
      ...
      if (indexRecord != null && indexRecord.getOriLang() == null) {
        fileName = (String) nodeService.getProperty(
          entry.getValue(),
  ...
  private void setMetaDatas(
    ...
    if (indexRecord != null) {
      ...
      if (nodeRef == null) {
        ...
        if (
          ...
          if (
            ...
            indexRecord.getRelTrans().length() != 0
          ) {
            // Lien vers pivot
            ...
            if (
              ...
              relatedTranslationNodeRef = searchFile(
                libraryNodeRef,
                relatedTranslation
              );
            } else {
            ...
            if (relatedTranslationNodeRef != null) {
              ...
              try {
                if (logger.isDebugEnabled()) {
                  logger.debug(
                    "addEmptyTranslation with DocLang:" +
                    indexRecord.getDocLang()
                  );
                }
                final String emptyTranslationName = getFileName(
                  translationName
                );
                currentNode = multilingualContentService.addEmptyTranslation(
          ...
          if (
            ...
            path.startsWith(File.separatorChar + library)
          ) {
      ...
      // Title
      if (
        indexRecord.getTitle() != null && indexRecord.getTitle().length() > 0
      ) {
      ...
      // Description
      if (
        indexRecord.getDescription() != null &&
        indexRecord.getDescription().length() > 0
      ) {
        setPropertiesGeneric(
          currentNode,
          fileName,
      ...
      // Keywords
      if (
        indexRecord.getKeywords() != null &&
        indexRecord.getKeywords().length() > 0
      ) {
        final String keywords[] = indexRecord.getKeywords().split(";");
        Keyword keyword;
        final NodeRef igNodeRef = managementService.getCurrentInterestGroup(
          currentNode
        );
        Locale locale;
        String localeValue;
        ...
        for (final String langKeywordValue : keywords) {
          ...
          if (open == -1 || close == -1) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "locale: " + localeValue + " keyword: " + keywordValue
              );
            }
      ...
      // Status
      if (
        indexRecord.getStatus() != null && indexRecord.getStatus().length() > 0
      ) {
        boolean valid = false;
      ...
      if (
        ...
        indexRecord.getIssueDate().length() > 0
      ) {
        try {
      ...
      // Reference
      if (
        indexRecord.getReference() != null &&
        indexRecord.getReference().length() > 0
      ) {
        setPropertiesGeneric(
          currentNode,
          fileName,
      ...
      if (
        ...
        if (valid) {
          ...
          if (logger.isErrorEnabled()) {
            logger.error(
              error_message + " value:" + indexRecord.getSecurityRanking()
            );
          }
        }
      } else {
        //Special case of OLAF - All documents should be marked as SNC by default
      ...
      //	Author
      if (
        indexRecord.getAuthor() != null && indexRecord.getAuthor().length() > 0
      ) {
      ...
      if (
        ...
        indexRecord.getTranslator().length() != 0
      ) {
        if (relatedTranslationNodeRef != null) {
          if (currentNode != null) {
            if (
      ...
      //We need to check the special case of OLAF
      //If there is no expiration date, we will use the default value: today + 1 month at 23:59:59.999
      //If there is an expiration date but if it not before the default value, we will use the default value as well
      if (CircabcConfig.OLAF) {
        Date defaultExpirationDate = DateUtils.inOneMonth23h59M59S();
        if (
          indexRecord.getExpirationDate() != null &&
          indexRecord.getExpirationDate().length() > 0
        ) {
          try {
            parsed = sdf.get().parse(indexRecord.getExpirationDate());
          } catch (final ParseException pe) {
            //in case of parsing error, we take the default expiration date
            parsed = defaultExpirationDate;
          }
          if (!parsed.before(defaultExpirationDate)) {
            parsed = defaultExpirationDate;
          }
        } else {
          parsed = defaultExpirationDate;
      ...
      if (
        ...
        try {
          //if we did not parse the value of the expiration date yet (not OLAF)
          if (parsed == null) {
            parsed = sdf.get().parse(indexRecord.getExpirationDate());
          }
          ...
          if (currentNode == pivotNode && pivotNode != null) {
            ...
            if (
              ...
              final String ERROR_DESCRIPTION =
                "bulk_upload_expir_date_error_on_translation";
              messages.add(
                new ValidationMessageImpl(
                  indexRecord.getRowNumber(),
                  fileName,
        ...
        if (
          ...
          final String ERROR_DESCRIPTION =
            "bulk_upload_mandatory_expir_date_error";
          messages.add(
            new ValidationMessageImpl(
              indexRecord.getRowNumber(),
              fileName,
    ...
    if (
      ...
      if (typeDef != null) {
        ...
        if (
          ...
          if (
            !childsNodeRefs.contains(nodeRef) && !allNodeRefs.contains(nodeRef)
          ) {
            if (multilingualContentService.isTranslation(nodeRef)) {
              ...
              if (nodeRef.equals(pivot)) {
                ...
                // Add the pivot
                if (
                  !childsNodeRefs.contains(pivot) &&
                  !allNodeRefs.contains(pivot)
                ) {
        ...
        // look for Space folder node
          ...
          dictionaryService.isSubClass(type, ApplicationModel.TYPE_FOLDERLINK)
        ) {
          // to avoid infinite recursion ignore folder links
          // createFolderLinkRepresentation(nodeRef, childsNodeRefs, allNodeRefs);
        } else if (
  ...
  private void createFileLinkRepresentation(
    ...
    if (
      destRef != null &&
      new Node(destRef).hasPermission(PermissionService.READ) == true
    ) {
    ...
    for (final NodeRef nodeRef : allNodeRefs) {
      if (isContent(nodeRef)) {
        ...
        if (multilingualContentService.isTranslation(nodeRef)) {
          ...
          securityRanking = (String) nodeService.getProperty(
        ...
        if (
          ...
          !(DocumentModel.SECURITY_RANKINGS_PUBLIC.equalsIgnoreCase(
            ...
            DocumentModel.SECURITY_RANKINGS_NORMAL.equalsIgnoreCase(
              securityRanking
            ))
        ) {
        ...
        if (indexRecord != null) {
          ...
          // Description
          final String description = (String) nodeService.getProperty(
            nodeRef,
            ContentModel.PROP_DESCRIPTION
          );
          ...
          // DOC LANG
          final Locale docLang = (Locale) nodeService.getProperty(
            nodeRef,
          ...
          // Author
          final String author = (String) nodeService.getProperty(
            nodeRef,
            ContentModel.PROP_AUTHOR
          );
          ...
          // Keywords
          final List<Keyword> keywords = keywordsService.getKeywordsForNode(
            nodeRef
          );
          ...
          // Status
          final String status = (String) nodeService.getProperty(
            nodeRef,
            DocumentModel.PROP_STATUS
          );
          ...
          // Issue Date
          final Date issueDate = (Date) nodeService.getProperty(
            nodeRef,
            DocumentModel.PROP_ISSUE_DATE
          );
          if (issueDate != null) {
          ...
          // Reference
          final String reference = (String) nodeService.getProperty(
            nodeRef,
            DocumentModel.PROP_REFERENCE
          );
          ...
          for (int i = 0; i < DocumentModel.ALL_DYN_PROPS.size(); i++) {
            QName item = DocumentModel.ALL_DYN_PROPS.get(i);
            final String dynamicProperty = (String) nodeService.getProperty(
              nodeRef,
              item
            );
            if (dynamicProperty != null && dynamicProperty.length() > 0) {
          ...
          if (multilingualContentService.isTranslation(nodeRef)) {
            ...
            if (nodeRef.equals(pivot)) {
              ...
              // Lien vers pivot
              //	Name (with relative path)
              final String pivotRelativePath =
                zipService.getRelativeLibraryPath(pivot);
              ...
              if (
                ...
                final String translator = (String) nodeService.getProperty(
                  nodeRef,
                  ContentModel.PROP_AUTHOR
                );
    ...
    for (final IndexRecord indexRecord : indexFileEntries) {
      ...
      if (!found) {
        if (
          ...
          if (logger.isTraceEnabled()) {
            logger.trace(
              "index entry for an empty translation" + indexRecord.getName()
            );
          }
          continue;
        } else {
          if (logger.isTraceEnabled()) {
            logger.trace(
              "index entry is NOT uploaded !!!" + indexRecord.getName()
            );
          }
          messages.add(
            new ValidationMessageImpl(
  ...
  private NodeRef searchFile(
    ...
    if (destinationFolderNodeRef != null) {
      if (logger.isDebugEnabled()) {
        try {
          final SimplePath simplePath = managementService.getNodePath(
            destinationFolderNodeRef
          );
          logger.debug(
            "Search file:" + fileName + " in:" + simplePath.toString()
          );
          if (logger.isTraceEnabled()) {
            final List<SimplePath> childs = managementService.getChildsPath(
              destinationFolderNodeRef
            );
          ...
          logger.error(
            "Error when geting node path for " +
            destinationFolderNodeRef.toString(),
            e
          );
        }
      }
      ...
      if (folder != null) {
        if (logger.isDebugEnabled()) {
          try {
            ...
            logger.debug(
              "Search:" + folderName + " in:" + simplePath.toString()
            );
            if (logger.isTraceEnabled()) {
              final List<SimplePath> childs = managementService.getChildsPath(
                folder

...
/**
 * Interface for Mail Service. Each mail should be sent via this service. An
 * email is always sent in
 * a non-blocking process. It means that if any error appears, it is will be
 * never throwed.
 *
 * @author Yanick Pignot
 *         <p>
 *         Migration 3.1 -> 3.4.6 - 02/12/2011 Commented the key parameter of
 *         the @Auditable
...
public interface MailService {
   ...
   * @param from        The email that send the email
   * @param to          The email that receive the email
   * @param others      The emails that receive the email in CC
   * @param subject     The subject of the email
   * @param body        The body of the email
   * @param html        True is the email is in HTML format
   * @return If whethever the mail is sent or not.
  ...
  /**
   * Sent an email as text or html email to each specified person with the given
   * node attached
   *
   * @param from    The person that send the email

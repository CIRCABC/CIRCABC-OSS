...
public class RenderJob implements StatefulJob {
  ...
  public void execute(JobExecutionContext context)
    ...
    renditionServiceConfig = (RenditionServiceConfig) jobData.get(
    ...
    final ServiceRegistry serviceRegistry = (ServiceRegistry) jobData.get(
      "serviceRegistry"
    );
    final CircabcServiceRegistry circabcServiceRegistry =
      (CircabcServiceRegistry) jobData.get("circabcServiceRegistry");
    circabcLockService =
      (eu.cec.digit.circabc.service.lock.LockService) jobData.get(
        "circabcLockService"
      );

    renditionDaoService =
      (RenditionDaoService) circabcServiceRegistry.getService(
        CircabcServiceRegistry.RENDITION_DAO_SERVICE
      );
      ...
      public Object doWork() throws Exception {
        ...
        try {
          ...
          for (Request request : requests) {
            ...
            if (
              checkOutCheckInService.isCheckedOut(nodeRef) ||
  ...
  private boolean render(Request request) {
    ...
    if (sourceContentQName == null) {
      ...
      String errorMessage =
        "Reader is null attempting to calculate " +
        "the rendition! Is this node a content item?";
      logger.error(errorMessage);
      updateForError(request, errorMessage, false);
      return false;
    }
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        "NodeRef " + nodeRef.getId() + " rendering time " + totalTime
      );
    }

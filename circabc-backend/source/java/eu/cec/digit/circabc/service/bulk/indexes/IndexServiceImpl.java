...
public class IndexServiceImpl implements IndexService {
  ...
  private IndexHeaders getAllHeaders() {
    ...
    localIndexHeaders.addHeader(
      indexHeaders.getHeader(Headers.EXPIRATION_DATE)
    );
    localIndexHeaders.addHeader(
      indexHeaders.getHeader(Headers.SECURITY_RANKING)
    );
  ...
  public void generateIndexRecords(
    ...
    try {
      ...
      IndexEntry indexEntry;
      final String[] indexEntriesValues = new String[indexHeaders
        .getHeaders()
        .size()];
      for (final IndexRecord indexRecord : indexRecords) {
        ...
        for (final IndexHeader indexHeader : indexHeaders.getHeaders()) {
          ...
          if (
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "No value found for header:" + indexHeader.getHeaderName()
              );
            }
          } else {
            if (logger.isDebugEnabled()) {

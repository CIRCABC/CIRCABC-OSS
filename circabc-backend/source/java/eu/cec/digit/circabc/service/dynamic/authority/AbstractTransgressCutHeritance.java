...
public abstract class AbstractTransgressCutHeritance
  ...
  protected static final Log logger = LogFactory.getLog(
    AbstractTransgressCutHeritance.class
  );
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (permissionsDaoComponent == null) {
      throw new IllegalStateException(
        "The permissions Dao Component must be set"
      );
    }
    ...
    transgressCutInheritancePermRef = modelDAO.getPermissionReference(
      ...
      new RunAsWork<Boolean>() {
        public Boolean doWork() throws Exception {
          ...
          if (mandatoryAspect == null) {
            if (logger.isDebugEnabled()) {
              logger.debug(
                "The node doesn't have one of the mandatory haspects:" +
                mandatoryAspects
              );
            }
            return false;
        ...
        public boolean isAdmin(
          ...
          if (nodeRef == null) {
            ...
            for (final PermissionEntry permissionEntry : permissions.getPermissionEntries()) {
              if (
                permissionEntry.getAuthority().equals(userName) ||
                containingGroupAuthorities.contains(
                ...
                adminOnService = grantingPermissions.contains(
            ...
            if (
              nodeService.hasAspect(
                nodeRef,
                mandatoryAspects.get(mandatoryAspect)
              )
            ) {
              hasTopAspect = true;
            }
  ...
  public Set<PermissionReference> requiredFor() {
    if (permissionReference == null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "(" + circabcService + ")" + " requiredFor:" + permissionReference
        );
      }
    }
  ...
  public void setPermissionsDaoComponent(
    final PermissionsDaoComponent permissionsDaoComponent
  ) {
  ...
  public abstract void setMandatoryAspects(
    final Map<String, String> mandatoryAspects
  );
}

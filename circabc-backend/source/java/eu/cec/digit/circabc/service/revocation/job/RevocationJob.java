...
public class RevocationJob implements Job {
  ...
  private void processJob(UserRevocationRequest job) {
    if (
      ...
      try {
        ...
        logger.error(
          "Security Exception when processing revocation job" + job.getId(),
          e
        );
        state = -1;
      } catch (IllegalStateException e) {
        logger.error(
          "IllegalStateException when processing revocation job" + job.getId(),
          e
        );
        state = -1;
      } catch (NotSupportedException e) {
        logger.error(
          "NotSupportedException when processing revocation job" + job.getId(),
          e
        );
        state = -1;
      } catch (SystemException e) {
        logger.error(
          "SystemException when processing revocation job" + job.getId(),
          e
        );
        state = -1;
      } catch (RollbackException e) {
        logger.error(
          "RollbackException when processing revocation job" + job.getId(),
          e
        );
        state = -1;
      } catch (HeuristicMixedException e) {
        logger.error(
          "HeuristicMixedException when processing revocation job" +
          job.getId(),
          e
        );
        state = -1;
        ...
        logger.error(
          "HeuristicRollbackException when processing revocation job" +
          job.getId(),
          e
        );
        state = -1;
  ...
  private void prepareServices(JobExecutionContext arg0) {
    ...
    setPersonService(
      (PersonService) arg0.getMergedJobDataMap().get("personService")
    );
  ...
  private void processCleaningGroupMembers(
    ...
    historyApi.cleanAndLogPermissions(
      groupNodeRef,
      new HashSet<String>(groupToCheck.getValue()),
      true
    );
  }
  ...
  private Map<String, List<String>> listGroupsFromMemberships(
    ...
    for (String userId : userIds) {
      if (personService.personExists(userId)) {
        List<InterestGroupProfile> profileMemberships =
          usersApi.getUserMembership(userId);

...
abstract class MailTemplateHelper {
    ...
    for (int x = 0; x < builder.length(); ++x) {
      ...
      if (inInstruction) {
        if (charat == '}') {
          ...
          result.append(
            escapeSingleInstruction(currentInstruction.toString(), locale)
          );
   ...
   * @return
   */
  private static String escapeSingleInstruction(
    ...
    if (currentInstruction.contains("${formatMessage(")) {
      ...
      final String params = currentInstruction.substring(
        i18nKeyEnd + 1,
        currentInstruction.length() - 2
      );

      return I18NUtil.getMessage(
        currentInstruction.substring(i18nKeyStart, i18nKeyEnd),
        locale,
        buildParams(params)
      );
    } else {

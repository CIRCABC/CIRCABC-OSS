...
public class MeetingResponseListener implements Job {
  ...
  private void initialize(final JobExecutionContext context) {
    if (!isInitialized) {
      ...
      emailProtocol = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_PROTOCOL
      );
      emailServer = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_SERVER
      );
      emailBox = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_BOX
      );
      emailUsername = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_USERNAME
      );
      emailPassword = CircabcConfiguration.getProperty(
        CircabcConfiguration.EMAIL_PASSWORD
      );
      emailServerPort = Integer.valueOf(
        CircabcConfiguration.getProperty(CircabcConfiguration.EMAIL_SERVER_PORT)
      );
      isActive = Boolean.valueOf(
        CircabcConfiguration.getProperty(
          CircabcConfiguration.IS_EMAIL_LISTENER_ACTIVE
        )
      );
      isEmailUseTls = Boolean.valueOf(
        CircabcConfiguration.getProperty(CircabcConfiguration.IS_EMAIL_USE_TLS)
      );
  ...
  private void processMessage(final POP3Message message) throws Exception {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (isResponse) {
        meetingRequestStatus = getMeetingRequestStatus(
        ...
        if (startPositionRecurrence > -1) {
          ...
          meetingNodeRef = getMeetingNodeRefByRecurrenceLine(
            meetingNodeRef,
            transactionService,
            recurrenceLine
          );
  ...
  private String getRecurrenceLine(
    String body,
    final int startPositionRecurrence
  ) {
    String recurrenceLine;
    final int endPositionRecurrence = body.indexOf(
      CRLF,
      startPositionRecurrence
    );
    recurrenceLine = body.substring(
      startPositionRecurrence,
      endPositionRecurrence
    );
  ...
  private String getBodyFromString(

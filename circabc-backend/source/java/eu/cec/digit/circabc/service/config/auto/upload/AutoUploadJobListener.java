...
public class AutoUploadJobListener implements Job {
    ...
    if (allConfigurations != null) {
      ...
      for (Configuration conf : allConfigurations) {
        ...
        try {
           ...
           * 0 = already locked
           */
          lock = autoUploadManagementService.lockJobFile(
          ...
          if (lock == 1) {
            Integer unlock = autoUploadManagementService.unlockJobFile(
              conf.getIdConfiguration()
            );
          }
        }
    ...
    try {
      ...
      if (tmpFile != null) {
        ...
        if (!isDeleted && logger.isWarnEnabled()) {
          try {
            logger.warn(
              "Unable to delete file : " + tmpFile.getCanonicalPath()
            );
          } catch (IOException e) {
            logger.warn(
              "Unable to get CanonicalPath for : " + tmpFile.getPath(),
              e
            );
          }
        }
  ...
  private List<Configuration> getConfigurations() {
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Error during fetching all configiration from autoupload DB",
          e
        );
      }

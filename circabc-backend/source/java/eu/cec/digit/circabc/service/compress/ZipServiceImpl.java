...
public class ZipServiceImpl implements ZipService {
  ...
  private static final String CREATE_FILE_INTO_REPOSITORY =
    "Create file into repository:";
  private static final String ERROR_WHEN_CLOSING_COMPRESSED_FILE =
    "Error when closing compressed file:";
  private static final String ERROR_DURING_UNCOMPRESSING_FILE =
    "Error during uncompressing file:";
    ...
    try {
      ...
      for (final NodeRef nodeRef : allNodeRefs) {
        ...
        if (
          ...
          contentReader = contentService.getReader(
            nodeRef,
    ...
    for (int i = 1; i < path.size(); i++) {
      ...
      if (element instanceof Path.ChildAssocElement) {
        ...
        if (elementRef.getParentRef() != null) {
          ...
          if (foundRoot == true) {
            nameProp = nodeService.getProperty(
  ...
  private Collection<? extends NodeRef> getAllChildsNodeRefs(
    final NodeRef nodeRef
  ) {
    final List<NodeRef> allNodeRefs = new ArrayList<>();
  ...
  private void createFolderLinkRepresentation(
    ...
    if (
      ...
      final List<ChildAssociationRef> listChildAssocs =
        nodeService.getChildAssocs(destRef);
      allNodeRefs.add(nodeRef);
  ...
  public Map<String, NodeRef> extract(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        UNCOMPRESS + compressedFile.getName() + TO + destinationNodeRef
      );
    }
    ...
    try {
      ...
      while (zipEntries.hasMoreElements()) {
        ...
        if (!excludedFileName.contains(filePath)) {
          ...
          if (createdNodeRef != null) {
            ...
            final String fileAllreadyExist = I18NUtil.getMessage(
              FILE_ALLREADY_EXIST
            );
            if (logger.isDebugEnabled()) {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          ERROR_DURING_UNCOMPRESSING_FILE + compressedFile.getName(),
          e
        );
      }
    } finally {

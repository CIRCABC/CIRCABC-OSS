...
public class ApiToolBox {
    ...
    try {
      ...
      if (bytesRead * -1 > size) {
        throw new IllegalArgumentException(
          "File exceeds 1Mb size: " + fileName
        );
      }
  ...
  public NodeRef getCurrentLibraryRoot(NodeRef nodeRef) {
    if (
      ...
      nodeService.hasAspect(nodeRef, CircabcModel.ASPECT_LIBRARY_ROOT)
    ) {
      return nodeRef;
    } else {
      return getCurrentLibraryRoot(
  ...
  public NodeRef getCurrentNewsgroupRoot(NodeRef nodeRef) {
    if (
      ...
      nodeService.hasAspect(nodeRef, CircabcModel.ASPECT_NEWSGROUP_ROOT)
    ) {
      return nodeRef;
    } else {
      return getCurrentNewsgroupRoot(
        nodeService.getPrimaryParent(nodeRef).getParentRef()
      );
    }
  }

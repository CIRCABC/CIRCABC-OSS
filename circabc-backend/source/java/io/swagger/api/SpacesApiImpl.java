...
public class SpacesApiImpl implements SpacesApi {
  ...
  public PagedNodes spaceGetChildren(
    ...
    if (
      ...
      final PagingResults<FileInfo> list = getFileFolderService()
        .list(spaceNodeRef, fileOnly, folderOnly, null, sortProps, pr);
      ...
      for (int i = iStart; i < iEnd; i++) {
        boolean isExpirededDate = false;
        final NodeRef childRef = items.get(i).getNodeRef();

        // retreive the expiration date from file properties
        String dateStr = null;
        if (
          items
            .get(i)
  ...
  public Node spacesIdSpacesPost(String id, Node body) {
    ...
    try {
      if (
        body.getProperties().get(EXPIRATION_DATE) != null &&
        ...
        this.secureNodeService.setProperty(
            ...
            Converter.convertStringToDate(
              body.getProperties().get(EXPIRATION_DATE)
            )
          );
      }
    } catch (ParseException e) {
  ...
  public void spacesIdPut(String id, Node body) {
    ...
    //We should call fileFolderService.rename to change the folder name in the Primary Path
    try {
  ...
  public Node spacesIdUrlPost(String id, Node body) {
    ...
    if (secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_LIBRARY)) {
      ...
      final FileInfo fileInfo = getFileFolderService()
        .create(nodeRef, name, ContentModel.TYPE_CONTENT);
  ...
  /**
   * @see io.swagger.api.SpacesApi#getInvitedInterestGroups(java.lang.String, int,
   *      int)
   */
  @Override
  public PagedShares getInvitedInterestGroups(
    ...
    for (int index = startItem; index < endItem; index++) {
      ...
      String igName = (String) secureNodeService.getProperty(
        invitedIG.getFirst(),
        ContentModel.PROP_NAME
      );
  ...
  /**
   * @see io.swagger.api.SpacesApi#changeSharePermission(java.lang.String,
   *      java.lang.String,
  ...
  /**
   * @see io.swagger.api.SpacesApi#addShare(java.lang.String,
   *      io.swagger.model.Share, boolean)
   */
  @Override
  ...
  /**
   * @see io.swagger.api.SpacesApi#createSharedSpaceLink(java.lang.String,
   *      java.lang.String,
  ...
  public void createSharedSpaceLink(
    ...
    linksBusinessSrv.createSharedSpaceLink(
      parentSpace,
      shareSpace,
      title,
      description

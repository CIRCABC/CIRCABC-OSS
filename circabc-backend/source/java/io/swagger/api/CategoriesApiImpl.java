...
public class CategoriesApiImpl implements CategoriesApi {
  ...
  private static final QName PROP_CIRCABC_PROFILE_GROUP_NAME = QName.createQName(
  ...
  private static final QName PROP_CATEGORY_ADMIN_SERVICE_NAME = QName.createQName(
      CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI,
      "circaCategoryServiceName");
  private static final QName PROP_CATEGORY_ADMIN_SERVICE_PERMISSION_SET = QName.createQName(
      CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI,
  ...
  private void setCircabcRoot() {
    final NodeRef companyHome = nodeLocatorService.getNode(
  ...
  public List<InterestGroup> getInterestGroupByCategoryId(String id) {
    ...
    if (circabcService.readFromDatabase()) {
      ...
      List<InterestGroupItem> interestGroups = circabcService.getInterestGroupByCategoryUser(
          categoryNodeRef,
          userName);
  ...
  private void setInterestGroups(
    ...
    for (ChildAssociationRef child : children) {
      ...
      if (isInterestGroup(igRef)) {
        InterestGroup interestGroup = groupsApi.getInterestGroupDetails(
            igRef,
            false);
   ...
   */
  public void setCircabcDaoServiceImpl(
      CircabcDaoServiceImpl circabcDaoServiceImpl) {
  ...
  public List<Profile> categoriesIdExportedProfilesGet(
    ...
    if (ignoreIgId != null && !"".equals(ignoreIgId)) {
      ...
      Long igId = (Long) nodeService.getProperty(
          igRef,
          ContentModel.PROP_NODE_DBID);
      exportedProfiles = circabcDaoServiceImpl.selectExpProfilesByCategoryIDInterestGroupID(
          categoryId,
          igId);
    } else {
  ...
  public InterestGroup categoriesIdGroupsPost(
      String id,
      final InterestGroupPostModel ig) {
  ...
  private void createKeywordContainer(NodeRef igRef) {
    QName ASSOC_IG_KEYWORDCONTAINER = QName.createQName(
        CircabcModel.CIRCABC_CONTENT_MODEL_1_0_URI,
        "igKwContainer");
  ...
  private List<NodeRef> createIGFolders(NodeRef igRef) {
    ...
    ////////////////////////////////////////
    // DIRECTORY
    NodeRef directoryRef = nodeService
        .createNode(
            igRef,
  ...
  private NodeRef createInformation(NodeRef igRef) {
    ...
    // Create Condition
    final ActionCondition actionConditionInformation = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
    ...
    // Create Condition
    final ActionCondition actionConditionInfoNotify = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
  ...
  private NodeRef createNewsGroup(NodeRef igRef) {
    ...
    // Create Condition
    final ActionCondition actionConditionNewsGroup = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
    ...
    // Create Condition
    final ActionCondition actionConditionNewsNotify = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
  ...
  private NodeRef createEvent(NodeRef igRef) {
    ...
    // Create Condition
    final ActionCondition actionConditionEvent = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
  ...
  private NodeRef createLibrary(NodeRef igRef) {
    ...
    // Create Condition
    final ActionCondition actionConditionCircaDocument = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
    ...
    // Create Condition
    final ActionCondition actionConditionLibrary = actionService.createActionCondition(NoConditionEvaluator.NAME);
    ...
    // Create Condition
    final ActionCondition actionConditionNotify = actionService.createActionCondition(IsSubTypeEvaluator.NAME);
  ...
  private String createSubsGroup(NodeRef nodeRef) {
    ...
    final String folderName = ((String) nodeService.getProperty(
        nodeRef,
  ...
  private NodeRef createIgNode(InterestGroupPostModel ig, NodeRef categoryRef) {
    ...
    getNodeService()
        .addAspect(igRef, ApplicationModel.ASPECT_UIFACETS, uiFacetsProps);
  ...
  /**
   * @see io.swagger.api.CategoriesApi#getIGStatisticsContents(java.lang.String,
   *      int, int)
   */
  @Override
  public PagedStatisticsContents getIGStatisticsContents(
    ...
    List<FileInfo> contents = globalStatisticsService.getCategoryGroupStatsFiles(categoryName, nodeRef);
  ...
  public void postCategoryLogoByCategoryId(
    ...
    if (inputStream != null) {
      ...
      String newfilename = calDate.get(Calendar.YEAR) +
          "-" +
      ...
      try {
        long attachmentTotalSize = Long.parseLong(
            CircabcConfiguration.getProperty(
                CircabcConfiguration.LOGO_ALLOWED_SIZE_BYTES));
    ...
    if (categoryId != null && category != null) {
      ...
      if (category.getContactEmails() != null) {
        ...
        Serializable contactMails = nodeService.getProperty(
            categoryRef,
            CircabcModel.PROP_CONTACT_EMAILS);
        if (!contactMails.equals(emails.toString()) &&
            category.getUseSingleContact()) {
          nodeService.setProperty(
              categoryRef,
  ...
  public Category headersIdCategoryPost(
    ...
    try {
      ...
      nodeService.setProperty(
          ...
          categories);
      trx.commit();
      UserTransaction trx1 = serviceRegistry
          .getTransactionService()
          .getNonPropagatingUserTransaction(false);
      trx1.begin();
      ...
      trx1.commit();
      UserTransaction trx2 = serviceRegistry
          .getTransactionService()
          .getNonPropagatingUserTransaction(false);
      trx2.begin();
      createCategoryPredefinedProfiles(categoryRef);
      trx2.commit();
      UserTransaction trx3 = serviceRegistry
          .getTransactionService()
          .getNonPropagatingUserTransaction(false);
      trx3.begin();
      for (ChildAssociationRef child : nodeService.getChildAssocs(
        ...
        if (nodeService
            ...
            .toString()
            .equals("CircaBCAdmin")) {
          String circabcAdminGroupName = nodeService
      ...
      trx3.commit();
      UserTransaction trx4 = serviceRegistry
          .getTransactionService()
          .getNonPropagatingUserTransaction(false);
      trx4.begin();
      permissionService.setInheritParentPermissions(categoryRef, false);
      trx4.commit();
      UserTransaction trx5 = serviceRegistry
          .getTransactionService()
          .getNonPropagatingUserTransaction(false);
      trx5.begin();
  ...
  private void createCategoryPredefinedProfiles(NodeRef categoryRef) {
    ...
    String finalAlfGroupName = authorityService.createAuthority(
        AuthorityType.GROUP,
        newAlfGroupName,
  ...
  public void categoriesIdAdminContactPost(
    ...
    if (body.getSendCopy()) {
      EmailDefinition emailConfirmation = emailApi.prepareConfirmationForAdminContact(
          categoryRef,
          body.getContent());
  ...
  public PagedGroupDeletionRequests categoriesIdGroupDeleteRequestsGet(
    ...
    List<GroupDeletionRequest> listGroupDeletionRequest = groupRequestsDaoService.getCategoryGroupDeletionRequests(
        categoryRef,
        limit,
        page,
        filter);

    Iterator<GroupDeletionRequest> iterator = listGroupDeletionRequest.iterator();
    while (iterator.hasNext()) {
      GroupDeletionRequest groupDeletionRequest = iterator.next();

      if (shouldDeleteRequest(groupDeletionRequest)) {
        groupRequestsDaoService.deleteRequestDeletion(groupDeletionRequest.getGroupId());
        iterator.remove();
      }
  ...
  private boolean shouldDeleteRequest(GroupDeletionRequest groupDeletionRequest) {
    if (groupDeletionRequest.getAgreement() == null || groupDeletionRequest.getAgreement() == 0) {
      NodeRef groupRef = Converter.createNodeRefFromId(groupDeletionRequest.getGroupId());
      if (!nodeService.exists(groupRef)) {
        return true;
      }
    }
    if (groupDeletionRequest.getAgreement() != null && groupDeletionRequest.getAgreement() == 1) {
      NodeRef groupRef = Converter.createNodeRefFromId(groupDeletionRequest.getGroupId());
      if (nodeService.exists(groupRef)) {
        return true;
      }
  ...
  public void setGroupRequestsDaoService(
      GroupRequestsDaoService groupRequestsDaoService) {
  ...
  public void categoriesIdGroupRequestApprovalPost(
    ...
    if (body.getAgreement() == -1) {
      ...
      GroupCreationRequest intialRequest = groupRequestGet(
          String.valueOf(body.getId()));
      EmailDefinition emailApproval = emailApi.prepareAcceptationGroupRequest(
          intialRequest,
          body.getArgument());
      emailApi.mailPost(emailApproval);
    }
  }
  ...
  public void groupIdDeleteRequestPost(GroupDeletionRequest body) {
    ...
    InterestGroup interestGroup = this.groupsApi.getInterestGroup(body.getGroupId());
    User userFrom = usersApi.usersUserIdGet(body.getFrom().getUserId());
    body.setLeaders(igLeaders);
    String title = interestGroup.getTitle() != null ? interestGroup.getTitle().getDefaultValue() : null;
    if (title == null && interestGroup.getTitle() != null) {
      title = interestGroup.getTitle().get("en-US");
    }
    body.setTitle(title != null ? title : "");

    String description = interestGroup.getDescription() != null ? interestGroup.getDescription().getDefaultValue()
        : null;
    if (description == null && interestGroup.getDescription() != null) {
      description = interestGroup.getDescription().get("en-US");
    }
    ...
    try {
      ...
      igLeaders.forEach(user -> {
        ...
        filteredCatAdmins.removeIf(admin -> admin.getEmail().equals(user.getEmail()));

        EmailDefinition email = emailApi.prepareEmailForGroupDeletionRequestLeaders(
            user,
            filteredCatAdmins,
            body,
  ...
  private List<User> getLeaders(String groupId) {
    ...
    for (Profile groupProfile : profiles) {
      ...
      if (perms != null &&
          ...
          NewsGroupPermissions.NWSADMIN.toString()
              .equals(perms.get("newsgroups"))
          &&
          InformationPermissions.INFADMIN.toString()
              .equals(perms.get("information"))) {
  ...
  public void categoriesIdGroupRequestDeleteApprovalPost(
      ...
      String username) {
    GroupDeletionRequest groupDeletionRequest = groupRequestsDaoService.getCategoryGroupDeletionRequests(
        String.valueOf(body.getId()));
    List<User> admins = categoriesIdAdminsGet(categoryId);
    ...
    if (body.getArgument() != null && body.getAgreement() == -1) {
      ...
      EmailDefinition emailDef = emailApi.prepareRefusalGroupDeleteRequest(
          ...
          ig.getName(), ig.getTitle().getDefaultValue());
      ...
      leaders
          ...
          .forEach(leader -> {
            EmailDefinition emailDef_leaders = emailApi.prepareRefusalGroupDeleteRequestLeaders(
                groupDeletionRequest,
                ig.getName(),
    ...
    if (body != null && body.getAgreement() == 1) {
      groupsApi.updateIgToBeDeleted(interestGroupID, true);
      toUsers.forEach(user -> {
        EmailDefinition emailDef = emailApi.prepareAcceptationGroupDeleteRequest(
            user,
            groupDeletionRequest,
            ig.getName(),
            ig.getTitle().getDefaultValue());
                  ...
                  AuthenticationUtil.runAs(
                      new AuthenticationUtil.RunAsWork<String>() {
                        ...
                        public String doWork() {
                          if (circabcLockService.isLocked(

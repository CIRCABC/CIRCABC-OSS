...
public class EmailApiImpl implements EmailApi {
   ...
   * @param extraModelParams Parameters added to the mail template model (used if
   *                         usingTemplate !=
   *                         null)
   * @param extraBodyParams  Parameters added to the mail body defined in the text
   *                         area (used if
   *                         usingTemplate == null)
   * @param isMailHtml       true if the mail is html format
   * @param usingTemplate    id of the template to be used, null if no custom
  ...
  public void mailToUser(
    ...
    if (to != null && to.length() != 0) {
      ...
      if (usingTemplate != null) {
        ...
        final NodeRef templateRef = Converter.createNodeRefFromId(
          usingTemplate
        );
        bodyToSend = new StringBuilder(
  ...
  private String buildLinkFromAttachmentWrapper(AttachementWrapper aw) {
    ...
    result += WebClientHelper.getGeneratedWaiFullUrl(
      n,
  ...
  public EmailDefinition prepareEmailForGroupDeletionRequest(
    ...
    model.put("categRef", categRef);
    model.put(CATEGORY_NAME, categoryName);
    model.put("interestGroupId", ig.getName());
    ...
    if (ig.getDescription() != null) {
      ...
      if (description == null) {
        description = ig.getDescription().get("en-US");
      }
    }
  ...
  public void clearTemplateCache() {
    ...
    try {
      Method clearTemplateCacheMethod =
        Configuration.class.getMethod("clearTemplateCache");
      clearTemplateCacheMethod.invoke(freemarkerConfig);
  ...
  @Override
  public void groupDeletionMailPost(
    EmailDefinition emailDefinition,
    String emailTo
  ) {
    String noReply = getMailService().getNoReplyEmailAddress();
    ...
    for (User catAdmin : emailDefinition.getUsers()) {
      uniqueEmails.add(catAdmin.getEmail());
    }
  ...
  /**
   * @see io.swagger.api.EmailApi#saveUserMailTemplate(java.lang.String,
   *      java.lang.String,
  ...
  public EmailDefinition prepareEmailForHelpdeskContact(
    ...
    if (GUEST.equals(currentUser)) {
      helpdesk.setEmail(
        CircabcConfiguration.getProperty("mail.from.circabc.helpdesk")
      );
    } else {
      ...
      if (
        ...
        helpdesk.setEmail(
          CircabcConfiguration.getProperty("mail.from.circabc.it.helpdesk")
        );
      } else {
        helpdesk.setEmail(
          CircabcConfiguration.getProperty("mail.from.circabc.helpdesk")
  ...
  public EmailDefinition prepareConfirmationForHelpdeskContact(
    ...
    if (serviceNowTicket != null) {
      result.setSubject(mail.getSubject(model) + " - " + serviceNowTicket);
    } else {
      result.setSubject(mail.getSubject(model));
    }
    result.setContent(mail.getBody(model));
  ...
  public EmailDefinition prepareRefusalGroupDeleteRequest(
    ...
    String userId = body.getFrom().getUserId() != null
      ? body.getFrom().getUserId()
      : "";
    NodeRef toUserRef = personService.getPerson(userId);
  ...
  public EmailDefinition prepareAcceptationGroupDeleteRequest(
    ...
    NodeRef toUserRef = personService.getPerson(user.getUserId());

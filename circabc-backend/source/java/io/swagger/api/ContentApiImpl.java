...
public class ContentApiImpl implements ContentApi {
  ...
  private static final List<String> officeMimetypes = new ArrayList<>(
    Arrays.asList(
    ...
    NodeRef copyOfDocument;
    {
      ...
      try {
        ...
        copyOfDocument = getTranslationService()
          .copyDocumentToBeTranslated(nodeRef);
  ...
  private NodeRef createContent(
    ...
    try {
      ...
      QName propContent = UtilsCircabc.getPropContent(
  ...
  public void contentIdMultilingualAspectPost(
    ...
    if (
      nodeAuthor == null ||
      (nodeAuthor.length() < 1 && body.getAuthor() != null)
    ) {
      nodeService.setProperty(
        nodeRef,
        ContentModel.PROP_AUTHOR,
  ...
  private OfficeEditResult checkEditInOffice(NodeRef nodeRef) {
    ...
    if (
      ...
      result.setErrorDescription(
        "1: Write permission denied. Cannot edit in Office."
      );
      return result;
    }
    ...
    if (!contentItem) {
      result.setErrorDescription(
        "7: Document's content property could not be found."
      );
      return result;
    }
    ...
    if (documentLocation.isEmpty()) {
      result.setErrorDescription(
        "8: Document's content could not be retrieved."
      );
      return result;
    }
  ...
  public PreviewResult checkPreview(String documentId) {
    ...
    if (
      reader != null && MimetypeMap.MIMETYPE_PDF.equals(reader.getMimetype())
    ) {
      return new PreviewResult(true, "", "", reader.getSize());
    }

    // else check if it has been converted
    NodeRef renditionNodeRef = circabcRenditionHelper.getRenditionNodeRef(
      documentNodeRef
    );
    ...
    List<Request> requests = renditionDaoService.getRequestsForDocument(
      documentId
    );
    ...
    for (Request request : requests) {
      if (request.isSuccess()) {
        ...
        request.getEndProcessingDate() == null ||
   ...
   */
  public void setCircabcRenditionHelper(

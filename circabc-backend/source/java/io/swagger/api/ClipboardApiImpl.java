...
public class ClipboardApiImpl implements ClipboardApi {
  ...
  private void paste(
    ...
    if (getParent(nodeRef) == null) {
      ...
      assocRef = foundRef;
      isPrimaryParent = parentNodeRef.equals(
    ...
    for (;;) {
      try {
        if (action == ClipboardAction.LINK.getValue()) {
          ...
          if (
            destRef.equals(assocRef.getParentRef()) && name.equals(originalName)
          ) {

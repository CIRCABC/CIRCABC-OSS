...
public class InformationApiImpl implements InformationApi {
  ...
  public PagedNews groupsIdInformationNewsGet(
    ...
    final PagingResults<FileInfo> list = getFileFolderService()
      .list(infoRef, false, true, ignoredQNames, sortProps, pr);
  ...
  private News newsIdGetInternal(NodeRef childRef) {
    if (
      ...
      if (
        ...
        news.setTitle(
          Converter.toI18NProperty(
            (String) secureNodeService.getProperty(
              childRef,
              ContentModel.PROP_TITLE
            )
          )
        ...
        news.setTitle(
          Converter.toI18NProperty(
            (MLText) secureNodeService.getProperty(
              childRef,
              ContentModel.PROP_TITLE
            )
          )
      ...
      Date date = (Date) secureNodeService.getProperty(
        childRef,
        ContentModel.PROP_MODIFIED
      );
      news.setModified(new DateTime(date));

      Date dateCreated = (Date) secureNodeService.getProperty(
        childRef,
      ...
      if (
        News.PatternEnum.DOCUMENT.equals(News.PatternEnum.fromValue(pattern)) ||
        ...
        News.PatternEnum.DATE.equals(News.PatternEnum.fromValue(pattern))
      ) {

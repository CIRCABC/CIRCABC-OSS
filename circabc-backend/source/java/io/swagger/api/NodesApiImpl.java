...
public class NodesApiImpl implements NodesApi {
    ...
    if (
      ...
      while (!stop) {
        ...
        if (element instanceof Path.ChildAssocElement) {
          NodeRef elementRef =
            ((Path.ChildAssocElement) element).getRef().getChildRef();
  ...
  public Node getNode(NodeRef nodeRef, Node node) {
    ...
    node.setName(
      (String) secureNodeService.getProperty(nodeRef, ContentModel.PROP_NAME)
    );
    ...
    if (secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_INFORMATION)) {
      ...
      if (libRef != null) {
        isLibRootAdmin = permissionService
          .hasPermission(libRef, "LibAdmin")
          .equals(AccessStatus.ALLOWED);
      }
    } else if (
      secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_EVENT)
    ) {
      ...
      if (newsRef != null) {
        isNewsRootAdmin = permissionService
          .hasPermission(newsRef, "NwsAdmin")
          .equals(AccessStatus.ALLOWED);
      }
    }

    node.setParentId(
      secureNodeService.getPrimaryParent(nodeRef).getParentRef().getId()
    );

    final Map<String, String> properties = new HashMap<>();
    boolean isMLAware = MLPropertyInterceptor.isMLAware();
    MLPropertyInterceptor.setMLAware(true);
    for (Entry<QName, Serializable> property : secureNodeService
    ...
    if (secureNodeService.getType(nodeRef).equals(ContentModel.TYPE_CONTENT)) {
      ContentData cData = (ContentData) secureNodeService.getProperty(
        nodeRef,
      ...
      boolean isMultilingual = secureNodeService.hasAspect(
        nodeRef,
      ...
      boolean isWorkingCopy = secureNodeService.hasAspect(
        nodeRef,
      ...
      if (
        ...
        try {
          properties.put(
            "originalNodeId",
            cociContentBusinessSrv.getWorkingCopyOf(nodeRef).getId()
          );
        } catch (Exception e) {
          logger.error(
      ...
      if (isWorkingCopy && currentUserHasAccess) {
        String workingCopyOwner = (String) secureNodeService.getProperty(
          nodeRef,
          ContentModel.PROP_WORKING_COPY_OWNER
        );
        currentUserHasAccess = workingCopyOwner.equals(
          AuthenticationUtil.getRunAsUser()
        );
      }
      properties.put(
        "currentUserHasAccess",
        String.valueOf(currentUserHasAccess)
      );
      if (isLocked) {
        NodeRef workingCopyNodeRef = cociContentBusinessSrv.getWorkingCopy(
          nodeRef
        );
    ...
    if (
      ...
      NodeRef destination = (NodeRef) secureNodeService.getProperty(
        nodeRef,
        PROP_DESTINATION
      );
      if (destination != null) {
    ...
    if (
      (secureNodeService.getType(nodeRef).equals(ContentModel.TYPE_CONTENT) ||
    ...
    if (secureNodeService.getType(nodeRef).equals(ContentModel.TYPE_FOLDER)) {
      ...
      secureNodeService.getType(nodeRef).equals(ForumModel.TYPE_FORUM)
    ) {
      Set<QName> typeForum = new HashSet<>();
      typeForum.add(ForumModel.TYPE_FORUM);
      Boolean hasFolders = !secureNodeService
        .getChildAssocs(nodeRef, typeForum)
        .isEmpty();
      node.setHasSubFolders(hasFolders);
    }

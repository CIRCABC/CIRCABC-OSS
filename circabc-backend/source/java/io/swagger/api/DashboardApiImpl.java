...
public class DashboardApiImpl implements DashboardApi {
  ...
  public UserNewsFeed usersUserIdDashboardNewsfeedGet(
    ...
    if (!groupIds.isEmpty() && !activityIds.isEmpty()) {
      ...
      List<UserActionLogDAO> activities = logService.getUserDashboardActivities(
        request
      );
      for (UserActionLogDAO activity : activities) {
        ...
        if (
          ...
          log.setActionDate(
            Converter.convertDateToString(activity.getLogDate())
          );
          ...
          if (
            action.equals(ADD_TRANSLATION) ||
            action.equals(ADD_CONTENT_TRANSLATION)
          ) {
          ...
          if (groupFeeds.containsKey(activity.getIgId())) {
            ...
            if (
              nodeService.getProperty(igRef, ContentModel.PROP_TITLE) != null
            ) {
              feed.setTitle(

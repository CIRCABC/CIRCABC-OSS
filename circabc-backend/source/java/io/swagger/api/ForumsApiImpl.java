...
public class ForumsApiImpl implements ForumsApi {
  ...
  public List<Node> getForumById(String id) {
    ...
    if (secureNodeService.hasAspect(forumRef, CircabcModel.ASPECT_NEWSGROUP)) {
      ...
      final PagingResults<FileInfo> list = getFileFolderService()
        .list(forumRef, true, true, null, sortProps, pr);
  ...
  @Override
  public PagedNodes getForumById(
    String id,
    Integer nbPage,
    Integer nbLimit,
    String sort
  ...
  private NodeRef createNode(Node body, NodeRef parentRef, QName nodeType) {
    if (
      !(secureNodeService.getType(parentRef).equals(ForumModel.TYPE_FORUM) ||
  ...
  public void toggleModeration(String id, boolean enable, boolean acceptAll) {
    ...
    Serializable moderationStatusSerializable = secureNodeService.getProperty(
      forumRef,
  ...
  private void reject(final NodeRef postRef, final String message)
    ...
    final String creator = (String) secureNodeService.getProperty(
      postRef,
    ...
    if (personService.personExists(creator)) {
      ...
      final Map<QName, Serializable> props = secureNodeService.getProperties(
        postRef
      );
  ...
  protected void signalAbuse(NodeRef nodeRef, final String message)
    ...
    for (final NodeRef moderator : moderators) {
      final String moderatorEmail = (String) secureNodeService.getProperty(
        moderator,
        ContentModel.PROP_EMAIL
      );
      String noReply = mailService.getNoReplyEmailAddress();
      String moderatorUserId = (String) secureNodeService.getProperty(
        moderator,
  ...
  protected Set<NodeRef> getModerators(NodeRef postRef) {
    final NodeRef topicRef = secureNodeService
  ...
  public List<Node> forumsIdSubforumsGet(String id, String sorting) {
    ...
    FileFilterMode.setClient(Client.cmis);
    final PagingResults<FileInfo> list = getFileFolderService()
      .list(forumRef, false, true, ignored, sortProps, pr);
    FileFilterMode.clearClient();

    ...
    while (!queue.isEmpty() && userIds != null && !userIds.isEmpty()) {
      ...
      for (ChildAssociationRef childAssoc : children) {
        ...
        if (
          ...
          ForumModel.TYPE_FORUMS.equals(type) ||
      ...
      for (AccessPermission acp : perms) {
        if (userIds.contains(acp.getAuthority()) && !acp.isInherited()) {
          try {
            ...
            logger.error(
              "Security Exception when processing clean permission of" +
              acp.getAuthority(),
              e
            );
          } catch (IllegalStateException e) {
            logger.error(
              "IllegalStateException when processing clean permission of" +
              acp.getAuthority(),
              e
            );
          } catch (NotSupportedException e) {
            logger.error(
              "NotSupportedException when processing clean permission of" +
              acp.getAuthority(),
              e
            );
          } catch (SystemException e) {
            logger.error(
              "SystemException when processing clean permission of" +
              acp.getAuthority(),
              e
            );
          } catch (RollbackException e) {
            logger.error(
              "RollbackException when processing clean permission of" +
              acp.getAuthority(),
              e
            );
          } catch (HeuristicMixedException e) {
            logger.error(
              "HeuristicMixedException when processing clean permission of" +
              acp.getAuthority(),
              e
            );

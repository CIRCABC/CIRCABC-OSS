...
/**
 * Low level service that manages notification subscriptions on repository nodes.
 *
 * <p>It resolves and mutates the notification status ({@link NotificationStatus}) granted to or
 * denied for authorities (users or groups) on a given {@link NodeRef}. Notification settings are
 * computed by walking from the target node up to its enclosing InterestGroup, allowing callers to
 * determine which users should be notified, inspect per-authority notification reports and
 * grant, remove or update subscriptions.
 *
...
public interface NotificationSubscriptionService {
  ...
  Set<NotifiableUser> getNotifiableUsers(final NodeRef nodeRef)
    throws InvalidNodeRefException;
   ...
   *
   * @param nodeRef       - the reference to the node
   * @param userAuthority - the authority that match the permission
   * @return the full report
   */
  @Auditable(
    /* key = Auditable.Key.ARG_0, */ parameters = { "nodeRef", "userAuthority" }
  )
  ...
  /**
   * Set a specific notification status on a node for the given authority.
   *
   * @param nodeRef   the reference to the node on which the notification status is set
   * @param authority the authority (user or group) whose notification status is being set
   * @param status    the notification status to apply
   */
  @Auditable(
    /* key = Auditable.Key.ARG_0, */ parameters = {
      "nodeRef", "authority", "status",
    }
  )

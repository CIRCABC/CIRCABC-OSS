...
public interface MailService {
  ...
  /**
   * Sends a calendar meeting request (iCalendar invitation) to the specified recipients. When an
   * {@code oldMeeting} is provided, the request is treated as an update to a previously sent
   * invitation according to the given {@code mode}.
   *
   * @param from       the sender email address of the meeting request
   * @param to         the recipient email addresses of the meeting request
   * @param replyTo    the reply-to email address
   * @param meeting    the meeting details to send in the invitation
   * @param oldMeeting the previous state of the meeting when this is an update, or {@code null} for
   *                   a new invitation
   * @param mode       the update mode indicating how the meeting request should be processed
   * @param useBCC     {@code true} to send the request using blind carbon copy
   * @return {@code true} if the meeting request was sent, {@code false} otherwise
   * @throws MailServiceException if the meeting request could not be built or sent
   */
  @Auditable(
    /* key = Auditable.Key.NO_KEY, */ parameters = {
      "subject", "from", "to", "replyTo", "meeting", "oldMeeting", "mode",
    }
  )
  ...
  /**
   * Sends a cancellation notice for a previously scheduled calendar meeting to the specified
   * recipients.
   *
   * @param from      the sender email address of the cancellation notice
   * @param to        the recipient email addresses of the cancellation notice
   * @param replyTo   the reply-to email address
   * @param meeting   the meeting being cancelled
   * @param eventDate the date of the meeting occurrence to cancel (relevant for recurring meetings)
   * @param mode      the update mode indicating how the cancellation should be processed
   * @return {@code true} if the cancellation notice was sent, {@code false} otherwise
   * @throws MailServiceException if the cancellation notice could not be built or sent
   */
  @Auditable(
    /* key = Auditable.Key.NO_KEY, */ parameters = {
      "subject", "from", "to", "replyTo", "meeting", "eventDate", "mode",
    }
  )
   ...
   * @param subject    The subject of the email
   * @param body       The body of the email
   * @param attachements List of content to add
   * @return If whethever the mail is sent or not.
   */
  @SuppressWarnings("java:S107") // Parameters map directly to email fields
  @Auditable(
    /* key = Auditable.Key.NO_KEY, */ parameters = {
      "from", "to", "replyTo", "subject", "body",
    }

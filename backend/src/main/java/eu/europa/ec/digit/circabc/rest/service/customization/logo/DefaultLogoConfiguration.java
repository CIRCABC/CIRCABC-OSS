...
/**
 * Base contract describing the logo customization configuration of a CIRCABC node (typically an
 * Interest Group).
 *
 * <p>Implementations expose, in a read-only fashion, the logo itself together with the display
 * options that control how it is rendered by the frontend: whether the logo is shown on the main
 * page (IG-HOME) and/or on the other navigation pages, its width/height, whether those dimensions
 * are enforced, and its horizontal placement. It also exposes the {@link NodeRef} of the node the

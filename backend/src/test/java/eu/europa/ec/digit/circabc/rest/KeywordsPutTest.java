...
public class KeywordsPutTest {

  private KeywordsPut keywordsPut;
  ...
  private static final String VALID_JSON =
    "{\"id\":\"keyword-node-id\",\"title\":{\"en\":\"Test Keyword\"}}";

  @Before
  public void setUp() throws Exception {
    keywordsPut = new KeywordsPut();
  ...
  @Test
  public void testExecuteImpl_whenUserIsAdmin_thenUpdatesKeyword()
    throws Exception {
    setupNodeHierarchy();
    ...
    KeywordDefinition expectedResult = new KeywordDefinition();
    expectedResult.setId(KEYWORD_ID);
    when(keywordsApi.keywordsKeywordIdPut(eq(KEYWORD_ID), any())).thenReturn(
      expectedResult
    );

    Map<String, Object> result = keywordsPut.executeImpl(req, status, cache);

    assertNotNull(result);
    assertEquals(expectedResult, result.get("keyword"));
    verify(keywordsApi).keywordsKeywordIdPut(eq(KEYWORD_ID), any());
  }
  ...
  public void testExecuteImpl_whenUserIsNotAdmin_thenReturnsForbidden()
    ...
    verify(keywordsApi, never()).keywordsKeywordIdPut(
      anyString(),
  ...
  private void setField(String fieldName, Object value) throws Exception {
    Field field = KeywordsPut.class.getDeclaredField(fieldName);
    field.setAccessible(true);
    field.set(keywordsPut, value);
  }
}

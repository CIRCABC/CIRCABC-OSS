...
public class ZipServiceTest {
  ...
  @Test
  public void testAddingFileIntoArchive_whenNodeRef_thenDelegatesToListVersion() {
    NodeRef nodeRef = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      "test-node"
    );

    // Node does not exist, so getAllChildsNodeRefs returns empty
    ...
    // Should not throw - just creates an empty zip
  ...
  public void testAddingFileIntoArchive_whenSecurityRankingSensitive_thenSkipped()
    ...
    // SENSITIVE ranking should be blocked
    when(
      nodeService.getProperty(nodeRef, DocumentModel.PROP_SECURITY_RANKING)
    ).thenReturn("SENSITIVE");

    // For path resolution (still called before security check in addNodeToArchive)
    NodeRef libraryRef = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      "library-node"
    );
    ...
    // Content should NOT be read since security ranking blocks it
  ...
  public void testAddingFileIntoArchive_whenFolderWithChildren_thenRecurses()
    ...
    // Child does not exist (to stop recursion)

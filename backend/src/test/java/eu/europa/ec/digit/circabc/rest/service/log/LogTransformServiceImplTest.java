...
public class LogTransformServiceImplTest {
  ...
  @Test
  public void testTransform_whenNodeRefNull_thenUsesNodeIDAndNodePath() {
    // To get null from resolveNodeRef: pathOneValue not a UUID, templateID matches transaction
    // template, and getExternalRepoNodeRef returns null (empty JSON payload)
    LogRestDAO input = new LogRestDAO();
    input.setTemplateID(99);
    input.setPathOneValue("not-a-uuid");
    input.setUserName("user2");
    input.setLogDate(new Date());
    input.setStatusCode(200);
    input.setInfo("{\"payload\":{\"nodes\":[]}}");
    input.setNodeID(999L);
    input.setNodePath("/some/path");
    input.setNodeParent("workspace://SpacesStore/parent-id");

    NodeRef parentRef = new NodeRef("workspace://SpacesStore/parent-id");

    when(logDaoService.getActivityID(99L)).thenReturn(300L);
    when(
      logDaoService.getTemplateID(
        "POST",
        "/circabc/repositories/{id}/transaction"
    ...
    when(
      nodeService.hasAspect(parentRef, CircabcModel.ASPECT_CATEGORY)
    ).thenReturn(false);
    when(apiToolBox.getCurrentInterestGroup(parentRef)).thenReturn(
      CIRCABC_NODE_REF
    );
    when(apiToolBox.getDatabaseID(CIRCABC_NODE_REF)).thenReturn(7L);
    when(apiToolBox.getName(CIRCABC_NODE_REF)).thenReturn("MyIG");

    LogRecordDAO result = service.transform(input);

    assertEquals(999L, result.getDocumentID());
    assertEquals("/some/path", result.getPath());
    assertEquals(7L, result.getIgID());
  }
  ...
  public void testTransform_whenProcessInfoFails_thenReturnsRawInfo() {
    ...
    // processInfo fails to parse, processInfoSafe catches and returns raw info

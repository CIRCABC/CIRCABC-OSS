...
public class CircabcDynamicAuthorityServiceImplTest {
  ...
  @Test
  public void testIsAdmin_whenNwsModerate_thenTrue() {
    NodeRef group = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      "group-id"
    );
    ...
    CircabcPermission perm = new CircabcPermission();
    perm.setLibraryPermission("LibAccess");
    perm.setNewsGroupPermission("NwsModerate");
    perm.setInformationPermission("InfAccess");
    when(circabcDAO.getGroupPermission(group.toString(), "user1")).thenReturn(
      Arrays.asList(perm)
    );
  ...
  @Test
  public void testIsAdmin_whenInfAdmin_thenTrue() {
    NodeRef group = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      "group-id"
    );
    ...
    perm.setLibraryPermission("LibAccess");
    perm.setNewsGroupPermission("NwsAccess");
    perm.setInformationPermission("InfAdmin");
    when(circabcDAO.getGroupPermission(group.toString(), "user1")).thenReturn(
      Arrays.asList(perm)
    );

    assertTrue(service.isAdmin(group, "user1", CircabcServiceType.INFORMATION));
  }
  ...
  @Test
  public void testFindServiceType_whenInformation_thenInformation() {
    NodeRef nodeRef = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      "node-id"
    );
    ...
    assertEquals(
      CircabcServiceType.INFORMATION,
      service.findServiceType(nodeRef)
    );
  }

...
public class HelpLinkPutTest {

  private HelpLinkPut helpLinkPut;
  ...
  @Test
  public void testExecuteImpl_whenAlfrescoAdmin_thenUpdatesLink()
    throws Exception {
    ...
    HelpLink updatedLink = new HelpLink();
    updatedLink.setId("link-1");
    when(helpApi.updateHelpLink(any(HelpLink.class))).thenReturn(updatedLink);

    Map<String, Object> result = helpLinkPut.executeImpl(req, status, cache);

    assertNotNull(result);
    assertEquals(updatedLink, result.get("link"));
    verify(helpApi).updateHelpLink(any(HelpLink.class));
  }

  @Test
  public void testExecuteImpl_whenCircabcAdmin_thenUpdatesLink()
    throws Exception {
    when(currentUserPermissionCheckerService.isAlfrescoAdmin()).thenReturn(
      false
    );
  ...
  public void testExecuteImpl_whenNotAdmin_thenReturnsForbidden() {
    ...
    verify(helpApi, never()).updateHelpLink(any(HelpLink.class));
  }
  ...
  private void setField(String fieldName, Object value) throws Exception {
    Field field = HelpLinkPut.class.getDeclaredField(fieldName);
    field.setAccessible(true);
    field.set(helpLinkPut, value);
  }
}
